#pragma once

// Fortnite (1.8) SDK

#pragma warning (disable : 4267)
#pragma warning (disable : 4244)
#pragma warning (disable : 26495)

#include <set>
#include <string>
#include <windows.h>
#include <psapi.h>
#include <vector>
#include <iostream>
#include <inttypes.h>
#include <filesystem>
#include <fstream>
#include <intrin.h>

#include "SDK/FN_Basic.hpp"

#include "SDK/FN_CoreUObject_structs.hpp"
#include "SDK/FN_CoreUObject_classes.hpp"
#include "SDK/FN_CoreUObject_parameters.hpp"
#include "SDK/FN_SlateCore_structs.hpp"
#include "SDK/FN_SlateCore_classes.hpp"
#include "SDK/FN_SlateCore_parameters.hpp"
#include "SDK/FN_InputCore_structs.hpp"
#include "SDK/FN_InputCore_classes.hpp"
#include "SDK/FN_InputCore_parameters.hpp"
#include "SDK/FN_Slate_structs.hpp"
#include "SDK/FN_Slate_classes.hpp"
#include "SDK/FN_Slate_parameters.hpp"
#include "SDK/FN_PacketHandler_structs.hpp"
#include "SDK/FN_PacketHandler_classes.hpp"
#include "SDK/FN_PacketHandler_parameters.hpp"
#include "SDK/FN_Rejoin_structs.hpp"
#include "SDK/FN_Rejoin_classes.hpp"
#include "SDK/FN_Rejoin_parameters.hpp"
#include "SDK/FN_Hotfix_structs.hpp"
#include "SDK/FN_Hotfix_classes.hpp"
#include "SDK/FN_Hotfix_parameters.hpp"
#include "SDK/FN_OnlineSubsystem_structs.hpp"
#include "SDK/FN_OnlineSubsystem_classes.hpp"
#include "SDK/FN_OnlineSubsystem_parameters.hpp"
#include "SDK/FN_Engine_structs.hpp"
#include "SDK/FN_Engine_classes.hpp"
#include "SDK/FN_Engine_parameters.hpp"
#include "SDK/FN_GameplayTasks_structs.hpp"
#include "SDK/FN_GameplayTasks_classes.hpp"
#include "SDK/FN_GameplayTasks_parameters.hpp"
#include "SDK/FN_GameplayTags_structs.hpp"
#include "SDK/FN_GameplayTags_classes.hpp"
#include "SDK/FN_GameplayTags_parameters.hpp"
#include "SDK/FN_MovieScene_structs.hpp"
#include "SDK/FN_MovieScene_classes.hpp"
#include "SDK/FN_MovieScene_parameters.hpp"
#include "SDK/FN_OnlineSubsystemUtils_structs.hpp"
#include "SDK/FN_OnlineSubsystemUtils_classes.hpp"
#include "SDK/FN_OnlineSubsystemUtils_parameters.hpp"
#include "SDK/FN_GameplayAbilities_structs.hpp"
#include "SDK/FN_GameplayAbilities_classes.hpp"
#include "SDK/FN_GameplayAbilities_parameters.hpp"
#include "SDK/FN_JsonUtilities_structs.hpp"
#include "SDK/FN_JsonUtilities_classes.hpp"
#include "SDK/FN_JsonUtilities_parameters.hpp"
#include "SDK/FN_NavLink_Stair_structs.hpp"
#include "SDK/FN_NavLink_Stair_classes.hpp"
#include "SDK/FN_NavLink_Stair_parameters.hpp"
#include "SDK/FN_McpProfileSys_structs.hpp"
#include "SDK/FN_McpProfileSys_classes.hpp"
#include "SDK/FN_McpProfileSys_parameters.hpp"
#include "SDK/FN_Lobby_structs.hpp"
#include "SDK/FN_Lobby_classes.hpp"
#include "SDK/FN_Lobby_parameters.hpp"
#include "SDK/FN_MovieSceneTracks_structs.hpp"
#include "SDK/FN_MovieSceneTracks_classes.hpp"
#include "SDK/FN_MovieSceneTracks_parameters.hpp"
#include "SDK/FN_Foliage_structs.hpp"
#include "SDK/FN_Foliage_classes.hpp"
#include "SDK/FN_Foliage_parameters.hpp"
#include "SDK/FN_AnimGraphRuntime_structs.hpp"
#include "SDK/FN_AnimGraphRuntime_classes.hpp"
#include "SDK/FN_AnimGraphRuntime_parameters.hpp"
#include "SDK/FN_Party_structs.hpp"
#include "SDK/FN_Party_classes.hpp"
#include "SDK/FN_Party_parameters.hpp"
#include "SDK/FN_Gauntlet_structs.hpp"
#include "SDK/FN_Gauntlet_classes.hpp"
#include "SDK/FN_Gauntlet_parameters.hpp"
#include "SDK/FN_SignificanceManager_structs.hpp"
#include "SDK/FN_SignificanceManager_classes.hpp"
#include "SDK/FN_SignificanceManager_parameters.hpp"
#include "SDK/FN_Account_structs.hpp"
#include "SDK/FN_Account_classes.hpp"
#include "SDK/FN_Account_parameters.hpp"
#include "SDK/FN_BlueprintContext_structs.hpp"
#include "SDK/FN_BlueprintContext_classes.hpp"
#include "SDK/FN_BlueprintContext_parameters.hpp"
#include "SDK/FN_UMG_structs.hpp"
#include "SDK/FN_UMG_classes.hpp"
#include "SDK/FN_UMG_parameters.hpp"
#include "SDK/FN_AIModule_structs.hpp"
#include "SDK/FN_AIModule_classes.hpp"
#include "SDK/FN_AIModule_parameters.hpp"
#include "SDK/FN_PhysXVehicles_structs.hpp"
#include "SDK/FN_PhysXVehicles_classes.hpp"
#include "SDK/FN_PhysXVehicles_parameters.hpp"
#include "SDK/FN_FortniteGame_structs.hpp"
#include "SDK/FN_FortniteGame_classes.hpp"
#include "SDK/FN_FortniteGame_parameters.hpp"
#include "SDK/FN_Commerce_StoreShelves_04_structs.hpp"
#include "SDK/FN_Commerce_StoreShelves_04_classes.hpp"
#include "SDK/FN_Commerce_StoreShelves_04_parameters.hpp"
#include "SDK/FN_En_LeafTypes_01_structs.hpp"
#include "SDK/FN_En_LeafTypes_01_classes.hpp"
#include "SDK/FN_En_LeafTypes_01_parameters.hpp"
#include "SDK/FN_Recreation_CampingRack04_structs.hpp"
#include "SDK/FN_Recreation_CampingRack04_classes.hpp"
#include "SDK/FN_Recreation_CampingRack04_parameters.hpp"
#include "SDK/FN_Crypt_BrokenFloor_Inner_structs.hpp"
#include "SDK/FN_Crypt_BrokenFloor_Inner_classes.hpp"
#include "SDK/FN_Crypt_BrokenFloor_Inner_parameters.hpp"
#include "SDK/FN_Parent_BuildingPropWall_structs.hpp"
#include "SDK/FN_Parent_BuildingPropWall_classes.hpp"
#include "SDK/FN_Parent_BuildingPropWall_parameters.hpp"
#include "SDK/FN_Art_Painting02_structs.hpp"
#include "SDK/FN_Art_Painting02_classes.hpp"
#include "SDK/FN_Art_Painting02_parameters.hpp"
#include "SDK/FN_DayPhaseFloats_structs.hpp"
#include "SDK/FN_DayPhaseFloats_classes.hpp"
#include "SDK/FN_DayPhaseFloats_parameters.hpp"
#include "SDK/FN_LCD_Grave_Dirt_Mound_structs.hpp"
#include "SDK/FN_LCD_Grave_Dirt_Mound_classes.hpp"
#include "SDK/FN_LCD_Grave_Dirt_Mound_parameters.hpp"
#include "SDK/FN_Crypt_02e_Quarter_structs.hpp"
#include "SDK/FN_Crypt_02e_Quarter_classes.hpp"
#include "SDK/FN_Crypt_02e_Quarter_parameters.hpp"
#include "SDK/FN_LCD_JunkPile_01a_structs.hpp"
#include "SDK/FN_LCD_JunkPile_01a_classes.hpp"
#include "SDK/FN_LCD_JunkPile_01a_parameters.hpp"
#include "SDK/FN_DTN_Corner_04e_Quarter_structs.hpp"
#include "SDK/FN_DTN_Corner_04e_Quarter_classes.hpp"
#include "SDK/FN_DTN_Corner_04e_Quarter_parameters.hpp"
#include "SDK/FN_Factory_QuarterWall_Curved_structs.hpp"
#include "SDK/FN_Factory_QuarterWall_Curved_classes.hpp"
#include "SDK/FN_Factory_QuarterWall_Curved_parameters.hpp"
#include "SDK/FN_Parent_BuildingWall_structs.hpp"
#include "SDK/FN_Parent_BuildingWall_classes.hpp"
#include "SDK/FN_Parent_BuildingWall_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_B09_structs.hpp"
#include "SDK/FN_Fence_Chainlink_B09_classes.hpp"
#include "SDK/FN_Fence_Chainlink_B09_parameters.hpp"
#include "SDK/FN_Athena_REM_3x3_RuinedBuilding_c_140_structs.hpp"
#include "SDK/FN_Athena_REM_3x3_RuinedBuilding_c_140_classes.hpp"
#include "SDK/FN_Athena_REM_3x3_RuinedBuilding_c_140_parameters.hpp"
#include "SDK/FN_Rural_House_Post_2_structs.hpp"
#include "SDK/FN_Rural_House_Post_2_classes.hpp"
#include "SDK/FN_Rural_House_Post_2_parameters.hpp"
#include "SDK/FN_Athena_REM_3x3_Shack_a_154_structs.hpp"
#include "SDK/FN_Athena_REM_3x3_Shack_a_154_classes.hpp"
#include "SDK/FN_Athena_REM_3x3_Shack_a_154_parameters.hpp"
#include "SDK/FN_Fence_Iron_Cemetary_02_structs.hpp"
#include "SDK/FN_Fence_Iron_Cemetary_02_classes.hpp"
#include "SDK/FN_Fence_Iron_Cemetary_02_parameters.hpp"
#include "SDK/FN_ParentBuildingContainerBlueprint_structs.hpp"
#include "SDK/FN_ParentBuildingContainerBlueprint_classes.hpp"
#include "SDK/FN_ParentBuildingContainerBlueprint_parameters.hpp"
#include "SDK/FN_PBWA_W2_ArchwaySupport_structs.hpp"
#include "SDK/FN_PBWA_W2_ArchwaySupport_classes.hpp"
#include "SDK/FN_PBWA_W2_ArchwaySupport_parameters.hpp"
#include "SDK/FN_PBWA_W3_ArchwayLargeSupport_structs.hpp"
#include "SDK/FN_PBWA_W3_ArchwayLargeSupport_classes.hpp"
#include "SDK/FN_PBWA_W3_ArchwayLargeSupport_parameters.hpp"
#include "SDK/FN_Roof_AirConditioner_02_structs.hpp"
#include "SDK/FN_Roof_AirConditioner_02_classes.hpp"
#include "SDK/FN_Roof_AirConditioner_02_parameters.hpp"
#include "SDK/FN_CryptWalls_00_structs.hpp"
#include "SDK/FN_CryptWalls_00_classes.hpp"
#include "SDK/FN_CryptWalls_00_parameters.hpp"
#include "SDK/FN_Rural_House_Wall_13_structs.hpp"
#include "SDK/FN_Rural_House_Wall_13_classes.hpp"
#include "SDK/FN_Rural_House_Wall_13_parameters.hpp"
#include "SDK/FN_BP_FloorLamp_structs.hpp"
#include "SDK/FN_BP_FloorLamp_classes.hpp"
#include "SDK/FN_BP_FloorLamp_parameters.hpp"
#include "SDK/FN_Crypt_03e_Quarter_structs.hpp"
#include "SDK/FN_Crypt_03e_Quarter_classes.hpp"
#include "SDK/FN_Crypt_03e_Quarter_parameters.hpp"
#include "SDK/FN_Rural_House_Stair_4_structs.hpp"
#include "SDK/FN_Rural_House_Stair_4_classes.hpp"
#include "SDK/FN_Rural_House_Stair_4_parameters.hpp"
#include "SDK/FN_CryptWalls_08_door_structs.hpp"
#include "SDK/FN_CryptWalls_08_door_classes.hpp"
#include "SDK/FN_CryptWalls_08_door_parameters.hpp"
#include "SDK/FN_Commerce_MagazineStand_02_structs.hpp"
#include "SDK/FN_Commerce_MagazineStand_02_classes.hpp"
#include "SDK/FN_Commerce_MagazineStand_02_parameters.hpp"
#include "SDK/FN_S_StormCellarDoors_structs.hpp"
#include "SDK/FN_S_StormCellarDoors_classes.hpp"
#include "SDK/FN_S_StormCellarDoors_parameters.hpp"
#include "SDK/FN_StormDoor_Stairs_02_structs.hpp"
#include "SDK/FN_StormDoor_Stairs_02_classes.hpp"
#include "SDK/FN_StormDoor_Stairs_02_parameters.hpp"
#include "SDK/FN_ShutterHouse_DoorSide_02_structs.hpp"
#include "SDK/FN_ShutterHouse_DoorSide_02_classes.hpp"
#include "SDK/FN_ShutterHouse_DoorSide_02_parameters.hpp"
#include "SDK/FN_Parent_Tree_structs.hpp"
#include "SDK/FN_Parent_Tree_classes.hpp"
#include "SDK/FN_Parent_Tree_parameters.hpp"
#include "SDK/FN_Athena_SUB_5x5_House_d_211_structs.hpp"
#include "SDK/FN_Athena_SUB_5x5_House_d_211_classes.hpp"
#include "SDK/FN_Athena_SUB_5x5_House_d_211_parameters.hpp"
#include "SDK/FN_Athena_SUB_5x5_CampingStore_a_212_structs.hpp"
#include "SDK/FN_Athena_SUB_5x5_CampingStore_a_212_classes.hpp"
#include "SDK/FN_Athena_SUB_5x5_CampingStore_a_212_parameters.hpp"
#include "SDK/FN_Commerce_Counter01_b_structs.hpp"
#include "SDK/FN_Commerce_Counter01_b_classes.hpp"
#include "SDK/FN_Commerce_Counter01_b_parameters.hpp"
#include "SDK/FN_SM_Trim_Corner_04_structs.hpp"
#include "SDK/FN_SM_Trim_Corner_04_classes.hpp"
#include "SDK/FN_SM_Trim_Corner_04_parameters.hpp"
#include "SDK/FN_Roof_Ducts03c_structs.hpp"
#include "SDK/FN_Roof_Ducts03c_classes.hpp"
#include "SDK/FN_Roof_Ducts03c_parameters.hpp"
#include "SDK/FN_Sign_PawnShop_6_structs.hpp"
#include "SDK/FN_Sign_PawnShop_6_classes.hpp"
#include "SDK/FN_Sign_PawnShop_6_parameters.hpp"
#include "SDK/FN_S_BuildingTrim_E_structs.hpp"
#include "SDK/FN_S_BuildingTrim_E_classes.hpp"
#include "SDK/FN_S_BuildingTrim_E_parameters.hpp"
#include "SDK/FN_Rural_House_Wall_12_structs.hpp"
#include "SDK/FN_Rural_House_Wall_12_classes.hpp"
#include "SDK/FN_Rural_House_Wall_12_parameters.hpp"
#include "SDK/FN_Parent_BuildingPropActor_structs.hpp"
#include "SDK/FN_Parent_BuildingPropActor_classes.hpp"
#include "SDK/FN_Parent_BuildingPropActor_parameters.hpp"
#include "SDK/FN_Tree_Medium_04_structs.hpp"
#include "SDK/FN_Tree_Medium_04_classes.hpp"
#include "SDK/FN_Tree_Medium_04_parameters.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge03_structs.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge03_classes.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge03_parameters.hpp"
#include "SDK/FN_Tree_Log_5b_structs.hpp"
#include "SDK/FN_Tree_Log_5b_classes.hpp"
#include "SDK/FN_Tree_Log_5b_parameters.hpp"
#include "SDK/FN_Prop_Rocks_07_structs.hpp"
#include "SDK/FN_Prop_Rocks_07_classes.hpp"
#include "SDK/FN_Prop_Rocks_07_parameters.hpp"
#include "SDK/FN_Tree_Pine_01_structs.hpp"
#include "SDK/FN_Tree_Pine_01_classes.hpp"
#include "SDK/FN_Tree_Pine_01_parameters.hpp"
#include "SDK/FN_Tree_Log_5c_structs.hpp"
#include "SDK/FN_Tree_Log_5c_classes.hpp"
#include "SDK/FN_Tree_Log_5c_parameters.hpp"
#include "SDK/FN_Tree_Log_5d_structs.hpp"
#include "SDK/FN_Tree_Log_5d_classes.hpp"
#include "SDK/FN_Tree_Log_5d_parameters.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge04_structs.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge04_classes.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge04_parameters.hpp"
#include "SDK/FN_Tree_Log_6b_structs.hpp"
#include "SDK/FN_Tree_Log_6b_classes.hpp"
#include "SDK/FN_Tree_Log_6b_parameters.hpp"
#include "SDK/FN_Athena_TheFarmstead_7x12_Field_a_237_structs.hpp"
#include "SDK/FN_Athena_TheFarmstead_7x12_Field_a_237_classes.hpp"
#include "SDK/FN_Athena_TheFarmstead_7x12_Field_a_237_parameters.hpp"
#include "SDK/FN_Athena_TheFarmstead_7x12_Field_c_238_structs.hpp"
#include "SDK/FN_Athena_TheFarmstead_7x12_Field_c_238_classes.hpp"
#include "SDK/FN_Athena_TheFarmstead_7x12_Field_c_238_parameters.hpp"
#include "SDK/FN_Art_Painting04_structs.hpp"
#include "SDK/FN_Art_Painting04_classes.hpp"
#include "SDK/FN_Art_Painting04_parameters.hpp"
#include "SDK/FN_Basic_StairF_structs.hpp"
#include "SDK/FN_Basic_StairF_classes.hpp"
#include "SDK/FN_Basic_StairF_parameters.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTileHalfTrimmed_structs.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTileHalfTrimmed_classes.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTileHalfTrimmed_parameters.hpp"
#include "SDK/FN_Sign_AddressNumber_5409_structs.hpp"
#include "SDK/FN_Sign_AddressNumber_5409_classes.hpp"
#include "SDK/FN_Sign_AddressNumber_5409_parameters.hpp"
#include "SDK/FN_SM_FireEscape_Walkway_Stair44_structs.hpp"
#include "SDK/FN_SM_FireEscape_Walkway_Stair44_classes.hpp"
#include "SDK/FN_SM_FireEscape_Walkway_Stair44_parameters.hpp"
#include "SDK/FN_WoodHouse_DoorSide_OneSideTrim_structs.hpp"
#include "SDK/FN_WoodHouse_DoorSide_OneSideTrim_classes.hpp"
#include "SDK/FN_WoodHouse_DoorSide_OneSideTrim_parameters.hpp"
#include "SDK/FN_Mailbox_01_structs.hpp"
#include "SDK/FN_Mailbox_01_classes.hpp"
#include "SDK/FN_Mailbox_01_parameters.hpp"
#include "SDK/FN_Farm_HayPulley_01_structs.hpp"
#include "SDK/FN_Farm_HayPulley_01_classes.hpp"
#include "SDK/FN_Farm_HayPulley_01_parameters.hpp"
#include "SDK/FN_LCD_Haystack_01_structs.hpp"
#include "SDK/FN_LCD_Haystack_01_classes.hpp"
#include "SDK/FN_LCD_Haystack_01_parameters.hpp"
#include "SDK/FN_WildWest_HayTough_01_structs.hpp"
#include "SDK/FN_WildWest_HayTough_01_classes.hpp"
#include "SDK/FN_WildWest_HayTough_01_parameters.hpp"
#include "SDK/FN_Plant_Corn01_structs.hpp"
#include "SDK/FN_Plant_Corn01_classes.hpp"
#include "SDK/FN_Plant_Corn01_parameters.hpp"
#include "SDK/FN_Haystack_Prop_structs.hpp"
#include "SDK/FN_Haystack_Prop_classes.hpp"
#include "SDK/FN_Haystack_Prop_parameters.hpp"
#include "SDK/FN_Prop_Sign_BikeXing_structs.hpp"
#include "SDK/FN_Prop_Sign_BikeXing_classes.hpp"
#include "SDK/FN_Prop_Sign_BikeXing_parameters.hpp"
#include "SDK/FN_Rural_House_Wall_2_structs.hpp"
#include "SDK/FN_Rural_House_Wall_2_classes.hpp"
#include "SDK/FN_Rural_House_Wall_2_parameters.hpp"
#include "SDK/FN_Fence_Rural_structs.hpp"
#include "SDK/FN_Fence_Rural_classes.hpp"
#include "SDK/FN_Fence_Rural_parameters.hpp"
#include "SDK/FN_Prop_WildWest_WoodenWindmill_01_structs.hpp"
#include "SDK/FN_Prop_WildWest_WoodenWindmill_01_classes.hpp"
#include "SDK/FN_Prop_WildWest_WoodenWindmill_01_parameters.hpp"
#include "SDK/FN_Chair_Kitchen01_structs.hpp"
#include "SDK/FN_Chair_Kitchen01_classes.hpp"
#include "SDK/FN_Chair_Kitchen01_parameters.hpp"
#include "SDK/FN_WildWest_Scarecrow_03_structs.hpp"
#include "SDK/FN_WildWest_Scarecrow_03_classes.hpp"
#include "SDK/FN_WildWest_Scarecrow_03_parameters.hpp"
#include "SDK/FN_Fence_Rural_End_structs.hpp"
#include "SDK/FN_Fence_Rural_End_classes.hpp"
#include "SDK/FN_Fence_Rural_End_parameters.hpp"
#include "SDK/FN_Prop_Boxes_01a_structs.hpp"
#include "SDK/FN_Prop_Boxes_01a_classes.hpp"
#include "SDK/FN_Prop_Boxes_01a_parameters.hpp"
#include "SDK/FN_Prop_StormDoor_01_structs.hpp"
#include "SDK/FN_Prop_StormDoor_01_classes.hpp"
#include "SDK/FN_Prop_StormDoor_01_parameters.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_WindowBoards02_structs.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_WindowBoards02_classes.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_WindowBoards02_parameters.hpp"
#include "SDK/FN_Fence_Rural_Broken_structs.hpp"
#include "SDK/FN_Fence_Rural_Broken_classes.hpp"
#include "SDK/FN_Fence_Rural_Broken_parameters.hpp"
#include "SDK/FN_SM_Farm_StablesWall01_structs.hpp"
#include "SDK/FN_SM_Farm_StablesWall01_classes.hpp"
#include "SDK/FN_SM_Farm_StablesWall01_parameters.hpp"
#include "SDK/FN_Tiered_Short_CraftingIngredients_3_Parent_structs.hpp"
#include "SDK/FN_Tiered_Short_CraftingIngredients_3_Parent_classes.hpp"
#include "SDK/FN_Tiered_Short_CraftingIngredients_3_Parent_parameters.hpp"
#include "SDK/FN_LF_3x3_Parent_structs.hpp"
#include "SDK/FN_LF_3x3_Parent_classes.hpp"
#include "SDK/FN_LF_3x3_Parent_parameters.hpp"
#include "SDK/FN_SM_Farm_StablesWall03_structs.hpp"
#include "SDK/FN_SM_Farm_StablesWall03_classes.hpp"
#include "SDK/FN_SM_Farm_StablesWall03_parameters.hpp"
#include "SDK/FN_Bedroom_Nightstand_02_structs.hpp"
#include "SDK/FN_Bedroom_Nightstand_02_classes.hpp"
#include "SDK/FN_Bedroom_Nightstand_02_parameters.hpp"
#include "SDK/FN_Farm_WeatherVane_01_structs.hpp"
#include "SDK/FN_Farm_WeatherVane_01_classes.hpp"
#include "SDK/FN_Farm_WeatherVane_01_parameters.hpp"
#include "SDK/FN_Prop_Church_Steeple_2_structs.hpp"
#include "SDK/FN_Prop_Church_Steeple_2_classes.hpp"
#include "SDK/FN_Prop_Church_Steeple_2_parameters.hpp"
#include "SDK/FN_Farm_Silo_Top_structs.hpp"
#include "SDK/FN_Farm_Silo_Top_classes.hpp"
#include "SDK/FN_Farm_Silo_Top_parameters.hpp"
#include "SDK/FN_Plant_Corn03_structs.hpp"
#include "SDK/FN_Plant_Corn03_classes.hpp"
#include "SDK/FN_Plant_Corn03_parameters.hpp"
#include "SDK/FN_Plant_Flowerbed_05_structs.hpp"
#include "SDK/FN_Plant_Flowerbed_05_classes.hpp"
#include "SDK/FN_Plant_Flowerbed_05_parameters.hpp"
#include "SDK/FN_Chimney_Narrow_Middle_structs.hpp"
#include "SDK/FN_Chimney_Narrow_Middle_classes.hpp"
#include "SDK/FN_Chimney_Narrow_Middle_parameters.hpp"
#include "SDK/FN_HousePile_05_structs.hpp"
#include "SDK/FN_HousePile_05_classes.hpp"
#include "SDK/FN_HousePile_05_parameters.hpp"
#include "SDK/FN_Tree_Medium_01_structs.hpp"
#include "SDK/FN_Tree_Medium_01_classes.hpp"
#include "SDK/FN_Tree_Medium_01_parameters.hpp"
#include "SDK/FN_HousePile_06_structs.hpp"
#include "SDK/FN_HousePile_06_classes.hpp"
#include "SDK/FN_HousePile_06_parameters.hpp"
#include "SDK/FN_Rural_House_Wall_7_structs.hpp"
#include "SDK/FN_Rural_House_Wall_7_classes.hpp"
#include "SDK/FN_Rural_House_Wall_7_parameters.hpp"
#include "SDK/FN_Plant_Flowerbed_11_structs.hpp"
#include "SDK/FN_Plant_Flowerbed_11_classes.hpp"
#include "SDK/FN_Plant_Flowerbed_11_parameters.hpp"
#include "SDK/FN_Farm_Silo_Mid_structs.hpp"
#include "SDK/FN_Farm_Silo_Mid_classes.hpp"
#include "SDK/FN_Farm_Silo_Mid_parameters.hpp"
#include "SDK/FN_LCD_JunkPile_01c_structs.hpp"
#include "SDK/FN_LCD_JunkPile_01c_classes.hpp"
#include "SDK/FN_LCD_JunkPile_01c_parameters.hpp"
#include "SDK/FN_Bedroom_EndTable_01_structs.hpp"
#include "SDK/FN_Bedroom_EndTable_01_classes.hpp"
#include "SDK/FN_Bedroom_EndTable_01_parameters.hpp"
#include "SDK/FN_Kitchen_Cabinets_Island_01_structs.hpp"
#include "SDK/FN_Kitchen_Cabinets_Island_01_classes.hpp"
#include "SDK/FN_Kitchen_Cabinets_Island_01_parameters.hpp"
#include "SDK/FN_Prop_WildWest_RockingChair_01_structs.hpp"
#include "SDK/FN_Prop_WildWest_RockingChair_01_classes.hpp"
#include "SDK/FN_Prop_WildWest_RockingChair_01_parameters.hpp"
#include "SDK/FN_Rural_House_Wall_8_structs.hpp"
#include "SDK/FN_Rural_House_Wall_8_classes.hpp"
#include "SDK/FN_Rural_House_Wall_8_parameters.hpp"
#include "SDK/FN_Rural_House_Wall_4_structs.hpp"
#include "SDK/FN_Rural_House_Wall_4_classes.hpp"
#include "SDK/FN_Rural_House_Wall_4_parameters.hpp"
#include "SDK/FN_Athena_SUB_5x5_PizzaPlace_a_248_structs.hpp"
#include "SDK/FN_Athena_SUB_5x5_PizzaPlace_a_248_classes.hpp"
#include "SDK/FN_Athena_SUB_5x5_PizzaPlace_a_248_parameters.hpp"
#include "SDK/FN_Athena_SUB_3x3_TacoStand_a_249_structs.hpp"
#include "SDK/FN_Athena_SUB_3x3_TacoStand_a_249_classes.hpp"
#include "SDK/FN_Athena_SUB_3x3_TacoStand_a_249_parameters.hpp"
#include "SDK/FN_Athena_SUB_5x5_House_a_255_structs.hpp"
#include "SDK/FN_Athena_SUB_5x5_House_a_255_classes.hpp"
#include "SDK/FN_Athena_SUB_5x5_House_a_255_parameters.hpp"
#include "SDK/FN_Athena_IND_5x5_Factory_b_271_structs.hpp"
#include "SDK/FN_Athena_IND_5x5_Factory_b_271_classes.hpp"
#include "SDK/FN_Athena_IND_5x5_Factory_b_271_parameters.hpp"
#include "SDK/FN_Gothic_Floor_structs.hpp"
#include "SDK/FN_Gothic_Floor_classes.hpp"
#include "SDK/FN_Gothic_Floor_parameters.hpp"
#include "SDK/FN_Construction_Roof_Wall_01_structs.hpp"
#include "SDK/FN_Construction_Roof_Wall_01_classes.hpp"
#include "SDK/FN_Construction_Roof_Wall_01_parameters.hpp"
#include "SDK/FN_Factory_Wall_13_structs.hpp"
#include "SDK/FN_Factory_Wall_13_classes.hpp"
#include "SDK/FN_Factory_Wall_13_parameters.hpp"
#include "SDK/FN_Rural_House_Rail_1_Full_structs.hpp"
#include "SDK/FN_Rural_House_Rail_1_Full_classes.hpp"
#include "SDK/FN_Rural_House_Rail_1_Full_parameters.hpp"
#include "SDK/FN_Building_Corner_G_structs.hpp"
#include "SDK/FN_Building_Corner_G_classes.hpp"
#include "SDK/FN_Building_Corner_G_parameters.hpp"
#include "SDK/FN_Kitchen_Cabinets_Counter_Corner_01_structs.hpp"
#include "SDK/FN_Kitchen_Cabinets_Counter_Corner_01_classes.hpp"
#include "SDK/FN_Kitchen_Cabinets_Counter_Corner_01_parameters.hpp"
#include "SDK/FN_Kitchen_Cabinets_Sink_Corner_01_structs.hpp"
#include "SDK/FN_Kitchen_Cabinets_Sink_Corner_01_classes.hpp"
#include "SDK/FN_Kitchen_Cabinets_Sink_Corner_01_parameters.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge02_structs.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge02_classes.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge02_parameters.hpp"
#include "SDK/FN_Tree_Small_3_structs.hpp"
#include "SDK/FN_Tree_Small_3_classes.hpp"
#include "SDK/FN_Tree_Small_3_parameters.hpp"
#include "SDK/FN_LCD_JunkPile_02a_structs.hpp"
#include "SDK/FN_LCD_JunkPile_02a_classes.hpp"
#include "SDK/FN_LCD_JunkPile_02a_parameters.hpp"
#include "SDK/FN_Storage_Metal_Crate_01_structs.hpp"
#include "SDK/FN_Storage_Metal_Crate_01_classes.hpp"
#include "SDK/FN_Storage_Metal_Crate_01_parameters.hpp"
#include "SDK/FN_Crypt_Dust_02_structs.hpp"
#include "SDK/FN_Crypt_Dust_02_classes.hpp"
#include "SDK/FN_Crypt_Dust_02_parameters.hpp"
#include "SDK/FN_Factory_Wall_9_structs.hpp"
#include "SDK/FN_Factory_Wall_9_classes.hpp"
#include "SDK/FN_Factory_Wall_9_parameters.hpp"
#include "SDK/FN_Fence_Picket01_Doorway2a_structs.hpp"
#include "SDK/FN_Fence_Picket01_Doorway2a_classes.hpp"
#include "SDK/FN_Fence_Picket01_Doorway2a_parameters.hpp"
#include "SDK/FN_Fence_Picket01_Half_a_structs.hpp"
#include "SDK/FN_Fence_Picket01_Half_a_classes.hpp"
#include "SDK/FN_Fence_Picket01_Half_a_parameters.hpp"
#include "SDK/FN_Kitchen_Shelf_structs.hpp"
#include "SDK/FN_Kitchen_Shelf_classes.hpp"
#include "SDK/FN_Kitchen_Shelf_parameters.hpp"
#include "SDK/FN_Kitchen_Cabinets_Counter_01_structs.hpp"
#include "SDK/FN_Kitchen_Cabinets_Counter_01_classes.hpp"
#include "SDK/FN_Kitchen_Cabinets_Counter_01_parameters.hpp"
#include "SDK/FN_Kitchen_Cabinets_Wall_01_structs.hpp"
#include "SDK/FN_Kitchen_Cabinets_Wall_01_classes.hpp"
#include "SDK/FN_Kitchen_Cabinets_Wall_01_parameters.hpp"
#include "SDK/FN_Kitchen_Cabinets_Wall_Corner_01_structs.hpp"
#include "SDK/FN_Kitchen_Cabinets_Wall_Corner_01_classes.hpp"
#include "SDK/FN_Kitchen_Cabinets_Wall_Corner_01_parameters.hpp"
#include "SDK/FN_Kitchen_Cabinets_Wall_Long_structs.hpp"
#include "SDK/FN_Kitchen_Cabinets_Wall_Long_classes.hpp"
#include "SDK/FN_Kitchen_Cabinets_Wall_Long_parameters.hpp"
#include "SDK/FN_Living_BookShelves_04_structs.hpp"
#include "SDK/FN_Living_BookShelves_04_classes.hpp"
#include "SDK/FN_Living_BookShelves_04_parameters.hpp"
#include "SDK/FN_Rural_House_Wall_3_structs.hpp"
#include "SDK/FN_Rural_House_Wall_3_classes.hpp"
#include "SDK/FN_Rural_House_Wall_3_parameters.hpp"
#include "SDK/FN_BrickSimple_Window_E_structs.hpp"
#include "SDK/FN_BrickSimple_Window_E_classes.hpp"
#include "SDK/FN_BrickSimple_Window_E_parameters.hpp"
#include "SDK/FN_WoodHouse_Archway_NoTrim_structs.hpp"
#include "SDK/FN_WoodHouse_Archway_NoTrim_classes.hpp"
#include "SDK/FN_WoodHouse_Archway_NoTrim_parameters.hpp"
#include "SDK/FN_WoodHouse_BayWindow_structs.hpp"
#include "SDK/FN_WoodHouse_BayWindow_classes.hpp"
#include "SDK/FN_WoodHouse_BayWindow_parameters.hpp"
#include "SDK/FN_WoodHouse_DoorCenter_OneSideTrim_structs.hpp"
#include "SDK/FN_WoodHouse_DoorCenter_OneSideTrim_classes.hpp"
#include "SDK/FN_WoodHouse_DoorCenter_OneSideTrim_parameters.hpp"
#include "SDK/FN_WoodHouse_SolidWall_OneSideTrim_structs.hpp"
#include "SDK/FN_WoodHouse_SolidWall_OneSideTrim_classes.hpp"
#include "SDK/FN_WoodHouse_SolidWall_OneSideTrim_parameters.hpp"
#include "SDK/FN_WoodHouse_WindowCenter_NoTrim_structs.hpp"
#include "SDK/FN_WoodHouse_WindowCenter_NoTrim_classes.hpp"
#include "SDK/FN_WoodHouse_WindowCenter_NoTrim_parameters.hpp"
#include "SDK/FN_Factory_1d_Half_structs.hpp"
#include "SDK/FN_Factory_1d_Half_classes.hpp"
#include "SDK/FN_Factory_1d_Half_parameters.hpp"
#include "SDK/FN_WoodHouse_Windows_NoTrim_structs.hpp"
#include "SDK/FN_WoodHouse_Windows_NoTrim_classes.hpp"
#include "SDK/FN_WoodHouse_Windows_NoTrim_parameters.hpp"
#include "SDK/FN_StoreGeneric_DoorC_structs.hpp"
#include "SDK/FN_StoreGeneric_DoorC_classes.hpp"
#include "SDK/FN_StoreGeneric_DoorC_parameters.hpp"
#include "SDK/FN_Kitchen_Modern_Cabinets03_structs.hpp"
#include "SDK/FN_Kitchen_Modern_Cabinets03_classes.hpp"
#include "SDK/FN_Kitchen_Modern_Cabinets03_parameters.hpp"
#include "SDK/FN_SM_Wood01_InteriorPillarSingle_structs.hpp"
#include "SDK/FN_SM_Wood01_InteriorPillarSingle_classes.hpp"
#include "SDK/FN_SM_Wood01_InteriorPillarSingle_parameters.hpp"
#include "SDK/FN_Kitchen_StoveHood_02_structs.hpp"
#include "SDK/FN_Kitchen_StoveHood_02_classes.hpp"
#include "SDK/FN_Kitchen_StoveHood_02_parameters.hpp"
#include "SDK/FN_StoreGeneric_DoorS_structs.hpp"
#include "SDK/FN_StoreGeneric_DoorS_classes.hpp"
#include "SDK/FN_StoreGeneric_DoorS_parameters.hpp"
#include "SDK/FN_ShutterHouse_BayWindow_02_structs.hpp"
#include "SDK/FN_ShutterHouse_BayWindow_02_classes.hpp"
#include "SDK/FN_ShutterHouse_BayWindow_02_parameters.hpp"
#include "SDK/FN_ShutterHouse_DoorC_Des_02_structs.hpp"
#include "SDK/FN_ShutterHouse_DoorC_Des_02_classes.hpp"
#include "SDK/FN_ShutterHouse_DoorC_Des_02_parameters.hpp"
#include "SDK/FN_ShutterHouse_DoorSide_structs.hpp"
#include "SDK/FN_ShutterHouse_DoorSide_classes.hpp"
#include "SDK/FN_ShutterHouse_DoorSide_parameters.hpp"
#include "SDK/FN_ShutterHouse_Windows_02_structs.hpp"
#include "SDK/FN_ShutterHouse_Windows_02_classes.hpp"
#include "SDK/FN_ShutterHouse_Windows_02_parameters.hpp"
#include "SDK/FN_Utility_WasherDryer_02_structs.hpp"
#include "SDK/FN_Utility_WasherDryer_02_classes.hpp"
#include "SDK/FN_Utility_WasherDryer_02_parameters.hpp"
#include "SDK/FN_WaterHeater02_structs.hpp"
#include "SDK/FN_WaterHeater02_classes.hpp"
#include "SDK/FN_WaterHeater02_parameters.hpp"
#include "SDK/FN_WildWest_Scarecrow_02_structs.hpp"
#include "SDK/FN_WildWest_Scarecrow_02_classes.hpp"
#include "SDK/FN_WildWest_Scarecrow_02_parameters.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_Dmg1_structs.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_Dmg1_classes.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_Dmg1_parameters.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_Dmg2_structs.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_Dmg2_classes.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_Dmg2_parameters.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_Dmg3_structs.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_Dmg3_classes.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_Dmg3_parameters.hpp"
#include "SDK/FN_ShutterHouse_WindowSide_02_structs.hpp"
#include "SDK/FN_ShutterHouse_WindowSide_02_classes.hpp"
#include "SDK/FN_ShutterHouse_WindowSide_02_parameters.hpp"
#include "SDK/FN_Tree_Forest_Bush_structs.hpp"
#include "SDK/FN_Tree_Forest_Bush_classes.hpp"
#include "SDK/FN_Tree_Forest_Bush_parameters.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Cap_structs.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Cap_classes.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Cap_parameters.hpp"
#include "SDK/FN_DestroyedHouse_RoofWall_structs.hpp"
#include "SDK/FN_DestroyedHouse_RoofWall_classes.hpp"
#include "SDK/FN_DestroyedHouse_RoofWall_parameters.hpp"
#include "SDK/FN_DestroyedHouse_Windows_03_structs.hpp"
#include "SDK/FN_DestroyedHouse_Windows_03_classes.hpp"
#include "SDK/FN_DestroyedHouse_Windows_03_parameters.hpp"
#include "SDK/FN_ShutterHouse_DoorS_structs.hpp"
#include "SDK/FN_ShutterHouse_DoorS_classes.hpp"
#include "SDK/FN_ShutterHouse_DoorS_parameters.hpp"
#include "SDK/FN_ShutterHouse_Solid_structs.hpp"
#include "SDK/FN_ShutterHouse_Solid_classes.hpp"
#include "SDK/FN_ShutterHouse_Solid_parameters.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_01_structs.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_01_classes.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_01_parameters.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_02_structs.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_02_classes.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_02_parameters.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_03_structs.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_03_classes.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_03_parameters.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_05_structs.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_05_classes.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_05_parameters.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_06_structs.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_06_classes.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_06_parameters.hpp"
#include "SDK/FN_ShutterHouse_WindowC_structs.hpp"
#include "SDK/FN_ShutterHouse_WindowC_classes.hpp"
#include "SDK/FN_ShutterHouse_WindowC_parameters.hpp"
#include "SDK/FN_Durr_CounterCorner_structs.hpp"
#include "SDK/FN_Durr_CounterCorner_classes.hpp"
#include "SDK/FN_Durr_CounterCorner_parameters.hpp"
#include "SDK/FN_Chair_FastFoodStool_structs.hpp"
#include "SDK/FN_Chair_FastFoodStool_classes.hpp"
#include "SDK/FN_Chair_FastFoodStool_parameters.hpp"
#include "SDK/FN_ShutterHouse_WindowSide_Des_structs.hpp"
#include "SDK/FN_ShutterHouse_WindowSide_Des_classes.hpp"
#include "SDK/FN_ShutterHouse_WindowSide_Des_parameters.hpp"
#include "SDK/FN_Plant_PalmBush1_structs.hpp"
#include "SDK/FN_Plant_PalmBush1_classes.hpp"
#include "SDK/FN_Plant_PalmBush1_parameters.hpp"
#include "SDK/FN_SM_Wood01_InteriorPillarSingle_ThreeQuarters_structs.hpp"
#include "SDK/FN_SM_Wood01_InteriorPillarSingle_ThreeQuarters_classes.hpp"
#include "SDK/FN_SM_Wood01_InteriorPillarSingle_ThreeQuarters_parameters.hpp"
#include "SDK/FN_Kitchen_Knives_structs.hpp"
#include "SDK/FN_Kitchen_Knives_classes.hpp"
#include "SDK/FN_Kitchen_Knives_parameters.hpp"
#include "SDK/FN_Prop_Doghouse_structs.hpp"
#include "SDK/FN_Prop_Doghouse_classes.hpp"
#include "SDK/FN_Prop_Doghouse_parameters.hpp"
#include "SDK/FN_StoreGeneric_Solid_structs.hpp"
#include "SDK/FN_StoreGeneric_Solid_classes.hpp"
#include "SDK/FN_StoreGeneric_Solid_parameters.hpp"
#include "SDK/FN_WoodHouse_Archway_structs.hpp"
#include "SDK/FN_WoodHouse_Archway_classes.hpp"
#include "SDK/FN_WoodHouse_Archway_parameters.hpp"
#include "SDK/FN_WoodHouse_DoorCenter_TwoSideTrim_structs.hpp"
#include "SDK/FN_WoodHouse_DoorCenter_TwoSideTrim_classes.hpp"
#include "SDK/FN_WoodHouse_DoorCenter_TwoSideTrim_parameters.hpp"
#include "SDK/FN_WoodHouse_DoorWindow_NoTrim_structs.hpp"
#include "SDK/FN_WoodHouse_DoorWindow_NoTrim_classes.hpp"
#include "SDK/FN_WoodHouse_DoorWindow_NoTrim_parameters.hpp"
#include "SDK/FN_WoodHouse_SolidWall_TwoSideTrim_structs.hpp"
#include "SDK/FN_WoodHouse_SolidWall_TwoSideTrim_classes.hpp"
#include "SDK/FN_WoodHouse_SolidWall_TwoSideTrim_parameters.hpp"
#include "SDK/FN_Kitchen_Cabinets_Counter_Small_01_structs.hpp"
#include "SDK/FN_Kitchen_Cabinets_Counter_Small_01_classes.hpp"
#include "SDK/FN_Kitchen_Cabinets_Counter_Small_01_parameters.hpp"
#include "SDK/FN_Kitchen_Cabinets_Sink_01_structs.hpp"
#include "SDK/FN_Kitchen_Cabinets_Sink_01_classes.hpp"
#include "SDK/FN_Kitchen_Cabinets_Sink_01_parameters.hpp"
#include "SDK/FN_SM_Wood01_InteriorPillarSingle_Half_structs.hpp"
#include "SDK/FN_SM_Wood01_InteriorPillarSingle_Half_classes.hpp"
#include "SDK/FN_SM_Wood01_InteriorPillarSingle_Half_parameters.hpp"
#include "SDK/FN_WoodHouse_Windows_OneSideTrim_structs.hpp"
#include "SDK/FN_WoodHouse_Windows_OneSideTrim_classes.hpp"
#include "SDK/FN_WoodHouse_Windows_OneSideTrim_parameters.hpp"
#include "SDK/FN_Construction_Wall_Arch_02_structs.hpp"
#include "SDK/FN_Construction_Wall_Arch_02_classes.hpp"
#include "SDK/FN_Construction_Wall_Arch_02_parameters.hpp"
#include "SDK/FN_Living_EndTable_02_structs.hpp"
#include "SDK/FN_Living_EndTable_02_classes.hpp"
#include "SDK/FN_Living_EndTable_02_parameters.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_Turn_NoTrim_structs.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_Turn_NoTrim_classes.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_Turn_NoTrim_parameters.hpp"
#include "SDK/FN_WoodHouse_DoorWindow_OneSideTrim_structs.hpp"
#include "SDK/FN_WoodHouse_DoorWindow_OneSideTrim_classes.hpp"
#include "SDK/FN_WoodHouse_DoorWindow_OneSideTrim_parameters.hpp"
#include "SDK/FN_ShutterHouse_DoorC_structs.hpp"
#include "SDK/FN_ShutterHouse_DoorC_classes.hpp"
#include "SDK/FN_ShutterHouse_DoorC_parameters.hpp"
#include "SDK/FN_ShutterHouse_WindowC_03_structs.hpp"
#include "SDK/FN_ShutterHouse_WindowC_03_classes.hpp"
#include "SDK/FN_ShutterHouse_WindowC_03_parameters.hpp"
#include "SDK/FN_DogHouse_01_structs.hpp"
#include "SDK/FN_DogHouse_01_classes.hpp"
#include "SDK/FN_DogHouse_01_parameters.hpp"
#include "SDK/FN_LCD_RubblePile_03_structs.hpp"
#include "SDK/FN_LCD_RubblePile_03_classes.hpp"
#include "SDK/FN_LCD_RubblePile_03_parameters.hpp"
#include "SDK/FN_Living_BookShelves_03_structs.hpp"
#include "SDK/FN_Living_BookShelves_03_classes.hpp"
#include "SDK/FN_Living_BookShelves_03_parameters.hpp"
#include "SDK/FN_Living_Table_Coffee_02_structs.hpp"
#include "SDK/FN_Living_Table_Coffee_02_classes.hpp"
#include "SDK/FN_Living_Table_Coffee_02_parameters.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_structs.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_classes.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_parameters.hpp"
#include "SDK/FN_ShutterHouse_Windows_Des_structs.hpp"
#include "SDK/FN_ShutterHouse_Windows_Des_classes.hpp"
#include "SDK/FN_ShutterHouse_Windows_Des_parameters.hpp"
#include "SDK/FN_Fence_Picket01_Half_c_structs.hpp"
#include "SDK/FN_Fence_Picket01_Half_c_classes.hpp"
#include "SDK/FN_Fence_Picket01_Half_c_parameters.hpp"
#include "SDK/FN_Art_Painting06_structs.hpp"
#include "SDK/FN_Art_Painting06_classes.hpp"
#include "SDK/FN_Art_Painting06_parameters.hpp"
#include "SDK/FN_FORT_Roof_Straight_Dormer2_structs.hpp"
#include "SDK/FN_FORT_Roof_Straight_Dormer2_classes.hpp"
#include "SDK/FN_FORT_Roof_Straight_Dormer2_parameters.hpp"
#include "SDK/FN_Rural_House_Garage_3_structs.hpp"
#include "SDK/FN_Rural_House_Garage_3_classes.hpp"
#include "SDK/FN_Rural_House_Garage_3_parameters.hpp"
#include "SDK/FN_Kitchen_Modern_Counter02_structs.hpp"
#include "SDK/FN_Kitchen_Modern_Counter02_classes.hpp"
#include "SDK/FN_Kitchen_Modern_Counter02_parameters.hpp"
#include "SDK/FN_Kitchen_Pans_structs.hpp"
#include "SDK/FN_Kitchen_Pans_classes.hpp"
#include "SDK/FN_Kitchen_Pans_parameters.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_Turn_structs.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_Turn_classes.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_Turn_parameters.hpp"
#include "SDK/FN_Rural_House_Wall_11_structs.hpp"
#include "SDK/FN_Rural_House_Wall_11_classes.hpp"
#include "SDK/FN_Rural_House_Wall_11_parameters.hpp"
#include "SDK/FN_Rural_House_Wall_9_structs.hpp"
#include "SDK/FN_Rural_House_Wall_9_classes.hpp"
#include "SDK/FN_Rural_House_Wall_9_parameters.hpp"
#include "SDK/FN_ShutterHouse_BayWindow_01_structs.hpp"
#include "SDK/FN_ShutterHouse_BayWindow_01_classes.hpp"
#include "SDK/FN_ShutterHouse_BayWindow_01_parameters.hpp"
#include "SDK/FN_ShutterHouse_DoorC_02_structs.hpp"
#include "SDK/FN_ShutterHouse_DoorC_02_classes.hpp"
#include "SDK/FN_ShutterHouse_DoorC_02_parameters.hpp"
#include "SDK/FN_ShutterHouse_DoorC_Des_structs.hpp"
#include "SDK/FN_ShutterHouse_DoorC_Des_classes.hpp"
#include "SDK/FN_ShutterHouse_DoorC_Des_parameters.hpp"
#include "SDK/FN_WaterTower01_structs.hpp"
#include "SDK/FN_WaterTower01_classes.hpp"
#include "SDK/FN_WaterTower01_parameters.hpp"
#include "SDK/FN_WaterTower02_structs.hpp"
#include "SDK/FN_WaterTower02_classes.hpp"
#include "SDK/FN_WaterTower02_parameters.hpp"
#include "SDK/FN_FORT_Pizza_Corner_A_structs.hpp"
#include "SDK/FN_FORT_Pizza_Corner_A_classes.hpp"
#include "SDK/FN_FORT_Pizza_Corner_A_parameters.hpp"
#include "SDK/FN_FORT_Pizza_Corner_B_structs.hpp"
#include "SDK/FN_FORT_Pizza_Corner_B_classes.hpp"
#include "SDK/FN_FORT_Pizza_Corner_B_parameters.hpp"
#include "SDK/FN_SM_HighRise_Corner_structs.hpp"
#include "SDK/FN_SM_HighRise_Corner_classes.hpp"
#include "SDK/FN_SM_HighRise_Corner_parameters.hpp"
#include "SDK/FN_Commerce_Awning_4_Corner_structs.hpp"
#include "SDK/FN_Commerce_Awning_4_Corner_classes.hpp"
#include "SDK/FN_Commerce_Awning_4_Corner_parameters.hpp"
#include "SDK/FN_FORT_Pizza_Clown_structs.hpp"
#include "SDK/FN_FORT_Pizza_Clown_classes.hpp"
#include "SDK/FN_FORT_Pizza_Clown_parameters.hpp"
#include "SDK/FN_Mailbox_01a_structs.hpp"
#include "SDK/FN_Mailbox_01a_classes.hpp"
#include "SDK/FN_Mailbox_01a_parameters.hpp"
#include "SDK/FN_FORT_Pizza_Roof_CornerO_structs.hpp"
#include "SDK/FN_FORT_Pizza_Roof_CornerO_classes.hpp"
#include "SDK/FN_FORT_Pizza_Roof_CornerO_parameters.hpp"
#include "SDK/FN_Sign_Pizza_structs.hpp"
#include "SDK/FN_Sign_Pizza_classes.hpp"
#include "SDK/FN_Sign_Pizza_parameters.hpp"
#include "SDK/FN_ShutterHouse_WindowC_02_structs.hpp"
#include "SDK/FN_ShutterHouse_WindowC_02_classes.hpp"
#include "SDK/FN_ShutterHouse_WindowC_02_parameters.hpp"
#include "SDK/FN_Pizza_DoorC_structs.hpp"
#include "SDK/FN_Pizza_DoorC_classes.hpp"
#include "SDK/FN_Pizza_DoorC_parameters.hpp"
#include "SDK/FN_Pizza_Solid01_structs.hpp"
#include "SDK/FN_Pizza_Solid01_classes.hpp"
#include "SDK/FN_Pizza_Solid01_parameters.hpp"
#include "SDK/FN_Pizza_Window02_structs.hpp"
#include "SDK/FN_Pizza_Window02_classes.hpp"
#include "SDK/FN_Pizza_Window02_parameters.hpp"
#include "SDK/FN_Pizza_Windows01_structs.hpp"
#include "SDK/FN_Pizza_Windows01_classes.hpp"
#include "SDK/FN_Pizza_Windows01_parameters.hpp"
#include "SDK/FN_S_BuildingTrim_C_structs.hpp"
#include "SDK/FN_S_BuildingTrim_C_classes.hpp"
#include "SDK/FN_S_BuildingTrim_C_parameters.hpp"
#include "SDK/FN_Commerce_CheckoutCounter03_structs.hpp"
#include "SDK/FN_Commerce_CheckoutCounter03_classes.hpp"
#include "SDK/FN_Commerce_CheckoutCounter03_parameters.hpp"
#include "SDK/FN_WildWest_Scarecrow_01_structs.hpp"
#include "SDK/FN_WildWest_Scarecrow_01_classes.hpp"
#include "SDK/FN_WildWest_Scarecrow_01_parameters.hpp"
#include "SDK/FN_Pizza_Windows_Upper_structs.hpp"
#include "SDK/FN_Pizza_Windows_Upper_classes.hpp"
#include "SDK/FN_Pizza_Windows_Upper_parameters.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_MetalRailing01_structs.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_MetalRailing01_classes.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_MetalRailing01_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_F_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_F_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_F_parameters.hpp"
#include "SDK/FN_Girder04_structs.hpp"
#include "SDK/FN_Girder04_classes.hpp"
#include "SDK/FN_Girder04_parameters.hpp"
#include "SDK/FN_Office_Desk_02_structs.hpp"
#include "SDK/FN_Office_Desk_02_classes.hpp"
#include "SDK/FN_Office_Desk_02_parameters.hpp"
#include "SDK/FN_Sign_UsedCarDealer_structs.hpp"
#include "SDK/FN_Sign_UsedCarDealer_classes.hpp"
#include "SDK/FN_Sign_UsedCarDealer_parameters.hpp"
#include "SDK/FN_LCD_JunkPile_01b_structs.hpp"
#include "SDK/FN_LCD_JunkPile_01b_classes.hpp"
#include "SDK/FN_LCD_JunkPile_01b_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_A09_structs.hpp"
#include "SDK/FN_Fence_Chainlink_A09_classes.hpp"
#include "SDK/FN_Fence_Chainlink_A09_parameters.hpp"
#include "SDK/FN_WoodHouse_HalfWall_Special2_structs.hpp"
#include "SDK/FN_WoodHouse_HalfWall_Special2_classes.hpp"
#include "SDK/FN_WoodHouse_HalfWall_Special2_parameters.hpp"
#include "SDK/FN_Streetprop_Contsruction_02b_structs.hpp"
#include "SDK/FN_Streetprop_Contsruction_02b_classes.hpp"
#include "SDK/FN_Streetprop_Contsruction_02b_parameters.hpp"
#include "SDK/FN_SM_Trim_Corner_02_structs.hpp"
#include "SDK/FN_SM_Trim_Corner_02_classes.hpp"
#include "SDK/FN_SM_Trim_Corner_02_parameters.hpp"
#include "SDK/FN_Living_Couch_07_structs.hpp"
#include "SDK/FN_Living_Couch_07_classes.hpp"
#include "SDK/FN_Living_Couch_07_parameters.hpp"
#include "SDK/FN_Sign_PassNGas_structs.hpp"
#include "SDK/FN_Sign_PassNGas_classes.hpp"
#include "SDK/FN_Sign_PassNGas_parameters.hpp"
#include "SDK/FN_Factory_Trim_19a_structs.hpp"
#include "SDK/FN_Factory_Trim_19a_classes.hpp"
#include "SDK/FN_Factory_Trim_19a_parameters.hpp"
#include "SDK/FN_Pipe03_structs.hpp"
#include "SDK/FN_Pipe03_classes.hpp"
#include "SDK/FN_Pipe03_parameters.hpp"
#include "SDK/FN_GE_Map_Fortitude_To_Health_structs.hpp"
#include "SDK/FN_GE_Map_Fortitude_To_Health_classes.hpp"
#include "SDK/FN_GE_Map_Fortitude_To_Health_parameters.hpp"
#include "SDK/FN_GE_Map_Offense_To_WeaponDamage_structs.hpp"
#include "SDK/FN_GE_Map_Offense_To_WeaponDamage_classes.hpp"
#include "SDK/FN_GE_Map_Offense_To_WeaponDamage_parameters.hpp"
#include "SDK/FN_LCD_JunkPile_01_structs.hpp"
#include "SDK/FN_LCD_JunkPile_01_classes.hpp"
#include "SDK/FN_LCD_JunkPile_01_parameters.hpp"
#include "SDK/FN_LCD_JunkPile_02b_structs.hpp"
#include "SDK/FN_LCD_JunkPile_02b_classes.hpp"
#include "SDK/FN_LCD_JunkPile_02b_parameters.hpp"
#include "SDK/FN_Prop_WildWest_Crate_01_structs.hpp"
#include "SDK/FN_Prop_WildWest_Crate_01_classes.hpp"
#include "SDK/FN_Prop_WildWest_Crate_01_parameters.hpp"
#include "SDK/FN_Prop_WildWest_Crate_02_structs.hpp"
#include "SDK/FN_Prop_WildWest_Crate_02_classes.hpp"
#include "SDK/FN_Prop_WildWest_Crate_02_parameters.hpp"
#include "SDK/FN_Storage_OilDrum_Barrel_3_structs.hpp"
#include "SDK/FN_Storage_OilDrum_Barrel_3_classes.hpp"
#include "SDK/FN_Storage_OilDrum_Barrel_3_parameters.hpp"
#include "SDK/FN_GE_Map_Athena_StartingShields_structs.hpp"
#include "SDK/FN_GE_Map_Athena_StartingShields_classes.hpp"
#include "SDK/FN_GE_Map_Athena_StartingShields_parameters.hpp"
#include "SDK/FN_FORT_Stop_Sign_structs.hpp"
#include "SDK/FN_FORT_Stop_Sign_classes.hpp"
#include "SDK/FN_FORT_Stop_Sign_parameters.hpp"
#include "SDK/FN_WoodMines_WindowC_structs.hpp"
#include "SDK/FN_WoodMines_WindowC_classes.hpp"
#include "SDK/FN_WoodMines_WindowC_parameters.hpp"
#include "SDK/FN_DestroyedHouse_Archway_structs.hpp"
#include "SDK/FN_DestroyedHouse_Archway_classes.hpp"
#include "SDK/FN_DestroyedHouse_Archway_parameters.hpp"
#include "SDK/FN_Tiered_Chest_6_Parent_structs.hpp"
#include "SDK/FN_Tiered_Chest_6_Parent_classes.hpp"
#include "SDK/FN_Tiered_Chest_6_Parent_parameters.hpp"
#include "SDK/FN_Storage_Crate06_structs.hpp"
#include "SDK/FN_Storage_Crate06_classes.hpp"
#include "SDK/FN_Storage_Crate06_parameters.hpp"
#include "SDK/FN_GE_SquadBuff_structs.hpp"
#include "SDK/FN_GE_SquadBuff_classes.hpp"
#include "SDK/FN_GE_SquadBuff_parameters.hpp"
#include "SDK/FN_Dartboard_structs.hpp"
#include "SDK/FN_Dartboard_classes.hpp"
#include "SDK/FN_Dartboard_parameters.hpp"
#include "SDK/FN_B_CameraRainDrops_01_structs.hpp"
#include "SDK/FN_B_CameraRainDrops_01_classes.hpp"
#include "SDK/FN_B_CameraRainDrops_01_parameters.hpp"
#include "SDK/FN_SM_Industrial_Pipe_01_structs.hpp"
#include "SDK/FN_SM_Industrial_Pipe_01_classes.hpp"
#include "SDK/FN_SM_Industrial_Pipe_01_parameters.hpp"
#include "SDK/FN_SM_Industrial_Pipe_03_structs.hpp"
#include "SDK/FN_SM_Industrial_Pipe_03_classes.hpp"
#include "SDK/FN_SM_Industrial_Pipe_03_parameters.hpp"
#include "SDK/FN_TrashPile_02_structs.hpp"
#include "SDK/FN_TrashPile_02_classes.hpp"
#include "SDK/FN_TrashPile_02_parameters.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_02_structs.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_02_classes.hpp"
#include "SDK/FN_DestroyedHouse_Roof_Straight_02_parameters.hpp"
#include "SDK/FN_BP_AmbientSplineActor_structs.hpp"
#include "SDK/FN_BP_AmbientSplineActor_classes.hpp"
#include "SDK/FN_BP_AmbientSplineActor_parameters.hpp"
#include "SDK/FN_BP_AmbientSplineActor_River_structs.hpp"
#include "SDK/FN_BP_AmbientSplineActor_River_classes.hpp"
#include "SDK/FN_BP_AmbientSplineActor_River_parameters.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_04_structs.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_04_classes.hpp"
#include "SDK/FN_ShutterHouse_Solid_Des_04_parameters.hpp"
#include "SDK/FN_BP_AmbientLakeAudioActor_structs.hpp"
#include "SDK/FN_BP_AmbientLakeAudioActor_classes.hpp"
#include "SDK/FN_BP_AmbientLakeAudioActor_parameters.hpp"
#include "SDK/FN_CCGameplayEffects_structs.hpp"
#include "SDK/FN_CCGameplayEffects_classes.hpp"
#include "SDK/FN_CCGameplayEffects_parameters.hpp"
#include "SDK/FN_Construction_Wall_Solid_01_structs.hpp"
#include "SDK/FN_Construction_Wall_Solid_01_classes.hpp"
#include "SDK/FN_Construction_Wall_Solid_01_parameters.hpp"
#include "SDK/FN_Utility_WheelBarrow_structs.hpp"
#include "SDK/FN_Utility_WheelBarrow_classes.hpp"
#include "SDK/FN_Utility_WheelBarrow_parameters.hpp"
#include "SDK/FN_NavLink_StairT_structs.hpp"
#include "SDK/FN_NavLink_StairT_classes.hpp"
#include "SDK/FN_NavLink_StairT_parameters.hpp"
#include "SDK/FN_NavLink_StairR_structs.hpp"
#include "SDK/FN_NavLink_StairR_classes.hpp"
#include "SDK/FN_NavLink_StairR_parameters.hpp"
#include "SDK/FN_NavLink_FloorI_structs.hpp"
#include "SDK/FN_NavLink_FloorI_classes.hpp"
#include "SDK/FN_NavLink_FloorI_parameters.hpp"
#include "SDK/FN_NavLink_BalconyI_structs.hpp"
#include "SDK/FN_NavLink_BalconyI_classes.hpp"
#include "SDK/FN_NavLink_BalconyI_parameters.hpp"
#include "SDK/FN_NavLink_BalconyO_structs.hpp"
#include "SDK/FN_NavLink_BalconyO_classes.hpp"
#include "SDK/FN_NavLink_BalconyO_parameters.hpp"
#include "SDK/FN_NavLink_BalconyS_structs.hpp"
#include "SDK/FN_NavLink_BalconyS_classes.hpp"
#include "SDK/FN_NavLink_BalconyS_parameters.hpp"
#include "SDK/FN_NavLink_FloorS_structs.hpp"
#include "SDK/FN_NavLink_FloorS_classes.hpp"
#include "SDK/FN_NavLink_FloorS_parameters.hpp"
#include "SDK/FN_Prop_SmallChimney_01_structs.hpp"
#include "SDK/FN_Prop_SmallChimney_01_classes.hpp"
#include "SDK/FN_Prop_SmallChimney_01_parameters.hpp"
#include "SDK/FN_NavLink_FloorO_structs.hpp"
#include "SDK/FN_NavLink_FloorO_classes.hpp"
#include "SDK/FN_NavLink_FloorO_parameters.hpp"
#include "SDK/FN_NavLink_StairF_structs.hpp"
#include "SDK/FN_NavLink_StairF_classes.hpp"
#include "SDK/FN_NavLink_StairF_parameters.hpp"
#include "SDK/FN_NavLink_RoofS_structs.hpp"
#include "SDK/FN_NavLink_RoofS_classes.hpp"
#include "SDK/FN_NavLink_RoofS_parameters.hpp"
#include "SDK/FN_NavLink_StairW_structs.hpp"
#include "SDK/FN_NavLink_StairW_classes.hpp"
#include "SDK/FN_NavLink_StairW_parameters.hpp"
#include "SDK/FN_PlayerPawn_Athena_Generic_Parent_structs.hpp"
#include "SDK/FN_PlayerPawn_Athena_Generic_Parent_classes.hpp"
#include "SDK/FN_PlayerPawn_Athena_Generic_Parent_parameters.hpp"
#include "SDK/FN_LightningLSystem_structs.hpp"
#include "SDK/FN_LightningLSystem_classes.hpp"
#include "SDK/FN_LightningLSystem_parameters.hpp"
#include "SDK/FN_FortMetaNavWallAreaDef_structs.hpp"
#include "SDK/FN_FortMetaNavWallAreaDef_classes.hpp"
#include "SDK/FN_FortMetaNavWallAreaDef_parameters.hpp"
#include "SDK/FN_TODM_A_structs.hpp"
#include "SDK/FN_TODM_A_classes.hpp"
#include "SDK/FN_TODM_A_parameters.hpp"
#include "SDK/FN_TODM_A_Child_Athena_structs.hpp"
#include "SDK/FN_TODM_A_Child_Athena_classes.hpp"
#include "SDK/FN_TODM_A_Child_Athena_parameters.hpp"
#include "SDK/FN_BP_Bus_Lobby_SoundController_structs.hpp"
#include "SDK/FN_BP_Bus_Lobby_SoundController_classes.hpp"
#include "SDK/FN_BP_Bus_Lobby_SoundController_parameters.hpp"
#include "SDK/FN_SM_FloorBigRampSplit_01_structs.hpp"
#include "SDK/FN_SM_FloorBigRampSplit_01_classes.hpp"
#include "SDK/FN_SM_FloorBigRampSplit_01_parameters.hpp"
#include "SDK/FN_SM_FloorBigRampSplit_02_structs.hpp"
#include "SDK/FN_SM_FloorBigRampSplit_02_classes.hpp"
#include "SDK/FN_SM_FloorBigRampSplit_02_parameters.hpp"
#include "SDK/FN_DestroyedHouse_Windows_02_structs.hpp"
#include "SDK/FN_DestroyedHouse_Windows_02_classes.hpp"
#include "SDK/FN_DestroyedHouse_Windows_02_parameters.hpp"
#include "SDK/FN_BrickSimple_Pillar_structs.hpp"
#include "SDK/FN_BrickSimple_Pillar_classes.hpp"
#include "SDK/FN_BrickSimple_Pillar_parameters.hpp"
#include "SDK/FN_Factory_6_structs.hpp"
#include "SDK/FN_Factory_6_classes.hpp"
#include "SDK/FN_Factory_6_parameters.hpp"
#include "SDK/FN_Athena_POI_ToiletFactory_001_structs.hpp"
#include "SDK/FN_Athena_POI_ToiletFactory_001_classes.hpp"
#include "SDK/FN_Athena_POI_ToiletFactory_001_parameters.hpp"
#include "SDK/FN_Basic_Corner_01f_Quarter_structs.hpp"
#include "SDK/FN_Basic_Corner_01f_Quarter_classes.hpp"
#include "SDK/FN_Basic_Corner_01f_Quarter_parameters.hpp"
#include "SDK/FN_Bathroom_ToiletSeat02_structs.hpp"
#include "SDK/FN_Bathroom_ToiletSeat02_classes.hpp"
#include "SDK/FN_Bathroom_ToiletSeat02_parameters.hpp"
#include "SDK/FN_Bathroom_ToiletTank01_structs.hpp"
#include "SDK/FN_Bathroom_ToiletTank01_classes.hpp"
#include "SDK/FN_Bathroom_ToiletTank01_parameters.hpp"
#include "SDK/FN_Bathroom_ToiletWithTank01_structs.hpp"
#include "SDK/FN_Bathroom_ToiletWithTank01_classes.hpp"
#include "SDK/FN_Bathroom_ToiletWithTank01_parameters.hpp"
#include "SDK/FN_Bathroom_Towels01_structs.hpp"
#include "SDK/FN_Bathroom_Towels01_classes.hpp"
#include "SDK/FN_Bathroom_Towels01_parameters.hpp"
#include "SDK/FN_Commerce_MagazineStand_03_structs.hpp"
#include "SDK/FN_Commerce_MagazineStand_03_classes.hpp"
#include "SDK/FN_Commerce_MagazineStand_03_parameters.hpp"
#include "SDK/FN_Living_EndTable_03_structs.hpp"
#include "SDK/FN_Living_EndTable_03_classes.hpp"
#include "SDK/FN_Living_EndTable_03_parameters.hpp"
#include "SDK/FN_IND_ConveyorBelt_05_structs.hpp"
#include "SDK/FN_IND_ConveyorBelt_05_classes.hpp"
#include "SDK/FN_IND_ConveyorBelt_05_parameters.hpp"
#include "SDK/FN_IND_FactoryArm_01_Orange_structs.hpp"
#include "SDK/FN_IND_FactoryArm_01_Orange_classes.hpp"
#include "SDK/FN_IND_FactoryArm_01_Orange_parameters.hpp"
#include "SDK/FN_PipeKit04_structs.hpp"
#include "SDK/FN_PipeKit04_classes.hpp"
#include "SDK/FN_PipeKit04_parameters.hpp"
#include "SDK/FN_PipeKit05_structs.hpp"
#include "SDK/FN_PipeKit05_classes.hpp"
#include "SDK/FN_PipeKit05_parameters.hpp"
#include "SDK/FN_Prop_CeilingTrack_01_91_structs.hpp"
#include "SDK/FN_Prop_CeilingTrack_01_91_classes.hpp"
#include "SDK/FN_Prop_CeilingTrack_01_91_parameters.hpp"
#include "SDK/FN_Roof_Ducts02_structs.hpp"
#include "SDK/FN_Roof_Ducts02_classes.hpp"
#include "SDK/FN_Roof_Ducts02_parameters.hpp"
#include "SDK/FN_ToiletFactory_DisplayDias_structs.hpp"
#include "SDK/FN_ToiletFactory_DisplayDias_classes.hpp"
#include "SDK/FN_ToiletFactory_DisplayDias_parameters.hpp"
#include "SDK/FN_ToiletFactory_Frame_01_structs.hpp"
#include "SDK/FN_ToiletFactory_Frame_01_classes.hpp"
#include "SDK/FN_ToiletFactory_Frame_01_parameters.hpp"
#include "SDK/FN_ToiletFactory_Frame_02_structs.hpp"
#include "SDK/FN_ToiletFactory_Frame_02_classes.hpp"
#include "SDK/FN_ToiletFactory_Frame_02_parameters.hpp"
#include "SDK/FN_ToiletFactory_Frame_03_structs.hpp"
#include "SDK/FN_ToiletFactory_Frame_03_classes.hpp"
#include "SDK/FN_ToiletFactory_Frame_03_parameters.hpp"
#include "SDK/FN_ToiletFactory_Frame_04_structs.hpp"
#include "SDK/FN_ToiletFactory_Frame_04_classes.hpp"
#include "SDK/FN_ToiletFactory_Frame_04_parameters.hpp"
#include "SDK/FN_ToiletFactory_Frame_05_structs.hpp"
#include "SDK/FN_ToiletFactory_Frame_05_classes.hpp"
#include "SDK/FN_ToiletFactory_Frame_05_parameters.hpp"
#include "SDK/FN_Factory_Roof_3_structs.hpp"
#include "SDK/FN_Factory_Roof_3_classes.hpp"
#include "SDK/FN_Factory_Roof_3_parameters.hpp"
#include "SDK/FN_Tree_Pine_02_structs.hpp"
#include "SDK/FN_Tree_Pine_02_classes.hpp"
#include "SDK/FN_Tree_Pine_02_parameters.hpp"
#include "SDK/FN_Factory_Roof_4_structs.hpp"
#include "SDK/FN_Factory_Roof_4_classes.hpp"
#include "SDK/FN_Factory_Roof_4_parameters.hpp"
#include "SDK/FN_BridgeGirder_H_structs.hpp"
#include "SDK/FN_BridgeGirder_H_classes.hpp"
#include "SDK/FN_BridgeGirder_H_parameters.hpp"
#include "SDK/FN_Construction_DlbWindow_02_structs.hpp"
#include "SDK/FN_Construction_DlbWindow_02_classes.hpp"
#include "SDK/FN_Construction_DlbWindow_02_parameters.hpp"
#include "SDK/FN_DanceClub_Wall_01_structs.hpp"
#include "SDK/FN_DanceClub_Wall_01_classes.hpp"
#include "SDK/FN_DanceClub_Wall_01_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_A06_structs.hpp"
#include "SDK/FN_Fence_Chainlink_A06_classes.hpp"
#include "SDK/FN_Fence_Chainlink_A06_parameters.hpp"
#include "SDK/FN_DanceClub_Wall_03_structs.hpp"
#include "SDK/FN_DanceClub_Wall_03_classes.hpp"
#include "SDK/FN_DanceClub_Wall_03_parameters.hpp"
#include "SDK/FN_Factory_4d_Half_structs.hpp"
#include "SDK/FN_Factory_4d_Half_classes.hpp"
#include "SDK/FN_Factory_4d_Half_parameters.hpp"
#include "SDK/FN_Pueblo_Floor_04_structs.hpp"
#include "SDK/FN_Pueblo_Floor_04_classes.hpp"
#include "SDK/FN_Pueblo_Floor_04_parameters.hpp"
#include "SDK/FN_Chimney_Narrow_Top_structs.hpp"
#include "SDK/FN_Chimney_Narrow_Top_classes.hpp"
#include "SDK/FN_Chimney_Narrow_Top_parameters.hpp"
#include "SDK/FN_Chimney_Transition_structs.hpp"
#include "SDK/FN_Chimney_Transition_classes.hpp"
#include "SDK/FN_Chimney_Transition_parameters.hpp"
#include "SDK/FN_Chimney_Wide_Middle_structs.hpp"
#include "SDK/FN_Chimney_Wide_Middle_classes.hpp"
#include "SDK/FN_Chimney_Wide_Middle_parameters.hpp"
#include "SDK/FN_Chimney_Wide_Top_structs.hpp"
#include "SDK/FN_Chimney_Wide_Top_classes.hpp"
#include "SDK/FN_Chimney_Wide_Top_parameters.hpp"
#include "SDK/FN_Hospital_Curtain_S_03_structs.hpp"
#include "SDK/FN_Hospital_Curtain_S_03_classes.hpp"
#include "SDK/FN_Hospital_Curtain_S_03_parameters.hpp"
#include "SDK/FN_Hospital_MountedTV_structs.hpp"
#include "SDK/FN_Hospital_MountedTV_classes.hpp"
#include "SDK/FN_Hospital_MountedTV_parameters.hpp"
#include "SDK/FN_IND_ControlModule01_structs.hpp"
#include "SDK/FN_IND_ControlModule01_classes.hpp"
#include "SDK/FN_IND_ControlModule01_parameters.hpp"
#include "SDK/FN_IND_ConveyorBelt_01_structs.hpp"
#include "SDK/FN_IND_ConveyorBelt_01_classes.hpp"
#include "SDK/FN_IND_ConveyorBelt_01_parameters.hpp"
#include "SDK/FN_IND_ConveyorBelt_03_structs.hpp"
#include "SDK/FN_IND_ConveyorBelt_03_classes.hpp"
#include "SDK/FN_IND_ConveyorBelt_03_parameters.hpp"
#include "SDK/FN_ToiletFactory_BrokenPile_02_structs.hpp"
#include "SDK/FN_ToiletFactory_BrokenPile_02_classes.hpp"
#include "SDK/FN_ToiletFactory_BrokenPile_02_parameters.hpp"
#include "SDK/FN_ToiletFactory_BrokenPile_03_structs.hpp"
#include "SDK/FN_ToiletFactory_BrokenPile_03_classes.hpp"
#include "SDK/FN_ToiletFactory_BrokenPile_03_parameters.hpp"
#include "SDK/FN_Factory_4_structs.hpp"
#include "SDK/FN_Factory_4_classes.hpp"
#include "SDK/FN_Factory_4_parameters.hpp"
#include "SDK/FN_Lamp01_structs.hpp"
#include "SDK/FN_Lamp01_classes.hpp"
#include "SDK/FN_Lamp01_parameters.hpp"
#include "SDK/FN_Factory_4c_ThreeQuarter_structs.hpp"
#include "SDK/FN_Factory_4c_ThreeQuarter_classes.hpp"
#include "SDK/FN_Factory_4c_ThreeQuarter_parameters.hpp"
#include "SDK/FN_IND_ConveyorBelt_04_structs.hpp"
#include "SDK/FN_IND_ConveyorBelt_04_classes.hpp"
#include "SDK/FN_IND_ConveyorBelt_04_parameters.hpp"
#include "SDK/FN_Athena_Prop_Toilet_Factory_Toilet_Future_structs.hpp"
#include "SDK/FN_Athena_Prop_Toilet_Factory_Toilet_Future_classes.hpp"
#include "SDK/FN_Athena_Prop_Toilet_Factory_Toilet_Future_parameters.hpp"
#include "SDK/FN_Athena_Prop_ToiletFactory_Crate_03_structs.hpp"
#include "SDK/FN_Athena_Prop_ToiletFactory_Crate_03_classes.hpp"
#include "SDK/FN_Athena_Prop_ToiletFactory_Crate_03_parameters.hpp"
#include "SDK/FN_Bathroom_ToiletBowl01_structs.hpp"
#include "SDK/FN_Bathroom_ToiletBowl01_classes.hpp"
#include "SDK/FN_Bathroom_ToiletBowl01_parameters.hpp"
#include "SDK/FN_Bathroom_ToiletSeat01_structs.hpp"
#include "SDK/FN_Bathroom_ToiletSeat01_classes.hpp"
#include "SDK/FN_Bathroom_ToiletSeat01_parameters.hpp"
#include "SDK/FN_Factory_Wall_12_structs.hpp"
#include "SDK/FN_Factory_Wall_12_classes.hpp"
#include "SDK/FN_Factory_Wall_12_parameters.hpp"
#include "SDK/FN_SM_Boardwalk_Stair44_structs.hpp"
#include "SDK/FN_SM_Boardwalk_Stair44_classes.hpp"
#include "SDK/FN_SM_Boardwalk_Stair44_parameters.hpp"
#include "SDK/FN_Tree_Bush_3_structs.hpp"
#include "SDK/FN_Tree_Bush_3_classes.hpp"
#include "SDK/FN_Tree_Bush_3_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_B04_structs.hpp"
#include "SDK/FN_Fence_Chainlink_B04_classes.hpp"
#include "SDK/FN_Fence_Chainlink_B04_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_A01_structs.hpp"
#include "SDK/FN_Fence_Chainlink_A01_classes.hpp"
#include "SDK/FN_Fence_Chainlink_A01_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_A04_structs.hpp"
#include "SDK/FN_Fence_Chainlink_A04_classes.hpp"
#include "SDK/FN_Fence_Chainlink_A04_parameters.hpp"
#include "SDK/FN_Utility_TrashCanMedium_02_structs.hpp"
#include "SDK/FN_Utility_TrashCanMedium_02_classes.hpp"
#include "SDK/FN_Utility_TrashCanMedium_02_parameters.hpp"
#include "SDK/FN_Streetprop_Contsruction_03_structs.hpp"
#include "SDK/FN_Streetprop_Contsruction_03_classes.hpp"
#include "SDK/FN_Streetprop_Contsruction_03_parameters.hpp"
#include "SDK/FN_Streetprop_Contsruction_03b_structs.hpp"
#include "SDK/FN_Streetprop_Contsruction_03b_classes.hpp"
#include "SDK/FN_Streetprop_Contsruction_03b_parameters.hpp"
#include "SDK/FN_Streetprop_Contsruction_03c_structs.hpp"
#include "SDK/FN_Streetprop_Contsruction_03c_classes.hpp"
#include "SDK/FN_Streetprop_Contsruction_03c_parameters.hpp"
#include "SDK/FN_Streetprop_Contsruction_01_structs.hpp"
#include "SDK/FN_Streetprop_Contsruction_01_classes.hpp"
#include "SDK/FN_Streetprop_Contsruction_01_parameters.hpp"
#include "SDK/FN_Streetprop_Contsruction_01b_structs.hpp"
#include "SDK/FN_Streetprop_Contsruction_01b_classes.hpp"
#include "SDK/FN_Streetprop_Contsruction_01b_parameters.hpp"
#include "SDK/FN_Factory_Trim_17b_structs.hpp"
#include "SDK/FN_Factory_Trim_17b_classes.hpp"
#include "SDK/FN_Factory_Trim_17b_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_B06_structs.hpp"
#include "SDK/FN_Fence_Chainlink_B06_classes.hpp"
#include "SDK/FN_Fence_Chainlink_B06_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_B07_structs.hpp"
#include "SDK/FN_Fence_Chainlink_B07_classes.hpp"
#include "SDK/FN_Fence_Chainlink_B07_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_A05_structs.hpp"
#include "SDK/FN_Fence_Chainlink_A05_classes.hpp"
#include "SDK/FN_Fence_Chainlink_A05_parameters.hpp"
#include "SDK/FN_Commerce_CoffeeSink_structs.hpp"
#include "SDK/FN_Commerce_CoffeeSink_classes.hpp"
#include "SDK/FN_Commerce_CoffeeSink_parameters.hpp"
#include "SDK/FN_Athena_POI_StripMall_structs.hpp"
#include "SDK/FN_Athena_POI_StripMall_classes.hpp"
#include "SDK/FN_Athena_POI_StripMall_parameters.hpp"
#include "SDK/FN_Commerce_CheckoutCounter01_structs.hpp"
#include "SDK/FN_Commerce_CheckoutCounter01_classes.hpp"
#include "SDK/FN_Commerce_CheckoutCounter01_parameters.hpp"
#include "SDK/FN_Commerce_Counter01_a_structs.hpp"
#include "SDK/FN_Commerce_Counter01_a_classes.hpp"
#include "SDK/FN_Commerce_Counter01_a_parameters.hpp"
#include "SDK/FN_Commerce_NewspaperBox_01_structs.hpp"
#include "SDK/FN_Commerce_NewspaperBox_01_classes.hpp"
#include "SDK/FN_Commerce_NewspaperBox_01_parameters.hpp"
#include "SDK/FN_Commerce_StoreShelves_01_structs.hpp"
#include "SDK/FN_Commerce_StoreShelves_01_classes.hpp"
#include "SDK/FN_Commerce_StoreShelves_01_parameters.hpp"
#include "SDK/FN_Commerce_GasPump_03_structs.hpp"
#include "SDK/FN_Commerce_GasPump_03_classes.hpp"
#include "SDK/FN_Commerce_GasPump_03_parameters.hpp"
#include "SDK/FN_Commerce_StoreShelves_18_structs.hpp"
#include "SDK/FN_Commerce_StoreShelves_18_classes.hpp"
#include "SDK/FN_Commerce_StoreShelves_18_parameters.hpp"
#include "SDK/FN_Living_BookShelves_06_structs.hpp"
#include "SDK/FN_Living_BookShelves_06_classes.hpp"
#include "SDK/FN_Living_BookShelves_06_parameters.hpp"
#include "SDK/FN_Living_BookShelves_06_2sided_structs.hpp"
#include "SDK/FN_Living_BookShelves_06_2sided_classes.hpp"
#include "SDK/FN_Living_BookShelves_06_2sided_parameters.hpp"
#include "SDK/FN_Living_BookShelves_06_CThru_structs.hpp"
#include "SDK/FN_Living_BookShelves_06_CThru_classes.hpp"
#include "SDK/FN_Living_BookShelves_06_CThru_parameters.hpp"
#include "SDK/FN_SM_Fort_Floors_Generic_BalconyO_structs.hpp"
#include "SDK/FN_SM_Fort_Floors_Generic_BalconyO_classes.hpp"
#include "SDK/FN_SM_Fort_Floors_Generic_BalconyO_parameters.hpp"
#include "SDK/FN_Commerce_StoreShelves_08_structs.hpp"
#include "SDK/FN_Commerce_StoreShelves_08_classes.hpp"
#include "SDK/FN_Commerce_StoreShelves_08_parameters.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BalconyS_structs.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BalconyS_classes.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BalconyS_parameters.hpp"
#include "SDK/FN_SM_Fort_Floors_Generic_TwoTone_Inner_structs.hpp"
#include "SDK/FN_SM_Fort_Floors_Generic_TwoTone_Inner_classes.hpp"
#include "SDK/FN_SM_Fort_Floors_Generic_TwoTone_Inner_parameters.hpp"
#include "SDK/FN_SM_Fort_Floors_Generic_TwoTone_Outer_structs.hpp"
#include "SDK/FN_SM_Fort_Floors_Generic_TwoTone_Outer_classes.hpp"
#include "SDK/FN_SM_Fort_Floors_Generic_TwoTone_Outer_parameters.hpp"
#include "SDK/FN_CoffeeShop_Menu_01_structs.hpp"
#include "SDK/FN_CoffeeShop_Menu_01_classes.hpp"
#include "SDK/FN_CoffeeShop_Menu_01_parameters.hpp"
#include "SDK/FN_CoffeeShop_Sign_01_structs.hpp"
#include "SDK/FN_CoffeeShop_Sign_01_classes.hpp"
#include "SDK/FN_CoffeeShop_Sign_01_parameters.hpp"
#include "SDK/FN_Commerce_Awning_4_Straight_structs.hpp"
#include "SDK/FN_Commerce_Awning_4_Straight_classes.hpp"
#include "SDK/FN_Commerce_Awning_4_Straight_parameters.hpp"
#include "SDK/FN_Commerce_Counter_Corner_structs.hpp"
#include "SDK/FN_Commerce_Counter_Corner_classes.hpp"
#include "SDK/FN_Commerce_Counter_Corner_parameters.hpp"
#include "SDK/FN_Commerce_Counter_HalfLength_structs.hpp"
#include "SDK/FN_Commerce_Counter_HalfLength_classes.hpp"
#include "SDK/FN_Commerce_Counter_HalfLength_parameters.hpp"
#include "SDK/FN_Prop_Sign_BookStore_Large_structs.hpp"
#include "SDK/FN_Prop_Sign_BookStore_Large_classes.hpp"
#include "SDK/FN_Prop_Sign_BookStore_Large_parameters.hpp"
#include "SDK/FN_Recreation_CampingRack03_structs.hpp"
#include "SDK/FN_Recreation_CampingRack03_classes.hpp"
#include "SDK/FN_Recreation_CampingRack03_parameters.hpp"
#include "SDK/FN_Sign_NOMS_Section_01_structs.hpp"
#include "SDK/FN_Sign_NOMS_Section_01_classes.hpp"
#include "SDK/FN_Sign_NOMS_Section_01_parameters.hpp"
#include "SDK/FN_TelephonePole_01_structs.hpp"
#include "SDK/FN_TelephonePole_01_classes.hpp"
#include "SDK/FN_TelephonePole_01_parameters.hpp"
#include "SDK/FN_Sign_NomsGrocery_structs.hpp"
#include "SDK/FN_Sign_NomsGrocery_classes.hpp"
#include "SDK/FN_Sign_NomsGrocery_parameters.hpp"
#include "SDK/FN_WarehouseShelves_06_structs.hpp"
#include "SDK/FN_WarehouseShelves_06_classes.hpp"
#include "SDK/FN_WarehouseShelves_06_parameters.hpp"
#include "SDK/FN_SM_FireEscape_Walkway_Stair34_structs.hpp"
#include "SDK/FN_SM_FireEscape_Walkway_Stair34_classes.hpp"
#include "SDK/FN_SM_FireEscape_Walkway_Stair34_parameters.hpp"
#include "SDK/FN_SM_FireEscape_Walkway_Stairs_structs.hpp"
#include "SDK/FN_SM_FireEscape_Walkway_Stairs_classes.hpp"
#include "SDK/FN_SM_FireEscape_Walkway_Stairs_parameters.hpp"
#include "SDK/FN_Fence_Picket01_Half_structs.hpp"
#include "SDK/FN_Fence_Picket01_Half_classes.hpp"
#include "SDK/FN_Fence_Picket01_Half_parameters.hpp"
#include "SDK/FN_BrickSimple_DoorWayCenter03_structs.hpp"
#include "SDK/FN_BrickSimple_DoorWayCenter03_classes.hpp"
#include "SDK/FN_BrickSimple_DoorWayCenter03_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_B08_structs.hpp"
#include "SDK/FN_Fence_Chainlink_B08_classes.hpp"
#include "SDK/FN_Fence_Chainlink_B08_parameters.hpp"
#include "SDK/FN_BrickSimple_Window_F_structs.hpp"
#include "SDK/FN_BrickSimple_Window_F_classes.hpp"
#include "SDK/FN_BrickSimple_Window_F_parameters.hpp"
#include "SDK/FN_Construction_ConcreteWall_Trim_01b_structs.hpp"
#include "SDK/FN_Construction_ConcreteWall_Trim_01b_classes.hpp"
#include "SDK/FN_Construction_ConcreteWall_Trim_01b_parameters.hpp"
#include "SDK/FN_S_BuildingTrim_B_structs.hpp"
#include "SDK/FN_S_BuildingTrim_B_classes.hpp"
#include "SDK/FN_S_BuildingTrim_B_parameters.hpp"
#include "SDK/FN_Construction_Wall_Solid_02_structs.hpp"
#include "SDK/FN_Construction_Wall_Solid_02_classes.hpp"
#include "SDK/FN_Construction_Wall_Solid_02_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_H_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_H_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_H_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_K2_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_K2_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_K2_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_L_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_L_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_L_parameters.hpp"
#include "SDK/FN_S_StoreFront_Wall_A2_structs.hpp"
#include "SDK/FN_S_StoreFront_Wall_A2_classes.hpp"
#include "SDK/FN_S_StoreFront_Wall_A2_parameters.hpp"
#include "SDK/FN_S_BuildingTrim_D_structs.hpp"
#include "SDK/FN_S_BuildingTrim_D_classes.hpp"
#include "SDK/FN_S_BuildingTrim_D_parameters.hpp"
#include "SDK/FN_Commerce_StoreShelves_03_structs.hpp"
#include "SDK/FN_Commerce_StoreShelves_03_classes.hpp"
#include "SDK/FN_Commerce_StoreShelves_03_parameters.hpp"
#include "SDK/FN_SM_Boardwalk_Stair34_structs.hpp"
#include "SDK/FN_SM_Boardwalk_Stair34_classes.hpp"
#include "SDK/FN_SM_Boardwalk_Stair34_parameters.hpp"
#include "SDK/FN_SM_Construction_Concrete_Floor_01_structs.hpp"
#include "SDK/FN_SM_Construction_Concrete_Floor_01_classes.hpp"
#include "SDK/FN_SM_Construction_Concrete_Floor_01_parameters.hpp"
#include "SDK/FN_SM_ParkingGarage_Floor_structs.hpp"
#include "SDK/FN_SM_ParkingGarage_Floor_classes.hpp"
#include "SDK/FN_SM_ParkingGarage_Floor_parameters.hpp"
#include "SDK/FN_Palettes_Medium_structs.hpp"
#include "SDK/FN_Palettes_Medium_classes.hpp"
#include "SDK/FN_Palettes_Medium_parameters.hpp"
#include "SDK/FN_Prop_LibraryBookPile_02_A_structs.hpp"
#include "SDK/FN_Prop_LibraryBookPile_02_A_classes.hpp"
#include "SDK/FN_Prop_LibraryBookPile_02_A_parameters.hpp"
#include "SDK/FN_Prop_LibraryBookPile_02_B_structs.hpp"
#include "SDK/FN_Prop_LibraryBookPile_02_B_classes.hpp"
#include "SDK/FN_Prop_LibraryBookPile_02_B_parameters.hpp"
#include "SDK/FN_S_StoreFront_Wall_B_structs.hpp"
#include "SDK/FN_S_StoreFront_Wall_B_classes.hpp"
#include "SDK/FN_S_StoreFront_Wall_B_parameters.hpp"
#include "SDK/FN_Construction_Wall_Arch_01_structs.hpp"
#include "SDK/FN_Construction_Wall_Arch_01_classes.hpp"
#include "SDK/FN_Construction_Wall_Arch_01_parameters.hpp"
#include "SDK/FN_Commerce_StoreShelves_09_structs.hpp"
#include "SDK/FN_Commerce_StoreShelves_09_classes.hpp"
#include "SDK/FN_Commerce_StoreShelves_09_parameters.hpp"
#include "SDK/FN_Athena_Prop_Fish_structs.hpp"
#include "SDK/FN_Athena_Prop_Fish_classes.hpp"
#include "SDK/FN_Athena_Prop_Fish_parameters.hpp"
#include "SDK/FN_Boardwalk_BalconyI_structs.hpp"
#include "SDK/FN_Boardwalk_BalconyI_classes.hpp"
#include "SDK/FN_Boardwalk_BalconyI_parameters.hpp"
#include "SDK/FN_Sign_TacoStand_01_structs.hpp"
#include "SDK/FN_Sign_TacoStand_01_classes.hpp"
#include "SDK/FN_Sign_TacoStand_01_parameters.hpp"
#include "SDK/FN_CobWebs01_structs.hpp"
#include "SDK/FN_CobWebs01_classes.hpp"
#include "SDK/FN_CobWebs01_parameters.hpp"
#include "SDK/FN_Recreation_Park_Swing01_structs.hpp"
#include "SDK/FN_Recreation_Park_Swing01_classes.hpp"
#include "SDK/FN_Recreation_Park_Swing01_parameters.hpp"
#include "SDK/FN_Recreation_Park_Swing03_structs.hpp"
#include "SDK/FN_Recreation_Park_Swing03_classes.hpp"
#include "SDK/FN_Recreation_Park_Swing03_parameters.hpp"
#include "SDK/FN_Tree_Log_3_structs.hpp"
#include "SDK/FN_Tree_Log_3_classes.hpp"
#include "SDK/FN_Tree_Log_3_parameters.hpp"
#include "SDK/FN_Sign_Open_structs.hpp"
#include "SDK/FN_Sign_Open_classes.hpp"
#include "SDK/FN_Sign_Open_parameters.hpp"
#include "SDK/FN_Construction_Wall_Arch_03_structs.hpp"
#include "SDK/FN_Construction_Wall_Arch_03_classes.hpp"
#include "SDK/FN_Construction_Wall_Arch_03_parameters.hpp"
#include "SDK/FN_SM_Farm_Doorway_3_structs.hpp"
#include "SDK/FN_SM_Farm_Doorway_3_classes.hpp"
#include "SDK/FN_SM_Farm_Doorway_3_parameters.hpp"
#include "SDK/FN_SM_Farm_QuarterWallHalf_structs.hpp"
#include "SDK/FN_SM_Farm_QuarterWallHalf_classes.hpp"
#include "SDK/FN_SM_Farm_QuarterWallHalf_parameters.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_Beams_structs.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_Beams_classes.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_Beams_parameters.hpp"
#include "SDK/FN_CobWebs02_structs.hpp"
#include "SDK/FN_CobWebs02_classes.hpp"
#include "SDK/FN_CobWebs02_parameters.hpp"
#include "SDK/FN_SM_Farm_Roof_1_Cap2_structs.hpp"
#include "SDK/FN_SM_Farm_Roof_1_Cap2_classes.hpp"
#include "SDK/FN_SM_Farm_Roof_1_Cap2_parameters.hpp"
#include "SDK/FN_Commerce_MagazineStand_01_structs.hpp"
#include "SDK/FN_Commerce_MagazineStand_01_classes.hpp"
#include "SDK/FN_Commerce_MagazineStand_01_parameters.hpp"
#include "SDK/FN_WildWest_Outhouse_01_structs.hpp"
#include "SDK/FN_WildWest_Outhouse_01_classes.hpp"
#include "SDK/FN_WildWest_Outhouse_01_parameters.hpp"
#include "SDK/FN_Living_Fireplace_01_structs.hpp"
#include "SDK/FN_Living_Fireplace_01_classes.hpp"
#include "SDK/FN_Living_Fireplace_01_parameters.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Corner_structs.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Corner_classes.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Corner_parameters.hpp"
#include "SDK/FN_Prop_OWL_SportPoster_02_structs.hpp"
#include "SDK/FN_Prop_OWL_SportPoster_02_classes.hpp"
#include "SDK/FN_Prop_OWL_SportPoster_02_parameters.hpp"
#include "SDK/FN_BP_Fort_Logcabin_Balcony_structs.hpp"
#include "SDK/FN_BP_Fort_Logcabin_Balcony_classes.hpp"
#include "SDK/FN_BP_Fort_Logcabin_Balcony_parameters.hpp"
#include "SDK/FN_deleteME_structs.hpp"
#include "SDK/FN_deleteME_classes.hpp"
#include "SDK/FN_deleteME_parameters.hpp"
#include "SDK/FN_deleteme2_structs.hpp"
#include "SDK/FN_deleteme2_classes.hpp"
#include "SDK/FN_deleteme2_parameters.hpp"
#include "SDK/FN_Mansion_DeerHead_structs.hpp"
#include "SDK/FN_Mansion_DeerHead_classes.hpp"
#include "SDK/FN_Mansion_DeerHead_parameters.hpp"
#include "SDK/FN_Prop_ArcheryTarget_01_structs.hpp"
#include "SDK/FN_Prop_ArcheryTarget_01_classes.hpp"
#include "SDK/FN_Prop_ArcheryTarget_01_parameters.hpp"
#include "SDK/FN_Prop_ArcheryTarget_3_structs.hpp"
#include "SDK/FN_Prop_ArcheryTarget_3_classes.hpp"
#include "SDK/FN_Prop_ArcheryTarget_3_parameters.hpp"
#include "SDK/FN_Prop_Desert_CowSkull_structs.hpp"
#include "SDK/FN_Prop_Desert_CowSkull_classes.hpp"
#include "SDK/FN_Prop_Desert_CowSkull_parameters.hpp"
#include "SDK/FN_Prop_OWL_ClothingRack_structs.hpp"
#include "SDK/FN_Prop_OWL_ClothingRack_classes.hpp"
#include "SDK/FN_Prop_OWL_ClothingRack_parameters.hpp"
#include "SDK/FN_Prop_OWL_WallFishA_structs.hpp"
#include "SDK/FN_Prop_OWL_WallFishA_classes.hpp"
#include "SDK/FN_Prop_OWL_WallFishA_parameters.hpp"
#include "SDK/FN_Prop_Sign_CampGate_structs.hpp"
#include "SDK/FN_Prop_Sign_CampGate_classes.hpp"
#include "SDK/FN_Prop_Sign_CampGate_parameters.hpp"
#include "SDK/FN_Prop_Sign_CampSign_02_structs.hpp"
#include "SDK/FN_Prop_Sign_CampSign_02_classes.hpp"
#include "SDK/FN_Prop_Sign_CampSign_02_parameters.hpp"
#include "SDK/FN_Prop_Sign_TrailMarker_01_structs.hpp"
#include "SDK/FN_Prop_Sign_TrailMarker_01_classes.hpp"
#include "SDK/FN_Prop_Sign_TrailMarker_01_parameters.hpp"
#include "SDK/FN_Recreation_CampingCanoe01_structs.hpp"
#include "SDK/FN_Recreation_CampingCanoe01_classes.hpp"
#include "SDK/FN_Recreation_CampingCanoe01_parameters.hpp"
#include "SDK/FN_Recreation_CampingKayak01_structs.hpp"
#include "SDK/FN_Recreation_CampingKayak01_classes.hpp"
#include "SDK/FN_Recreation_CampingKayak01_parameters.hpp"
#include "SDK/FN_Recreation_CampingRack01_structs.hpp"
#include "SDK/FN_Recreation_CampingRack01_classes.hpp"
#include "SDK/FN_Recreation_CampingRack01_parameters.hpp"
#include "SDK/FN_Recreation_CampingRack02_structs.hpp"
#include "SDK/FN_Recreation_CampingRack02_classes.hpp"
#include "SDK/FN_Recreation_CampingRack02_parameters.hpp"
#include "SDK/FN_Recreation_CampingRack05_structs.hpp"
#include "SDK/FN_Recreation_CampingRack05_classes.hpp"
#include "SDK/FN_Recreation_CampingRack05_parameters.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Roof1_structs.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Roof1_classes.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Roof1_parameters.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Roof2_structs.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Roof2_classes.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Roof2_parameters.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Roof2_Cap_structs.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Roof2_Cap_classes.hpp"
#include "SDK/FN_BP_FORT_LogCabin_Roof2_Cap_parameters.hpp"
#include "SDK/FN_BP_FORT_LogCabin_RoofCorner_structs.hpp"
#include "SDK/FN_BP_FORT_LogCabin_RoofCorner_classes.hpp"
#include "SDK/FN_BP_FORT_LogCabin_RoofCorner_parameters.hpp"
#include "SDK/FN_LogCabin_Stairs1_structs.hpp"
#include "SDK/FN_LogCabin_Stairs1_classes.hpp"
#include "SDK/FN_LogCabin_Stairs1_parameters.hpp"
#include "SDK/FN_SM_Farm_StablesWall02_structs.hpp"
#include "SDK/FN_SM_Farm_StablesWall02_classes.hpp"
#include "SDK/FN_SM_Farm_StablesWall02_parameters.hpp"
#include "SDK/FN_Fence_Rural_Low_1missing_structs.hpp"
#include "SDK/FN_Fence_Rural_Low_1missing_classes.hpp"
#include "SDK/FN_Fence_Rural_Low_1missing_parameters.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_MetalRailing02_structs.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_MetalRailing02_classes.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_MetalRailing02_parameters.hpp"
#include "SDK/FN_Logcabin_Railing1_structs.hpp"
#include "SDK/FN_Logcabin_Railing1_classes.hpp"
#include "SDK/FN_Logcabin_Railing1_parameters.hpp"
#include "SDK/FN_Fence_Rural_Low_Broken_structs.hpp"
#include "SDK/FN_Fence_Rural_Low_Broken_classes.hpp"
#include "SDK/FN_Fence_Rural_Low_Broken_parameters.hpp"
#include "SDK/FN_LogCabin_RoofWall_structs.hpp"
#include "SDK/FN_LogCabin_RoofWall_classes.hpp"
#include "SDK/FN_LogCabin_RoofWall_parameters.hpp"
#include "SDK/FN_LogCabin_RoofWall_Window_structs.hpp"
#include "SDK/FN_LogCabin_RoofWall_Window_classes.hpp"
#include "SDK/FN_LogCabin_RoofWall_Window_parameters.hpp"
#include "SDK/FN_LogCabin_Wall_structs.hpp"
#include "SDK/FN_LogCabin_Wall_classes.hpp"
#include "SDK/FN_LogCabin_Wall_parameters.hpp"
#include "SDK/FN_LogCabin_Wall_Brace2_structs.hpp"
#include "SDK/FN_LogCabin_Wall_Brace2_classes.hpp"
#include "SDK/FN_LogCabin_Wall_Brace2_parameters.hpp"
#include "SDK/FN_LogCabin_Wall_Door_Center_structs.hpp"
#include "SDK/FN_LogCabin_Wall_Door_Center_classes.hpp"
#include "SDK/FN_LogCabin_Wall_Door_Center_parameters.hpp"
#include "SDK/FN_LogCabin_Wall_Door_Left_structs.hpp"
#include "SDK/FN_LogCabin_Wall_Door_Left_classes.hpp"
#include "SDK/FN_LogCabin_Wall_Door_Left_parameters.hpp"
#include "SDK/FN_LogCabin_wall_Door_Window_structs.hpp"
#include "SDK/FN_LogCabin_wall_Door_Window_classes.hpp"
#include "SDK/FN_LogCabin_wall_Door_Window_parameters.hpp"
#include "SDK/FN_LogCabin_Wall_Window_Left_structs.hpp"
#include "SDK/FN_LogCabin_Wall_Window_Left_classes.hpp"
#include "SDK/FN_LogCabin_Wall_Window_Left_parameters.hpp"
#include "SDK/FN_SM_Farm_Doorway_2_structs.hpp"
#include "SDK/FN_SM_Farm_Doorway_2_classes.hpp"
#include "SDK/FN_SM_Farm_Doorway_2_parameters.hpp"
#include "SDK/FN_WildWest_WoodPile_02_structs.hpp"
#include "SDK/FN_WildWest_WoodPile_02_classes.hpp"
#include "SDK/FN_WildWest_WoodPile_02_parameters.hpp"
#include "SDK/FN_Radio_01_structs.hpp"
#include "SDK/FN_Radio_01_classes.hpp"
#include "SDK/FN_Radio_01_parameters.hpp"
#include "SDK/FN_WoodHouse_SolidWall_NoTrim_structs.hpp"
#include "SDK/FN_WoodHouse_SolidWall_NoTrim_classes.hpp"
#include "SDK/FN_WoodHouse_SolidWall_NoTrim_parameters.hpp"
#include "SDK/FN_Bedroom_PoliceStation_01_structs.hpp"
#include "SDK/FN_Bedroom_PoliceStation_01_classes.hpp"
#include "SDK/FN_Bedroom_PoliceStation_01_parameters.hpp"
#include "SDK/FN_ElementarySchool_CafeteriaBench_01_structs.hpp"
#include "SDK/FN_ElementarySchool_CafeteriaBench_01_classes.hpp"
#include "SDK/FN_ElementarySchool_CafeteriaBench_01_parameters.hpp"
#include "SDK/FN_Bathroom_PrisonToilet_structs.hpp"
#include "SDK/FN_Bathroom_PrisonToilet_classes.hpp"
#include "SDK/FN_Bathroom_PrisonToilet_parameters.hpp"
#include "SDK/FN_BigClock_01_structs.hpp"
#include "SDK/FN_BigClock_01_classes.hpp"
#include "SDK/FN_BigClock_01_parameters.hpp"
#include "SDK/FN_Kitchen_IND_Counter_01_structs.hpp"
#include "SDK/FN_Kitchen_IND_Counter_01_classes.hpp"
#include "SDK/FN_Kitchen_IND_Counter_01_parameters.hpp"
#include "SDK/FN_Kitchen_Modern_Cabinets01_structs.hpp"
#include "SDK/FN_Kitchen_Modern_Cabinets01_classes.hpp"
#include "SDK/FN_Kitchen_Modern_Cabinets01_parameters.hpp"
#include "SDK/FN_Kitchen_Modern_Cabinets02_structs.hpp"
#include "SDK/FN_Kitchen_Modern_Cabinets02_classes.hpp"
#include "SDK/FN_Kitchen_Modern_Cabinets02_parameters.hpp"
#include "SDK/FN_Kitchen_Modern_Counter01_structs.hpp"
#include "SDK/FN_Kitchen_Modern_Counter01_classes.hpp"
#include "SDK/FN_Kitchen_Modern_Counter01_parameters.hpp"
#include "SDK/FN_Kitchen_Modern_Counter03_structs.hpp"
#include "SDK/FN_Kitchen_Modern_Counter03_classes.hpp"
#include "SDK/FN_Kitchen_Modern_Counter03_parameters.hpp"
#include "SDK/FN_Kitchen_Modern_Sink_structs.hpp"
#include "SDK/FN_Kitchen_Modern_Sink_classes.hpp"
#include "SDK/FN_Kitchen_Modern_Sink_parameters.hpp"
#include "SDK/FN_Kitchen_Range_01_structs.hpp"
#include "SDK/FN_Kitchen_Range_01_classes.hpp"
#include "SDK/FN_Kitchen_Range_01_parameters.hpp"
#include "SDK/FN_Kitchen_StoveHood_01_structs.hpp"
#include "SDK/FN_Kitchen_StoveHood_01_classes.hpp"
#include "SDK/FN_Kitchen_StoveHood_01_parameters.hpp"
#include "SDK/FN_Living_BookShelves_05_CThru_structs.hpp"
#include "SDK/FN_Living_BookShelves_05_CThru_classes.hpp"
#include "SDK/FN_Living_BookShelves_05_CThru_parameters.hpp"
#include "SDK/FN_Prop_Sign_Detour_structs.hpp"
#include "SDK/FN_Prop_Sign_Detour_classes.hpp"
#include "SDK/FN_Prop_Sign_Detour_parameters.hpp"
#include "SDK/FN_Living_BookShelves_07_2Sided_CThru_structs.hpp"
#include "SDK/FN_Living_BookShelves_07_2Sided_CThru_classes.hpp"
#include "SDK/FN_Living_BookShelves_07_2Sided_CThru_parameters.hpp"
#include "SDK/FN_Office_FileCabinet01a_structs.hpp"
#include "SDK/FN_Office_FileCabinet01a_classes.hpp"
#include "SDK/FN_Office_FileCabinet01a_parameters.hpp"
#include "SDK/FN_Commerce_StoreShelves_11_structs.hpp"
#include "SDK/FN_Commerce_StoreShelves_11_classes.hpp"
#include "SDK/FN_Commerce_StoreShelves_11_parameters.hpp"
#include "SDK/FN_Prison_Bathroom_Sink_structs.hpp"
#include "SDK/FN_Prison_Bathroom_Sink_classes.hpp"
#include "SDK/FN_Prison_Bathroom_Sink_parameters.hpp"
#include "SDK/FN_School_Lockers_01_structs.hpp"
#include "SDK/FN_School_Lockers_01_classes.hpp"
#include "SDK/FN_School_Lockers_01_parameters.hpp"
#include "SDK/FN_DanceClub_Corner_01_structs.hpp"
#include "SDK/FN_DanceClub_Corner_01_classes.hpp"
#include "SDK/FN_DanceClub_Corner_01_parameters.hpp"
#include "SDK/FN_Prop_OWL_SportPoster_01_structs.hpp"
#include "SDK/FN_Prop_OWL_SportPoster_01_classes.hpp"
#include "SDK/FN_Prop_OWL_SportPoster_01_parameters.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TilesBrokenBeamsO_structs.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TilesBrokenBeamsO_classes.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TilesBrokenBeamsO_parameters.hpp"
#include "SDK/FN_Chair_Lobby03_structs.hpp"
#include "SDK/FN_Chair_Lobby03_classes.hpp"
#include "SDK/FN_Chair_Lobby03_parameters.hpp"
#include "SDK/FN_Hospital_Bed_structs.hpp"
#include "SDK/FN_Hospital_Bed_classes.hpp"
#include "SDK/FN_Hospital_Bed_parameters.hpp"
#include "SDK/FN_IND_DamControl_02_structs.hpp"
#include "SDK/FN_IND_DamControl_02_classes.hpp"
#include "SDK/FN_IND_DamControl_02_parameters.hpp"
#include "SDK/FN_IND_DamControl_03_structs.hpp"
#include "SDK/FN_IND_DamControl_03_classes.hpp"
#include "SDK/FN_IND_DamControl_03_parameters.hpp"
#include "SDK/FN_Prop_LibraryBookPile_01_A_structs.hpp"
#include "SDK/FN_Prop_LibraryBookPile_01_A_classes.hpp"
#include "SDK/FN_Prop_LibraryBookPile_01_A_parameters.hpp"
#include "SDK/FN_Prop_LibraryBookPile_01_B_structs.hpp"
#include "SDK/FN_Prop_LibraryBookPile_01_B_classes.hpp"
#include "SDK/FN_Prop_LibraryBookPile_01_B_parameters.hpp"
#include "SDK/FN_WoodMines_FrameA_structs.hpp"
#include "SDK/FN_WoodMines_FrameA_classes.hpp"
#include "SDK/FN_WoodMines_FrameA_parameters.hpp"
#include "SDK/FN_CryptWalls_08_structs.hpp"
#include "SDK/FN_CryptWalls_08_classes.hpp"
#include "SDK/FN_CryptWalls_08_parameters.hpp"
#include "SDK/FN_SM_Prison_Walls_CellDoorWay2_structs.hpp"
#include "SDK/FN_SM_Prison_Walls_CellDoorWay2_classes.hpp"
#include "SDK/FN_SM_Prison_Walls_CellDoorWay2_parameters.hpp"
#include "SDK/FN_PhysicsBall_Master_structs.hpp"
#include "SDK/FN_PhysicsBall_Master_classes.hpp"
#include "SDK/FN_PhysicsBall_Master_parameters.hpp"
#include "SDK/FN_Recreation_SportingGoods_Basketball_structs.hpp"
#include "SDK/FN_Recreation_SportingGoods_Basketball_classes.hpp"
#include "SDK/FN_Recreation_SportingGoods_Basketball_parameters.hpp"
#include "SDK/FN_Pipe01_structs.hpp"
#include "SDK/FN_Pipe01_classes.hpp"
#include "SDK/FN_Pipe01_parameters.hpp"
#include "SDK/FN_Pipe02_structs.hpp"
#include "SDK/FN_Pipe02_classes.hpp"
#include "SDK/FN_Pipe02_parameters.hpp"
#include "SDK/FN_Prop_Sign_CampSites_02_structs.hpp"
#include "SDK/FN_Prop_Sign_CampSites_02_classes.hpp"
#include "SDK/FN_Prop_Sign_CampSites_02_parameters.hpp"
#include "SDK/FN_Prop_Newspaper_Rack_01_structs.hpp"
#include "SDK/FN_Prop_Newspaper_Rack_01_classes.hpp"
#include "SDK/FN_Prop_Newspaper_Rack_01_parameters.hpp"
#include "SDK/FN_Prop_SecurityCamera_01_structs.hpp"
#include "SDK/FN_Prop_SecurityCamera_01_classes.hpp"
#include "SDK/FN_Prop_SecurityCamera_01_parameters.hpp"
#include "SDK/FN_Roof_Ducts03b_structs.hpp"
#include "SDK/FN_Roof_Ducts03b_classes.hpp"
#include "SDK/FN_Roof_Ducts03b_parameters.hpp"
#include "SDK/FN_Roof_Ducts03d_structs.hpp"
#include "SDK/FN_Roof_Ducts03d_classes.hpp"
#include "SDK/FN_Roof_Ducts03d_parameters.hpp"
#include "SDK/FN_Roof_Ducts04_structs.hpp"
#include "SDK/FN_Roof_Ducts04_classes.hpp"
#include "SDK/FN_Roof_Ducts04_parameters.hpp"
#include "SDK/FN_Roof_Ducts04a_structs.hpp"
#include "SDK/FN_Roof_Ducts04a_classes.hpp"
#include "SDK/FN_Roof_Ducts04a_parameters.hpp"
#include "SDK/FN_Roof_Ducts04d_structs.hpp"
#include "SDK/FN_Roof_Ducts04d_classes.hpp"
#include "SDK/FN_Roof_Ducts04d_parameters.hpp"
#include "SDK/FN_WigglyPipe_02_structs.hpp"
#include "SDK/FN_WigglyPipe_02_classes.hpp"
#include "SDK/FN_WigglyPipe_02_parameters.hpp"
#include "SDK/FN_Neo_Class_Stair_6_structs.hpp"
#include "SDK/FN_Neo_Class_Stair_6_classes.hpp"
#include "SDK/FN_Neo_Class_Stair_6_parameters.hpp"
#include "SDK/FN_SM_Prison_Walls_CellDoorWay_Circle22_structs.hpp"
#include "SDK/FN_SM_Prison_Walls_CellDoorWay_Circle22_classes.hpp"
#include "SDK/FN_SM_Prison_Walls_CellDoorWay_Circle22_parameters.hpp"
#include "SDK/FN_BrickSimple_Archway02_structs.hpp"
#include "SDK/FN_BrickSimple_Archway02_classes.hpp"
#include "SDK/FN_BrickSimple_Archway02_parameters.hpp"
#include "SDK/FN_Construction_ConcreteWall_Solid_02_structs.hpp"
#include "SDK/FN_Construction_ConcreteWall_Solid_02_classes.hpp"
#include "SDK/FN_Construction_ConcreteWall_Solid_02_parameters.hpp"
#include "SDK/FN_CryptWalls_03_structs.hpp"
#include "SDK/FN_CryptWalls_03_classes.hpp"
#include "SDK/FN_CryptWalls_03_parameters.hpp"
#include "SDK/FN_Logcabin_Railing_Half_structs.hpp"
#include "SDK/FN_Logcabin_Railing_Half_classes.hpp"
#include "SDK/FN_Logcabin_Railing_Half_parameters.hpp"
#include "SDK/FN_CryptWalls_04_structs.hpp"
#include "SDK/FN_CryptWalls_04_classes.hpp"
#include "SDK/FN_CryptWalls_04_parameters.hpp"
#include "SDK/FN_Factory_Wall_14_structs.hpp"
#include "SDK/FN_Factory_Wall_14_classes.hpp"
#include "SDK/FN_Factory_Wall_14_parameters.hpp"
#include "SDK/FN_Factory_Wall_15_structs.hpp"
#include "SDK/FN_Factory_Wall_15_classes.hpp"
#include "SDK/FN_Factory_Wall_15_parameters.hpp"
#include "SDK/FN_Factory_Wall_3_structs.hpp"
#include "SDK/FN_Factory_Wall_3_classes.hpp"
#include "SDK/FN_Factory_Wall_3_parameters.hpp"
#include "SDK/FN_Factory_Wall_2Trim_structs.hpp"
#include "SDK/FN_Factory_Wall_2Trim_classes.hpp"
#include "SDK/FN_Factory_Wall_2Trim_parameters.hpp"
#include "SDK/FN_Factory_Wall_3Trim_structs.hpp"
#include "SDK/FN_Factory_Wall_3Trim_classes.hpp"
#include "SDK/FN_Factory_Wall_3Trim_parameters.hpp"
#include "SDK/FN_Factory_Trim_10b_structs.hpp"
#include "SDK/FN_Factory_Trim_10b_classes.hpp"
#include "SDK/FN_Factory_Trim_10b_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_C01_structs.hpp"
#include "SDK/FN_Fence_Chainlink_C01_classes.hpp"
#include "SDK/FN_Fence_Chainlink_C01_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_C_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_C_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_C_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_E_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_E_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_E_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_G_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_G_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_G_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_J_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_J_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_J_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_C02_structs.hpp"
#include "SDK/FN_Fence_Chainlink_C02_classes.hpp"
#include "SDK/FN_Fence_Chainlink_C02_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_L2_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_L2_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_L2_parameters.hpp"
#include "SDK/FN_PoliceProps_Tape_structs.hpp"
#include "SDK/FN_PoliceProps_Tape_classes.hpp"
#include "SDK/FN_PoliceProps_Tape_parameters.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge01_structs.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge01_classes.hpp"
#include "SDK/FN_Prop_Rocks_StoneLarge01_parameters.hpp"
#include "SDK/FN_Commerce_ShoppingCart_01_structs.hpp"
#include "SDK/FN_Commerce_ShoppingCart_01_classes.hpp"
#include "SDK/FN_Commerce_ShoppingCart_01_parameters.hpp"
#include "SDK/FN_WoodMines_BalconyI_structs.hpp"
#include "SDK/FN_WoodMines_BalconyI_classes.hpp"
#include "SDK/FN_WoodMines_BalconyI_parameters.hpp"
#include "SDK/FN_Bedroom_Bed_03_structs.hpp"
#include "SDK/FN_Bedroom_Bed_03_classes.hpp"
#include "SDK/FN_Bedroom_Bed_03_parameters.hpp"
#include "SDK/FN_Bedroom_Dresser_02_structs.hpp"
#include "SDK/FN_Bedroom_Dresser_02_classes.hpp"
#include "SDK/FN_Bedroom_Dresser_02_parameters.hpp"
#include "SDK/FN_Mansion_Art_Painting_08_structs.hpp"
#include "SDK/FN_Mansion_Art_Painting_08_classes.hpp"
#include "SDK/FN_Mansion_Art_Painting_08_parameters.hpp"
#include "SDK/FN_Durr_Corner2_structs.hpp"
#include "SDK/FN_Durr_Corner2_classes.hpp"
#include "SDK/FN_Durr_Corner2_parameters.hpp"
#include "SDK/FN_WoodMines_Corner01_structs.hpp"
#include "SDK/FN_WoodMines_Corner01_classes.hpp"
#include "SDK/FN_WoodMines_Corner01_parameters.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TwoTone_Straight_structs.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TwoTone_Straight_classes.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TwoTone_Straight_parameters.hpp"
#include "SDK/FN_BP_SinkHole_Boat_01_structs.hpp"
#include "SDK/FN_BP_SinkHole_Boat_01_classes.hpp"
#include "SDK/FN_BP_SinkHole_Boat_01_parameters.hpp"
#include "SDK/FN_HotelBench_02_structs.hpp"
#include "SDK/FN_HotelBench_02_classes.hpp"
#include "SDK/FN_HotelBench_02_parameters.hpp"
#include "SDK/FN_Prop_Junk_Pile_structs.hpp"
#include "SDK/FN_Prop_Junk_Pile_classes.hpp"
#include "SDK/FN_Prop_Junk_Pile_parameters.hpp"
#include "SDK/FN_FORT_Roof_Corner_In_structs.hpp"
#include "SDK/FN_FORT_Roof_Corner_In_classes.hpp"
#include "SDK/FN_FORT_Roof_Corner_In_parameters.hpp"
#include "SDK/FN_FORT_Roof_Straight_Dormer_structs.hpp"
#include "SDK/FN_FORT_Roof_Straight_Dormer_classes.hpp"
#include "SDK/FN_FORT_Roof_Straight_Dormer_parameters.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_DoubleBack_structs.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_DoubleBack_classes.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_DoubleBack_parameters.hpp"
#include "SDK/FN_SM_SM_PrisonWal_CheckinStation_structs.hpp"
#include "SDK/FN_SM_SM_PrisonWal_CheckinStation_classes.hpp"
#include "SDK/FN_SM_SM_PrisonWal_CheckinStation_parameters.hpp"
#include "SDK/FN_BrickSimple_Archway03_structs.hpp"
#include "SDK/FN_BrickSimple_Archway03_classes.hpp"
#include "SDK/FN_BrickSimple_Archway03_parameters.hpp"
#include "SDK/FN_BrickSimple_DoorWayCenter_INT_structs.hpp"
#include "SDK/FN_BrickSimple_DoorWayCenter_INT_classes.hpp"
#include "SDK/FN_BrickSimple_DoorWayCenter_INT_parameters.hpp"
#include "SDK/FN_BrickSimple_DoorWaySide_EXT_structs.hpp"
#include "SDK/FN_BrickSimple_DoorWaySide_EXT_classes.hpp"
#include "SDK/FN_BrickSimple_DoorWaySide_EXT_parameters.hpp"
#include "SDK/FN_BrickSimple_RoofWall_Trim_structs.hpp"
#include "SDK/FN_BrickSimple_RoofWall_Trim_classes.hpp"
#include "SDK/FN_BrickSimple_RoofWall_Trim_parameters.hpp"
#include "SDK/FN_BrickSimple_RoundedWall_structs.hpp"
#include "SDK/FN_BrickSimple_RoundedWall_classes.hpp"
#include "SDK/FN_BrickSimple_RoundedWall_parameters.hpp"
#include "SDK/FN_BrickSimple_Window_D_structs.hpp"
#include "SDK/FN_BrickSimple_Window_D_classes.hpp"
#include "SDK/FN_BrickSimple_Window_D_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_A_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_A_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_A_parameters.hpp"
#include "SDK/FN_S_StoreFront_Glass_B_structs.hpp"
#include "SDK/FN_S_StoreFront_Glass_B_classes.hpp"
#include "SDK/FN_S_StoreFront_Glass_B_parameters.hpp"
#include "SDK/FN_Bathroom_Bathtub_01_structs.hpp"
#include "SDK/FN_Bathroom_Bathtub_01_classes.hpp"
#include "SDK/FN_Bathroom_Bathtub_01_parameters.hpp"
#include "SDK/FN_Storage_Crate03_structs.hpp"
#include "SDK/FN_Storage_Crate03_classes.hpp"
#include "SDK/FN_Storage_Crate03_parameters.hpp"
#include "SDK/FN_TrashPile_03_structs.hpp"
#include "SDK/FN_TrashPile_03_classes.hpp"
#include "SDK/FN_TrashPile_03_parameters.hpp"
#include "SDK/FN_Street_Light_Urban02_structs.hpp"
#include "SDK/FN_Street_Light_Urban02_classes.hpp"
#include "SDK/FN_Street_Light_Urban02_parameters.hpp"
#include "SDK/FN_WarehouseShelves_01_structs.hpp"
#include "SDK/FN_WarehouseShelves_01_classes.hpp"
#include "SDK/FN_WarehouseShelves_01_parameters.hpp"
#include "SDK/FN_WaterHeater01_structs.hpp"
#include "SDK/FN_WaterHeater01_classes.hpp"
#include "SDK/FN_WaterHeater01_parameters.hpp"
#include "SDK/FN_Prop_Sign_Cliff_structs.hpp"
#include "SDK/FN_Prop_Sign_Cliff_classes.hpp"
#include "SDK/FN_Prop_Sign_Cliff_parameters.hpp"
#include "SDK/FN_Fort_Roof_Peak_Open_structs.hpp"
#include "SDK/FN_Fort_Roof_Peak_Open_classes.hpp"
#include "SDK/FN_Fort_Roof_Peak_Open_parameters.hpp"
#include "SDK/FN_BP_TRV_SWA_Boat_01_structs.hpp"
#include "SDK/FN_BP_TRV_SWA_Boat_01_classes.hpp"
#include "SDK/FN_BP_TRV_SWA_Boat_01_parameters.hpp"
#include "SDK/FN_WoodMines_HalfWall_structs.hpp"
#include "SDK/FN_WoodMines_HalfWall_classes.hpp"
#include "SDK/FN_WoodMines_HalfWall_parameters.hpp"
#include "SDK/FN_BrickSimple_DoorWaySide_INT_structs.hpp"
#include "SDK/FN_BrickSimple_DoorWaySide_INT_classes.hpp"
#include "SDK/FN_BrickSimple_DoorWaySide_INT_parameters.hpp"
#include "SDK/FN_SM_Fort_Neo_Class_Rail_3_structs.hpp"
#include "SDK/FN_SM_Fort_Neo_Class_Rail_3_classes.hpp"
#include "SDK/FN_SM_Fort_Neo_Class_Rail_3_parameters.hpp"
#include "SDK/FN_Barriers_Wood02_structs.hpp"
#include "SDK/FN_Barriers_Wood02_classes.hpp"
#include "SDK/FN_Barriers_Wood02_parameters.hpp"
#include "SDK/FN_Storage_CardboardBoxes_06_structs.hpp"
#include "SDK/FN_Storage_CardboardBoxes_06_classes.hpp"
#include "SDK/FN_Storage_CardboardBoxes_06_parameters.hpp"
#include "SDK/FN_Commerce_Awning_3_structs.hpp"
#include "SDK/FN_Commerce_Awning_3_classes.hpp"
#include "SDK/FN_Commerce_Awning_3_parameters.hpp"
#include "SDK/FN_DoorMat_GoAway_structs.hpp"
#include "SDK/FN_DoorMat_GoAway_classes.hpp"
#include "SDK/FN_DoorMat_GoAway_parameters.hpp"
#include "SDK/FN_Hedge_High_90Degree_structs.hpp"
#include "SDK/FN_Hedge_High_90Degree_classes.hpp"
#include "SDK/FN_Hedge_High_90Degree_parameters.hpp"
#include "SDK/FN_TelephonePole_02_structs.hpp"
#include "SDK/FN_TelephonePole_02_classes.hpp"
#include "SDK/FN_TelephonePole_02_parameters.hpp"
#include "SDK/FN_Shelters_CannedGoods01_structs.hpp"
#include "SDK/FN_Shelters_CannedGoods01_classes.hpp"
#include "SDK/FN_Shelters_CannedGoods01_parameters.hpp"
#include "SDK/FN_Shelters_CannedGoods03_structs.hpp"
#include "SDK/FN_Shelters_CannedGoods03_classes.hpp"
#include "SDK/FN_Shelters_CannedGoods03_parameters.hpp"
#include "SDK/FN_IND_BarbedWire_01_structs.hpp"
#include "SDK/FN_IND_BarbedWire_01_classes.hpp"
#include "SDK/FN_IND_BarbedWire_01_parameters.hpp"
#include "SDK/FN_Shelters_Flashlight01_structs.hpp"
#include "SDK/FN_Shelters_Flashlight01_classes.hpp"
#include "SDK/FN_Shelters_Flashlight01_parameters.hpp"
#include "SDK/FN_Sign_Exit_Leave_structs.hpp"
#include "SDK/FN_Sign_Exit_Leave_classes.hpp"
#include "SDK/FN_Sign_Exit_Leave_parameters.hpp"
#include "SDK/FN_Roof_Dock_1Unit_structs.hpp"
#include "SDK/FN_Roof_Dock_1Unit_classes.hpp"
#include "SDK/FN_Roof_Dock_1Unit_parameters.hpp"
#include "SDK/FN_Hedge_High_Rectangle_structs.hpp"
#include "SDK/FN_Hedge_High_Rectangle_classes.hpp"
#include "SDK/FN_Hedge_High_Rectangle_parameters.hpp"
#include "SDK/FN_BrickSimple_Roof_Peak_Cap_structs.hpp"
#include "SDK/FN_BrickSimple_Roof_Peak_Cap_classes.hpp"
#include "SDK/FN_BrickSimple_Roof_Peak_Cap_parameters.hpp"
#include "SDK/FN_Athena_POI_FarmStead_002_structs.hpp"
#include "SDK/FN_Athena_POI_FarmStead_002_classes.hpp"
#include "SDK/FN_Athena_POI_FarmStead_002_parameters.hpp"
#include "SDK/FN_Athena_POI_FarmStead_structs.hpp"
#include "SDK/FN_Athena_POI_FarmStead_classes.hpp"
#include "SDK/FN_Athena_POI_FarmStead_parameters.hpp"
#include "SDK/FN_CartWithWheels_structs.hpp"
#include "SDK/FN_CartWithWheels_classes.hpp"
#include "SDK/FN_CartWithWheels_parameters.hpp"
#include "SDK/FN_Haybale_02_structs.hpp"
#include "SDK/FN_Haybale_02_classes.hpp"
#include "SDK/FN_Haybale_02_parameters.hpp"
#include "SDK/FN_LF_5x9_Parent_structs.hpp"
#include "SDK/FN_LF_5x9_Parent_classes.hpp"
#include "SDK/FN_LF_5x9_Parent_parameters.hpp"
#include "SDK/FN_LF_9x12_Parent_structs.hpp"
#include "SDK/FN_LF_9x12_Parent_classes.hpp"
#include "SDK/FN_LF_9x12_Parent_parameters.hpp"
#include "SDK/FN_LF_Athena_POI_5x9_FarmBuildings_structs.hpp"
#include "SDK/FN_LF_Athena_POI_5x9_FarmBuildings_classes.hpp"
#include "SDK/FN_LF_Athena_POI_5x9_FarmBuildings_parameters.hpp"
#include "SDK/FN_LF_Athena_POI_9x12_WoodCluster_structs.hpp"
#include "SDK/FN_LF_Athena_POI_9x12_WoodCluster_classes.hpp"
#include "SDK/FN_LF_Athena_POI_9x12_WoodCluster_parameters.hpp"
#include "SDK/FN_Storage_Barrel_02_structs.hpp"
#include "SDK/FN_Storage_Barrel_02_classes.hpp"
#include "SDK/FN_Storage_Barrel_02_parameters.hpp"
#include "SDK/FN_Chair_Kitchen02_structs.hpp"
#include "SDK/FN_Chair_Kitchen02_classes.hpp"
#include "SDK/FN_Chair_Kitchen02_parameters.hpp"
#include "SDK/FN_LF_7x12_Parent_structs.hpp"
#include "SDK/FN_LF_7x12_Parent_classes.hpp"
#include "SDK/FN_LF_7x12_Parent_parameters.hpp"
#include "SDK/FN_LF_Athena_POI_7x12_Fields_structs.hpp"
#include "SDK/FN_LF_Athena_POI_7x12_Fields_classes.hpp"
#include "SDK/FN_LF_Athena_POI_7x12_Fields_parameters.hpp"
#include "SDK/FN_Commerce_CashRegister_structs.hpp"
#include "SDK/FN_Commerce_CashRegister_classes.hpp"
#include "SDK/FN_Commerce_CashRegister_parameters.hpp"
#include "SDK/FN_Commerce_CondimentBar_01_structs.hpp"
#include "SDK/FN_Commerce_CondimentBar_01_classes.hpp"
#include "SDK/FN_Commerce_CondimentBar_01_parameters.hpp"
#include "SDK/FN_Commerce_FastFoodTable_01_structs.hpp"
#include "SDK/FN_Commerce_FastFoodTable_01_classes.hpp"
#include "SDK/FN_Commerce_FastFoodTable_01_parameters.hpp"
#include "SDK/FN_Durr_Counter_2_structs.hpp"
#include "SDK/FN_Durr_Counter_2_classes.hpp"
#include "SDK/FN_Durr_Counter_2_parameters.hpp"
#include "SDK/FN_Durr_Counter_3_structs.hpp"
#include "SDK/FN_Durr_Counter_3_classes.hpp"
#include "SDK/FN_Durr_Counter_3_parameters.hpp"
#include "SDK/FN_Athena_POI_DurrrBurger_001_structs.hpp"
#include "SDK/FN_Athena_POI_DurrrBurger_001_classes.hpp"
#include "SDK/FN_Athena_POI_DurrrBurger_001_parameters.hpp"
#include "SDK/FN_Commerce_CheckoutCounter02_structs.hpp"
#include "SDK/FN_Commerce_CheckoutCounter02_classes.hpp"
#include "SDK/FN_Commerce_CheckoutCounter02_parameters.hpp"
#include "SDK/FN_Durr_Food_Trashcan_3_structs.hpp"
#include "SDK/FN_Durr_Food_Trashcan_3_classes.hpp"
#include "SDK/FN_Durr_Food_Trashcan_3_parameters.hpp"
#include "SDK/FN_Durr_Food_Trashcan_4_structs.hpp"
#include "SDK/FN_Durr_Food_Trashcan_4_classes.hpp"
#include "SDK/FN_Durr_Food_Trashcan_4_parameters.hpp"
#include "SDK/FN_Durr_Food_Trashcan_5_structs.hpp"
#include "SDK/FN_Durr_Food_Trashcan_5_classes.hpp"
#include "SDK/FN_Durr_Food_Trashcan_5_parameters.hpp"
#include "SDK/FN_Durr_Large_Storage_Box_Group_2_structs.hpp"
#include "SDK/FN_Durr_Large_Storage_Box_Group_2_classes.hpp"
#include "SDK/FN_Durr_Large_Storage_Box_Group_2_parameters.hpp"
#include "SDK/FN_Durr_Watercooler_Shelves_structs.hpp"
#include "SDK/FN_Durr_Watercooler_Shelves_classes.hpp"
#include "SDK/FN_Durr_Watercooler_Shelves_parameters.hpp"
#include "SDK/FN_Durr_Watercooler_Shelves2_structs.hpp"
#include "SDK/FN_Durr_Watercooler_Shelves2_classes.hpp"
#include "SDK/FN_Durr_Watercooler_Shelves2_parameters.hpp"
#include "SDK/FN_Plant_Corn02_structs.hpp"
#include "SDK/FN_Plant_Corn02_classes.hpp"
#include "SDK/FN_Plant_Corn02_parameters.hpp"
#include "SDK/FN_Kitchen_DeepFryer_01_structs.hpp"
#include "SDK/FN_Kitchen_DeepFryer_01_classes.hpp"
#include "SDK/FN_Kitchen_DeepFryer_01_parameters.hpp"
#include "SDK/FN_Kitchen_Freezer_02_structs.hpp"
#include "SDK/FN_Kitchen_Freezer_02_classes.hpp"
#include "SDK/FN_Kitchen_Freezer_02_parameters.hpp"
#include "SDK/FN_Kitchen_Griddle_02_structs.hpp"
#include "SDK/FN_Kitchen_Griddle_02_classes.hpp"
#include "SDK/FN_Kitchen_Griddle_02_parameters.hpp"
#include "SDK/FN_Mailbox_02_structs.hpp"
#include "SDK/FN_Mailbox_02_classes.hpp"
#include "SDK/FN_Mailbox_02_parameters.hpp"
#include "SDK/FN_Office_FileCabinet01b_structs.hpp"
#include "SDK/FN_Office_FileCabinet01b_classes.hpp"
#include "SDK/FN_Office_FileCabinet01b_parameters.hpp"
#include "SDK/FN_School_Lockers_02_structs.hpp"
#include "SDK/FN_School_Lockers_02_classes.hpp"
#include "SDK/FN_School_Lockers_02_parameters.hpp"
#include "SDK/FN_Storage_Crate2a_side_structs.hpp"
#include "SDK/FN_Storage_Crate2a_side_classes.hpp"
#include "SDK/FN_Storage_Crate2a_side_parameters.hpp"
#include "SDK/FN_Prop_Boxes_01b_structs.hpp"
#include "SDK/FN_Prop_Boxes_01b_classes.hpp"
#include "SDK/FN_Prop_Boxes_01b_parameters.hpp"
#include "SDK/FN_Prop_Boxes_01c_structs.hpp"
#include "SDK/FN_Prop_Boxes_01c_classes.hpp"
#include "SDK/FN_Prop_Boxes_01c_parameters.hpp"
#include "SDK/FN_Prop_Boxes_01d_structs.hpp"
#include "SDK/FN_Prop_Boxes_01d_classes.hpp"
#include "SDK/FN_Prop_Boxes_01d_parameters.hpp"
#include "SDK/FN_Prop_Boxes_01e_structs.hpp"
#include "SDK/FN_Prop_Boxes_01e_classes.hpp"
#include "SDK/FN_Prop_Boxes_01e_parameters.hpp"
#include "SDK/FN_Plant_Sunflower01_structs.hpp"
#include "SDK/FN_Plant_Sunflower01_classes.hpp"
#include "SDK/FN_Plant_Sunflower01_parameters.hpp"
#include "SDK/FN_Durr_Corner_structs.hpp"
#include "SDK/FN_Durr_Corner_classes.hpp"
#include "SDK/FN_Durr_Corner_parameters.hpp"
#include "SDK/FN_Durr_PillarCorner_structs.hpp"
#include "SDK/FN_Durr_PillarCorner_classes.hpp"
#include "SDK/FN_Durr_PillarCorner_parameters.hpp"
#include "SDK/FN_Durr_SimpleTrim_Corner_structs.hpp"
#include "SDK/FN_Durr_SimpleTrim_Corner_classes.hpp"
#include "SDK/FN_Durr_SimpleTrim_Corner_parameters.hpp"
#include "SDK/FN_Durr_Chart_2_structs.hpp"
#include "SDK/FN_Durr_Chart_2_classes.hpp"
#include "SDK/FN_Durr_Chart_2_parameters.hpp"
#include "SDK/FN_Durr_Chart_3_structs.hpp"
#include "SDK/FN_Durr_Chart_3_classes.hpp"
#include "SDK/FN_Durr_Chart_3_parameters.hpp"
#include "SDK/FN_Durr_EmployeesOnly_Sign_structs.hpp"
#include "SDK/FN_Durr_EmployeesOnly_Sign_classes.hpp"
#include "SDK/FN_Durr_EmployeesOnly_Sign_parameters.hpp"
#include "SDK/FN_Durr_NoEntry_Sign_structs.hpp"
#include "SDK/FN_Durr_NoEntry_Sign_classes.hpp"
#include "SDK/FN_Durr_NoEntry_Sign_parameters.hpp"
#include "SDK/FN_Durr_Wall_Poster_2_structs.hpp"
#include "SDK/FN_Durr_Wall_Poster_2_classes.hpp"
#include "SDK/FN_Durr_Wall_Poster_2_parameters.hpp"
#include "SDK/FN_Durr_Wall_Poster_3_structs.hpp"
#include "SDK/FN_Durr_Wall_Poster_3_classes.hpp"
#include "SDK/FN_Durr_Wall_Poster_3_parameters.hpp"
#include "SDK/FN_Durr_Wall_Poster_4_structs.hpp"
#include "SDK/FN_Durr_Wall_Poster_4_classes.hpp"
#include "SDK/FN_Durr_Wall_Poster_4_parameters.hpp"
#include "SDK/FN_Durr_Wall_Poster_5_structs.hpp"
#include "SDK/FN_Durr_Wall_Poster_5_classes.hpp"
#include "SDK/FN_Durr_Wall_Poster_5_parameters.hpp"
#include "SDK/FN_Durr_Wall_Poster_6_structs.hpp"
#include "SDK/FN_Durr_Wall_Poster_6_classes.hpp"
#include "SDK/FN_Durr_Wall_Poster_6_parameters.hpp"
#include "SDK/FN_Durr_Wall_Poster_7_structs.hpp"
#include "SDK/FN_Durr_Wall_Poster_7_classes.hpp"
#include "SDK/FN_Durr_Wall_Poster_7_parameters.hpp"
#include "SDK/FN_Durr_WashHands_Sign_structs.hpp"
#include "SDK/FN_Durr_WashHands_Sign_classes.hpp"
#include "SDK/FN_Durr_WashHands_Sign_parameters.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTileHalf_structs.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTileHalf_classes.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTileHalf_parameters.hpp"
#include "SDK/FN_Chair_FastFood_structs.hpp"
#include "SDK/FN_Chair_FastFood_classes.hpp"
#include "SDK/FN_Chair_FastFood_parameters.hpp"
#include "SDK/FN_Chair_Lobby02_structs.hpp"
#include "SDK/FN_Chair_Lobby02_classes.hpp"
#include "SDK/FN_Chair_Lobby02_parameters.hpp"
#include "SDK/FN_FastFood_Bench_01_structs.hpp"
#include "SDK/FN_FastFood_Bench_01_classes.hpp"
#include "SDK/FN_FastFood_Bench_01_parameters.hpp"
#include "SDK/FN_PipeKit02_structs.hpp"
#include "SDK/FN_PipeKit02_classes.hpp"
#include "SDK/FN_PipeKit02_parameters.hpp"
#include "SDK/FN_PipeKit03_structs.hpp"
#include "SDK/FN_PipeKit03_classes.hpp"
#include "SDK/FN_PipeKit03_parameters.hpp"
#include "SDK/FN_Prop_Rocks_01_structs.hpp"
#include "SDK/FN_Prop_Rocks_01_classes.hpp"
#include "SDK/FN_Prop_Rocks_01_parameters.hpp"
#include "SDK/FN_Shelters_Lights01_structs.hpp"
#include "SDK/FN_Shelters_Lights01_classes.hpp"
#include "SDK/FN_Shelters_Lights01_parameters.hpp"
#include "SDK/FN_Prop_Rocks_09_structs.hpp"
#include "SDK/FN_Prop_Rocks_09_classes.hpp"
#include "SDK/FN_Prop_Rocks_09_parameters.hpp"
#include "SDK/FN_Commerce_SodaFountain_01_structs.hpp"
#include "SDK/FN_Commerce_SodaFountain_01_classes.hpp"
#include "SDK/FN_Commerce_SodaFountain_01_parameters.hpp"
#include "SDK/FN_Prop_Sign_BacklitMenu_01_structs.hpp"
#include "SDK/FN_Prop_Sign_BacklitMenu_01_classes.hpp"
#include "SDK/FN_Prop_Sign_BacklitMenu_01_parameters.hpp"
#include "SDK/FN_Prop_Sign_BacklitMenu_02_structs.hpp"
#include "SDK/FN_Prop_Sign_BacklitMenu_02_classes.hpp"
#include "SDK/FN_Prop_Sign_BacklitMenu_02_parameters.hpp"
#include "SDK/FN_Prop_Sign_BacklitMenu_03_structs.hpp"
#include "SDK/FN_Prop_Sign_BacklitMenu_03_classes.hpp"
#include "SDK/FN_Prop_Sign_BacklitMenu_03_parameters.hpp"
#include "SDK/FN_Prop_SmallChimney_01b_structs.hpp"
#include "SDK/FN_Prop_SmallChimney_01b_classes.hpp"
#include "SDK/FN_Prop_SmallChimney_01b_parameters.hpp"
#include "SDK/FN_Recreation_BikeRack_structs.hpp"
#include "SDK/FN_Recreation_BikeRack_classes.hpp"
#include "SDK/FN_Recreation_BikeRack_parameters.hpp"
#include "SDK/FN_Recreation_Park_Carousel01_structs.hpp"
#include "SDK/FN_Recreation_Park_Carousel01_classes.hpp"
#include "SDK/FN_Recreation_Park_Carousel01_parameters.hpp"
#include "SDK/FN_Recreation_Park_JungleGym02_structs.hpp"
#include "SDK/FN_Recreation_Park_JungleGym02_classes.hpp"
#include "SDK/FN_Recreation_Park_JungleGym02_parameters.hpp"
#include "SDK/FN_Recreation_Park_JungleGym03_structs.hpp"
#include "SDK/FN_Recreation_Park_JungleGym03_classes.hpp"
#include "SDK/FN_Recreation_Park_JungleGym03_parameters.hpp"
#include "SDK/FN_Recreation_Park_Seesaw01_structs.hpp"
#include "SDK/FN_Recreation_Park_Seesaw01_classes.hpp"
#include "SDK/FN_Recreation_Park_Seesaw01_parameters.hpp"
#include "SDK/FN_Prop_SmallChimney_02_structs.hpp"
#include "SDK/FN_Prop_SmallChimney_02_classes.hpp"
#include "SDK/FN_Prop_SmallChimney_02_parameters.hpp"
#include "SDK/FN_Recreation_Park_SpringRide01_structs.hpp"
#include "SDK/FN_Recreation_Park_SpringRide01_classes.hpp"
#include "SDK/FN_Recreation_Park_SpringRide01_parameters.hpp"
#include "SDK/FN_Storage_OilDrum_Barrel_2_structs.hpp"
#include "SDK/FN_Storage_OilDrum_Barrel_2_classes.hpp"
#include "SDK/FN_Storage_OilDrum_Barrel_2_parameters.hpp"
#include "SDK/FN_Recreation_Park_Slide02_structs.hpp"
#include "SDK/FN_Recreation_Park_Slide02_classes.hpp"
#include "SDK/FN_Recreation_Park_Slide02_parameters.hpp"
#include "SDK/FN_Shelters_Lights02_structs.hpp"
#include "SDK/FN_Shelters_Lights02_classes.hpp"
#include "SDK/FN_Shelters_Lights02_parameters.hpp"
#include "SDK/FN_Recreation_Park_SpringRide02_structs.hpp"
#include "SDK/FN_Recreation_Park_SpringRide02_classes.hpp"
#include "SDK/FN_Recreation_Park_SpringRide02_parameters.hpp"
#include "SDK/FN_Restaurant_Table__3_structs.hpp"
#include "SDK/FN_Restaurant_Table__3_classes.hpp"
#include "SDK/FN_Restaurant_Table__3_parameters.hpp"
#include "SDK/FN_Roof_AirConditioner_01_structs.hpp"
#include "SDK/FN_Roof_AirConditioner_01_classes.hpp"
#include "SDK/FN_Roof_AirConditioner_01_parameters.hpp"
#include "SDK/FN_Roof_AirConditioner_03_structs.hpp"
#include "SDK/FN_Roof_AirConditioner_03_classes.hpp"
#include "SDK/FN_Roof_AirConditioner_03_parameters.hpp"
#include "SDK/FN_Roof_Ducts03_structs.hpp"
#include "SDK/FN_Roof_Ducts03_classes.hpp"
#include "SDK/FN_Roof_Ducts03_parameters.hpp"
#include "SDK/FN_Roof_Ducts03a_structs.hpp"
#include "SDK/FN_Roof_Ducts03a_classes.hpp"
#include "SDK/FN_Roof_Ducts03a_parameters.hpp"
#include "SDK/FN_Roof_Ducts05_structs.hpp"
#include "SDK/FN_Roof_Ducts05_classes.hpp"
#include "SDK/FN_Roof_Ducts05_parameters.hpp"
#include "SDK/FN_Recreation_Park_SpringRide03_structs.hpp"
#include "SDK/FN_Recreation_Park_SpringRide03_classes.hpp"
#include "SDK/FN_Recreation_Park_SpringRide03_parameters.hpp"
#include "SDK/FN_Sign_Durrr_2_structs.hpp"
#include "SDK/FN_Sign_Durrr_2_classes.hpp"
#include "SDK/FN_Sign_Durrr_2_parameters.hpp"
#include "SDK/FN_Sign_Durrr_4_structs.hpp"
#include "SDK/FN_Sign_Durrr_4_classes.hpp"
#include "SDK/FN_Sign_Durrr_4_parameters.hpp"
#include "SDK/FN_Sign_Durrr_5_structs.hpp"
#include "SDK/FN_Sign_Durrr_5_classes.hpp"
#include "SDK/FN_Sign_Durrr_5_parameters.hpp"
#include "SDK/FN_Sign_Durrr_8_structs.hpp"
#include "SDK/FN_Sign_Durrr_8_classes.hpp"
#include "SDK/FN_Sign_Durrr_8_parameters.hpp"
#include "SDK/FN_Sign_Durrr_9_structs.hpp"
#include "SDK/FN_Sign_Durrr_9_classes.hpp"
#include "SDK/FN_Sign_Durrr_9_parameters.hpp"
#include "SDK/FN_Umbrella01_structs.hpp"
#include "SDK/FN_Umbrella01_classes.hpp"
#include "SDK/FN_Umbrella01_parameters.hpp"
#include "SDK/FN_Waterpipe01_structs.hpp"
#include "SDK/FN_Waterpipe01_classes.hpp"
#include "SDK/FN_Waterpipe01_parameters.hpp"
#include "SDK/FN_FORT_Food_Roof_A_structs.hpp"
#include "SDK/FN_FORT_Food_Roof_A_classes.hpp"
#include "SDK/FN_FORT_Food_Roof_A_parameters.hpp"
#include "SDK/FN_FORT_Food_Roof_B_structs.hpp"
#include "SDK/FN_FORT_Food_Roof_B_classes.hpp"
#include "SDK/FN_FORT_Food_Roof_B_parameters.hpp"
#include "SDK/FN_FORT_Food_Roof_I_structs.hpp"
#include "SDK/FN_FORT_Food_Roof_I_classes.hpp"
#include "SDK/FN_FORT_Food_Roof_I_parameters.hpp"
#include "SDK/FN_Gothic_RoofC_structs.hpp"
#include "SDK/FN_Gothic_RoofC_classes.hpp"
#include "SDK/FN_Gothic_RoofC_parameters.hpp"
#include "SDK/FN_WoodMines_DoorC_structs.hpp"
#include "SDK/FN_WoodMines_DoorC_classes.hpp"
#include "SDK/FN_WoodMines_DoorC_parameters.hpp"
#include "SDK/FN_Durrr_Food_Wall_E_structs.hpp"
#include "SDK/FN_Durrr_Food_Wall_E_classes.hpp"
#include "SDK/FN_Durrr_Food_Wall_E_parameters.hpp"
#include "SDK/FN_Rural_House_Stair_2_structs.hpp"
#include "SDK/FN_Rural_House_Stair_2_classes.hpp"
#include "SDK/FN_Rural_House_Stair_2_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_A03_structs.hpp"
#include "SDK/FN_Fence_Chainlink_A03_classes.hpp"
#include "SDK/FN_Fence_Chainlink_A03_parameters.hpp"
#include "SDK/FN_Bridge_Railing_01b_structs.hpp"
#include "SDK/FN_Bridge_Railing_01b_classes.hpp"
#include "SDK/FN_Bridge_Railing_01b_parameters.hpp"
#include "SDK/FN_Durr_HalfWall_Straight_structs.hpp"
#include "SDK/FN_Durr_HalfWall_Straight_classes.hpp"
#include "SDK/FN_Durr_HalfWall_Straight_parameters.hpp"
#include "SDK/FN_Durr_HalfWall_Straight_3_structs.hpp"
#include "SDK/FN_Durr_HalfWall_Straight_3_classes.hpp"
#include "SDK/FN_Durr_HalfWall_Straight_3_parameters.hpp"
#include "SDK/FN_Durrr_Food_Counter_A_structs.hpp"
#include "SDK/FN_Durrr_Food_Counter_A_classes.hpp"
#include "SDK/FN_Durrr_Food_Counter_A_parameters.hpp"
#include "SDK/FN_BrickSimple_RoofWall_structs.hpp"
#include "SDK/FN_BrickSimple_RoofWall_classes.hpp"
#include "SDK/FN_BrickSimple_RoofWall_parameters.hpp"
#include "SDK/FN_Durrr_Food_Pillars_structs.hpp"
#include "SDK/FN_Durrr_Food_Pillars_classes.hpp"
#include "SDK/FN_Durrr_Food_Pillars_parameters.hpp"
#include "SDK/FN_Durrr_Food_Pillars_Counter_structs.hpp"
#include "SDK/FN_Durrr_Food_Pillars_Counter_classes.hpp"
#include "SDK/FN_Durrr_Food_Pillars_Counter_parameters.hpp"
#include "SDK/FN_Durrr_Food_Wall_B_structs.hpp"
#include "SDK/FN_Durrr_Food_Wall_B_classes.hpp"
#include "SDK/FN_Durrr_Food_Wall_B_parameters.hpp"
#include "SDK/FN_Durrr_Food_Wall_C_structs.hpp"
#include "SDK/FN_Durrr_Food_Wall_C_classes.hpp"
#include "SDK/FN_Durrr_Food_Wall_C_parameters.hpp"
#include "SDK/FN_Durrr_Food_Wall_D_structs.hpp"
#include "SDK/FN_Durrr_Food_Wall_D_classes.hpp"
#include "SDK/FN_Durrr_Food_Wall_D_parameters.hpp"
#include "SDK/FN_Durrr_Food_Counter_A_Half_structs.hpp"
#include "SDK/FN_Durrr_Food_Counter_A_Half_classes.hpp"
#include "SDK/FN_Durrr_Food_Counter_A_Half_parameters.hpp"
#include "SDK/FN_Durrr_Food_Wall_SimpleTrim_structs.hpp"
#include "SDK/FN_Durrr_Food_Wall_SimpleTrim_classes.hpp"
#include "SDK/FN_Durrr_Food_Wall_SimpleTrim_parameters.hpp"
#include "SDK/FN_Durrr_Trim_Piece_structs.hpp"
#include "SDK/FN_Durrr_Trim_Piece_classes.hpp"
#include "SDK/FN_Durrr_Trim_Piece_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_A02_structs.hpp"
#include "SDK/FN_Fence_Chainlink_A02_classes.hpp"
#include "SDK/FN_Fence_Chainlink_A02_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_B01_structs.hpp"
#include "SDK/FN_Fence_Chainlink_B01_classes.hpp"
#include "SDK/FN_Fence_Chainlink_B01_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_B05_structs.hpp"
#include "SDK/FN_Fence_Chainlink_B05_classes.hpp"
#include "SDK/FN_Fence_Chainlink_B05_parameters.hpp"
#include "SDK/FN_Fence_Iron_01_structs.hpp"
#include "SDK/FN_Fence_Iron_01_classes.hpp"
#include "SDK/FN_Fence_Iron_01_parameters.hpp"
#include "SDK/FN_Fence_Iron_02_structs.hpp"
#include "SDK/FN_Fence_Iron_02_classes.hpp"
#include "SDK/FN_Fence_Iron_02_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_A07_structs.hpp"
#include "SDK/FN_Fence_Chainlink_A07_classes.hpp"
#include "SDK/FN_Fence_Chainlink_A07_parameters.hpp"
#include "SDK/FN_Rural_House_Rail_1_Half_structs.hpp"
#include "SDK/FN_Rural_House_Rail_1_Half_classes.hpp"
#include "SDK/FN_Rural_House_Rail_1_Half_parameters.hpp"
#include "SDK/FN_Fence_Iron_Archway_structs.hpp"
#include "SDK/FN_Fence_Iron_Archway_classes.hpp"
#include "SDK/FN_Fence_Iron_Archway_parameters.hpp"
#include "SDK/FN_Storage_Crate04_structs.hpp"
#include "SDK/FN_Storage_Crate04_classes.hpp"
#include "SDK/FN_Storage_Crate04_parameters.hpp"
#include "SDK/FN_Office_Copier_01_structs.hpp"
#include "SDK/FN_Office_Copier_01_classes.hpp"
#include "SDK/FN_Office_Copier_01_parameters.hpp"
#include "SDK/FN_Bathroom_Mirror_structs.hpp"
#include "SDK/FN_Bathroom_Mirror_classes.hpp"
#include "SDK/FN_Bathroom_Mirror_parameters.hpp"
#include "SDK/FN_Bathroom_Stall_structs.hpp"
#include "SDK/FN_Bathroom_Stall_classes.hpp"
#include "SDK/FN_Bathroom_Stall_parameters.hpp"
#include "SDK/FN_Office_WaterCooler_01_structs.hpp"
#include "SDK/FN_Office_WaterCooler_01_classes.hpp"
#include "SDK/FN_Office_WaterCooler_01_parameters.hpp"
#include "SDK/FN_Storage_CardboardBoxes_05_structs.hpp"
#include "SDK/FN_Storage_CardboardBoxes_05_classes.hpp"
#include "SDK/FN_Storage_CardboardBoxes_05_parameters.hpp"
#include "SDK/FN_Utility_ToolBox_02_structs.hpp"
#include "SDK/FN_Utility_ToolBox_02_classes.hpp"
#include "SDK/FN_Utility_ToolBox_02_parameters.hpp"
#include "SDK/FN_GamePoster01_structs.hpp"
#include "SDK/FN_GamePoster01_classes.hpp"
#include "SDK/FN_GamePoster01_parameters.hpp"
#include "SDK/FN_Garage_Door_03_structs.hpp"
#include "SDK/FN_Garage_Door_03_classes.hpp"
#include "SDK/FN_Garage_Door_03_parameters.hpp"
#include "SDK/FN_IND_ControlModule02_structs.hpp"
#include "SDK/FN_IND_ControlModule02_classes.hpp"
#include "SDK/FN_IND_ControlModule02_parameters.hpp"
#include "SDK/FN_Shelters_CannedGoods02_structs.hpp"
#include "SDK/FN_Shelters_CannedGoods02_classes.hpp"
#include "SDK/FN_Shelters_CannedGoods02_parameters.hpp"
#include "SDK/FN_Hospital_IVDrip_structs.hpp"
#include "SDK/FN_Hospital_IVDrip_classes.hpp"
#include "SDK/FN_Hospital_IVDrip_parameters.hpp"
#include "SDK/FN_IND_ConveyorBelt_02_structs.hpp"
#include "SDK/FN_IND_ConveyorBelt_02_classes.hpp"
#include "SDK/FN_IND_ConveyorBelt_02_parameters.hpp"
#include "SDK/FN_IND_ConveyorBelt_06_structs.hpp"
#include "SDK/FN_IND_ConveyorBelt_06_classes.hpp"
#include "SDK/FN_IND_ConveyorBelt_06_parameters.hpp"
#include "SDK/FN_Mine_Track_Straight_01_structs.hpp"
#include "SDK/FN_Mine_Track_Straight_01_classes.hpp"
#include "SDK/FN_Mine_Track_Straight_01_parameters.hpp"
#include "SDK/FN_PalletJack_structs.hpp"
#include "SDK/FN_PalletJack_classes.hpp"
#include "SDK/FN_PalletJack_parameters.hpp"
#include "SDK/FN_Prop_CarCrusher_ControlBox_structs.hpp"
#include "SDK/FN_Prop_CarCrusher_ControlBox_classes.hpp"
#include "SDK/FN_Prop_CarCrusher_ControlBox_parameters.hpp"
#include "SDK/FN_Prop_CeilingTrack_01_S_structs.hpp"
#include "SDK/FN_Prop_CeilingTrack_01_S_classes.hpp"
#include "SDK/FN_Prop_CeilingTrack_01_S_parameters.hpp"
#include "SDK/FN_Prop_TV_01_structs.hpp"
#include "SDK/FN_Prop_TV_01_classes.hpp"
#include "SDK/FN_Prop_TV_01_parameters.hpp"
#include "SDK/FN_Shelters_FoldingTable01_structs.hpp"
#include "SDK/FN_Shelters_FoldingTable01_classes.hpp"
#include "SDK/FN_Shelters_FoldingTable01_parameters.hpp"
#include "SDK/FN_S_BuildingTrim_A_structs.hpp"
#include "SDK/FN_S_BuildingTrim_A_classes.hpp"
#include "SDK/FN_S_BuildingTrim_A_parameters.hpp"
#include "SDK/FN_Fence_Picket01_Doorway2c_structs.hpp"
#include "SDK/FN_Fence_Picket01_Doorway2c_classes.hpp"
#include "SDK/FN_Fence_Picket01_Doorway2c_parameters.hpp"
#include "SDK/FN_CryptWalls_01_structs.hpp"
#include "SDK/FN_CryptWalls_01_classes.hpp"
#include "SDK/FN_CryptWalls_01_parameters.hpp"
#include "SDK/FN_BridgeGirder_Trim_E_structs.hpp"
#include "SDK/FN_BridgeGirder_Trim_E_classes.hpp"
#include "SDK/FN_BridgeGirder_Trim_E_parameters.hpp"
#include "SDK/FN_CryptWalls_02_structs.hpp"
#include "SDK/FN_CryptWalls_02_classes.hpp"
#include "SDK/FN_CryptWalls_02_parameters.hpp"
#include "SDK/FN_Girder00_structs.hpp"
#include "SDK/FN_Girder00_classes.hpp"
#include "SDK/FN_Girder00_parameters.hpp"
#include "SDK/FN_Girder00_alt_structs.hpp"
#include "SDK/FN_Girder00_alt_classes.hpp"
#include "SDK/FN_Girder00_alt_parameters.hpp"
#include "SDK/FN_Girder01_structs.hpp"
#include "SDK/FN_Girder01_classes.hpp"
#include "SDK/FN_Girder01_parameters.hpp"
#include "SDK/FN_Factory_Wall_24_structs.hpp"
#include "SDK/FN_Factory_Wall_24_classes.hpp"
#include "SDK/FN_Factory_Wall_24_parameters.hpp"
#include "SDK/FN_Prop_CeilingTrack_Claw2_structs.hpp"
#include "SDK/FN_Prop_CeilingTrack_Claw2_classes.hpp"
#include "SDK/FN_Prop_CeilingTrack_Claw2_parameters.hpp"
#include "SDK/FN_Prop_Tire_single_structs.hpp"
#include "SDK/FN_Prop_Tire_single_classes.hpp"
#include "SDK/FN_Prop_Tire_single_parameters.hpp"
#include "SDK/FN_Factory_Roof_6_structs.hpp"
#include "SDK/FN_Factory_Roof_6_classes.hpp"
#include "SDK/FN_Factory_Roof_6_parameters.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair3_structs.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair3_classes.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair3_parameters.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair4_structs.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair4_classes.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair4_parameters.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_structs.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_classes.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_parameters.hpp"
#include "SDK/FN_Hedge_High_Square_structs.hpp"
#include "SDK/FN_Hedge_High_Square_classes.hpp"
#include "SDK/FN_Hedge_High_Square_parameters.hpp"
#include "SDK/FN_Bedroom_Dresser_03_structs.hpp"
#include "SDK/FN_Bedroom_Dresser_03_classes.hpp"
#include "SDK/FN_Bedroom_Dresser_03_parameters.hpp"
#include "SDK/FN_Utility_TrashCanLarge_01_structs.hpp"
#include "SDK/FN_Utility_TrashCanLarge_01_classes.hpp"
#include "SDK/FN_Utility_TrashCanLarge_01_parameters.hpp"
#include "SDK/FN_Living_EntertainmentCenter_01_structs.hpp"
#include "SDK/FN_Living_EntertainmentCenter_01_classes.hpp"
#include "SDK/FN_Living_EntertainmentCenter_01_parameters.hpp"
#include "SDK/FN_Recreation_SoccerGoal_structs.hpp"
#include "SDK/FN_Recreation_SoccerGoal_classes.hpp"
#include "SDK/FN_Recreation_SoccerGoal_parameters.hpp"
#include "SDK/FN_Girder02_Archway_structs.hpp"
#include "SDK/FN_Girder02_Archway_classes.hpp"
#include "SDK/FN_Girder02_Archway_parameters.hpp"
#include "SDK/FN_WoodHouse_DoorCenter_NoTrim_structs.hpp"
#include "SDK/FN_WoodHouse_DoorCenter_NoTrim_classes.hpp"
#include "SDK/FN_WoodHouse_DoorCenter_NoTrim_parameters.hpp"
#include "SDK/FN_Storage_Suitcases_01_structs.hpp"
#include "SDK/FN_Storage_Suitcases_01_classes.hpp"
#include "SDK/FN_Storage_Suitcases_01_parameters.hpp"
#include "SDK/FN_Athena_POI_CommunityPark_001_structs.hpp"
#include "SDK/FN_Athena_POI_CommunityPark_001_classes.hpp"
#include "SDK/FN_Athena_POI_CommunityPark_001_parameters.hpp"
#include "SDK/FN_Living_BookShelves_05_structs.hpp"
#include "SDK/FN_Living_BookShelves_05_classes.hpp"
#include "SDK/FN_Living_BookShelves_05_parameters.hpp"
#include "SDK/FN_Sign_Durrr_10_structs.hpp"
#include "SDK/FN_Sign_Durrr_10_classes.hpp"
#include "SDK/FN_Sign_Durrr_10_parameters.hpp"
#include "SDK/FN_Tree_Log_5_structs.hpp"
#include "SDK/FN_Tree_Log_5_classes.hpp"
#include "SDK/FN_Tree_Log_5_parameters.hpp"
#include "SDK/FN_FireHydrant__3_structs.hpp"
#include "SDK/FN_FireHydrant__3_classes.hpp"
#include "SDK/FN_FireHydrant__3_parameters.hpp"
#include "SDK/FN_Hydrant01_structs.hpp"
#include "SDK/FN_Hydrant01_classes.hpp"
#include "SDK/FN_Hydrant01_parameters.hpp"
#include "SDK/FN_Office_Desk_01_structs.hpp"
#include "SDK/FN_Office_Desk_01_classes.hpp"
#include "SDK/FN_Office_Desk_01_parameters.hpp"
#include "SDK/FN_Mailbox_01b_structs.hpp"
#include "SDK/FN_Mailbox_01b_classes.hpp"
#include "SDK/FN_Mailbox_01b_parameters.hpp"
#include "SDK/FN_Living_Table_Dining_01_structs.hpp"
#include "SDK/FN_Living_Table_Dining_01_classes.hpp"
#include "SDK/FN_Living_Table_Dining_01_parameters.hpp"
#include "SDK/FN_Living_BookShelves_01_structs.hpp"
#include "SDK/FN_Living_BookShelves_01_classes.hpp"
#include "SDK/FN_Living_BookShelves_01_parameters.hpp"
#include "SDK/FN_WoodHouse_DoorSide_NoTrim_structs.hpp"
#include "SDK/FN_WoodHouse_DoorSide_NoTrim_classes.hpp"
#include "SDK/FN_WoodHouse_DoorSide_NoTrim_parameters.hpp"
#include "SDK/FN_Fence_Picket01_Doorway1_structs.hpp"
#include "SDK/FN_Fence_Picket01_Doorway1_classes.hpp"
#include "SDK/FN_Fence_Picket01_Doorway1_parameters.hpp"
#include "SDK/FN_Bedroom_Bed_Queen_01_structs.hpp"
#include "SDK/FN_Bedroom_Bed_Queen_01_classes.hpp"
#include "SDK/FN_Bedroom_Bed_Queen_01_parameters.hpp"
#include "SDK/FN_Fence_Picket01_Doorway2_structs.hpp"
#include "SDK/FN_Fence_Picket01_Doorway2_classes.hpp"
#include "SDK/FN_Fence_Picket01_Doorway2_parameters.hpp"
#include "SDK/FN_Bedroom_HopeChest_02_structs.hpp"
#include "SDK/FN_Bedroom_HopeChest_02_classes.hpp"
#include "SDK/FN_Bedroom_HopeChest_02_parameters.hpp"
#include "SDK/FN_Bedroom_HopeChest_03_structs.hpp"
#include "SDK/FN_Bedroom_HopeChest_03_classes.hpp"
#include "SDK/FN_Bedroom_HopeChest_03_parameters.hpp"
#include "SDK/FN_Chair_Dining_structs.hpp"
#include "SDK/FN_Chair_Dining_classes.hpp"
#include "SDK/FN_Chair_Dining_parameters.hpp"
#include "SDK/FN_Hydrant02_structs.hpp"
#include "SDK/FN_Hydrant02_classes.hpp"
#include "SDK/FN_Hydrant02_parameters.hpp"
#include "SDK/FN_Living_BookShelves_07_structs.hpp"
#include "SDK/FN_Living_BookShelves_07_classes.hpp"
#include "SDK/FN_Living_BookShelves_07_parameters.hpp"
#include "SDK/FN_Office_Desk_03_structs.hpp"
#include "SDK/FN_Office_Desk_03_classes.hpp"
#include "SDK/FN_Office_Desk_03_parameters.hpp"
#include "SDK/FN_Storage_CardboardBoxes_02_structs.hpp"
#include "SDK/FN_Storage_CardboardBoxes_02_classes.hpp"
#include "SDK/FN_Storage_CardboardBoxes_02_parameters.hpp"
#include "SDK/FN_BrickSimple_02c_ThreeQuarter_structs.hpp"
#include "SDK/FN_BrickSimple_02c_ThreeQuarter_classes.hpp"
#include "SDK/FN_BrickSimple_02c_ThreeQuarter_parameters.hpp"
#include "SDK/FN_Construction_Concrete_Corner_2_structs.hpp"
#include "SDK/FN_Construction_Concrete_Corner_2_classes.hpp"
#include "SDK/FN_Construction_Concrete_Corner_2_parameters.hpp"
#include "SDK/FN_FORT_StonePath_01_structs.hpp"
#include "SDK/FN_FORT_StonePath_01_classes.hpp"
#include "SDK/FN_FORT_StonePath_01_parameters.hpp"
#include "SDK/FN_TrashPile_04_structs.hpp"
#include "SDK/FN_TrashPile_04_classes.hpp"
#include "SDK/FN_TrashPile_04_parameters.hpp"
#include "SDK/FN_FORT_Bay_Floor_structs.hpp"
#include "SDK/FN_FORT_Bay_Floor_classes.hpp"
#include "SDK/FN_FORT_Bay_Floor_parameters.hpp"
#include "SDK/FN_FORT_Bay_Roof_structs.hpp"
#include "SDK/FN_FORT_Bay_Roof_classes.hpp"
#include "SDK/FN_FORT_Bay_Roof_parameters.hpp"
#include "SDK/FN_Chair_Office02_structs.hpp"
#include "SDK/FN_Chair_Office02_classes.hpp"
#include "SDK/FN_Chair_Office02_parameters.hpp"
#include "SDK/FN_Hedge_Low_90Degree_structs.hpp"
#include "SDK/FN_Hedge_Low_90Degree_classes.hpp"
#include "SDK/FN_Hedge_Low_90Degree_parameters.hpp"
#include "SDK/FN_MetalBleachers_structs.hpp"
#include "SDK/FN_MetalBleachers_classes.hpp"
#include "SDK/FN_MetalBleachers_parameters.hpp"
#include "SDK/FN_PowerTransformer02_structs.hpp"
#include "SDK/FN_PowerTransformer02_classes.hpp"
#include "SDK/FN_PowerTransformer02_parameters.hpp"
#include "SDK/FN_Sign_Durrr_Big_structs.hpp"
#include "SDK/FN_Sign_Durrr_Big_classes.hpp"
#include "SDK/FN_Sign_Durrr_Big_parameters.hpp"
#include "SDK/FN_Prop_Sign_PedXing_structs.hpp"
#include "SDK/FN_Prop_Sign_PedXing_classes.hpp"
#include "SDK/FN_Prop_Sign_PedXing_parameters.hpp"
#include "SDK/FN_FORT_Roof_Corner_structs.hpp"
#include "SDK/FN_FORT_Roof_Corner_classes.hpp"
#include "SDK/FN_FORT_Roof_Corner_parameters.hpp"
#include "SDK/FN_FORT_Roof_Straight_structs.hpp"
#include "SDK/FN_FORT_Roof_Straight_classes.hpp"
#include "SDK/FN_FORT_Roof_Straight_parameters.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_DoubleBack_NoTrim_structs.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_DoubleBack_NoTrim_classes.hpp"
#include "SDK/FN_SM_Wood01_WoodStairs_DoubleBack_NoTrim_parameters.hpp"
#include "SDK/FN_Rural_House_Wall_10_structs.hpp"
#include "SDK/FN_Rural_House_Wall_10_classes.hpp"
#include "SDK/FN_Rural_House_Wall_10_parameters.hpp"
#include "SDK/FN_BrickSimple_AAA_SolidWall_structs.hpp"
#include "SDK/FN_BrickSimple_AAA_SolidWall_classes.hpp"
#include "SDK/FN_BrickSimple_AAA_SolidWall_parameters.hpp"
#include "SDK/FN_S_BuildingTrim_G_Half_structs.hpp"
#include "SDK/FN_S_BuildingTrim_G_Half_classes.hpp"
#include "SDK/FN_S_BuildingTrim_G_Half_parameters.hpp"
#include "SDK/FN_BrickSimple_Chimney_structs.hpp"
#include "SDK/FN_BrickSimple_Chimney_classes.hpp"
#include "SDK/FN_BrickSimple_Chimney_parameters.hpp"
#include "SDK/FN_BrickSimple_Archway01_structs.hpp"
#include "SDK/FN_BrickSimple_Archway01_classes.hpp"
#include "SDK/FN_BrickSimple_Archway01_parameters.hpp"
#include "SDK/FN_BrickSimple_RoofWall_Trim2_structs.hpp"
#include "SDK/FN_BrickSimple_RoofWall_Trim2_classes.hpp"
#include "SDK/FN_BrickSimple_RoofWall_Trim2_parameters.hpp"
#include "SDK/FN_BrickSimple_Window_A_structs.hpp"
#include "SDK/FN_BrickSimple_Window_A_classes.hpp"
#include "SDK/FN_BrickSimple_Window_A_parameters.hpp"
#include "SDK/FN_BrickSimple_Window_C_structs.hpp"
#include "SDK/FN_BrickSimple_Window_C_classes.hpp"
#include "SDK/FN_BrickSimple_Window_C_parameters.hpp"
#include "SDK/FN_Factory_Wall_Garage_structs.hpp"
#include "SDK/FN_Factory_Wall_Garage_classes.hpp"
#include "SDK/FN_Factory_Wall_Garage_parameters.hpp"
#include "SDK/FN_Fence_Picket01_structs.hpp"
#include "SDK/FN_Fence_Picket01_classes.hpp"
#include "SDK/FN_Fence_Picket01_parameters.hpp"
#include "SDK/FN_Fence_Picket01a_structs.hpp"
#include "SDK/FN_Fence_Picket01a_classes.hpp"
#include "SDK/FN_Fence_Picket01a_parameters.hpp"
#include "SDK/FN_BrickSimple_ChimneyTop_structs.hpp"
#include "SDK/FN_BrickSimple_ChimneyTop_classes.hpp"
#include "SDK/FN_BrickSimple_ChimneyTop_parameters.hpp"
#include "SDK/FN_Garage_Door_01_structs.hpp"
#include "SDK/FN_Garage_Door_01_classes.hpp"
#include "SDK/FN_Garage_Door_01_parameters.hpp"
#include "SDK/FN_Prop_Sign_Turn_structs.hpp"
#include "SDK/FN_Prop_Sign_Turn_classes.hpp"
#include "SDK/FN_Prop_Sign_Turn_parameters.hpp"
#include "SDK/FN_Streetlight_Suburban01_structs.hpp"
#include "SDK/FN_Streetlight_Suburban01_classes.hpp"
#include "SDK/FN_Streetlight_Suburban01_parameters.hpp"
#include "SDK/FN_WarehouseShelves_02_structs.hpp"
#include "SDK/FN_WarehouseShelves_02_classes.hpp"
#include "SDK/FN_WarehouseShelves_02_parameters.hpp"
#include "SDK/FN_Fence_Picket01c_structs.hpp"
#include "SDK/FN_Fence_Picket01c_classes.hpp"
#include "SDK/FN_Fence_Picket01c_parameters.hpp"
#include "SDK/FN_WoodMines_Solid_structs.hpp"
#include "SDK/FN_WoodMines_Solid_classes.hpp"
#include "SDK/FN_WoodMines_Solid_parameters.hpp"
#include "SDK/FN_Haybale_01_structs.hpp"
#include "SDK/FN_Haybale_01_classes.hpp"
#include "SDK/FN_Haybale_01_parameters.hpp"
#include "SDK/FN_Car_Copper_structs.hpp"
#include "SDK/FN_Car_Copper_classes.hpp"
#include "SDK/FN_Car_Copper_parameters.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TilesBrokenBeams_structs.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TilesBrokenBeams_classes.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TilesBrokenBeams_parameters.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TilesBrokenBeamsI_structs.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TilesBrokenBeamsI_classes.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_TilesBrokenBeamsI_parameters.hpp"
#include "SDK/FN_Tree_Log_4_structs.hpp"
#include "SDK/FN_Tree_Log_4_classes.hpp"
#include "SDK/FN_Tree_Log_4_parameters.hpp"
#include "SDK/FN_Farm_Roof2_S_structs.hpp"
#include "SDK/FN_Farm_Roof2_S_classes.hpp"
#include "SDK/FN_Farm_Roof2_S_parameters.hpp"
#include "SDK/FN_WoodHouse_Railing_Half_structs.hpp"
#include "SDK/FN_WoodHouse_Railing_Half_classes.hpp"
#include "SDK/FN_WoodHouse_Railing_Half_parameters.hpp"
#include "SDK/FN_SM_Farm_Roof1_PeakCap_structs.hpp"
#include "SDK/FN_SM_Farm_Roof1_PeakCap_classes.hpp"
#include "SDK/FN_SM_Farm_Roof1_PeakCap_parameters.hpp"
#include "SDK/FN_SM_Farm_Roof_1_Cap_structs.hpp"
#include "SDK/FN_SM_Farm_Roof_1_Cap_classes.hpp"
#include "SDK/FN_SM_Farm_Roof_1_Cap_parameters.hpp"
#include "SDK/FN_SM_Farm_Window_2_structs.hpp"
#include "SDK/FN_SM_Farm_Window_2_classes.hpp"
#include "SDK/FN_SM_Farm_Window_2_parameters.hpp"
#include "SDK/FN_WoodMines_Frame02_structs.hpp"
#include "SDK/FN_WoodMines_Frame02_classes.hpp"
#include "SDK/FN_WoodMines_Frame02_parameters.hpp"
#include "SDK/FN_SM_Construction_Concrete_Floor_03_structs.hpp"
#include "SDK/FN_SM_Construction_Concrete_Floor_03_classes.hpp"
#include "SDK/FN_SM_Construction_Concrete_Floor_03_parameters.hpp"
#include "SDK/FN_LCD_PayPhone_01_structs.hpp"
#include "SDK/FN_LCD_PayPhone_01_classes.hpp"
#include "SDK/FN_LCD_PayPhone_01_parameters.hpp"
#include "SDK/FN_Prop_Rocks_StoneSmall01_structs.hpp"
#include "SDK/FN_Prop_Rocks_StoneSmall01_classes.hpp"
#include "SDK/FN_Prop_Rocks_StoneSmall01_parameters.hpp"
#include "SDK/FN_Prop_Rocks_StoneSmall02_structs.hpp"
#include "SDK/FN_Prop_Rocks_StoneSmall02_classes.hpp"
#include "SDK/FN_Prop_Rocks_StoneSmall02_parameters.hpp"
#include "SDK/FN_Military_Bag_structs.hpp"
#include "SDK/FN_Military_Bag_classes.hpp"
#include "SDK/FN_Military_Bag_parameters.hpp"
#include "SDK/FN_Plant_Shrub_04_structs.hpp"
#include "SDK/FN_Plant_Shrub_04_classes.hpp"
#include "SDK/FN_Plant_Shrub_04_parameters.hpp"
#include "SDK/FN_Plant_Mushrooms_Yellow_Generic_structs.hpp"
#include "SDK/FN_Plant_Mushrooms_Yellow_Generic_classes.hpp"
#include "SDK/FN_Plant_Mushrooms_Yellow_Generic_parameters.hpp"
#include "SDK/FN_Utility_TrashCanMedium_01_structs.hpp"
#include "SDK/FN_Utility_TrashCanMedium_01_classes.hpp"
#include "SDK/FN_Utility_TrashCanMedium_01_parameters.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Inner_structs.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Inner_classes.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Inner_parameters.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Outer_structs.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Outer_classes.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Outer_parameters.hpp"
#include "SDK/FN_Prop_IndMedPipe_01_S_structs.hpp"
#include "SDK/FN_Prop_IndMedPipe_01_S_classes.hpp"
#include "SDK/FN_Prop_IndMedPipe_01_S_parameters.hpp"
#include "SDK/FN_WoodMines_Frame03_structs.hpp"
#include "SDK/FN_WoodMines_Frame03_classes.hpp"
#include "SDK/FN_WoodMines_Frame03_parameters.hpp"
#include "SDK/FN_BridgeGirder_A_structs.hpp"
#include "SDK/FN_BridgeGirder_A_classes.hpp"
#include "SDK/FN_BridgeGirder_A_parameters.hpp"
#include "SDK/FN_Construction_ConcreteWall_Solid_03_structs.hpp"
#include "SDK/FN_Construction_ConcreteWall_Solid_03_classes.hpp"
#include "SDK/FN_Construction_ConcreteWall_Solid_03_parameters.hpp"
#include "SDK/FN_BridgeGirder_A2_structs.hpp"
#include "SDK/FN_BridgeGirder_A2_classes.hpp"
#include "SDK/FN_BridgeGirder_A2_parameters.hpp"
#include "SDK/FN_Plant_Shrub_11_structs.hpp"
#include "SDK/FN_Plant_Shrub_11_classes.hpp"
#include "SDK/FN_Plant_Shrub_11_parameters.hpp"
#include "SDK/FN_LCD_Constr_PortoPotty_structs.hpp"
#include "SDK/FN_LCD_Constr_PortoPotty_classes.hpp"
#include "SDK/FN_LCD_Constr_PortoPotty_parameters.hpp"
#include "SDK/FN_Utility_TrashBag_01_structs.hpp"
#include "SDK/FN_Utility_TrashBag_01_classes.hpp"
#include "SDK/FN_Utility_TrashBag_01_parameters.hpp"
#include "SDK/FN_Factory_Wall_19_structs.hpp"
#include "SDK/FN_Factory_Wall_19_classes.hpp"
#include "SDK/FN_Factory_Wall_19_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_B02_structs.hpp"
#include "SDK/FN_Fence_Chainlink_B02_classes.hpp"
#include "SDK/FN_Fence_Chainlink_B02_parameters.hpp"
#include "SDK/FN_GuardRail_3_structs.hpp"
#include "SDK/FN_GuardRail_3_classes.hpp"
#include "SDK/FN_GuardRail_3_parameters.hpp"
#include "SDK/FN_GuardRail_2b_structs.hpp"
#include "SDK/FN_GuardRail_2b_classes.hpp"
#include "SDK/FN_GuardRail_2b_parameters.hpp"
#include "SDK/FN_GuardRail_2end_structs.hpp"
#include "SDK/FN_GuardRail_2end_classes.hpp"
#include "SDK/FN_GuardRail_2end_parameters.hpp"
#include "SDK/FN_Prop_Sign_Stop_structs.hpp"
#include "SDK/FN_Prop_Sign_Stop_classes.hpp"
#include "SDK/FN_Prop_Sign_Stop_parameters.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair1_structs.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair1_classes.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair1_parameters.hpp"
#include "SDK/FN_S_BuildingTrim_G_structs.hpp"
#include "SDK/FN_S_BuildingTrim_G_classes.hpp"
#include "SDK/FN_S_BuildingTrim_G_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_B03_structs.hpp"
#include "SDK/FN_Fence_Chainlink_B03_classes.hpp"
#include "SDK/FN_Fence_Chainlink_B03_parameters.hpp"
#include "SDK/FN_BridgeBase_A_structs.hpp"
#include "SDK/FN_BridgeBase_A_classes.hpp"
#include "SDK/FN_BridgeBase_A_parameters.hpp"
#include "SDK/FN_BridgeBase_B_structs.hpp"
#include "SDK/FN_BridgeBase_B_classes.hpp"
#include "SDK/FN_BridgeBase_B_parameters.hpp"
#include "SDK/FN_BridgeBase_C_structs.hpp"
#include "SDK/FN_BridgeBase_C_classes.hpp"
#include "SDK/FN_BridgeBase_C_parameters.hpp"
#include "SDK/FN_BridgeGirder_A3_structs.hpp"
#include "SDK/FN_BridgeGirder_A3_classes.hpp"
#include "SDK/FN_BridgeGirder_A3_parameters.hpp"
#include "SDK/FN_BridgeGirder_B_structs.hpp"
#include "SDK/FN_BridgeGirder_B_classes.hpp"
#include "SDK/FN_BridgeGirder_B_parameters.hpp"
#include "SDK/FN_BridgeGirder_C_structs.hpp"
#include "SDK/FN_BridgeGirder_C_classes.hpp"
#include "SDK/FN_BridgeGirder_C_parameters.hpp"
#include "SDK/FN_BridgeGirder_D_structs.hpp"
#include "SDK/FN_BridgeGirder_D_classes.hpp"
#include "SDK/FN_BridgeGirder_D_parameters.hpp"
#include "SDK/FN_BridgeGirder_F_structs.hpp"
#include "SDK/FN_BridgeGirder_F_classes.hpp"
#include "SDK/FN_BridgeGirder_F_parameters.hpp"
#include "SDK/FN_BridgeGirder_G_structs.hpp"
#include "SDK/FN_BridgeGirder_G_classes.hpp"
#include "SDK/FN_BridgeGirder_G_parameters.hpp"
#include "SDK/FN_BridgeGirder_K_structs.hpp"
#include "SDK/FN_BridgeGirder_K_classes.hpp"
#include "SDK/FN_BridgeGirder_K_parameters.hpp"
#include "SDK/FN_BridgeGirder_L_structs.hpp"
#include "SDK/FN_BridgeGirder_L_classes.hpp"
#include "SDK/FN_BridgeGirder_L_parameters.hpp"
#include "SDK/FN_Factory_Wall_16_structs.hpp"
#include "SDK/FN_Factory_Wall_16_classes.hpp"
#include "SDK/FN_Factory_Wall_16_parameters.hpp"
#include "SDK/FN_Factory_Wall_20_structs.hpp"
#include "SDK/FN_Factory_Wall_20_classes.hpp"
#include "SDK/FN_Factory_Wall_20_parameters.hpp"
#include "SDK/FN_Factory_Wall_33_structs.hpp"
#include "SDK/FN_Factory_Wall_33_classes.hpp"
#include "SDK/FN_Factory_Wall_33_parameters.hpp"
#include "SDK/FN_Bridge_Railing_01_structs.hpp"
#include "SDK/FN_Bridge_Railing_01_classes.hpp"
#include "SDK/FN_Bridge_Railing_01_parameters.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Railing_structs.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Railing_classes.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Railing_parameters.hpp"
#include "SDK/FN_Ammo_Spawn_structs.hpp"
#include "SDK/FN_Ammo_Spawn_classes.hpp"
#include "SDK/FN_Ammo_Spawn_parameters.hpp"
#include "SDK/FN_Hobopile_A_structs.hpp"
#include "SDK/FN_Hobopile_A_classes.hpp"
#include "SDK/FN_Hobopile_A_parameters.hpp"
#include "SDK/FN_BrickSimple_01c_ThreeQuarter_structs.hpp"
#include "SDK/FN_BrickSimple_01c_ThreeQuarter_classes.hpp"
#include "SDK/FN_BrickSimple_01c_ThreeQuarter_parameters.hpp"
#include "SDK/FN_BridgeBase_CornerA_structs.hpp"
#include "SDK/FN_BridgeBase_CornerA_classes.hpp"
#include "SDK/FN_BridgeBase_CornerA_parameters.hpp"
#include "SDK/FN_BridgeBase_CornerB_structs.hpp"
#include "SDK/FN_BridgeBase_CornerB_classes.hpp"
#include "SDK/FN_BridgeBase_CornerB_parameters.hpp"
#include "SDK/FN_BridgeBase_CornerC_structs.hpp"
#include "SDK/FN_BridgeBase_CornerC_classes.hpp"
#include "SDK/FN_BridgeBase_CornerC_parameters.hpp"
#include "SDK/FN_Factory_4b_Whole_structs.hpp"
#include "SDK/FN_Factory_4b_Whole_classes.hpp"
#include "SDK/FN_Factory_4b_Whole_parameters.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Straight_structs.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Straight_classes.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Straight_parameters.hpp"
#include "SDK/FN_Bridge_Railing_Curve_B_structs.hpp"
#include "SDK/FN_Bridge_Railing_Curve_B_classes.hpp"
#include "SDK/FN_Bridge_Railing_Curve_B_parameters.hpp"
#include "SDK/FN_BridgeGirder_E_structs.hpp"
#include "SDK/FN_BridgeGirder_E_classes.hpp"
#include "SDK/FN_BridgeGirder_E_parameters.hpp"
#include "SDK/FN_BridgeGirder_End_structs.hpp"
#include "SDK/FN_BridgeGirder_End_classes.hpp"
#include "SDK/FN_BridgeGirder_End_parameters.hpp"
#include "SDK/FN_B_Keep_AmmoStash_structs.hpp"
#include "SDK/FN_B_Keep_AmmoStash_classes.hpp"
#include "SDK/FN_B_Keep_AmmoStash_parameters.hpp"
#include "SDK/FN_Recreation_Cooler_structs.hpp"
#include "SDK/FN_Recreation_Cooler_classes.hpp"
#include "SDK/FN_Recreation_Cooler_parameters.hpp"
#include "SDK/FN_Storage_CardboardBoxes_01_structs.hpp"
#include "SDK/FN_Storage_CardboardBoxes_01_classes.hpp"
#include "SDK/FN_Storage_CardboardBoxes_01_parameters.hpp"
#include "SDK/FN_Chair_Cafeteria_01_structs.hpp"
#include "SDK/FN_Chair_Cafeteria_01_classes.hpp"
#include "SDK/FN_Chair_Cafeteria_01_parameters.hpp"
#include "SDK/FN_Outdoor_Grill_structs.hpp"
#include "SDK/FN_Outdoor_Grill_classes.hpp"
#include "SDK/FN_Outdoor_Grill_parameters.hpp"
#include "SDK/FN_Sign_NationalPark_structs.hpp"
#include "SDK/FN_Sign_NationalPark_classes.hpp"
#include "SDK/FN_Sign_NationalPark_parameters.hpp"
#include "SDK/FN_Factory_Wall_6_structs.hpp"
#include "SDK/FN_Factory_Wall_6_classes.hpp"
#include "SDK/FN_Factory_Wall_6_parameters.hpp"
#include "SDK/FN_Boardwalk_Archway_structs.hpp"
#include "SDK/FN_Boardwalk_Archway_classes.hpp"
#include "SDK/FN_Boardwalk_Archway_parameters.hpp"
#include "SDK/FN_Prop_TirePile_01_structs.hpp"
#include "SDK/FN_Prop_TirePile_01_classes.hpp"
#include "SDK/FN_Prop_TirePile_01_parameters.hpp"
#include "SDK/FN_Plant_Shrub_03_structs.hpp"
#include "SDK/FN_Plant_Shrub_03_classes.hpp"
#include "SDK/FN_Plant_Shrub_03_parameters.hpp"
#include "SDK/FN_Haybale_03_structs.hpp"
#include "SDK/FN_Haybale_03_classes.hpp"
#include "SDK/FN_Haybale_03_parameters.hpp"
#include "SDK/FN_ListOfWaterComponentsThatTheCharactersInteractingWithByIndex_structs.hpp"
#include "SDK/FN_ListOfWaterComponentsThatTheCharactersInteractingWithByIndex_classes.hpp"
#include "SDK/FN_ListOfWaterComponentsThatTheCharactersInteractingWithByIndex_parameters.hpp"
#include "SDK/FN_Palettes_Short_structs.hpp"
#include "SDK/FN_Palettes_Short_classes.hpp"
#include "SDK/FN_Palettes_Short_parameters.hpp"
#include "SDK/FN_Picnic_Table_structs.hpp"
#include "SDK/FN_Picnic_Table_classes.hpp"
#include "SDK/FN_Picnic_Table_parameters.hpp"
#include "SDK/FN_Prop_Sign_NatureSign_01_structs.hpp"
#include "SDK/FN_Prop_Sign_NatureSign_01_classes.hpp"
#include "SDK/FN_Prop_Sign_NatureSign_01_parameters.hpp"
#include "SDK/FN_Prop_Sign_Slow_structs.hpp"
#include "SDK/FN_Prop_Sign_Slow_classes.hpp"
#include "SDK/FN_Prop_Sign_Slow_parameters.hpp"
#include "SDK/FN_Prop_Sign_TrailSign_02_structs.hpp"
#include "SDK/FN_Prop_Sign_TrailSign_02_classes.hpp"
#include "SDK/FN_Prop_Sign_TrailSign_02_parameters.hpp"
#include "SDK/FN_Recreation_Canopy_structs.hpp"
#include "SDK/FN_Recreation_Canopy_classes.hpp"
#include "SDK/FN_Recreation_Canopy_parameters.hpp"
#include "SDK/FN_Prop_Sign_TrailSign_01_structs.hpp"
#include "SDK/FN_Prop_Sign_TrailSign_01_classes.hpp"
#include "SDK/FN_Prop_Sign_TrailSign_01_parameters.hpp"
#include "SDK/FN_Living_Television_01_structs.hpp"
#include "SDK/FN_Living_Television_01_classes.hpp"
#include "SDK/FN_Living_Television_01_parameters.hpp"
#include "SDK/FN_Kitchen_Refrigerator_01_structs.hpp"
#include "SDK/FN_Kitchen_Refrigerator_01_classes.hpp"
#include "SDK/FN_Kitchen_Refrigerator_01_parameters.hpp"
#include "SDK/FN_SpawnTeleportPadPlayerData_structs.hpp"
#include "SDK/FN_SpawnTeleportPadPlayerData_classes.hpp"
#include "SDK/FN_SpawnTeleportPadPlayerData_parameters.hpp"
#include "SDK/FN_Kitchen_Stove_01_structs.hpp"
#include "SDK/FN_Kitchen_Stove_01_classes.hpp"
#include "SDK/FN_Kitchen_Stove_01_parameters.hpp"
#include "SDK/FN_Haybale_04_structs.hpp"
#include "SDK/FN_Haybale_04_classes.hpp"
#include "SDK/FN_Haybale_04_parameters.hpp"
#include "SDK/FN_SupplyDropUnlocks_structs.hpp"
#include "SDK/FN_SupplyDropUnlocks_classes.hpp"
#include "SDK/FN_SupplyDropUnlocks_parameters.hpp"
#include "SDK/FN_Haybale_05_structs.hpp"
#include "SDK/FN_Haybale_05_classes.hpp"
#include "SDK/FN_Haybale_05_parameters.hpp"
#include "SDK/FN_Living_Couch_01_structs.hpp"
#include "SDK/FN_Living_Couch_01_classes.hpp"
#include "SDK/FN_Living_Couch_01_parameters.hpp"
#include "SDK/FN_Plant_Shrub_02_structs.hpp"
#include "SDK/FN_Plant_Shrub_02_classes.hpp"
#include "SDK/FN_Plant_Shrub_02_parameters.hpp"
#include "SDK/FN_Tree_Bush_4_structs.hpp"
#include "SDK/FN_Tree_Bush_4_classes.hpp"
#include "SDK/FN_Tree_Bush_4_parameters.hpp"
#include "SDK/FN_Boardwalk_BalconyO_structs.hpp"
#include "SDK/FN_Boardwalk_BalconyO_classes.hpp"
#include "SDK/FN_Boardwalk_BalconyO_parameters.hpp"
#include "SDK/FN_Boardwalk_BalconyS_structs.hpp"
#include "SDK/FN_Boardwalk_BalconyS_classes.hpp"
#include "SDK/FN_Boardwalk_BalconyS_parameters.hpp"
#include "SDK/FN_Prop_WildWest_SimpleChair_02_structs.hpp"
#include "SDK/FN_Prop_WildWest_SimpleChair_02_classes.hpp"
#include "SDK/FN_Prop_WildWest_SimpleChair_02_parameters.hpp"
#include "SDK/FN_Plant_Shrub_bush_sm_01_structs.hpp"
#include "SDK/FN_Plant_Shrub_bush_sm_01_classes.hpp"
#include "SDK/FN_Plant_Shrub_bush_sm_01_parameters.hpp"
#include "SDK/FN_B_SupplyDropCameraShake_structs.hpp"
#include "SDK/FN_B_SupplyDropCameraShake_classes.hpp"
#include "SDK/FN_B_SupplyDropCameraShake_parameters.hpp"
#include "SDK/FN_B_PlasmaCameraShake_structs.hpp"
#include "SDK/FN_B_PlasmaCameraShake_classes.hpp"
#include "SDK/FN_B_PlasmaCameraShake_parameters.hpp"
#include "SDK/FN_B_Post_Explosion_Generic_structs.hpp"
#include "SDK/FN_B_Post_Explosion_Generic_classes.hpp"
#include "SDK/FN_B_Post_Explosion_Generic_parameters.hpp"
#include "SDK/FN_B_Post_SupplyDrop_Impact_structs.hpp"
#include "SDK/FN_B_Post_SupplyDrop_Impact_classes.hpp"
#include "SDK/FN_B_Post_SupplyDrop_Impact_parameters.hpp"
#include "SDK/FN_Plant_Mushrooms_Red_Forest_structs.hpp"
#include "SDK/FN_Plant_Mushrooms_Red_Forest_classes.hpp"
#include "SDK/FN_Plant_Mushrooms_Red_Forest_parameters.hpp"
#include "SDK/FN_Plant_Mushrooms_Red_Forest_3_structs.hpp"
#include "SDK/FN_Plant_Mushrooms_Red_Forest_3_classes.hpp"
#include "SDK/FN_Plant_Mushrooms_Red_Forest_3_parameters.hpp"
#include "SDK/FN_Plant_Mushrooms_Yellow_Generic_3_structs.hpp"
#include "SDK/FN_Plant_Mushrooms_Yellow_Generic_3_classes.hpp"
#include "SDK/FN_Plant_Mushrooms_Yellow_Generic_3_parameters.hpp"
#include "SDK/FN_BP_SafeZoneStorm_structs.hpp"
#include "SDK/FN_BP_SafeZoneStorm_classes.hpp"
#include "SDK/FN_BP_SafeZoneStorm_parameters.hpp"
#include "SDK/FN_Plant_Shrub_08_structs.hpp"
#include "SDK/FN_Plant_Shrub_08_classes.hpp"
#include "SDK/FN_Plant_Shrub_08_parameters.hpp"
#include "SDK/FN_Car_Tractor_structs.hpp"
#include "SDK/FN_Car_Tractor_classes.hpp"
#include "SDK/FN_Car_Tractor_parameters.hpp"
#include "SDK/FN_Prop_Rocks_08_structs.hpp"
#include "SDK/FN_Prop_Rocks_08_classes.hpp"
#include "SDK/FN_Prop_Rocks_08_parameters.hpp"
#include "SDK/FN_Shelters_Matress04_structs.hpp"
#include "SDK/FN_Shelters_Matress04_classes.hpp"
#include "SDK/FN_Shelters_Matress04_parameters.hpp"
#include "SDK/FN_Tree_Log_7_structs.hpp"
#include "SDK/FN_Tree_Log_7_classes.hpp"
#include "SDK/FN_Tree_Log_7_parameters.hpp"
#include "SDK/FN_Bathroom_Toilet_01_structs.hpp"
#include "SDK/FN_Bathroom_Toilet_01_classes.hpp"
#include "SDK/FN_Bathroom_Toilet_01_parameters.hpp"
#include "SDK/FN_Bathroom_Sink_01_structs.hpp"
#include "SDK/FN_Bathroom_Sink_01_classes.hpp"
#include "SDK/FN_Bathroom_Sink_01_parameters.hpp"
#include "SDK/FN_SM_Farm_QuarterWall_structs.hpp"
#include "SDK/FN_SM_Farm_QuarterWall_classes.hpp"
#include "SDK/FN_SM_Farm_QuarterWall_parameters.hpp"
#include "SDK/FN_LF_Athena_POI_7x12_StoneCluster_structs.hpp"
#include "SDK/FN_LF_Athena_POI_7x12_StoneCluster_classes.hpp"
#include "SDK/FN_LF_Athena_POI_7x12_StoneCluster_parameters.hpp"
#include "SDK/FN_Utility_TrashCanSmall_01_structs.hpp"
#include "SDK/FN_Utility_TrashCanSmall_01_classes.hpp"
#include "SDK/FN_Utility_TrashCanSmall_01_parameters.hpp"
#include "SDK/FN_TrashPile_01_structs.hpp"
#include "SDK/FN_TrashPile_01_classes.hpp"
#include "SDK/FN_TrashPile_01_parameters.hpp"
#include "SDK/FN_GuardRail_2_structs.hpp"
#include "SDK/FN_GuardRail_2_classes.hpp"
#include "SDK/FN_GuardRail_2_parameters.hpp"
#include "SDK/FN_GuardRail_1b_structs.hpp"
#include "SDK/FN_GuardRail_1b_classes.hpp"
#include "SDK/FN_GuardRail_1b_parameters.hpp"
#include "SDK/FN_Prop_Rocks_05_structs.hpp"
#include "SDK/FN_Prop_Rocks_05_classes.hpp"
#include "SDK/FN_Prop_Rocks_05_parameters.hpp"
#include "SDK/FN_LF_7x12_GenericPlot_structs.hpp"
#include "SDK/FN_LF_7x12_GenericPlot_classes.hpp"
#include "SDK/FN_LF_7x12_GenericPlot_parameters.hpp"
#include "SDK/FN_Tree_StumpSet02_structs.hpp"
#include "SDK/FN_Tree_StumpSet02_classes.hpp"
#include "SDK/FN_Tree_StumpSet02_parameters.hpp"
#include "SDK/FN_BP_PlayerSpawnTeleportPad_structs.hpp"
#include "SDK/FN_BP_PlayerSpawnTeleportPad_classes.hpp"
#include "SDK/FN_BP_PlayerSpawnTeleportPad_parameters.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Railing_Half_structs.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Railing_Half_classes.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Railing_Half_parameters.hpp"
#include "SDK/FN_AthenaAircraft_CameraMode_structs.hpp"
#include "SDK/FN_AthenaAircraft_CameraMode_classes.hpp"
#include "SDK/FN_AthenaAircraft_CameraMode_parameters.hpp"
#include "SDK/FN_GET_DamageParent_structs.hpp"
#include "SDK/FN_GET_DamageParent_classes.hpp"
#include "SDK/FN_GET_DamageParent_parameters.hpp"
#include "SDK/FN_Boardwalk_QuarterWallS_structs.hpp"
#include "SDK/FN_Boardwalk_QuarterWallS_classes.hpp"
#include "SDK/FN_Boardwalk_QuarterWallS_parameters.hpp"
#include "SDK/FN_Tiered_Short_Ammo_3_Parent_structs.hpp"
#include "SDK/FN_Tiered_Short_Ammo_3_Parent_classes.hpp"
#include "SDK/FN_Tiered_Short_Ammo_3_Parent_parameters.hpp"
#include "SDK/FN_DefaultMapInfo_structs.hpp"
#include "SDK/FN_DefaultMapInfo_classes.hpp"
#include "SDK/FN_DefaultMapInfo_parameters.hpp"
#include "SDK/FN_Athena_GameMode_structs.hpp"
#include "SDK/FN_Athena_GameMode_classes.hpp"
#include "SDK/FN_Athena_GameMode_parameters.hpp"
#include "SDK/FN_SafeZoneIndicator_structs.hpp"
#include "SDK/FN_SafeZoneIndicator_classes.hpp"
#include "SDK/FN_SafeZoneIndicator_parameters.hpp"
#include "SDK/FN_BP_DamageBalloon_Athena_structs.hpp"
#include "SDK/FN_BP_DamageBalloon_Athena_classes.hpp"
#include "SDK/FN_BP_DamageBalloon_Athena_parameters.hpp"
#include "SDK/FN_AthenaSupplyDrop_02_structs.hpp"
#include "SDK/FN_AthenaSupplyDrop_02_classes.hpp"
#include "SDK/FN_AthenaSupplyDrop_02_parameters.hpp"
#include "SDK/FN_AthenaAircraft_structs.hpp"
#include "SDK/FN_AthenaAircraft_classes.hpp"
#include "SDK/FN_AthenaAircraft_parameters.hpp"
#include "SDK/FN_Tiered_Chest_3_Child_structs.hpp"
#include "SDK/FN_Tiered_Chest_3_Child_classes.hpp"
#include "SDK/FN_Tiered_Chest_3_Child_parameters.hpp"
#include "SDK/FN_Athena_Tree_Hero_02_structs.hpp"
#include "SDK/FN_Athena_Tree_Hero_02_classes.hpp"
#include "SDK/FN_Athena_Tree_Hero_02_parameters.hpp"
#include "SDK/FN_Athena_Tree_Pine_01_structs.hpp"
#include "SDK/FN_Athena_Tree_Pine_01_classes.hpp"
#include "SDK/FN_Athena_Tree_Pine_01_parameters.hpp"
#include "SDK/FN_Plant_Shrub_09_structs.hpp"
#include "SDK/FN_Plant_Shrub_09_classes.hpp"
#include "SDK/FN_Plant_Shrub_09_parameters.hpp"
#include "SDK/FN_BP_PlayerSpawnTeleportPadDeployableBase_structs.hpp"
#include "SDK/FN_BP_PlayerSpawnTeleportPadDeployableBase_classes.hpp"
#include "SDK/FN_BP_PlayerSpawnTeleportPadDeployableBase_parameters.hpp"
#include "SDK/FN_Athena_Terrain_structs.hpp"
#include "SDK/FN_Athena_Terrain_classes.hpp"
#include "SDK/FN_Athena_Terrain_parameters.hpp"
#include "SDK/FN_Fence_Picket01_Half_b_structs.hpp"
#include "SDK/FN_Fence_Picket01_Half_b_classes.hpp"
#include "SDK/FN_Fence_Picket01_Half_b_parameters.hpp"
#include "SDK/FN_WoodMines_FrameX_structs.hpp"
#include "SDK/FN_WoodMines_FrameX_classes.hpp"
#include "SDK/FN_WoodMines_FrameX_parameters.hpp"
#include "SDK/FN_WoodMines_Frame01_structs.hpp"
#include "SDK/FN_WoodMines_Frame01_classes.hpp"
#include "SDK/FN_WoodMines_Frame01_parameters.hpp"
#include "SDK/FN_WoodMines_Archway_Support_structs.hpp"
#include "SDK/FN_WoodMines_Archway_Support_classes.hpp"
#include "SDK/FN_WoodMines_Archway_Support_parameters.hpp"
#include "SDK/FN_WoodMines_Archway_Large02_structs.hpp"
#include "SDK/FN_WoodMines_Archway_Large02_classes.hpp"
#include "SDK/FN_WoodMines_Archway_Large02_parameters.hpp"
#include "SDK/FN_WoodMines_Archway_Large01_structs.hpp"
#include "SDK/FN_WoodMines_Archway_Large01_classes.hpp"
#include "SDK/FN_WoodMines_Archway_Large01_parameters.hpp"
#include "SDK/FN_StoreGeneric_WindowWide_structs.hpp"
#include "SDK/FN_StoreGeneric_WindowWide_classes.hpp"
#include "SDK/FN_StoreGeneric_WindowWide_parameters.hpp"
#include "SDK/FN_SM_Farm_Wall_5_structs.hpp"
#include "SDK/FN_SM_Farm_Wall_5_classes.hpp"
#include "SDK/FN_SM_Farm_Wall_5_parameters.hpp"
#include "SDK/FN_SM_Farm_Wall_4_structs.hpp"
#include "SDK/FN_SM_Farm_Wall_4_classes.hpp"
#include "SDK/FN_SM_Farm_Wall_4_parameters.hpp"
#include "SDK/FN_SM_Farm_Wall_3_structs.hpp"
#include "SDK/FN_SM_Farm_Wall_3_classes.hpp"
#include "SDK/FN_SM_Farm_Wall_3_parameters.hpp"
#include "SDK/FN_SM_Farm_Roof_2_Cap_structs.hpp"
#include "SDK/FN_SM_Farm_Roof_2_Cap_classes.hpp"
#include "SDK/FN_SM_Farm_Roof_2_Cap_parameters.hpp"
#include "SDK/FN_Pueblo_Trim_01_structs.hpp"
#include "SDK/FN_Pueblo_Trim_01_classes.hpp"
#include "SDK/FN_Pueblo_Trim_01_parameters.hpp"
#include "SDK/FN_Girder03_structs.hpp"
#include "SDK/FN_Girder03_classes.hpp"
#include "SDK/FN_Girder03_parameters.hpp"
#include "SDK/FN_Factory_Wall_5_structs.hpp"
#include "SDK/FN_Factory_Wall_5_classes.hpp"
#include "SDK/FN_Factory_Wall_5_parameters.hpp"
#include "SDK/FN_Factory_Wall_4_structs.hpp"
#include "SDK/FN_Factory_Wall_4_classes.hpp"
#include "SDK/FN_Factory_Wall_4_parameters.hpp"
#include "SDK/FN_Factory_Wall_25_structs.hpp"
#include "SDK/FN_Factory_Wall_25_classes.hpp"
#include "SDK/FN_Factory_Wall_25_parameters.hpp"
#include "SDK/FN_Factory_Wall_22_structs.hpp"
#include "SDK/FN_Factory_Wall_22_classes.hpp"
#include "SDK/FN_Factory_Wall_22_parameters.hpp"
#include "SDK/FN_GE_Generic_Character_Launch_structs.hpp"
#include "SDK/FN_GE_Generic_Character_Launch_classes.hpp"
#include "SDK/FN_GE_Generic_Character_Launch_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_A06_BarbedWire_structs.hpp"
#include "SDK/FN_Fence_Chainlink_A06_BarbedWire_classes.hpp"
#include "SDK/FN_Fence_Chainlink_A06_BarbedWire_parameters.hpp"
#include "SDK/FN_Fence_Chainlink_A01_BarbedWire_structs.hpp"
#include "SDK/FN_Fence_Chainlink_A01_BarbedWire_classes.hpp"
#include "SDK/FN_Fence_Chainlink_A01_BarbedWire_parameters.hpp"
#include "SDK/FN_Factory_Wall_18_structs.hpp"
#include "SDK/FN_Factory_Wall_18_classes.hpp"
#include "SDK/FN_Factory_Wall_18_parameters.hpp"
#include "SDK/FN_Factory_Wall_2_structs.hpp"
#include "SDK/FN_Factory_Wall_2_classes.hpp"
#include "SDK/FN_Factory_Wall_2_parameters.hpp"
#include "SDK/FN_Factory_Trim_19d_structs.hpp"
#include "SDK/FN_Factory_Trim_19d_classes.hpp"
#include "SDK/FN_Factory_Trim_19d_parameters.hpp"
#include "SDK/FN_Factory_Trim_17a_structs.hpp"
#include "SDK/FN_Factory_Trim_17a_classes.hpp"
#include "SDK/FN_Factory_Trim_17a_parameters.hpp"
#include "SDK/FN_Factory_Trim_10a_structs.hpp"
#include "SDK/FN_Factory_Trim_10a_classes.hpp"
#include "SDK/FN_Factory_Trim_10a_parameters.hpp"
#include "SDK/FN_Factory_QuarterWall_structs.hpp"
#include "SDK/FN_Factory_QuarterWall_classes.hpp"
#include "SDK/FN_Factory_QuarterWall_parameters.hpp"
#include "SDK/FN_Factory_HalfWall_Transition_structs.hpp"
#include "SDK/FN_Factory_HalfWall_Transition_classes.hpp"
#include "SDK/FN_Factory_HalfWall_Transition_parameters.hpp"
#include "SDK/FN_Construction_ConcreteWall_Trim_01_structs.hpp"
#include "SDK/FN_Construction_ConcreteWall_Trim_01_classes.hpp"
#include "SDK/FN_Construction_ConcreteWall_Trim_01_parameters.hpp"
#include "SDK/FN_Commerce_VendingMachine_Drink_structs.hpp"
#include "SDK/FN_Commerce_VendingMachine_Drink_classes.hpp"
#include "SDK/FN_Commerce_VendingMachine_Drink_parameters.hpp"
#include "SDK/FN_Prop_Sign_Private_Property_structs.hpp"
#include "SDK/FN_Prop_Sign_Private_Property_classes.hpp"
#include "SDK/FN_Prop_Sign_Private_Property_parameters.hpp"
#include "SDK/FN_Prop_TirePile_02_structs.hpp"
#include "SDK/FN_Prop_TirePile_02_classes.hpp"
#include "SDK/FN_Prop_TirePile_02_parameters.hpp"
#include "SDK/FN_Prop_TirePile_04_structs.hpp"
#include "SDK/FN_Prop_TirePile_04_classes.hpp"
#include "SDK/FN_Prop_TirePile_04_parameters.hpp"
#include "SDK/FN_Prop_TirePile_03_structs.hpp"
#include "SDK/FN_Prop_TirePile_03_classes.hpp"
#include "SDK/FN_Prop_TirePile_03_parameters.hpp"
#include "SDK/FN_Prop_Sign_Reserved_Parking_structs.hpp"
#include "SDK/FN_Prop_Sign_Reserved_Parking_classes.hpp"
#include "SDK/FN_Prop_Sign_Reserved_Parking_parameters.hpp"
#include "SDK/FN_Prop_Sign_Permit_Parking_structs.hpp"
#include "SDK/FN_Prop_Sign_Permit_Parking_classes.hpp"
#include "SDK/FN_Prop_Sign_Permit_Parking_parameters.hpp"
#include "SDK/FN_Prop_Sign_NoParking_structs.hpp"
#include "SDK/FN_Prop_Sign_NoParking_classes.hpp"
#include "SDK/FN_Prop_Sign_NoParking_parameters.hpp"
#include "SDK/FN_Prop_Sign_Handicapped_structs.hpp"
#include "SDK/FN_Prop_Sign_Handicapped_classes.hpp"
#include "SDK/FN_Prop_Sign_Handicapped_parameters.hpp"
#include "SDK/FN_Boardwalk_QuarterWallHalf_structs.hpp"
#include "SDK/FN_Boardwalk_QuarterWallHalf_classes.hpp"
#include "SDK/FN_Boardwalk_QuarterWallHalf_parameters.hpp"
#include "SDK/FN_SM_Boardwalk_Stairs_structs.hpp"
#include "SDK/FN_SM_Boardwalk_Stairs_classes.hpp"
#include "SDK/FN_SM_Boardwalk_Stairs_parameters.hpp"
#include "SDK/FN_SM_Boardwalk_Stair2_structs.hpp"
#include "SDK/FN_SM_Boardwalk_Stair2_classes.hpp"
#include "SDK/FN_SM_Boardwalk_Stair2_parameters.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair2_structs.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair2_classes.hpp"
#include "SDK/FN_IND_Rusty_Catwalk_Stair2_parameters.hpp"
#include "SDK/FN_Farm_Roof1_S_structs.hpp"
#include "SDK/FN_Farm_Roof1_S_classes.hpp"
#include "SDK/FN_Farm_Roof1_S_parameters.hpp"
#include "SDK/FN_Farm_Roof1_Peak_structs.hpp"
#include "SDK/FN_Farm_Roof1_Peak_classes.hpp"
#include "SDK/FN_Farm_Roof1_Peak_parameters.hpp"
#include "SDK/FN_Factory_Roof_5_structs.hpp"
#include "SDK/FN_Factory_Roof_5_classes.hpp"
#include "SDK/FN_Factory_Roof_5_parameters.hpp"
#include "SDK/FN_Factory_Roof_2_structs.hpp"
#include "SDK/FN_Factory_Roof_2_classes.hpp"
#include "SDK/FN_Factory_Roof_2_parameters.hpp"
#include "SDK/FN_WaterPipe02_structs.hpp"
#include "SDK/FN_WaterPipe02_classes.hpp"
#include "SDK/FN_WaterPipe02_parameters.hpp"
#include "SDK/FN_WarehouseShelves_06b_structs.hpp"
#include "SDK/FN_WarehouseShelves_06b_classes.hpp"
#include "SDK/FN_WarehouseShelves_06b_parameters.hpp"
#include "SDK/FN_WarehouseShelves_03_structs.hpp"
#include "SDK/FN_WarehouseShelves_03_classes.hpp"
#include "SDK/FN_WarehouseShelves_03_parameters.hpp"
#include "SDK/FN_Tree_Small_2_structs.hpp"
#include "SDK/FN_Tree_Small_2_classes.hpp"
#include "SDK/FN_Tree_Small_2_parameters.hpp"
#include "SDK/FN_Tree_Bush_1a_structs.hpp"
#include "SDK/FN_Tree_Bush_1a_classes.hpp"
#include "SDK/FN_Tree_Bush_1a_parameters.hpp"
#include "SDK/FN_Street_Light_Urban01_structs.hpp"
#include "SDK/FN_Street_Light_Urban01_classes.hpp"
#include "SDK/FN_Street_Light_Urban01_parameters.hpp"
#include "SDK/FN_Street_Light__3_structs.hpp"
#include "SDK/FN_Street_Light__3_classes.hpp"
#include "SDK/FN_Street_Light__3_parameters.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_WindowBoards01_structs.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_WindowBoards01_classes.hpp"
#include "SDK/FN_SM_FORT_DecoBarriers_WindowBoards01_parameters.hpp"
#include "SDK/FN_Shelters_TargetPractice01_structs.hpp"
#include "SDK/FN_Shelters_TargetPractice01_classes.hpp"
#include "SDK/FN_Shelters_TargetPractice01_parameters.hpp"
#include "SDK/FN_S_ParkingBlock_4_structs.hpp"
#include "SDK/FN_S_ParkingBlock_4_classes.hpp"
#include "SDK/FN_S_ParkingBlock_4_parameters.hpp"
#include "SDK/FN_S_ParkingBlock_3_structs.hpp"
#include "SDK/FN_S_ParkingBlock_3_classes.hpp"
#include "SDK/FN_S_ParkingBlock_3_parameters.hpp"
#include "SDK/FN_S_Parking_Spot_Stopper_structs.hpp"
#include "SDK/FN_S_Parking_Spot_Stopper_classes.hpp"
#include "SDK/FN_S_Parking_Spot_Stopper_parameters.hpp"
#include "SDK/FN_Recreation_ParkBench_04_structs.hpp"
#include "SDK/FN_Recreation_ParkBench_04_classes.hpp"
#include "SDK/FN_Recreation_ParkBench_04_parameters.hpp"
#include "SDK/FN_Ranger_Lightpost_structs.hpp"
#include "SDK/FN_Ranger_Lightpost_classes.hpp"
#include "SDK/FN_Ranger_Lightpost_parameters.hpp"
#include "SDK/FN_Prop_Rocks_Piles_6_structs.hpp"
#include "SDK/FN_Prop_Rocks_Piles_6_classes.hpp"
#include "SDK/FN_Prop_Rocks_Piles_6_parameters.hpp"
#include "SDK/FN_Prop_Rocks_Piles_5_structs.hpp"
#include "SDK/FN_Prop_Rocks_Piles_5_classes.hpp"
#include "SDK/FN_Prop_Rocks_Piles_5_parameters.hpp"
#include "SDK/FN_Prop_Rocks_04_structs.hpp"
#include "SDK/FN_Prop_Rocks_04_classes.hpp"
#include "SDK/FN_Prop_Rocks_04_parameters.hpp"
#include "SDK/FN_Prop_Rocks_03_structs.hpp"
#include "SDK/FN_Prop_Rocks_03_classes.hpp"
#include "SDK/FN_Prop_Rocks_03_parameters.hpp"
#include "SDK/FN_Prop_Rocks_02_structs.hpp"
#include "SDK/FN_Prop_Rocks_02_classes.hpp"
#include "SDK/FN_Prop_Rocks_02_parameters.hpp"
#include "SDK/FN_PowerTransformer01_structs.hpp"
#include "SDK/FN_PowerTransformer01_classes.hpp"
#include "SDK/FN_PowerTransformer01_parameters.hpp"
#include "SDK/FN_Parking_Bollard_structs.hpp"
#include "SDK/FN_Parking_Bollard_classes.hpp"
#include "SDK/FN_Parking_Bollard_parameters.hpp"
#include "SDK/FN_Palettes_Tall_structs.hpp"
#include "SDK/FN_Palettes_Tall_classes.hpp"
#include "SDK/FN_Palettes_Tall_parameters.hpp"
#include "SDK/FN_JackStraw_structs.hpp"
#include "SDK/FN_JackStraw_classes.hpp"
#include "SDK/FN_JackStraw_parameters.hpp"
#include "SDK/FN_Hedge_Low_Rectangle_structs.hpp"
#include "SDK/FN_Hedge_Low_Rectangle_classes.hpp"
#include "SDK/FN_Hedge_Low_Rectangle_parameters.hpp"
#include "SDK/FN_Crypt_Dust_01_structs.hpp"
#include "SDK/FN_Crypt_Dust_01_classes.hpp"
#include "SDK/FN_Crypt_Dust_01_parameters.hpp"
#include "SDK/FN_Container_Support_structs.hpp"
#include "SDK/FN_Container_Support_classes.hpp"
#include "SDK/FN_Container_Support_parameters.hpp"
#include "SDK/FN_Car_SemiTruck_Dump_structs.hpp"
#include "SDK/FN_Car_SemiTruck_Dump_classes.hpp"
#include "SDK/FN_Car_SemiTruck_Dump_parameters.hpp"
#include "SDK/FN_Car_Stocker_structs.hpp"
#include "SDK/FN_Car_Stocker_classes.hpp"
#include "SDK/FN_Car_Stocker_parameters.hpp"
#include "SDK/FN_Car_SemiTruck_Container_structs.hpp"
#include "SDK/FN_Car_SemiTruck_Container_classes.hpp"
#include "SDK/FN_Car_SemiTruck_Container_parameters.hpp"
#include "SDK/FN_Car_SchoolBus_structs.hpp"
#include "SDK/FN_Car_SchoolBus_classes.hpp"
#include "SDK/FN_Car_SchoolBus_parameters.hpp"
#include "SDK/FN_Car_Rat_structs.hpp"
#include "SDK/FN_Car_Rat_classes.hpp"
#include "SDK/FN_Car_Rat_parameters.hpp"
#include "SDK/FN_BridgeGirder_Lamp_structs.hpp"
#include "SDK/FN_BridgeGirder_Lamp_classes.hpp"
#include "SDK/FN_BridgeGirder_Lamp_parameters.hpp"
#include "SDK/FN_WoodMines_BalconyO_structs.hpp"
#include "SDK/FN_WoodMines_BalconyO_classes.hpp"
#include "SDK/FN_WoodMines_BalconyO_parameters.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTileBeams_structs.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTileBeams_classes.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTileBeams_parameters.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTile_structs.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTile_classes.hpp"
#include "SDK/FN_SM_FORT_Floors_Generic_BasicTile_parameters.hpp"
#include "SDK/FN_SM_FloorQuarter01_structs.hpp"
#include "SDK/FN_SM_FloorQuarter01_classes.hpp"
#include "SDK/FN_SM_FloorQuarter01_parameters.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Floor_structs.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Floor_classes.hpp"
#include "SDK/FN_IDN_Rusty_Catwalk_Floor_parameters.hpp"
#include "SDK/FN_TrashPile_05_structs.hpp"
#include "SDK/FN_TrashPile_05_classes.hpp"
#include "SDK/FN_TrashPile_05_parameters.hpp"
#include "SDK/FN_Factory_9_structs.hpp"
#include "SDK/FN_Factory_9_classes.hpp"
#include "SDK/FN_Factory_9_parameters.hpp"
#include "SDK/FN_Factory_3_structs.hpp"
#include "SDK/FN_Factory_3_classes.hpp"
#include "SDK/FN_Factory_3_parameters.hpp"
#include "SDK/FN_Factory_1c_ThreeQuarter_structs.hpp"
#include "SDK/FN_Factory_1c_ThreeQuarter_classes.hpp"
#include "SDK/FN_Factory_1c_ThreeQuarter_parameters.hpp"
#include "SDK/FN_Factory_1b_Whole_structs.hpp"
#include "SDK/FN_Factory_1b_Whole_classes.hpp"
#include "SDK/FN_Factory_1b_Whole_parameters.hpp"
#include "SDK/FN_Utility_TrashCanDowntown_01_structs.hpp"
#include "SDK/FN_Utility_TrashCanDowntown_01_classes.hpp"
#include "SDK/FN_Utility_TrashCanDowntown_01_parameters.hpp"
#include "SDK/FN_Utility_TrashCan_01_structs.hpp"
#include "SDK/FN_Utility_TrashCan_01_classes.hpp"
#include "SDK/FN_Utility_TrashCan_01_parameters.hpp"
#include "SDK/FN_Utility_Dumpster_01_structs.hpp"
#include "SDK/FN_Utility_Dumpster_01_classes.hpp"
#include "SDK/FN_Utility_Dumpster_01_parameters.hpp"
#include "SDK/FN_Storage_Crate3a_side_structs.hpp"
#include "SDK/FN_Storage_Crate3a_side_classes.hpp"
#include "SDK/FN_Storage_Crate3a_side_parameters.hpp"
#include "SDK/FN_Storage_Crate02_structs.hpp"
#include "SDK/FN_Storage_Crate02_classes.hpp"
#include "SDK/FN_Storage_Crate02_parameters.hpp"
#include "SDK/FN_Storage_Crate01a_Side_structs.hpp"
#include "SDK/FN_Storage_Crate01a_Side_classes.hpp"
#include "SDK/FN_Storage_Crate01a_Side_parameters.hpp"
#include "SDK/FN_Plant_Shrub_06_structs.hpp"
#include "SDK/FN_Plant_Shrub_06_classes.hpp"
#include "SDK/FN_Plant_Shrub_06_parameters.hpp"
#include "SDK/FN_Plant_Shrub_01_structs.hpp"
#include "SDK/FN_Plant_Shrub_01_classes.hpp"
#include "SDK/FN_Plant_Shrub_01_parameters.hpp"
#include "SDK/FN_Living_EasyChair_02_structs.hpp"
#include "SDK/FN_Living_EasyChair_02_classes.hpp"
#include "SDK/FN_Living_EasyChair_02_parameters.hpp"
#include "SDK/FN_LCD_RubblePile_02_structs.hpp"
#include "SDK/FN_LCD_RubblePile_02_classes.hpp"
#include "SDK/FN_LCD_RubblePile_02_parameters.hpp"
#include "SDK/FN_Tiered_Athena_FloorLoot_01_structs.hpp"
#include "SDK/FN_Tiered_Athena_FloorLoot_01_classes.hpp"
#include "SDK/FN_Tiered_Athena_FloorLoot_01_parameters.hpp"
#include "SDK/FN_Basic_Corner_01e_Quarter_structs.hpp"
#include "SDK/FN_Basic_Corner_01e_Quarter_classes.hpp"
#include "SDK/FN_Basic_Corner_01e_Quarter_parameters.hpp"
#include "SDK/FN_NavLink_Terrain_Inner_structs.hpp"
#include "SDK/FN_NavLink_Terrain_Inner_classes.hpp"
#include "SDK/FN_NavLink_Terrain_Inner_parameters.hpp"
#include "SDK/FN_Barriers_Wood01_structs.hpp"
#include "SDK/FN_Barriers_Wood01_classes.hpp"
#include "SDK/FN_Barriers_Wood01_parameters.hpp"
#include "SDK/FN_WoodMines_Archway_structs.hpp"
#include "SDK/FN_WoodMines_Archway_classes.hpp"
#include "SDK/FN_WoodMines_Archway_parameters.hpp"
#include "SDK/FN_Prop_Bear_structs.hpp"
#include "SDK/FN_Prop_Bear_classes.hpp"
#include "SDK/FN_Prop_Bear_parameters.hpp"
#include "SDK/FN_WoodMines_Floor_structs.hpp"
#include "SDK/FN_WoodMines_Floor_classes.hpp"
#include "SDK/FN_WoodMines_Floor_parameters.hpp"
#include "SDK/FN_WoodMines_BalconyS_structs.hpp"
#include "SDK/FN_WoodMines_BalconyS_classes.hpp"
#include "SDK/FN_WoodMines_BalconyS_parameters.hpp"
#include "SDK/FN_Athena_Tree_Pine_4_structs.hpp"
#include "SDK/FN_Athena_Tree_Pine_4_classes.hpp"
#include "SDK/FN_Athena_Tree_Pine_4_parameters.hpp"
#include "SDK/FN_NavLink_Terrain_Straight_structs.hpp"
#include "SDK/FN_NavLink_Terrain_Straight_classes.hpp"
#include "SDK/FN_NavLink_Terrain_Straight_parameters.hpp"
#include "SDK/FN_Athena_Tree_Pine_05_structs.hpp"
#include "SDK/FN_Athena_Tree_Pine_05_classes.hpp"
#include "SDK/FN_Athena_Tree_Pine_05_parameters.hpp"
#include "SDK/FN_Athena_Tree_Medium_04__structs.hpp"
#include "SDK/FN_Athena_Tree_Medium_04__classes.hpp"
#include "SDK/FN_Athena_Tree_Medium_04__parameters.hpp"
#include "SDK/FN_Athena_Tree_Medium_03__structs.hpp"
#include "SDK/FN_Athena_Tree_Medium_03__classes.hpp"
#include "SDK/FN_Athena_Tree_Medium_03__parameters.hpp"
#include "SDK/FN_NavLink_Terrain_Outer_structs.hpp"
#include "SDK/FN_NavLink_Terrain_Outer_classes.hpp"
#include "SDK/FN_NavLink_Terrain_Outer_parameters.hpp"
#include "SDK/FN_Athena_Tree_Medium_01__structs.hpp"
#include "SDK/FN_Athena_Tree_Medium_01__classes.hpp"
#include "SDK/FN_Athena_Tree_Medium_01__parameters.hpp"
#include "SDK/FN_Athena_Tree_Large_2_structs.hpp"
#include "SDK/FN_Athena_Tree_Large_2_classes.hpp"
#include "SDK/FN_Athena_Tree_Large_2_parameters.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_Open_B_structs.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_Open_B_classes.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_Open_B_parameters.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_Closed_structs.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_Closed_classes.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_Closed_parameters.hpp"
#include "SDK/FN_Athena_Prop_Rocks_StoneLarge01_structs.hpp"
#include "SDK/FN_Athena_Prop_Rocks_StoneLarge01_classes.hpp"
#include "SDK/FN_Athena_Prop_Rocks_StoneLarge01_parameters.hpp"
#include "SDK/FN_Car_Pickup_structs.hpp"
#include "SDK/FN_Car_Pickup_classes.hpp"
#include "SDK/FN_Car_Pickup_parameters.hpp"
#include "SDK/FN_Car_StationWagon_structs.hpp"
#include "SDK/FN_Car_StationWagon_classes.hpp"
#include "SDK/FN_Car_StationWagon_parameters.hpp"
#include "SDK/FN_Car_SemiTruck_Flatbed_structs.hpp"
#include "SDK/FN_Car_SemiTruck_Flatbed_classes.hpp"
#include "SDK/FN_Car_SemiTruck_Flatbed_parameters.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_Open_A_structs.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_Open_A_classes.hpp"
#include "SDK/FN_Athena_Prop_ShippingContainer_01_Open_A_parameters.hpp"
#include "SDK/FN_Athena_Prop_Rocks_StoneLarge03_structs.hpp"
#include "SDK/FN_Athena_Prop_Rocks_StoneLarge03_classes.hpp"
#include "SDK/FN_Athena_Prop_Rocks_StoneLarge03_parameters.hpp"
#include "SDK/FN_Athena_Prop_Rocks_StoneLarge02_structs.hpp"
#include "SDK/FN_Athena_Prop_Rocks_StoneLarge02_classes.hpp"
#include "SDK/FN_Athena_Prop_Rocks_StoneLarge02_parameters.hpp"
#include "SDK/FN_Car_SemiTruck_ContainerB_structs.hpp"
#include "SDK/FN_Car_SemiTruck_ContainerB_classes.hpp"
#include "SDK/FN_Car_SemiTruck_ContainerB_parameters.hpp"
#include "SDK/FN_Boardwalk_Floor_structs.hpp"
#include "SDK/FN_Boardwalk_Floor_classes.hpp"
#include "SDK/FN_Boardwalk_Floor_parameters.hpp"
#include "SDK/FN_Storage_Crate_Small01_structs.hpp"
#include "SDK/FN_Storage_Crate_Small01_classes.hpp"
#include "SDK/FN_Storage_Crate_Small01_parameters.hpp"
#include "SDK/FN_Recreation_CampFire_01_structs.hpp"
#include "SDK/FN_Recreation_CampFire_01_classes.hpp"
#include "SDK/FN_Recreation_CampFire_01_parameters.hpp"
#include "SDK/FN_Recreation_Camp_Tent_01_structs.hpp"
#include "SDK/FN_Recreation_Camp_Tent_01_classes.hpp"
#include "SDK/FN_Recreation_Camp_Tent_01_parameters.hpp"
#include "SDK/FN_Ranger_LodgeStatue_01_structs.hpp"
#include "SDK/FN_Ranger_LodgeStatue_01_classes.hpp"
#include "SDK/FN_Ranger_LodgeStatue_01_parameters.hpp"
#include "SDK/FN_LF_1x1_Foundation_structs.hpp"
#include "SDK/FN_LF_1x1_Foundation_classes.hpp"
#include "SDK/FN_LF_1x1_Foundation_parameters.hpp"
#include "SDK/FN_SM_Farm_Wall_2_structs.hpp"
#include "SDK/FN_SM_Farm_Wall_2_classes.hpp"
#include "SDK/FN_SM_Farm_Wall_2_parameters.hpp"
#include "SDK/FN_SM_Farm_HalfWallHalf_structs.hpp"
#include "SDK/FN_SM_Farm_HalfWallHalf_classes.hpp"
#include "SDK/FN_SM_Farm_HalfWallHalf_parameters.hpp"
#include "SDK/FN_SM_Farm_HalfWall_structs.hpp"
#include "SDK/FN_SM_Farm_HalfWall_classes.hpp"
#include "SDK/FN_SM_Farm_HalfWall_parameters.hpp"
#include "SDK/FN_Fence_Rural_Low_End_structs.hpp"
#include "SDK/FN_Fence_Rural_Low_End_classes.hpp"
#include "SDK/FN_Fence_Rural_Low_End_parameters.hpp"
#include "SDK/FN_Fence_Rural_Low_structs.hpp"
#include "SDK/FN_Fence_Rural_Low_classes.hpp"
#include "SDK/FN_Fence_Rural_Low_parameters.hpp"
#include "SDK/FN_Fence_Rural_1missing_structs.hpp"
#include "SDK/FN_Fence_Rural_1missing_classes.hpp"
#include "SDK/FN_Fence_Rural_1missing_parameters.hpp"
#include "SDK/FN_Factory_HalfWall_structs.hpp"
#include "SDK/FN_Factory_HalfWall_classes.hpp"
#include "SDK/FN_Factory_HalfWall_parameters.hpp"
#include "SDK/FN_SubGameSelectRotatorItems_structs.hpp"
#include "SDK/FN_SubGameSelectRotatorItems_classes.hpp"
#include "SDK/FN_SubGameSelectRotatorItems_parameters.hpp"
#include "SDK/FN_CryptWalls_07_structs.hpp"
#include "SDK/FN_CryptWalls_07_classes.hpp"
#include "SDK/FN_CryptWalls_07_parameters.hpp"
#include "SDK/FN_CryptWalls_06_structs.hpp"
#include "SDK/FN_CryptWalls_06_classes.hpp"
#include "SDK/FN_CryptWalls_06_parameters.hpp"
#include "SDK/FN_Boardwalk_ArchwayLong_structs.hpp"
#include "SDK/FN_Boardwalk_ArchwayLong_classes.hpp"
#include "SDK/FN_Boardwalk_ArchwayLong_parameters.hpp"
#include "SDK/FN_Tree_Log_2_structs.hpp"
#include "SDK/FN_Tree_Log_2_classes.hpp"
#include "SDK/FN_Tree_Log_2_parameters.hpp"
#include "SDK/FN_Tree_04_structs.hpp"
#include "SDK/FN_Tree_04_classes.hpp"
#include "SDK/FN_Tree_04_parameters.hpp"
#include "SDK/FN_WidgetCarousel_structs.hpp"
#include "SDK/FN_WidgetCarousel_classes.hpp"
#include "SDK/FN_WidgetCarousel_parameters.hpp"
#include "SDK/FN_Recreation_Tent_structs.hpp"
#include "SDK/FN_Recreation_Tent_classes.hpp"
#include "SDK/FN_Recreation_Tent_parameters.hpp"
#include "SDK/FN_SocialTypes_structs.hpp"
#include "SDK/FN_SocialTypes_classes.hpp"
#include "SDK/FN_SocialTypes_parameters.hpp"
#include "SDK/FN_Recreation_BasketballHoop__3_structs.hpp"
#include "SDK/FN_Recreation_BasketballHoop__3_classes.hpp"
#include "SDK/FN_Recreation_BasketballHoop__3_parameters.hpp"
#include "SDK/FN_CommonUI_structs.hpp"
#include "SDK/FN_CommonUI_classes.hpp"
#include "SDK/FN_CommonUI_parameters.hpp"
#include "SDK/FN_Prop_WildWest_TelegraphPole_02_structs.hpp"
#include "SDK/FN_Prop_WildWest_TelegraphPole_02_classes.hpp"
#include "SDK/FN_Prop_WildWest_TelegraphPole_02_parameters.hpp"
#include "SDK/FN_ButtonStyle-Primary-XL-Yellow_structs.hpp"
#include "SDK/FN_ButtonStyle-Primary-XL-Yellow_classes.hpp"
#include "SDK/FN_ButtonStyle-Primary-XL-Yellow_parameters.hpp"
#include "SDK/FN_Prop_Rocks_Piles_3_structs.hpp"
#include "SDK/FN_Prop_Rocks_Piles_3_classes.hpp"
#include "SDK/FN_Prop_Rocks_Piles_3_parameters.hpp"
#include "SDK/FN_TextStyle-Button-Primary-M-FadedDisabled_structs.hpp"
#include "SDK/FN_TextStyle-Button-Primary-M-FadedDisabled_classes.hpp"
#include "SDK/FN_TextStyle-Button-Primary-M-FadedDisabled_parameters.hpp"
#include "SDK/FN_Prop_Rocks_Piles_2_structs.hpp"
#include "SDK/FN_Prop_Rocks_Piles_2_classes.hpp"
#include "SDK/FN_Prop_Rocks_Piles_2_parameters.hpp"
#include "SDK/FN_Prop_Rocks_06_structs.hpp"
#include "SDK/FN_Prop_Rocks_06_classes.hpp"
#include "SDK/FN_Prop_Rocks_06_parameters.hpp"
#include "SDK/FN_ButtonStyle-Skew_structs.hpp"
#include "SDK/FN_ButtonStyle-Skew_classes.hpp"
#include "SDK/FN_ButtonStyle-Skew_parameters.hpp"
#include "SDK/FN_EpicCMSUIFramework_structs.hpp"
#include "SDK/FN_EpicCMSUIFramework_classes.hpp"
#include "SDK/FN_EpicCMSUIFramework_parameters.hpp"
#include "SDK/FN_ConsoleProfileWidget_structs.hpp"
#include "SDK/FN_ConsoleProfileWidget_classes.hpp"
#include "SDK/FN_ConsoleProfileWidget_parameters.hpp"
#include "SDK/FN_PowerTower02_structs.hpp"
#include "SDK/FN_PowerTower02_classes.hpp"
#include "SDK/FN_PowerTower02_parameters.hpp"
#include "SDK/FN_CinematicLanuageToTrackStruct_structs.hpp"
#include "SDK/FN_CinematicLanuageToTrackStruct_classes.hpp"
#include "SDK/FN_CinematicLanuageToTrackStruct_parameters.hpp"
#include "SDK/FN_Car_IceCreamTruck_structs.hpp"
#include "SDK/FN_Car_IceCreamTruck_classes.hpp"
#include "SDK/FN_Car_IceCreamTruck_parameters.hpp"
#include "SDK/FN_Car_SemiTruck_Tractor_structs.hpp"
#include "SDK/FN_Car_SemiTruck_Tractor_classes.hpp"
#include "SDK/FN_Car_SemiTruck_Tractor_parameters.hpp"
#include "SDK/FN_TextStyle-Base_structs.hpp"
#include "SDK/FN_TextStyle-Base_classes.hpp"
#include "SDK/FN_TextStyle-Base_parameters.hpp"
#include "SDK/FN_Storage_Suitcases_03_structs.hpp"
#include "SDK/FN_Storage_Suitcases_03_classes.hpp"
#include "SDK/FN_Storage_Suitcases_03_parameters.hpp"
#include "SDK/FN_Storage_Crate01_structs.hpp"
#include "SDK/FN_Storage_Crate01_classes.hpp"
#include "SDK/FN_Storage_Crate01_parameters.hpp"
#include "SDK/FN_Storage_CardboardBoxes_04_structs.hpp"
#include "SDK/FN_Storage_CardboardBoxes_04_classes.hpp"
#include "SDK/FN_Storage_CardboardBoxes_04_parameters.hpp"
#include "SDK/FN_Storage_CardboardBoxes_03_structs.hpp"
#include "SDK/FN_Storage_CardboardBoxes_03_classes.hpp"
#include "SDK/FN_Storage_CardboardBoxes_03_parameters.hpp"
#include "SDK/FN_Hobopile_B_structs.hpp"
#include "SDK/FN_Hobopile_B_classes.hpp"
#include "SDK/FN_Hobopile_B_parameters.hpp"
#include "SDK/FN_Athena_Tree_WeepingWillow_01_structs.hpp"
#include "SDK/FN_Athena_Tree_WeepingWillow_01_classes.hpp"
#include "SDK/FN_Athena_Tree_WeepingWillow_01_parameters.hpp"
#include "SDK/FN_Prop_Rocks_Piles_4_structs.hpp"
#include "SDK/FN_Prop_Rocks_Piles_4_classes.hpp"
#include "SDK/FN_Prop_Rocks_Piles_4_parameters.hpp"
#include "SDK/FN_LoginResultWIdget_structs.hpp"
#include "SDK/FN_LoginResultWIdget_classes.hpp"
#include "SDK/FN_LoginResultWIdget_parameters.hpp"
#include "SDK/FN_GameSubCatalog_structs.hpp"
#include "SDK/FN_GameSubCatalog_classes.hpp"
#include "SDK/FN_GameSubCatalog_parameters.hpp"
#include "SDK/FN_Athena_Terrain_LS_01_structs.hpp"
#include "SDK/FN_Athena_Terrain_LS_01_classes.hpp"
#include "SDK/FN_Athena_Terrain_LS_01_parameters.hpp"
#include "SDK/FN_FortniteUI_structs.hpp"
#include "SDK/FN_FortniteUI_classes.hpp"
#include "SDK/FN_FortniteUI_parameters.hpp"
#include "SDK/FN_TextStyle-BaseParent_structs.hpp"
#include "SDK/FN_TextStyle-BaseParent_classes.hpp"
#include "SDK/FN_TextStyle-BaseParent_parameters.hpp"
#include "SDK/FN_SubgameSelectScreen_structs.hpp"
#include "SDK/FN_SubgameSelectScreen_classes.hpp"
#include "SDK/FN_SubgameSelectScreen_parameters.hpp"
#include "SDK/FN_EFortUITheme_structs.hpp"
#include "SDK/FN_EFortUITheme_classes.hpp"
#include "SDK/FN_EFortUITheme_parameters.hpp"
#include "SDK/FN_LoginScreen_structs.hpp"
#include "SDK/FN_LoginScreen_classes.hpp"
#include "SDK/FN_LoginScreen_parameters.hpp"
#include "SDK/FN_LoginBackground_structs.hpp"
#include "SDK/FN_LoginBackground_classes.hpp"
#include "SDK/FN_LoginBackground_parameters.hpp"
#include "SDK/FN_TextStyle-HeaderParent_structs.hpp"
#include "SDK/FN_TextStyle-HeaderParent_classes.hpp"
#include "SDK/FN_TextStyle-HeaderParent_parameters.hpp"
#include "SDK/FN_StatusWidget_structs.hpp"
#include "SDK/FN_StatusWidget_classes.hpp"
#include "SDK/FN_StatusWidget_parameters.hpp"
#include "SDK/FN_JoinServer_structs.hpp"
#include "SDK/FN_JoinServer_classes.hpp"
#include "SDK/FN_JoinServer_parameters.hpp"
#include "SDK/FN_SubgameSelect_structs.hpp"
#include "SDK/FN_SubgameSelect_classes.hpp"
#include "SDK/FN_SubgameSelect_parameters.hpp"
#include "SDK/FN_AdditionalEntriesIndicator_structs.hpp"
#include "SDK/FN_AdditionalEntriesIndicator_classes.hpp"
#include "SDK/FN_AdditionalEntriesIndicator_parameters.hpp"
#include "SDK/FN_RejoinWindow_structs.hpp"
#include "SDK/FN_RejoinWindow_classes.hpp"
#include "SDK/FN_RejoinWindow_parameters.hpp"
#include "SDK/FN_AccountLinkingWindow_structs.hpp"
#include "SDK/FN_AccountLinkingWindow_classes.hpp"
#include "SDK/FN_AccountLinkingWindow_parameters.hpp"
#include "SDK/FN_ProgressBarType_structs.hpp"
#include "SDK/FN_ProgressBarType_classes.hpp"
#include "SDK/FN_ProgressBarType_parameters.hpp"
#include "SDK/FN_EulaWidget_structs.hpp"
#include "SDK/FN_EulaWidget_classes.hpp"
#include "SDK/FN_EulaWidget_parameters.hpp"
#include "SDK/FN_LoginAccountSelect_structs.hpp"
#include "SDK/FN_LoginAccountSelect_classes.hpp"
#include "SDK/FN_LoginAccountSelect_parameters.hpp"
#include "SDK/FN_SignInWidget_structs.hpp"
#include "SDK/FN_SignInWidget_classes.hpp"
#include "SDK/FN_SignInWidget_parameters.hpp"
#include "SDK/FN_SplashScreenWidget_structs.hpp"
#include "SDK/FN_SplashScreenWidget_classes.hpp"
#include "SDK/FN_SplashScreenWidget_parameters.hpp"
#include "SDK/FN_TutorialWindow_structs.hpp"
#include "SDK/FN_TutorialWindow_classes.hpp"
#include "SDK/FN_TutorialWindow_parameters.hpp"
#include "SDK/FN_FrontEnd_structs.hpp"
#include "SDK/FN_FrontEnd_classes.hpp"
#include "SDK/FN_FrontEnd_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S_structs.hpp"
#include "SDK/FN_TextStyle-Base-S_classes.hpp"
#include "SDK/FN_TextStyle-Base-S_parameters.hpp"
#include "SDK/FN_Cinematic-TeamMics_structs.hpp"
#include "SDK/FN_Cinematic-TeamMics_classes.hpp"
#include "SDK/FN_Cinematic-TeamMics_parameters.hpp"
#include "SDK/FN_DynamicQuestUpdateInfo_structs.hpp"
#include "SDK/FN_DynamicQuestUpdateInfo_classes.hpp"
#include "SDK/FN_DynamicQuestUpdateInfo_parameters.hpp"
#include "SDK/FN_TextStyle-SignInPage-S_structs.hpp"
#include "SDK/FN_TextStyle-SignInPage-S_classes.hpp"
#include "SDK/FN_TextStyle-SignInPage-S_parameters.hpp"
#include "SDK/FN_HUD_structs.hpp"
#include "SDK/FN_HUD_classes.hpp"
#include "SDK/FN_HUD_parameters.hpp"
#include "SDK/FN_Cinematic_structs.hpp"
#include "SDK/FN_Cinematic_classes.hpp"
#include "SDK/FN_Cinematic_parameters.hpp"
#include "SDK/FN_TextStyle-Base-L_structs.hpp"
#include "SDK/FN_TextStyle-Base-L_classes.hpp"
#include "SDK/FN_TextStyle-Base-L_parameters.hpp"
#include "SDK/FN_TeamMicStack_structs.hpp"
#include "SDK/FN_TeamMicStack_classes.hpp"
#include "SDK/FN_TeamMicStack_parameters.hpp"
#include "SDK/FN_Results_Widget_structs.hpp"
#include "SDK/FN_Results_Widget_classes.hpp"
#include "SDK/FN_Results_Widget_parameters.hpp"
#include "SDK/FN_HUD-TeamMemberInfo_structs.hpp"
#include "SDK/FN_HUD-TeamMemberInfo_classes.hpp"
#include "SDK/FN_HUD-TeamMemberInfo_parameters.hpp"
#include "SDK/FN_BluGloInventory_structs.hpp"
#include "SDK/FN_BluGloInventory_classes.hpp"
#include "SDK/FN_BluGloInventory_parameters.hpp"
#include "SDK/FN_EquippedItem-Bandolier_structs.hpp"
#include "SDK/FN_EquippedItem-Bandolier_classes.hpp"
#include "SDK/FN_EquippedItem-Bandolier_parameters.hpp"
#include "SDK/FN_MicIndicator_structs.hpp"
#include "SDK/FN_MicIndicator_classes.hpp"
#include "SDK/FN_MicIndicator_parameters.hpp"
#include "SDK/FN_HUD_TeamInfo_structs.hpp"
#include "SDK/FN_HUD_TeamInfo_classes.hpp"
#include "SDK/FN_HUD_TeamInfo_parameters.hpp"
#include "SDK/FN_Announcement_Layout_structs.hpp"
#include "SDK/FN_Announcement_Layout_classes.hpp"
#include "SDK/FN_Announcement_Layout_parameters.hpp"
#include "SDK/FN_Announcement_Basic_structs.hpp"
#include "SDK/FN_Announcement_Basic_classes.hpp"
#include "SDK/FN_Announcement_Basic_parameters.hpp"
#include "SDK/FN_ZoneDayCompletion_ScoreBlock_structs.hpp"
#include "SDK/FN_ZoneDayCompletion_ScoreBlock_classes.hpp"
#include "SDK/FN_ZoneDayCompletion_ScoreBlock_parameters.hpp"
#include "SDK/FN_ZoneScoreWidget_structs.hpp"
#include "SDK/FN_ZoneScoreWidget_classes.hpp"
#include "SDK/FN_ZoneScoreWidget_parameters.hpp"
#include "SDK/FN_TeamScoreToPlayerXPRewardWidget_structs.hpp"
#include "SDK/FN_TeamScoreToPlayerXPRewardWidget_classes.hpp"
#include "SDK/FN_TeamScoreToPlayerXPRewardWidget_parameters.hpp"
#include "SDK/FN_PlayerInfo_structs.hpp"
#include "SDK/FN_PlayerInfo_classes.hpp"
#include "SDK/FN_PlayerInfo_parameters.hpp"
#include "SDK/FN_Border-PlayerXPBar-Frame_structs.hpp"
#include "SDK/FN_Border-PlayerXPBar-Frame_classes.hpp"
#include "SDK/FN_Border-PlayerXPBar-Frame_parameters.hpp"
#include "SDK/FN_StoreOfferCosts_structs.hpp"
#include "SDK/FN_StoreOfferCosts_classes.hpp"
#include "SDK/FN_StoreOfferCosts_parameters.hpp"
#include "SDK/FN_TextStyle-PageTitle_structs.hpp"
#include "SDK/FN_TextStyle-PageTitle_classes.hpp"
#include "SDK/FN_TextStyle-PageTitle_parameters.hpp"
#include "SDK/FN_PickupManager_structs.hpp"
#include "SDK/FN_PickupManager_classes.hpp"
#include "SDK/FN_PickupManager_parameters.hpp"
#include "SDK/FN_QuestUpdateEntry_structs.hpp"
#include "SDK/FN_QuestUpdateEntry_classes.hpp"
#include "SDK/FN_QuestUpdateEntry_parameters.hpp"
#include "SDK/FN_CraftingBar_structs.hpp"
#include "SDK/FN_CraftingBar_classes.hpp"
#include "SDK/FN_CraftingBar_parameters.hpp"
#include "SDK/FN_QuestObjectiveEntry_structs.hpp"
#include "SDK/FN_QuestObjectiveEntry_classes.hpp"
#include "SDK/FN_QuestObjectiveEntry_parameters.hpp"
#include "SDK/FN_QuestProgressWidget_structs.hpp"
#include "SDK/FN_QuestProgressWidget_classes.hpp"
#include "SDK/FN_QuestProgressWidget_parameters.hpp"
#include "SDK/FN_PinnedSchematicItemWidget_structs.hpp"
#include "SDK/FN_PinnedSchematicItemWidget_classes.hpp"
#include "SDK/FN_PinnedSchematicItemWidget_parameters.hpp"
#include "SDK/FN_MissionFocusWidget_structs.hpp"
#include "SDK/FN_MissionFocusWidget_classes.hpp"
#include "SDK/FN_MissionFocusWidget_parameters.hpp"
#include "SDK/FN_PinnedSchematicItemsWidget_structs.hpp"
#include "SDK/FN_PinnedSchematicItemsWidget_classes.hpp"
#include "SDK/FN_PinnedSchematicItemsWidget_parameters.hpp"
#include "SDK/FN_DeathWidget_structs.hpp"
#include "SDK/FN_DeathWidget_classes.hpp"
#include "SDK/FN_DeathWidget_parameters.hpp"
#include "SDK/FN_EquippedItemWidget_structs.hpp"
#include "SDK/FN_EquippedItemWidget_classes.hpp"
#include "SDK/FN_EquippedItemWidget_parameters.hpp"
#include "SDK/FN_PlayerZoneTeamScoreContributionWidget_structs.hpp"
#include "SDK/FN_PlayerZoneTeamScoreContributionWidget_classes.hpp"
#include "SDK/FN_PlayerZoneTeamScoreContributionWidget_parameters.hpp"
#include "SDK/FN_TeamMemberBluGloIndicator_structs.hpp"
#include "SDK/FN_TeamMemberBluGloIndicator_classes.hpp"
#include "SDK/FN_TeamMemberBluGloIndicator_parameters.hpp"
#include "SDK/FN_HordeTierResultsWidget_structs.hpp"
#include "SDK/FN_HordeTierResultsWidget_classes.hpp"
#include "SDK/FN_HordeTierResultsWidget_parameters.hpp"
#include "SDK/FN_Results_NameplateWidget_structs.hpp"
#include "SDK/FN_Results_NameplateWidget_classes.hpp"
#include "SDK/FN_Results_NameplateWidget_parameters.hpp"
#include "SDK/FN_LocalPlayerHitPointInfo_structs.hpp"
#include "SDK/FN_LocalPlayerHitPointInfo_classes.hpp"
#include "SDK/FN_LocalPlayerHitPointInfo_parameters.hpp"
#include "SDK/FN_HordeWaveAlertWidget_structs.hpp"
#include "SDK/FN_HordeWaveAlertWidget_classes.hpp"
#include "SDK/FN_HordeWaveAlertWidget_parameters.hpp"
#include "SDK/FN_SurvivorRescuedCounter_structs.hpp"
#include "SDK/FN_SurvivorRescuedCounter_classes.hpp"
#include "SDK/FN_SurvivorRescuedCounter_parameters.hpp"
#include "SDK/FN_ButtonStyle-Tab-Main_structs.hpp"
#include "SDK/FN_ButtonStyle-Tab-Main_classes.hpp"
#include "SDK/FN_ButtonStyle-Tab-Main_parameters.hpp"
#include "SDK/FN_HordeWaveModifiersWidget_structs.hpp"
#include "SDK/FN_HordeWaveModifiersWidget_classes.hpp"
#include "SDK/FN_HordeWaveModifiersWidget_parameters.hpp"
#include "SDK/FN_HordeWaveModifiersTile_structs.hpp"
#include "SDK/FN_HordeWaveModifiersTile_classes.hpp"
#include "SDK/FN_HordeWaveModifiersTile_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B_parameters.hpp"
#include "SDK/FN_KillerPortraitWidget_structs.hpp"
#include "SDK/FN_KillerPortraitWidget_classes.hpp"
#include "SDK/FN_KillerPortraitWidget_parameters.hpp"
#include "SDK/FN_Announce_QuestUpdate_structs.hpp"
#include "SDK/FN_Announce_QuestUpdate_classes.hpp"
#include "SDK/FN_Announce_QuestUpdate_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S-Green_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S-Green_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S-Green_parameters.hpp"
#include "SDK/FN_QuestUpdatesLog_structs.hpp"
#include "SDK/FN_QuestUpdatesLog_classes.hpp"
#include "SDK/FN_QuestUpdatesLog_parameters.hpp"
#include "SDK/FN_Announcement_QuestUpdate_structs.hpp"
#include "SDK/FN_Announcement_QuestUpdate_classes.hpp"
#include "SDK/FN_Announcement_QuestUpdate_parameters.hpp"
#include "SDK/FN_Results_TeamScore_structs.hpp"
#include "SDK/FN_Results_TeamScore_classes.hpp"
#include "SDK/FN_Results_TeamScore_parameters.hpp"
#include "SDK/FN_Results_PlayerScoreBox_structs.hpp"
#include "SDK/FN_Results_PlayerScoreBox_classes.hpp"
#include "SDK/FN_Results_PlayerScoreBox_parameters.hpp"
#include "SDK/FN_MinimapContainer_structs.hpp"
#include "SDK/FN_MinimapContainer_classes.hpp"
#include "SDK/FN_MinimapContainer_parameters.hpp"
#include "SDK/FN_MissionTracker_structs.hpp"
#include "SDK/FN_MissionTracker_classes.hpp"
#include "SDK/FN_MissionTracker_parameters.hpp"
#include "SDK/FN_Border-Bottom-Box-Rounded_structs.hpp"
#include "SDK/FN_Border-Bottom-Box-Rounded_classes.hpp"
#include "SDK/FN_Border-Bottom-Box-Rounded_parameters.hpp"
#include "SDK/FN_QuickbarBase_structs.hpp"
#include "SDK/FN_QuickbarBase_classes.hpp"
#include "SDK/FN_QuickbarBase_parameters.hpp"
#include "SDK/FN_CampaignMapWidget_structs.hpp"
#include "SDK/FN_CampaignMapWidget_classes.hpp"
#include "SDK/FN_CampaignMapWidget_parameters.hpp"
#include "SDK/FN_Halloween_QuestMapWidget_structs.hpp"
#include "SDK/FN_Halloween_QuestMapWidget_classes.hpp"
#include "SDK/FN_Halloween_QuestMapWidget_parameters.hpp"
#include "SDK/FN_RatingWidget_NUI_structs.hpp"
#include "SDK/FN_RatingWidget_NUI_classes.hpp"
#include "SDK/FN_RatingWidget_NUI_parameters.hpp"
#include "SDK/FN_ButtonStyle-Outline-M-Yellow_structs.hpp"
#include "SDK/FN_ButtonStyle-Outline-M-Yellow_classes.hpp"
#include "SDK/FN_ButtonStyle-Outline-M-Yellow_parameters.hpp"
#include "SDK/FN_QuickbarPrimary_structs.hpp"
#include "SDK/FN_QuickbarPrimary_classes.hpp"
#include "SDK/FN_QuickbarPrimary_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-LowOpacity_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-LowOpacity_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-LowOpacity_parameters.hpp"
#include "SDK/FN_ButtonStyle-Primary-L-Yellow_structs.hpp"
#include "SDK/FN_ButtonStyle-Primary-L-Yellow_classes.hpp"
#include "SDK/FN_ButtonStyle-Primary-L-Yellow_parameters.hpp"
#include "SDK/FN_TalkingHeadWidget_structs.hpp"
#include "SDK/FN_TalkingHeadWidget_classes.hpp"
#include "SDK/FN_TalkingHeadWidget_parameters.hpp"
#include "SDK/FN_HalloweenMapPage_05_structs.hpp"
#include "SDK/FN_HalloweenMapPage_05_classes.hpp"
#include "SDK/FN_HalloweenMapPage_05_parameters.hpp"
#include "SDK/FN_WaveModifiersWidget_structs.hpp"
#include "SDK/FN_WaveModifiersWidget_classes.hpp"
#include "SDK/FN_WaveModifiersWidget_parameters.hpp"
#include "SDK/FN_WaveModifiersTile_structs.hpp"
#include "SDK/FN_WaveModifiersTile_classes.hpp"
#include "SDK/FN_WaveModifiersTile_parameters.hpp"
#include "SDK/FN_ProtoCampaignMapPage_structs.hpp"
#include "SDK/FN_ProtoCampaignMapPage_classes.hpp"
#include "SDK/FN_ProtoCampaignMapPage_parameters.hpp"
#include "SDK/FN_AthenaTabsScreen_structs.hpp"
#include "SDK/FN_AthenaTabsScreen_classes.hpp"
#include "SDK/FN_AthenaTabsScreen_parameters.hpp"
#include "SDK/FN_HomeScreenNextQuest_structs.hpp"
#include "SDK/FN_HomeScreenNextQuest_classes.hpp"
#include "SDK/FN_HomeScreenNextQuest_parameters.hpp"
#include "SDK/FN_Border-FloatingShadow_structs.hpp"
#include "SDK/FN_Border-FloatingShadow_classes.hpp"
#include "SDK/FN_Border-FloatingShadow_parameters.hpp"
#include "SDK/FN_QuestsCountIconTabButton_structs.hpp"
#include "SDK/FN_QuestsCountIconTabButton_classes.hpp"
#include "SDK/FN_QuestsCountIconTabButton_parameters.hpp"
#include "SDK/FN_Results_TeamSubtotalScore_structs.hpp"
#include "SDK/FN_Results_TeamSubtotalScore_classes.hpp"
#include "SDK/FN_Results_TeamSubtotalScore_parameters.hpp"
#include "SDK/FN_StoreMain_OfferDetailsAttribute_structs.hpp"
#include "SDK/FN_StoreMain_OfferDetailsAttribute_classes.hpp"
#include "SDK/FN_StoreMain_OfferDetailsAttribute_parameters.hpp"
#include "SDK/FN_Results_BadgeLoot_structs.hpp"
#include "SDK/FN_Results_BadgeLoot_classes.hpp"
#include "SDK/FN_Results_BadgeLoot_parameters.hpp"
#include "SDK/FN_Border_BattleLoot_ColorBG_Dynamic_structs.hpp"
#include "SDK/FN_Border_BattleLoot_ColorBG_Dynamic_classes.hpp"
#include "SDK/FN_Border_BattleLoot_ColorBG_Dynamic_parameters.hpp"
#include "SDK/FN_DailyQuestObjectiveEntry_structs.hpp"
#include "SDK/FN_DailyQuestObjectiveEntry_classes.hpp"
#include "SDK/FN_DailyQuestObjectiveEntry_parameters.hpp"
#include "SDK/FN_DailyQuestProgressWidget_structs.hpp"
#include "SDK/FN_DailyQuestProgressWidget_classes.hpp"
#include "SDK/FN_DailyQuestProgressWidget_parameters.hpp"
#include "SDK/FN_Results_CommanderXP_structs.hpp"
#include "SDK/FN_Results_CommanderXP_classes.hpp"
#include "SDK/FN_Results_CommanderXP_parameters.hpp"
#include "SDK/FN_AthenaCustomizationTab_structs.hpp"
#include "SDK/FN_AthenaCustomizationTab_classes.hpp"
#include "SDK/FN_AthenaCustomizationTab_parameters.hpp"
#include "SDK/FN_AthenaNewsModal_structs.hpp"
#include "SDK/FN_AthenaNewsModal_classes.hpp"
#include "SDK/FN_AthenaNewsModal_parameters.hpp"
#include "SDK/FN_AthenaRotator_structs.hpp"
#include "SDK/FN_AthenaRotator_classes.hpp"
#include "SDK/FN_AthenaRotator_parameters.hpp"
#include "SDK/FN_Border-TabM-2-Blue_structs.hpp"
#include "SDK/FN_Border-TabM-2-Blue_classes.hpp"
#include "SDK/FN_Border-TabM-2-Blue_parameters.hpp"
#include "SDK/FN_AthenaDirectAcquisitionScreen_structs.hpp"
#include "SDK/FN_AthenaDirectAcquisitionScreen_classes.hpp"
#include "SDK/FN_AthenaDirectAcquisitionScreen_parameters.hpp"
#include "SDK/FN_Border_StatRow_RedFill_structs.hpp"
#include "SDK/FN_Border_StatRow_RedFill_classes.hpp"
#include "SDK/FN_Border_StatRow_RedFill_parameters.hpp"
#include "SDK/FN_AthenaLeaderboardTab_structs.hpp"
#include "SDK/FN_AthenaLeaderboardTab_classes.hpp"
#include "SDK/FN_AthenaLeaderboardTab_parameters.hpp"
#include "SDK/FN_AthenaLobby_structs.hpp"
#include "SDK/FN_AthenaLobby_classes.hpp"
#include "SDK/FN_AthenaLobby_parameters.hpp"
#include "SDK/FN_AthenaSeasonDirectAcquisitionScreen_structs.hpp"
#include "SDK/FN_AthenaSeasonDirectAcquisitionScreen_classes.hpp"
#include "SDK/FN_AthenaSeasonDirectAcquisitionScreen_parameters.hpp"
#include "SDK/FN_MainQuestsTile_structs.hpp"
#include "SDK/FN_MainQuestsTile_classes.hpp"
#include "SDK/FN_MainQuestsTile_parameters.hpp"
#include "SDK/FN_StoreMain_MTXOffer_structs.hpp"
#include "SDK/FN_StoreMain_MTXOffer_classes.hpp"
#include "SDK/FN_StoreMain_MTXOffer_parameters.hpp"
#include "SDK/FN_REsults_CommanderXP_MaterialData_structs.hpp"
#include "SDK/FN_REsults_CommanderXP_MaterialData_classes.hpp"
#include "SDK/FN_REsults_CommanderXP_MaterialData_parameters.hpp"
#include "SDK/FN_Results_CommanderXP_Data_structs.hpp"
#include "SDK/FN_Results_CommanderXP_Data_classes.hpp"
#include "SDK/FN_Results_CommanderXP_Data_parameters.hpp"
#include "SDK/FN_AthenaCustomizationBannerButton_structs.hpp"
#include "SDK/FN_AthenaCustomizationBannerButton_classes.hpp"
#include "SDK/FN_AthenaCustomizationBannerButton_parameters.hpp"
#include "SDK/FN_StoreMain_OfferDetails_structs.hpp"
#include "SDK/FN_StoreMain_OfferDetails_classes.hpp"
#include "SDK/FN_StoreMain_OfferDetails_parameters.hpp"
#include "SDK/FN_Border-TabM_structs.hpp"
#include "SDK/FN_Border-TabM_classes.hpp"
#include "SDK/FN_Border-TabM_parameters.hpp"
#include "SDK/FN_AthenaDirectAcquisitionOfferWidget_structs.hpp"
#include "SDK/FN_AthenaDirectAcquisitionOfferWidget_classes.hpp"
#include "SDK/FN_AthenaDirectAcquisitionOfferWidget_parameters.hpp"
#include "SDK/FN_AthenaLobbyPlayerPanel_structs.hpp"
#include "SDK/FN_AthenaLobbyPlayerPanel_classes.hpp"
#include "SDK/FN_AthenaLobbyPlayerPanel_parameters.hpp"
#include "SDK/FN_DailyQuestWidget_structs.hpp"
#include "SDK/FN_DailyQuestWidget_classes.hpp"
#include "SDK/FN_DailyQuestWidget_parameters.hpp"
#include "SDK/FN_AthenaDirectAcquisitionOfferPurchasedWidget_structs.hpp"
#include "SDK/FN_AthenaDirectAcquisitionOfferPurchasedWidget_classes.hpp"
#include "SDK/FN_AthenaDirectAcquisitionOfferPurchasedWidget_parameters.hpp"
#include "SDK/FN_HalloweenMapPage_01_structs.hpp"
#include "SDK/FN_HalloweenMapPage_01_classes.hpp"
#include "SDK/FN_HalloweenMapPage_01_parameters.hpp"
#include "SDK/FN_AthenaDirectAcquisitionOfferDetailsWidget_structs.hpp"
#include "SDK/FN_AthenaDirectAcquisitionOfferDetailsWidget_classes.hpp"
#include "SDK/FN_AthenaDirectAcquisitionOfferDetailsWidget_parameters.hpp"
#include "SDK/FN_HalloweenMapPage_02_structs.hpp"
#include "SDK/FN_HalloweenMapPage_02_classes.hpp"
#include "SDK/FN_HalloweenMapPage_02_parameters.hpp"
#include "SDK/FN_HalloweenMapPage_03_structs.hpp"
#include "SDK/FN_HalloweenMapPage_03_classes.hpp"
#include "SDK/FN_HalloweenMapPage_03_parameters.hpp"
#include "SDK/FN_MissionGen_Athena_structs.hpp"
#include "SDK/FN_MissionGen_Athena_classes.hpp"
#include "SDK/FN_MissionGen_Athena_parameters.hpp"
#include "SDK/FN_MissionGen_AthenaDuo_structs.hpp"
#include "SDK/FN_MissionGen_AthenaDuo_classes.hpp"
#include "SDK/FN_MissionGen_AthenaDuo_parameters.hpp"
#include "SDK/FN_HalloweenMapPage_04_structs.hpp"
#include "SDK/FN_HalloweenMapPage_04_classes.hpp"
#include "SDK/FN_HalloweenMapPage_04_parameters.hpp"
#include "SDK/FN_Results_BonusXpType_structs.hpp"
#include "SDK/FN_Results_BonusXpType_classes.hpp"
#include "SDK/FN_Results_BonusXpType_parameters.hpp"
#include "SDK/FN_Results_TeleportPadPlayerTop_structs.hpp"
#include "SDK/FN_Results_TeleportPadPlayerTop_classes.hpp"
#include "SDK/FN_Results_TeleportPadPlayerTop_parameters.hpp"
#include "SDK/FN_Results_Badge_structs.hpp"
#include "SDK/FN_Results_Badge_classes.hpp"
#include "SDK/FN_Results_Badge_parameters.hpp"
#include "SDK/FN_Results_BadgeLootBar_structs.hpp"
#include "SDK/FN_Results_BadgeLootBar_classes.hpp"
#include "SDK/FN_Results_BadgeLootBar_parameters.hpp"
#include "SDK/FN_Results_SummaryBadge_structs.hpp"
#include "SDK/FN_Results_SummaryBadge_classes.hpp"
#include "SDK/FN_Results_SummaryBadge_parameters.hpp"
#include "SDK/FN_MissionGen_AthenaSquad_structs.hpp"
#include "SDK/FN_MissionGen_AthenaSquad_classes.hpp"
#include "SDK/FN_MissionGen_AthenaSquad_parameters.hpp"
#include "SDK/FN_ButtonStyle-Solid-SquareM-Selectable_structs.hpp"
#include "SDK/FN_ButtonStyle-Solid-SquareM-Selectable_classes.hpp"
#include "SDK/FN_ButtonStyle-Solid-SquareM-Selectable_parameters.hpp"
#include "SDK/FN_DailyQuestRewardInfo_structs.hpp"
#include "SDK/FN_DailyQuestRewardInfo_classes.hpp"
#include "SDK/FN_DailyQuestRewardInfo_parameters.hpp"
#include "SDK/FN_TextStyle-Header-L_structs.hpp"
#include "SDK/FN_TextStyle-Header-L_classes.hpp"
#include "SDK/FN_TextStyle-Header-L_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_05_structs.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_05_classes.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_05_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_01_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_01_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_01_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_02_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_02_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_02_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_03_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_03_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_03_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_04_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_04_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_04_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_05_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_05_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_05_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_06_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_06_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_06_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_07_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_07_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_07_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_08_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_08_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_08_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_09_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_09_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_09_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_11_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_11_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_11_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_12_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_12_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_12_parameters.hpp"
#include "SDK/FN_AthenaLobbyPlayerPanelDetails_structs.hpp"
#include "SDK/FN_AthenaLobbyPlayerPanelDetails_classes.hpp"
#include "SDK/FN_AthenaLobbyPlayerPanelDetails_parameters.hpp"
#include "SDK/FN_RewardOrWidget_structs.hpp"
#include "SDK/FN_RewardOrWidget_classes.hpp"
#include "SDK/FN_RewardOrWidget_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_13_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_13_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_13_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_14_structs.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_14_classes.hpp"
#include "SDK/FN_CampaignMapPage_Plankerton_14_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_01_structs.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_01_classes.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_01_parameters.hpp"
#include "SDK/FN_AthenaLobbyPlayerPanelActions_structs.hpp"
#include "SDK/FN_AthenaLobbyPlayerPanelActions_classes.hpp"
#include "SDK/FN_AthenaLobbyPlayerPanelActions_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_02_structs.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_02_classes.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_02_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_03_structs.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_03_classes.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_03_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_04_structs.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_04_classes.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_04_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_06_structs.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_06_classes.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_06_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_07_structs.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_07_classes.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_07_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_08_structs.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_08_classes.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_08_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_09_structs.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_09_classes.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_09_parameters.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_11_structs.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_11_classes.hpp"
#include "SDK/FN_CampaignMapPage_Stonewood_11_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-S_LightGray_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-S_LightGray_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-S_LightGray_parameters.hpp"
#include "SDK/FN_ButtonStyle-Outline-Green-Shine_structs.hpp"
#include "SDK/FN_ButtonStyle-Outline-Green-Shine_classes.hpp"
#include "SDK/FN_ButtonStyle-Outline-Green-Shine_parameters.hpp"
#include "SDK/FN_Border_StatRow_StaticStroke_structs.hpp"
#include "SDK/FN_Border_StatRow_StaticStroke_classes.hpp"
#include "SDK/FN_Border_StatRow_StaticStroke_parameters.hpp"
#include "SDK/FN_DailyQuestsPanel_structs.hpp"
#include "SDK/FN_DailyQuestsPanel_classes.hpp"
#include "SDK/FN_DailyQuestsPanel_parameters.hpp"
#include "SDK/FN_AthenaMatchmakingWidget_structs.hpp"
#include "SDK/FN_AthenaMatchmakingWidget_classes.hpp"
#include "SDK/FN_AthenaMatchmakingWidget_parameters.hpp"
#include "SDK/FN_AthenaNews_structs.hpp"
#include "SDK/FN_AthenaNews_classes.hpp"
#include "SDK/FN_AthenaNews_parameters.hpp"
#include "SDK/FN_ButtonStyle_TransparentList_structs.hpp"
#include "SDK/FN_ButtonStyle_TransparentList_classes.hpp"
#include "SDK/FN_ButtonStyle_TransparentList_parameters.hpp"
#include "SDK/FN_ButtonStyle-Base_structs.hpp"
#include "SDK/FN_ButtonStyle-Base_classes.hpp"
#include "SDK/FN_ButtonStyle-Base_parameters.hpp"
#include "SDK/FN_LeaderboardHeaderRowWidget_structs.hpp"
#include "SDK/FN_LeaderboardHeaderRowWidget_classes.hpp"
#include "SDK/FN_LeaderboardHeaderRowWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S-LightGray_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S-LightGray_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S-LightGray_parameters.hpp"
#include "SDK/FN_Node_structs.hpp"
#include "SDK/FN_Node_classes.hpp"
#include "SDK/FN_Node_parameters.hpp"
#include "SDK/FN_HalloweenQuestsTile_structs.hpp"
#include "SDK/FN_HalloweenQuestsTile_classes.hpp"
#include "SDK/FN_HalloweenQuestsTile_parameters.hpp"
#include "SDK/FN_LeaderboardListView_structs.hpp"
#include "SDK/FN_LeaderboardListView_classes.hpp"
#include "SDK/FN_LeaderboardListView_parameters.hpp"
#include "SDK/FN_Node_Arrows_structs.hpp"
#include "SDK/FN_Node_Arrows_classes.hpp"
#include "SDK/FN_Node_Arrows_parameters.hpp"
#include "SDK/FN_QuestMiniTile_structs.hpp"
#include "SDK/FN_QuestMiniTile_classes.hpp"
#include "SDK/FN_QuestMiniTile_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-LowOpacity_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-LowOpacity_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-LowOpacity_parameters.hpp"
#include "SDK/FN_AthenaBannerSelectModal_structs.hpp"
#include "SDK/FN_AthenaBannerSelectModal_classes.hpp"
#include "SDK/FN_AthenaBannerSelectModal_parameters.hpp"
#include "SDK/FN_Results_TeamScoreRow_structs.hpp"
#include "SDK/FN_Results_TeamScoreRow_classes.hpp"
#include "SDK/FN_Results_TeamScoreRow_parameters.hpp"
#include "SDK/FN_TextStyle-Base-M_structs.hpp"
#include "SDK/FN_TextStyle-Base-M_classes.hpp"
#include "SDK/FN_TextStyle-Base-M_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-I-40pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-I-40pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-I-40pc_parameters.hpp"
#include "SDK/FN_TextStyle-Header-L-Horizon_structs.hpp"
#include "SDK/FN_TextStyle-Header-L-Horizon_classes.hpp"
#include "SDK/FN_TextStyle-Header-L-Horizon_parameters.hpp"
#include "SDK/FN_TextStyle-Header-L-Horizon-Yellow_structs.hpp"
#include "SDK/FN_TextStyle-Header-L-Horizon-Yellow_classes.hpp"
#include "SDK/FN_TextStyle-Header-L-Horizon-Yellow_parameters.hpp"
#include "SDK/FN_TextStyle-Header-M-Horizon_structs.hpp"
#include "SDK/FN_TextStyle-Header-M-Horizon_classes.hpp"
#include "SDK/FN_TextStyle-Header-M-Horizon_parameters.hpp"
#include "SDK/FN_AthenaMainStatTile_structs.hpp"
#include "SDK/FN_AthenaMainStatTile_classes.hpp"
#include "SDK/FN_AthenaMainStatTile_parameters.hpp"
#include "SDK/FN_AthenaStatsRow_structs.hpp"
#include "SDK/FN_AthenaStatsRow_classes.hpp"
#include "SDK/FN_AthenaStatsRow_parameters.hpp"
#include "SDK/FN_TextStyle-Base-M-B_structs.hpp"
#include "SDK/FN_TextStyle-Base-M-B_classes.hpp"
#include "SDK/FN_TextStyle-Base-M-B_parameters.hpp"
#include "SDK/FN_Results_TeamScoreBox_structs.hpp"
#include "SDK/FN_Results_TeamScoreBox_classes.hpp"
#include "SDK/FN_Results_TeamScoreBox_parameters.hpp"
#include "SDK/FN_Results_Summary_structs.hpp"
#include "SDK/FN_Results_Summary_classes.hpp"
#include "SDK/FN_Results_Summary_parameters.hpp"
#include "SDK/FN_TextStyle-Base-M-B-70pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-M-B-70pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-M-B-70pc_parameters.hpp"
#include "SDK/FN_Results_TopPanel_structs.hpp"
#include "SDK/FN_Results_TopPanel_classes.hpp"
#include "SDK/FN_Results_TopPanel_parameters.hpp"
#include "SDK/FN_ComingSoonPlaceholderWidget_structs.hpp"
#include "SDK/FN_ComingSoonPlaceholderWidget_classes.hpp"
#include "SDK/FN_ComingSoonPlaceholderWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Header-S-Horizon_structs.hpp"
#include "SDK/FN_TextStyle-Header-S-Horizon_classes.hpp"
#include "SDK/FN_TextStyle-Header-S-Horizon_parameters.hpp"
#include "SDK/FN_MainTabsScreen_structs.hpp"
#include "SDK/FN_MainTabsScreen_classes.hpp"
#include "SDK/FN_MainTabsScreen_parameters.hpp"
#include "SDK/FN_Results_CommanderXPBar_structs.hpp"
#include "SDK/FN_Results_CommanderXPBar_classes.hpp"
#include "SDK/FN_Results_CommanderXPBar_parameters.hpp"
#include "SDK/FN_Results_TeleportPadPlayer_structs.hpp"
#include "SDK/FN_Results_TeleportPadPlayer_classes.hpp"
#include "SDK/FN_Results_TeleportPadPlayer_parameters.hpp"
#include "SDK/FN_Results_StatsSubtypeBox_structs.hpp"
#include "SDK/FN_Results_StatsSubtypeBox_classes.hpp"
#include "SDK/FN_Results_StatsSubtypeBox_parameters.hpp"
#include "SDK/FN_Results_PlayerScoreRow_structs.hpp"
#include "SDK/FN_Results_PlayerScoreRow_classes.hpp"
#include "SDK/FN_Results_PlayerScoreRow_parameters.hpp"
#include "SDK/FN_NullLeaderboardContent_structs.hpp"
#include "SDK/FN_NullLeaderboardContent_classes.hpp"
#include "SDK/FN_NullLeaderboardContent_parameters.hpp"
#include "SDK/FN_PlayerRankEmblem_structs.hpp"
#include "SDK/FN_PlayerRankEmblem_classes.hpp"
#include "SDK/FN_PlayerRankEmblem_parameters.hpp"
#include "SDK/FN_Border-SolidBG-Yellow_structs.hpp"
#include "SDK/FN_Border-SolidBG-Yellow_classes.hpp"
#include "SDK/FN_Border-SolidBG-Yellow_parameters.hpp"
#include "SDK/FN_Border-CampaignMapTile-Normal_structs.hpp"
#include "SDK/FN_Border-CampaignMapTile-Normal_classes.hpp"
#include "SDK/FN_Border-CampaignMapTile-Normal_parameters.hpp"
#include "SDK/FN_Results_TeleportPad_structs.hpp"
#include "SDK/FN_Results_TeleportPad_classes.hpp"
#include "SDK/FN_Results_TeleportPad_parameters.hpp"
#include "SDK/FN_Results_TeamTotalScore_structs.hpp"
#include "SDK/FN_Results_TeamTotalScore_classes.hpp"
#include "SDK/FN_Results_TeamTotalScore_parameters.hpp"
#include "SDK/FN_AthenaCompendiumTab_structs.hpp"
#include "SDK/FN_AthenaCompendiumTab_classes.hpp"
#include "SDK/FN_AthenaCompendiumTab_parameters.hpp"
#include "SDK/FN_AthenaCustomizationPicker_structs.hpp"
#include "SDK/FN_AthenaCustomizationPicker_classes.hpp"
#include "SDK/FN_AthenaCustomizationPicker_parameters.hpp"
#include "SDK/FN_AthenaCustomizationPickerTileButton_structs.hpp"
#include "SDK/FN_AthenaCustomizationPickerTileButton_classes.hpp"
#include "SDK/FN_AthenaCustomizationPickerTileButton_parameters.hpp"
#include "SDK/FN_AthenaCustomizationSlotButton_structs.hpp"
#include "SDK/FN_AthenaCustomizationSlotButton_classes.hpp"
#include "SDK/FN_AthenaCustomizationSlotButton_parameters.hpp"
#include "SDK/FN_TextStyle-Header-S-70pc_structs.hpp"
#include "SDK/FN_TextStyle-Header-S-70pc_classes.hpp"
#include "SDK/FN_TextStyle-Header-S-70pc_parameters.hpp"
#include "SDK/FN_AthenaLeaderboardTabButton_structs.hpp"
#include "SDK/FN_AthenaLeaderboardTabButton_classes.hpp"
#include "SDK/FN_AthenaLeaderboardTabButton_parameters.hpp"
#include "SDK/FN_CampaignMapIconTextButton_structs.hpp"
#include "SDK/FN_CampaignMapIconTextButton_classes.hpp"
#include "SDK/FN_CampaignMapIconTextButton_parameters.hpp"
#include "SDK/FN_JournalObjectiveEntry_structs.hpp"
#include "SDK/FN_JournalObjectiveEntry_classes.hpp"
#include "SDK/FN_JournalObjectiveEntry_parameters.hpp"
#include "SDK/FN_LeaderboardThrobber_structs.hpp"
#include "SDK/FN_LeaderboardThrobber_classes.hpp"
#include "SDK/FN_LeaderboardThrobber_parameters.hpp"
#include "SDK/FN_LeaderboardRowWidget_structs.hpp"
#include "SDK/FN_LeaderboardRowWidget_classes.hpp"
#include "SDK/FN_LeaderboardRowWidget_parameters.hpp"
#include "SDK/FN_ConversationUtil_structs.hpp"
#include "SDK/FN_ConversationUtil_classes.hpp"
#include "SDK/FN_ConversationUtil_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-NavyBlue_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-NavyBlue_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-NavyBlue_parameters.hpp"
#include "SDK/FN_JournalObjectiveProgressWidget_structs.hpp"
#include "SDK/FN_JournalObjectiveProgressWidget_classes.hpp"
#include "SDK/FN_JournalObjectiveProgressWidget_parameters.hpp"
#include "SDK/FN_QuestPlayButton_structs.hpp"
#include "SDK/FN_QuestPlayButton_classes.hpp"
#include "SDK/FN_QuestPlayButton_parameters.hpp"
#include "SDK/FN_PlayerEmblemAndXp_structs.hpp"
#include "SDK/FN_PlayerEmblemAndXp_classes.hpp"
#include "SDK/FN_PlayerEmblemAndXp_parameters.hpp"
#include "SDK/FN_AthenaStatsTab_structs.hpp"
#include "SDK/FN_AthenaStatsTab_classes.hpp"
#include "SDK/FN_AthenaStatsTab_parameters.hpp"
#include "SDK/FN_FrontEndRewards_Definition_structs.hpp"
#include "SDK/FN_FrontEndRewards_Definition_classes.hpp"
#include "SDK/FN_FrontEndRewards_Definition_parameters.hpp"
#include "SDK/FN_TextStyle-Header-XL-Horizon_structs.hpp"
#include "SDK/FN_TextStyle-Header-XL-Horizon_classes.hpp"
#include "SDK/FN_TextStyle-Header-XL-Horizon_parameters.hpp"
#include "SDK/FN_ActiveModifierItemHUD_structs.hpp"
#include "SDK/FN_ActiveModifierItemHUD_classes.hpp"
#include "SDK/FN_ActiveModifierItemHUD_parameters.hpp"
#include "SDK/FN_TextStyle-Header-L-Critical_structs.hpp"
#include "SDK/FN_TextStyle-Header-L-Critical_classes.hpp"
#include "SDK/FN_TextStyle-Header-L-Critical_parameters.hpp"
#include "SDK/FN_TextStyle-Base-L-B-S_structs.hpp"
#include "SDK/FN_TextStyle-Base-L-B-S_classes.hpp"
#include "SDK/FN_TextStyle-Base-L-B-S_parameters.hpp"
#include "SDK/FN_StoreMain_Root_structs.hpp"
#include "SDK/FN_StoreMain_Root_classes.hpp"
#include "SDK/FN_StoreMain_Root_parameters.hpp"
#include "SDK/FN_TextStyle-Base-L-B_structs.hpp"
#include "SDK/FN_TextStyle-Base-L-B_classes.hpp"
#include "SDK/FN_TextStyle-Base-L-B_parameters.hpp"
#include "SDK/FN_ReportSpectatingPlayerButton_structs.hpp"
#include "SDK/FN_ReportSpectatingPlayerButton_classes.hpp"
#include "SDK/FN_ReportSpectatingPlayerButton_parameters.hpp"
#include "SDK/FN_CheckExpeditionRewardsAction_structs.hpp"
#include "SDK/FN_CheckExpeditionRewardsAction_classes.hpp"
#include "SDK/FN_CheckExpeditionRewardsAction_parameters.hpp"
#include "SDK/FN_CheckRateExperienceAction_structs.hpp"
#include "SDK/FN_CheckRateExperienceAction_classes.hpp"
#include "SDK/FN_CheckRateExperienceAction_parameters.hpp"
#include "SDK/FN_Border_AngledTab_LBlue_Edge_structs.hpp"
#include "SDK/FN_Border_AngledTab_LBlue_Edge_classes.hpp"
#include "SDK/FN_Border_AngledTab_LBlue_Edge_parameters.hpp"
#include "SDK/FN_AthenaInventoryDragDrop_structs.hpp"
#include "SDK/FN_AthenaInventoryDragDrop_classes.hpp"
#include "SDK/FN_AthenaInventoryDragDrop_parameters.hpp"
#include "SDK/FN_HitPointBar_structs.hpp"
#include "SDK/FN_HitPointBar_classes.hpp"
#include "SDK/FN_HitPointBar_parameters.hpp"
#include "SDK/FN_Border_AthenaStore_OfferHeader_structs.hpp"
#include "SDK/FN_Border_AthenaStore_OfferHeader_classes.hpp"
#include "SDK/FN_Border_AthenaStore_OfferHeader_parameters.hpp"
#include "SDK/FN_CheckFrontEndDailyRewardsAction_structs.hpp"
#include "SDK/FN_CheckFrontEndDailyRewardsAction_classes.hpp"
#include "SDK/FN_CheckFrontEndDailyRewardsAction_parameters.hpp"
#include "SDK/FN_Feedback_RateExperience_structs.hpp"
#include "SDK/FN_Feedback_RateExperience_classes.hpp"
#include "SDK/FN_Feedback_RateExperience_parameters.hpp"
#include "SDK/FN_AthenaInventoryLimitStatusIndicator_structs.hpp"
#include "SDK/FN_AthenaInventoryLimitStatusIndicator_classes.hpp"
#include "SDK/FN_AthenaInventoryLimitStatusIndicator_parameters.hpp"
#include "SDK/FN_LegacyButtonStyle-Base_structs.hpp"
#include "SDK/FN_LegacyButtonStyle-Base_classes.hpp"
#include "SDK/FN_LegacyButtonStyle-Base_parameters.hpp"
#include "SDK/FN_AthenaWinWidget_structs.hpp"
#include "SDK/FN_AthenaWinWidget_classes.hpp"
#include "SDK/FN_AthenaWinWidget_parameters.hpp"
#include "SDK/FN_FriendNotification_structs.hpp"
#include "SDK/FN_FriendNotification_classes.hpp"
#include "SDK/FN_FriendNotification_parameters.hpp"
#include "SDK/FN_ButtonStyle-Primary-L_structs.hpp"
#include "SDK/FN_ButtonStyle-Primary-L_classes.hpp"
#include "SDK/FN_ButtonStyle-Primary-L_parameters.hpp"
#include "SDK/FN_TextStyle-Button-Primary-L-Disabled_structs.hpp"
#include "SDK/FN_TextStyle-Button-Primary-L-Disabled_classes.hpp"
#include "SDK/FN_TextStyle-Button-Primary-L-Disabled_parameters.hpp"
#include "SDK/FN_BasicGradientFill_structs.hpp"
#include "SDK/FN_BasicGradientFill_classes.hpp"
#include "SDK/FN_BasicGradientFill_parameters.hpp"
#include "SDK/FN_ActiveModifiersHUD_structs.hpp"
#include "SDK/FN_ActiveModifiersHUD_classes.hpp"
#include "SDK/FN_ActiveModifiersHUD_parameters.hpp"
#include "SDK/FN_ChatWidget_structs.hpp"
#include "SDK/FN_ChatWidget_classes.hpp"
#include "SDK/FN_ChatWidget_parameters.hpp"
#include "SDK/FN_AthenaHUD_structs.hpp"
#include "SDK/FN_AthenaHUD_classes.hpp"
#include "SDK/FN_AthenaHUD_parameters.hpp"
#include "SDK/FN_BannerSelectionWidget_structs.hpp"
#include "SDK/FN_BannerSelectionWidget_classes.hpp"
#include "SDK/FN_BannerSelectionWidget_parameters.hpp"
#include "SDK/FN_AthenaGameOverScreen_structs.hpp"
#include "SDK/FN_AthenaGameOverScreen_classes.hpp"
#include "SDK/FN_AthenaGameOverScreen_parameters.hpp"
#include "SDK/FN_AthenaGamePhaseChangeWidget_structs.hpp"
#include "SDK/FN_AthenaGamePhaseChangeWidget_classes.hpp"
#include "SDK/FN_AthenaGamePhaseChangeWidget_parameters.hpp"
#include "SDK/FN_AthenaGamePhaseWidget_structs.hpp"
#include "SDK/FN_AthenaGamePhaseWidget_classes.hpp"
#include "SDK/FN_AthenaGamePhaseWidget_parameters.hpp"
#include "SDK/FN_ItemDisplayStyle_structs.hpp"
#include "SDK/FN_ItemDisplayStyle_classes.hpp"
#include "SDK/FN_ItemDisplayStyle_parameters.hpp"
#include "SDK/FN_AthenaInventoryPanel_structs.hpp"
#include "SDK/FN_AthenaInventoryPanel_classes.hpp"
#include "SDK/FN_AthenaInventoryPanel_parameters.hpp"
#include "SDK/FN_AthenaInventoryDropSlot_structs.hpp"
#include "SDK/FN_AthenaInventoryDropSlot_classes.hpp"
#include "SDK/FN_AthenaInventoryDropSlot_parameters.hpp"
#include "SDK/FN_TextStyle-Button-Primary-L_structs.hpp"
#include "SDK/FN_TextStyle-Button-Primary-L_classes.hpp"
#include "SDK/FN_TextStyle-Button-Primary-L_parameters.hpp"
#include "SDK/FN_AthenaInventoryEquipSlot_structs.hpp"
#include "SDK/FN_AthenaInventoryEquipSlot_classes.hpp"
#include "SDK/FN_AthenaInventoryEquipSlot_parameters.hpp"
#include "SDK/FN_AthenaDeathWidget_structs.hpp"
#include "SDK/FN_AthenaDeathWidget_classes.hpp"
#include "SDK/FN_AthenaDeathWidget_parameters.hpp"
#include "SDK/FN_AthenaInventoryFortItemTileButton_structs.hpp"
#include "SDK/FN_AthenaInventoryFortItemTileButton_classes.hpp"
#include "SDK/FN_AthenaInventoryFortItemTileButton_parameters.hpp"
#include "SDK/FN_AthenaKillerPortraitWidget_structs.hpp"
#include "SDK/FN_AthenaKillerPortraitWidget_classes.hpp"
#include "SDK/FN_AthenaKillerPortraitWidget_parameters.hpp"
#include "SDK/FN_AthenaViewTargetHitPointBar_structs.hpp"
#include "SDK/FN_AthenaViewTargetHitPointBar_classes.hpp"
#include "SDK/FN_AthenaViewTargetHitPointBar_parameters.hpp"
#include "SDK/FN_AthenaViewTargetHitPointInfo_structs.hpp"
#include "SDK/FN_AthenaViewTargetHitPointInfo_classes.hpp"
#include "SDK/FN_AthenaViewTargetHitPointInfo_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-S_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-S_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-S_parameters.hpp"
#include "SDK/FN_AthenaInventoryDragVisual_structs.hpp"
#include "SDK/FN_AthenaInventoryDragVisual_classes.hpp"
#include "SDK/FN_AthenaInventoryDragVisual_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-S_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-S_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-S_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-DBNOBarMax_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-DBNOBarMax_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-DBNOBarMax_parameters.hpp"
#include "SDK/FN_AthenaAerialFeedback_structs.hpp"
#include "SDK/FN_AthenaAerialFeedback_classes.hpp"
#include "SDK/FN_AthenaAerialFeedback_parameters.hpp"
#include "SDK/FN_AthenaTeamMemberDBNOState_structs.hpp"
#include "SDK/FN_AthenaTeamMemberDBNOState_classes.hpp"
#include "SDK/FN_AthenaTeamMemberDBNOState_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-HealthBarMax_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-HealthBarMax_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-HealthBarMax_parameters.hpp"
#include "SDK/FN_AthenaCompass_structs.hpp"
#include "SDK/FN_AthenaCompass_classes.hpp"
#include "SDK/FN_AthenaCompass_parameters.hpp"
#include "SDK/FN_Border-ShellTopBar-Glow_structs.hpp"
#include "SDK/FN_Border-ShellTopBar-Glow_classes.hpp"
#include "SDK/FN_Border-ShellTopBar-Glow_parameters.hpp"
#include "SDK/FN_Border-ShellTopBar-LeftSide_structs.hpp"
#include "SDK/FN_Border-ShellTopBar-LeftSide_classes.hpp"
#include "SDK/FN_Border-ShellTopBar-LeftSide_parameters.hpp"
#include "SDK/FN_Border-ShellTopBar-RightSide_structs.hpp"
#include "SDK/FN_Border-ShellTopBar-RightSide_classes.hpp"
#include "SDK/FN_Border-ShellTopBar-RightSide_parameters.hpp"
#include "SDK/FN_Border-ShellTopBar_structs.hpp"
#include "SDK/FN_Border-ShellTopBar_classes.hpp"
#include "SDK/FN_Border-ShellTopBar_parameters.hpp"
#include "SDK/FN_LobbyTimer_structs.hpp"
#include "SDK/FN_LobbyTimer_classes.hpp"
#include "SDK/FN_LobbyTimer_parameters.hpp"
#include "SDK/FN_Border-TopBar-Timer_structs.hpp"
#include "SDK/FN_Border-TopBar-Timer_classes.hpp"
#include "SDK/FN_Border-TopBar-Timer_parameters.hpp"
#include "SDK/FN_AthenaEquippedItemWidget_structs.hpp"
#include "SDK/FN_AthenaEquippedItemWidget_classes.hpp"
#include "SDK/FN_AthenaEquippedItemWidget_parameters.hpp"
#include "SDK/FN_AthenaEquipProgress_structs.hpp"
#include "SDK/FN_AthenaEquipProgress_classes.hpp"
#include "SDK/FN_AthenaEquipProgress_parameters.hpp"
#include "SDK/FN_AthenaHitPointBar_structs.hpp"
#include "SDK/FN_AthenaHitPointBar_classes.hpp"
#include "SDK/FN_AthenaHitPointBar_parameters.hpp"
#include "SDK/FN_AthenaLocalPlayerHitPointInfo_structs.hpp"
#include "SDK/FN_AthenaLocalPlayerHitPointInfo_classes.hpp"
#include "SDK/FN_AthenaLocalPlayerHitPointInfo_parameters.hpp"
#include "SDK/FN_Border-TopBar-TimerFlash_structs.hpp"
#include "SDK/FN_Border-TopBar-TimerFlash_classes.hpp"
#include "SDK/FN_Border-TopBar-TimerFlash_parameters.hpp"
#include "SDK/FN_Border-TopBar-Twitch_structs.hpp"
#include "SDK/FN_Border-TopBar-Twitch_classes.hpp"
#include "SDK/FN_Border-TopBar-Twitch_parameters.hpp"
#include "SDK/FN_Border_HUD_Line_Horz_structs.hpp"
#include "SDK/FN_Border_HUD_Line_Horz_classes.hpp"
#include "SDK/FN_Border_HUD_Line_Horz_parameters.hpp"
#include "SDK/FN_Border_HUD_Line_Vert_structs.hpp"
#include "SDK/FN_Border_HUD_Line_Vert_classes.hpp"
#include "SDK/FN_Border_HUD_Line_Vert_parameters.hpp"
#include "SDK/FN_AthenaKillFeedItem_structs.hpp"
#include "SDK/FN_AthenaKillFeedItem_classes.hpp"
#include "SDK/FN_AthenaKillFeedItem_parameters.hpp"
#include "SDK/FN_AthenaKillFeedWidget_structs.hpp"
#include "SDK/FN_AthenaKillFeedWidget_classes.hpp"
#include "SDK/FN_AthenaKillFeedWidget_parameters.hpp"
#include "SDK/FN_AthenaKillsWidget_structs.hpp"
#include "SDK/FN_AthenaKillsWidget_classes.hpp"
#include "SDK/FN_AthenaKillsWidget_parameters.hpp"
#include "SDK/FN_AthenaMinimapContainer_structs.hpp"
#include "SDK/FN_AthenaMinimapContainer_classes.hpp"
#include "SDK/FN_AthenaMinimapContainer_parameters.hpp"
#include "SDK/FN_AthenaPickupManager_structs.hpp"
#include "SDK/FN_AthenaPickupManager_classes.hpp"
#include "SDK/FN_AthenaPickupManager_parameters.hpp"
#include "SDK/FN_PickupMessageItem_structs.hpp"
#include "SDK/FN_PickupMessageItem_classes.hpp"
#include "SDK/FN_PickupMessageItem_parameters.hpp"
#include "SDK/FN_AthenaPlayerActionAlert_structs.hpp"
#include "SDK/FN_AthenaPlayerActionAlert_classes.hpp"
#include "SDK/FN_AthenaPlayerActionAlert_parameters.hpp"
#include "SDK/FN_Border_OutlineButton_ReadOnly_structs.hpp"
#include "SDK/FN_Border_OutlineButton_ReadOnly_classes.hpp"
#include "SDK/FN_Border_OutlineButton_ReadOnly_parameters.hpp"
#include "SDK/FN_AthenaPlayerInfo_structs.hpp"
#include "SDK/FN_AthenaPlayerInfo_classes.hpp"
#include "SDK/FN_AthenaPlayerInfo_parameters.hpp"
#include "SDK/FN_BRCurrency_structs.hpp"
#include "SDK/FN_BRCurrency_classes.hpp"
#include "SDK/FN_BRCurrency_parameters.hpp"
#include "SDK/FN_QuickbarSecondary_structs.hpp"
#include "SDK/FN_QuickbarSecondary_classes.hpp"
#include "SDK/FN_QuickbarSecondary_parameters.hpp"
#include "SDK/FN_Border_ShellUberBG_structs.hpp"
#include "SDK/FN_Border_ShellUberBG_classes.hpp"
#include "SDK/FN_Border_ShellUberBG_parameters.hpp"
#include "SDK/FN_ButtonStyle-Hamburger_structs.hpp"
#include "SDK/FN_ButtonStyle-Hamburger_classes.hpp"
#include "SDK/FN_ButtonStyle-Hamburger_parameters.hpp"
#include "SDK/FN_RotatorSelector_structs.hpp"
#include "SDK/FN_RotatorSelector_classes.hpp"
#include "SDK/FN_RotatorSelector_parameters.hpp"
#include "SDK/FN_AthenaQuickbarPrimary_structs.hpp"
#include "SDK/FN_AthenaQuickbarPrimary_classes.hpp"
#include "SDK/FN_AthenaQuickbarPrimary_parameters.hpp"
#include "SDK/FN_OptionsMenuSlider_Light_structs.hpp"
#include "SDK/FN_OptionsMenuSlider_Light_classes.hpp"
#include "SDK/FN_OptionsMenuSlider_Light_parameters.hpp"
#include "SDK/FN_AthenaQuickbarSecondary_structs.hpp"
#include "SDK/FN_AthenaQuickbarSecondary_classes.hpp"
#include "SDK/FN_AthenaQuickbarSecondary_parameters.hpp"
#include "SDK/FN_TextRotator_structs.hpp"
#include "SDK/FN_TextRotator_classes.hpp"
#include "SDK/FN_TextRotator_parameters.hpp"
#include "SDK/FN_OptionsMenuSlider_structs.hpp"
#include "SDK/FN_OptionsMenuSlider_classes.hpp"
#include "SDK/FN_OptionsMenuSlider_parameters.hpp"
#include "SDK/FN_ButtonStyle-RotatorBorder_structs.hpp"
#include "SDK/FN_ButtonStyle-RotatorBorder_classes.hpp"
#include "SDK/FN_ButtonStyle-RotatorBorder_parameters.hpp"
#include "SDK/FN_AthenaPlayersLeftWidget_structs.hpp"
#include "SDK/FN_AthenaPlayersLeftWidget_classes.hpp"
#include "SDK/FN_AthenaPlayersLeftWidget_parameters.hpp"
#include "SDK/FN_OptionsMenuHudRotator_structs.hpp"
#include "SDK/FN_OptionsMenuHudRotator_classes.hpp"
#include "SDK/FN_OptionsMenuHudRotator_parameters.hpp"
#include "SDK/FN_AthenaTeamMemberInfo_structs.hpp"
#include "SDK/FN_AthenaTeamMemberInfo_classes.hpp"
#include "SDK/FN_AthenaTeamMemberInfo_parameters.hpp"
#include "SDK/FN_AthenaTeamMemberIndicator_structs.hpp"
#include "SDK/FN_AthenaTeamMemberIndicator_classes.hpp"
#include "SDK/FN_AthenaTeamMemberIndicator_parameters.hpp"
#include "SDK/FN_Border-MainModal-Dark_structs.hpp"
#include "SDK/FN_Border-MainModal-Dark_classes.hpp"
#include "SDK/FN_Border-MainModal-Dark_parameters.hpp"
#include "SDK/FN_ButtonStyle-Outline-LeftShade-Red_structs.hpp"
#include "SDK/FN_ButtonStyle-Outline-LeftShade-Red_classes.hpp"
#include "SDK/FN_ButtonStyle-Outline-LeftShade-Red_parameters.hpp"
#include "SDK/FN_TextStyle-Button-Outline-M-Disabled_structs.hpp"
#include "SDK/FN_TextStyle-Button-Outline-M-Disabled_classes.hpp"
#include "SDK/FN_TextStyle-Button-Outline-M-Disabled_parameters.hpp"
#include "SDK/FN_AthenaSessionId_structs.hpp"
#include "SDK/FN_AthenaSessionId_classes.hpp"
#include "SDK/FN_AthenaSessionId_parameters.hpp"
#include "SDK/FN_ButtonStyle-Outline-Red_structs.hpp"
#include "SDK/FN_ButtonStyle-Outline-Red_classes.hpp"
#include "SDK/FN_ButtonStyle-Outline-Red_parameters.hpp"
#include "SDK/FN_AthenaTeamInfo_structs.hpp"
#include "SDK/FN_AthenaTeamInfo_classes.hpp"
#include "SDK/FN_AthenaTeamInfo_parameters.hpp"
#include "SDK/FN_Credits_structs.hpp"
#include "SDK/FN_Credits_classes.hpp"
#include "SDK/FN_Credits_parameters.hpp"
#include "SDK/FN_Border-ItemInfo-Unlocked_structs.hpp"
#include "SDK/FN_Border-ItemInfo-Unlocked_classes.hpp"
#include "SDK/FN_Border-ItemInfo-Unlocked_parameters.hpp"
#include "SDK/FN_TextStyle-Header-XL-S_structs.hpp"
#include "SDK/FN_TextStyle-Header-XL-S_classes.hpp"
#include "SDK/FN_TextStyle-Header-XL-S_parameters.hpp"
#include "SDK/FN_AthenaWatchers_structs.hpp"
#include "SDK/FN_AthenaWatchers_classes.hpp"
#include "SDK/FN_AthenaWatchers_parameters.hpp"
#include "SDK/FN_TopBar_structs.hpp"
#include "SDK/FN_TopBar_classes.hpp"
#include "SDK/FN_TopBar_parameters.hpp"
#include "SDK/FN_BP_FortLiveStreamGrantWindowExpires_structs.hpp"
#include "SDK/FN_BP_FortLiveStreamGrantWindowExpires_classes.hpp"
#include "SDK/FN_BP_FortLiveStreamGrantWindowExpires_parameters.hpp"
#include "SDK/FN_TextStyle-Header-M-70pc_structs.hpp"
#include "SDK/FN_TextStyle-Header-M-70pc_classes.hpp"
#include "SDK/FN_TextStyle-Header-M-70pc_parameters.hpp"
#include "SDK/FN_ButtonStyle-Outline-XPBar_structs.hpp"
#include "SDK/FN_ButtonStyle-Outline-XPBar_classes.hpp"
#include "SDK/FN_ButtonStyle-Outline-XPBar_parameters.hpp"
#include "SDK/FN_ButtonStyle-Outline_structs.hpp"
#include "SDK/FN_ButtonStyle-Outline_classes.hpp"
#include "SDK/FN_ButtonStyle-Outline_parameters.hpp"
#include "SDK/FN_BuildingInfoIndicator_structs.hpp"
#include "SDK/FN_BuildingInfoIndicator_classes.hpp"
#include "SDK/FN_BuildingInfoIndicator_parameters.hpp"
#include "SDK/FN_BuildingBar_structs.hpp"
#include "SDK/FN_BuildingBar_classes.hpp"
#include "SDK/FN_BuildingBar_parameters.hpp"
#include "SDK/FN_ButtonStyle-OutlineEmptyFill_structs.hpp"
#include "SDK/FN_ButtonStyle-OutlineEmptyFill_classes.hpp"
#include "SDK/FN_ButtonStyle-OutlineEmptyFill_parameters.hpp"
#include "SDK/FN_ButtonStyle-OutlineLeftShade_structs.hpp"
#include "SDK/FN_ButtonStyle-OutlineLeftShade_classes.hpp"
#include "SDK/FN_ButtonStyle-OutlineLeftShade_parameters.hpp"
#include "SDK/FN_ButtonStyle-OutlineRightShade_structs.hpp"
#include "SDK/FN_ButtonStyle-OutlineRightShade_classes.hpp"
#include "SDK/FN_ButtonStyle-OutlineRightShade_parameters.hpp"
#include "SDK/FN_OptionsMenuRowSelector_structs.hpp"
#include "SDK/FN_OptionsMenuRowSelector_classes.hpp"
#include "SDK/FN_OptionsMenuRowSelector_parameters.hpp"
#include "SDK/FN_OptionsMenuInput_structs.hpp"
#include "SDK/FN_OptionsMenuInput_classes.hpp"
#include "SDK/FN_OptionsMenuInput_parameters.hpp"
#include "SDK/FN_TabVideoOptions_structs.hpp"
#include "SDK/FN_TabVideoOptions_classes.hpp"
#include "SDK/FN_TabVideoOptions_parameters.hpp"
#include "SDK/FN_Border_Navy_RGrad_structs.hpp"
#include "SDK/FN_Border_Navy_RGrad_classes.hpp"
#include "SDK/FN_Border_Navy_RGrad_parameters.hpp"
#include "SDK/FN_TabAccountLinkage_structs.hpp"
#include "SDK/FN_TabAccountLinkage_classes.hpp"
#include "SDK/FN_TabAccountLinkage_parameters.hpp"
#include "SDK/FN_Border_Solid_LtBlue_structs.hpp"
#include "SDK/FN_Border_Solid_LtBlue_classes.hpp"
#include "SDK/FN_Border_Solid_LtBlue_parameters.hpp"
#include "SDK/FN_GamepadMappingInfo_structs.hpp"
#include "SDK/FN_GamepadMappingInfo_classes.hpp"
#include "SDK/FN_GamepadMappingInfo_parameters.hpp"
#include "SDK/FN_TabAudioOptions_structs.hpp"
#include "SDK/FN_TabAudioOptions_classes.hpp"
#include "SDK/FN_TabAudioOptions_parameters.hpp"
#include "SDK/FN_TabBrightnessOptions_structs.hpp"
#include "SDK/FN_TabBrightnessOptions_classes.hpp"
#include "SDK/FN_TabBrightnessOptions_parameters.hpp"
#include "SDK/FN_TabGamePadConfig_structs.hpp"
#include "SDK/FN_TabGamePadConfig_classes.hpp"
#include "SDK/FN_TabGamePadConfig_parameters.hpp"
#include "SDK/FN_ButtonStyle-PanelSelector_structs.hpp"
#include "SDK/FN_ButtonStyle-PanelSelector_classes.hpp"
#include "SDK/FN_ButtonStyle-PanelSelector_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-ShieldBarMax_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-ShieldBarMax_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-ShieldBarMax_parameters.hpp"
#include "SDK/FN_TabGameOptions_structs.hpp"
#include "SDK/FN_TabGameOptions_classes.hpp"
#include "SDK/FN_TabGameOptions_parameters.hpp"
#include "SDK/FN_TextStyle-Button-Outline-XS-Disabled_structs.hpp"
#include "SDK/FN_TextStyle-Button-Outline-XS-Disabled_classes.hpp"
#include "SDK/FN_TextStyle-Button-Outline-XS-Disabled_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS_parameters.hpp"
#include "SDK/FN_Border-PowerBar_structs.hpp"
#include "SDK/FN_Border-PowerBar_classes.hpp"
#include "SDK/FN_Border-PowerBar_parameters.hpp"
#include "SDK/FN_TextStyle-Header-M-NavyBlue_structs.hpp"
#include "SDK/FN_TextStyle-Header-M-NavyBlue_classes.hpp"
#include "SDK/FN_TextStyle-Header-M-NavyBlue_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-BlueGray_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-BlueGray_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-BlueGray_parameters.hpp"
#include "SDK/FN_FullPartyMember_structs.hpp"
#include "SDK/FN_FullPartyMember_classes.hpp"
#include "SDK/FN_FullPartyMember_parameters.hpp"
#include "SDK/FN_TextStyle-Base-L-B-Yellow_structs.hpp"
#include "SDK/FN_TextStyle-Base-L-B-Yellow_classes.hpp"
#include "SDK/FN_TextStyle-Base-L-B-Yellow_parameters.hpp"
#include "SDK/FN_FortTwitchLogin_structs.hpp"
#include "SDK/FN_FortTwitchLogin_classes.hpp"
#include "SDK/FN_FortTwitchLogin_parameters.hpp"
#include "SDK/FN_FullPartyMemberConnected_structs.hpp"
#include "SDK/FN_FullPartyMemberConnected_classes.hpp"
#include "SDK/FN_FullPartyMemberConnected_parameters.hpp"
#include "SDK/FN_LeaveButton_structs.hpp"
#include "SDK/FN_LeaveButton_classes.hpp"
#include "SDK/FN_LeaveButton_parameters.hpp"
#include "SDK/FN_BottomBarWidget_structs.hpp"
#include "SDK/FN_BottomBarWidget_classes.hpp"
#include "SDK/FN_BottomBarWidget_parameters.hpp"
#include "SDK/FN_TwitchLoginModalWidget_structs.hpp"
#include "SDK/FN_TwitchLoginModalWidget_classes.hpp"
#include "SDK/FN_TwitchLoginModalWidget_parameters.hpp"
#include "SDK/FN_EulaTab_structs.hpp"
#include "SDK/FN_EulaTab_classes.hpp"
#include "SDK/FN_EulaTab_parameters.hpp"
#include "SDK/FN_ThirdPartyTab_structs.hpp"
#include "SDK/FN_ThirdPartyTab_classes.hpp"
#include "SDK/FN_ThirdPartyTab_parameters.hpp"
#include "SDK/FN_MainMenu_structs.hpp"
#include "SDK/FN_MainMenu_classes.hpp"
#include "SDK/FN_MainMenu_parameters.hpp"
#include "SDK/FN_TextStyle-MessageCenter-Description_structs.hpp"
#include "SDK/FN_TextStyle-MessageCenter-Description_classes.hpp"
#include "SDK/FN_TextStyle-MessageCenter-Description_parameters.hpp"
#include "SDK/FN_PopupCenterMessageModalPanel_structs.hpp"
#include "SDK/FN_PopupCenterMessageModalPanel_classes.hpp"
#include "SDK/FN_PopupCenterMessageModalPanel_parameters.hpp"
#include "SDK/FN_MessageCenterListItem_structs.hpp"
#include "SDK/FN_MessageCenterListItem_classes.hpp"
#include "SDK/FN_MessageCenterListItem_parameters.hpp"
#include "SDK/FN_BPI_StormShieldinterface_structs.hpp"
#include "SDK/FN_BPI_StormShieldinterface_classes.hpp"
#include "SDK/FN_BPI_StormShieldinterface_parameters.hpp"
#include "SDK/FN_TabGameOptionsMain_structs.hpp"
#include "SDK/FN_TabGameOptionsMain_classes.hpp"
#include "SDK/FN_TabGameOptionsMain_parameters.hpp"
#include "SDK/FN_OutpostScreenStormShieldContent_structs.hpp"
#include "SDK/FN_OutpostScreenStormShieldContent_classes.hpp"
#include "SDK/FN_OutpostScreenStormShieldContent_parameters.hpp"
#include "SDK/FN_TabGameOptionsHud_structs.hpp"
#include "SDK/FN_TabGameOptionsHud_classes.hpp"
#include "SDK/FN_TabGameOptionsHud_parameters.hpp"
#include "SDK/FN_FullPartyBar_structs.hpp"
#include "SDK/FN_FullPartyBar_classes.hpp"
#include "SDK/FN_FullPartyBar_parameters.hpp"
#include "SDK/FN_OptionsMenu_structs.hpp"
#include "SDK/FN_OptionsMenu_classes.hpp"
#include "SDK/FN_OptionsMenu_parameters.hpp"
#include "SDK/FN_TextStyle-Header-L-NavyBlue_structs.hpp"
#include "SDK/FN_TextStyle-Header-L-NavyBlue_classes.hpp"
#include "SDK/FN_TextStyle-Header-L-NavyBlue_parameters.hpp"
#include "SDK/FN_TopBarSkillPoints_structs.hpp"
#include "SDK/FN_TopBarSkillPoints_classes.hpp"
#include "SDK/FN_TopBarSkillPoints_parameters.hpp"
#include "SDK/FN_TabInputOptions_structs.hpp"
#include "SDK/FN_TabInputOptions_classes.hpp"
#include "SDK/FN_TabInputOptions_parameters.hpp"
#include "SDK/FN_ScrollingTextButton_structs.hpp"
#include "SDK/FN_ScrollingTextButton_classes.hpp"
#include "SDK/FN_ScrollingTextButton_parameters.hpp"
#include "SDK/FN_News_structs.hpp"
#include "SDK/FN_News_classes.hpp"
#include "SDK/FN_News_parameters.hpp"
#include "SDK/FN_NewsEntry_structs.hpp"
#include "SDK/FN_NewsEntry_classes.hpp"
#include "SDK/FN_NewsEntry_parameters.hpp"
#include "SDK/FN_Border-ShellTopBar-Solid_structs.hpp"
#include "SDK/FN_Border-ShellTopBar-Solid_classes.hpp"
#include "SDK/FN_Border-ShellTopBar-Solid_parameters.hpp"
#include "SDK/FN_TextStyle-SectionHeader_structs.hpp"
#include "SDK/FN_TextStyle-SectionHeader_classes.hpp"
#include "SDK/FN_TextStyle-SectionHeader_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-BlueGray_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-BlueGray_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-BlueGray_parameters.hpp"
#include "SDK/FN_OutpostScreenStormShield_structs.hpp"
#include "SDK/FN_OutpostScreenStormShield_classes.hpp"
#include "SDK/FN_OutpostScreenStormShield_parameters.hpp"
#include "SDK/FN_ChangeSubgameButton_structs.hpp"
#include "SDK/FN_ChangeSubgameButton_classes.hpp"
#include "SDK/FN_ChangeSubgameButton_parameters.hpp"
#include "SDK/FN_ButtonStyle-Primary-Radio-M-NoText_structs.hpp"
#include "SDK/FN_ButtonStyle-Primary-Radio-M-NoText_classes.hpp"
#include "SDK/FN_ButtonStyle-Primary-Radio-M-NoText_parameters.hpp"
#include "SDK/FN_Border-KeyBase-Center-M_structs.hpp"
#include "SDK/FN_Border-KeyBase-Center-M_classes.hpp"
#include "SDK/FN_Border-KeyBase-Center-M_parameters.hpp"
#include "SDK/FN_HUD-PickupItemWidget_structs.hpp"
#include "SDK/FN_HUD-PickupItemWidget_classes.hpp"
#include "SDK/FN_HUD-PickupItemWidget_parameters.hpp"
#include "SDK/FN_BasicInteractionWidget_structs.hpp"
#include "SDK/FN_BasicInteractionWidget_classes.hpp"
#include "SDK/FN_BasicInteractionWidget_parameters.hpp"
#include "SDK/FN_MTXButton_structs.hpp"
#include "SDK/FN_MTXButton_classes.hpp"
#include "SDK/FN_MTXButton_parameters.hpp"
#include "SDK/FN_HomebaseRatingBar_structs.hpp"
#include "SDK/FN_HomebaseRatingBar_classes.hpp"
#include "SDK/FN_HomebaseRatingBar_parameters.hpp"
#include "SDK/FN_TutorialRichText_structs.hpp"
#include "SDK/FN_TutorialRichText_classes.hpp"
#include "SDK/FN_TutorialRichText_parameters.hpp"
#include "SDK/FN_TutorialTransparentRichText_structs.hpp"
#include "SDK/FN_TutorialTransparentRichText_classes.hpp"
#include "SDK/FN_TutorialTransparentRichText_parameters.hpp"
#include "SDK/FN_TutorialOverlay_structs.hpp"
#include "SDK/FN_TutorialOverlay_classes.hpp"
#include "SDK/FN_TutorialOverlay_parameters.hpp"
#include "SDK/FN_PowerWidget_structs.hpp"
#include "SDK/FN_PowerWidget_classes.hpp"
#include "SDK/FN_PowerWidget_parameters.hpp"
#include "SDK/FN_Border-PowerToast_structs.hpp"
#include "SDK/FN_Border-PowerToast_classes.hpp"
#include "SDK/FN_Border-PowerToast_parameters.hpp"
#include "SDK/FN_ResourceAggregationWidget_structs.hpp"
#include "SDK/FN_ResourceAggregationWidget_classes.hpp"
#include "SDK/FN_ResourceAggregationWidget_parameters.hpp"
#include "SDK/FN_BuildWatermark_structs.hpp"
#include "SDK/FN_BuildWatermark_classes.hpp"
#include "SDK/FN_BuildWatermark_parameters.hpp"
#include "SDK/FN_ButtonStyle-Primary-Radio-M_structs.hpp"
#include "SDK/FN_ButtonStyle-Primary-Radio-M_classes.hpp"
#include "SDK/FN_ButtonStyle-Primary-Radio-M_parameters.hpp"
#include "SDK/FN_Border-PowerToastGlow_structs.hpp"
#include "SDK/FN_Border-PowerToastGlow_classes.hpp"
#include "SDK/FN_Border-PowerToastGlow_parameters.hpp"
#include "SDK/FN_InteractionIndicator_structs.hpp"
#include "SDK/FN_InteractionIndicator_classes.hpp"
#include "SDK/FN_InteractionIndicator_parameters.hpp"
#include "SDK/FN_Border-SolidBG-StatItem_structs.hpp"
#include "SDK/FN_Border-SolidBG-StatItem_classes.hpp"
#include "SDK/FN_Border-SolidBG-StatItem_parameters.hpp"
#include "SDK/FN_ToastWidget_structs.hpp"
#include "SDK/FN_ToastWidget_classes.hpp"
#include "SDK/FN_ToastWidget_parameters.hpp"
#include "SDK/FN_InterestIndicatorWidget_structs.hpp"
#include "SDK/FN_InterestIndicatorWidget_classes.hpp"
#include "SDK/FN_InterestIndicatorWidget_parameters.hpp"
#include "SDK/FN_LegalInfo_structs.hpp"
#include "SDK/FN_LegalInfo_classes.hpp"
#include "SDK/FN_LegalInfo_parameters.hpp"
#include "SDK/FN_MessageCenterWidget_structs.hpp"
#include "SDK/FN_MessageCenterWidget_classes.hpp"
#include "SDK/FN_MessageCenterWidget_parameters.hpp"
#include "SDK/FN_ShowFriendCodesSelection_structs.hpp"
#include "SDK/FN_ShowFriendCodesSelection_classes.hpp"
#include "SDK/FN_ShowFriendCodesSelection_parameters.hpp"
#include "SDK/FN_TextStyle-Header-L-S_structs.hpp"
#include "SDK/FN_TextStyle-Header-L-S_classes.hpp"
#include "SDK/FN_TextStyle-Header-L-S_parameters.hpp"
#include "SDK/FN_FriendCodeConsole_structs.hpp"
#include "SDK/FN_FriendCodeConsole_classes.hpp"
#include "SDK/FN_FriendCodeConsole_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-Blue_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-Blue_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-Blue_parameters.hpp"
#include "SDK/FN_FriendCodePC_structs.hpp"
#include "SDK/FN_FriendCodePC_classes.hpp"
#include "SDK/FN_FriendCodePC_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-EnchantedBlue_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-EnchantedBlue_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-EnchantedBlue_parameters.hpp"
#include "SDK/FN_TextStyle-Toast-Description_structs.hpp"
#include "SDK/FN_TextStyle-Toast-Description_classes.hpp"
#include "SDK/FN_TextStyle-Toast-Description_parameters.hpp"
#include "SDK/FN_PermissionsRoot_structs.hpp"
#include "SDK/FN_PermissionsRoot_classes.hpp"
#include "SDK/FN_PermissionsRoot_parameters.hpp"
#include "SDK/FN_QuickbarSlot_structs.hpp"
#include "SDK/FN_QuickbarSlot_classes.hpp"
#include "SDK/FN_QuickbarSlot_parameters.hpp"
#include "SDK/FN_TextStyle-Toast-Title_structs.hpp"
#include "SDK/FN_TextStyle-Toast-Title_classes.hpp"
#include "SDK/FN_TextStyle-Toast-Title_parameters.hpp"
#include "SDK/FN_Border-ModalHeader_structs.hpp"
#include "SDK/FN_Border-ModalHeader_classes.hpp"
#include "SDK/FN_Border-ModalHeader_parameters.hpp"
#include "SDK/FN_PrivacyWidget_structs.hpp"
#include "SDK/FN_PrivacyWidget_classes.hpp"
#include "SDK/FN_PrivacyWidget_parameters.hpp"
#include "SDK/FN_ProgressWidget_structs.hpp"
#include "SDK/FN_ProgressWidget_classes.hpp"
#include "SDK/FN_ProgressWidget_parameters.hpp"
#include "SDK/FN_OutpostScreenStormShieldPermissions_structs.hpp"
#include "SDK/FN_OutpostScreenStormShieldPermissions_classes.hpp"
#include "SDK/FN_OutpostScreenStormShieldPermissions_parameters.hpp"
#include "SDK/FN_OutpostScreenCanEditPanel_structs.hpp"
#include "SDK/FN_OutpostScreenCanEditPanel_classes.hpp"
#include "SDK/FN_OutpostScreenCanEditPanel_parameters.hpp"
#include "SDK/FN_Border-ModalHeader-Recycle_structs.hpp"
#include "SDK/FN_Border-ModalHeader-Recycle_classes.hpp"
#include "SDK/FN_Border-ModalHeader-Recycle_parameters.hpp"
#include "SDK/FN_BP_PlayerControllerOutpost_structs.hpp"
#include "SDK/FN_BP_PlayerControllerOutpost_classes.hpp"
#include "SDK/FN_BP_PlayerControllerOutpost_parameters.hpp"
#include "SDK/FN_OutpostScreenCanEditRow_structs.hpp"
#include "SDK/FN_OutpostScreenCanEditRow_classes.hpp"
#include "SDK/FN_OutpostScreenCanEditRow_parameters.hpp"
#include "SDK/FN_Interaction_DefenderBeacon_structs.hpp"
#include "SDK/FN_Interaction_DefenderBeacon_classes.hpp"
#include "SDK/FN_Interaction_DefenderBeacon_parameters.hpp"
#include "SDK/FN_ButtonStyle-Left_structs.hpp"
#include "SDK/FN_ButtonStyle-Left_classes.hpp"
#include "SDK/FN_ButtonStyle-Left_parameters.hpp"
#include "SDK/FN_Feedback_structs.hpp"
#include "SDK/FN_Feedback_classes.hpp"
#include "SDK/FN_Feedback_parameters.hpp"
#include "SDK/FN_ButtonStyle-Right_structs.hpp"
#include "SDK/FN_ButtonStyle-Right_classes.hpp"
#include "SDK/FN_ButtonStyle-Right_parameters.hpp"
#include "SDK/FN_LeaveWarningInfo_structs.hpp"
#include "SDK/FN_LeaveWarningInfo_classes.hpp"
#include "SDK/FN_LeaveWarningInfo_parameters.hpp"
#include "SDK/FN_Border_HUD_ResourceBg_structs.hpp"
#include "SDK/FN_Border_HUD_ResourceBg_classes.hpp"
#include "SDK/FN_Border_HUD_ResourceBg_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-EnchantedBlue_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-EnchantedBlue_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-EnchantedBlue_parameters.hpp"
#include "SDK/FN_UIManager_structs.hpp"
#include "SDK/FN_UIManager_classes.hpp"
#include "SDK/FN_UIManager_parameters.hpp"
#include "SDK/FN_Announcement_Tutorial_structs.hpp"
#include "SDK/FN_Announcement_Tutorial_classes.hpp"
#include "SDK/FN_Announcement_Tutorial_parameters.hpp"
#include "SDK/FN_MissionRewardBoostBadgeTooltip_structs.hpp"
#include "SDK/FN_MissionRewardBoostBadgeTooltip_classes.hpp"
#include "SDK/FN_MissionRewardBoostBadgeTooltip_parameters.hpp"
#include "SDK/FN_ConfirmationWindow_structs.hpp"
#include "SDK/FN_ConfirmationWindow_classes.hpp"
#include "SDK/FN_ConfirmationWindow_parameters.hpp"
#include "SDK/FN_Border-SolidBG-RewardBack_structs.hpp"
#include "SDK/FN_Border-SolidBG-RewardBack_classes.hpp"
#include "SDK/FN_Border-SolidBG-RewardBack_parameters.hpp"
#include "SDK/FN_WebPurchase_structs.hpp"
#include "SDK/FN_WebPurchase_classes.hpp"
#include "SDK/FN_WebPurchase_parameters.hpp"
#include "SDK/FN_ToastDisplayArea_structs.hpp"
#include "SDK/FN_ToastDisplayArea_classes.hpp"
#include "SDK/FN_ToastDisplayArea_parameters.hpp"
#include "SDK/FN_Border_HUD_ResourceBg_Highlighted_structs.hpp"
#include "SDK/FN_Border_HUD_ResourceBg_Highlighted_classes.hpp"
#include "SDK/FN_Border_HUD_ResourceBg_Highlighted_parameters.hpp"
#include "SDK/FN_FounderBadgeTooltip_structs.hpp"
#include "SDK/FN_FounderBadgeTooltip_classes.hpp"
#include "SDK/FN_FounderBadgeTooltip_parameters.hpp"
#include "SDK/FN_PowerToastWidget_structs.hpp"
#include "SDK/FN_PowerToastWidget_classes.hpp"
#include "SDK/FN_PowerToastWidget_parameters.hpp"
#include "SDK/FN_ErrorWindow_structs.hpp"
#include "SDK/FN_ErrorWindow_classes.hpp"
#include "SDK/FN_ErrorWindow_parameters.hpp"
#include "SDK/FN_ErrorEntry_structs.hpp"
#include "SDK/FN_ErrorEntry_classes.hpp"
#include "SDK/FN_ErrorEntry_parameters.hpp"
#include "SDK/FN_MonthlyVIPBadgeTooltip_structs.hpp"
#include "SDK/FN_MonthlyVIPBadgeTooltip_classes.hpp"
#include "SDK/FN_MonthlyVIPBadgeTooltip_parameters.hpp"
#include "SDK/FN_XpBonusToolTip_structs.hpp"
#include "SDK/FN_XpBonusToolTip_classes.hpp"
#include "SDK/FN_XpBonusToolTip_parameters.hpp"
#include "SDK/FN_Athena_PlayerController_structs.hpp"
#include "SDK/FN_Athena_PlayerController_classes.hpp"
#include "SDK/FN_Athena_PlayerController_parameters.hpp"
#include "SDK/FN_AmbientControllerComponent_Athena_structs.hpp"
#include "SDK/FN_AmbientControllerComponent_Athena_classes.hpp"
#include "SDK/FN_AmbientControllerComponent_Athena_parameters.hpp"
#include "SDK/FN_Athena_GameState_structs.hpp"
#include "SDK/FN_Athena_GameState_classes.hpp"
#include "SDK/FN_Athena_GameState_parameters.hpp"
#include "SDK/FN_RadialPickerItem_structs.hpp"
#include "SDK/FN_RadialPickerItem_classes.hpp"
#include "SDK/FN_RadialPickerItem_parameters.hpp"
#include "SDK/FN_ProgressModalWidget_structs.hpp"
#include "SDK/FN_ProgressModalWidget_classes.hpp"
#include "SDK/FN_ProgressModalWidget_parameters.hpp"
#include "SDK/FN_BP_LocalPlayerBannerEditor_structs.hpp"
#include "SDK/FN_BP_LocalPlayerBannerEditor_classes.hpp"
#include "SDK/FN_BP_LocalPlayerBannerEditor_parameters.hpp"
#include "SDK/FN_BP_LocalPlayerProfileManagement_structs.hpp"
#include "SDK/FN_BP_LocalPlayerProfileManagement_classes.hpp"
#include "SDK/FN_BP_LocalPlayerProfileManagement_parameters.hpp"
#include "SDK/FN_PackResource_structs.hpp"
#include "SDK/FN_PackResource_classes.hpp"
#include "SDK/FN_PackResource_parameters.hpp"
#include "SDK/FN_BP_LocalPlayerProfileModal_structs.hpp"
#include "SDK/FN_BP_LocalPlayerProfileModal_classes.hpp"
#include "SDK/FN_BP_LocalPlayerProfileModal_parameters.hpp"
#include "SDK/FN_BP_BannerEditorTile_structs.hpp"
#include "SDK/FN_BP_BannerEditorTile_classes.hpp"
#include "SDK/FN_BP_BannerEditorTile_parameters.hpp"
#include "SDK/FN_PotentialResourceWidget_structs.hpp"
#include "SDK/FN_PotentialResourceWidget_classes.hpp"
#include "SDK/FN_PotentialResourceWidget_parameters.hpp"
#include "SDK/FN_ButtonStyle-Secondary-M_structs.hpp"
#include "SDK/FN_ButtonStyle-Secondary-M_classes.hpp"
#include "SDK/FN_ButtonStyle-Secondary-M_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-40pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-40pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-40pc_parameters.hpp"
#include "SDK/FN_MiniPartyBar_structs.hpp"
#include "SDK/FN_MiniPartyBar_classes.hpp"
#include "SDK/FN_MiniPartyBar_parameters.hpp"
#include "SDK/FN_TextStyle-Base-L-S_structs.hpp"
#include "SDK/FN_TextStyle-Base-L-S_classes.hpp"
#include "SDK/FN_TextStyle-Base-L-S_parameters.hpp"
#include "SDK/FN_TextStyle-Base-L-B-Black_structs.hpp"
#include "SDK/FN_TextStyle-Base-L-B-Black_classes.hpp"
#include "SDK/FN_TextStyle-Base-L-B-Black_parameters.hpp"
#include "SDK/FN_MiniPartyMember_structs.hpp"
#include "SDK/FN_MiniPartyMember_classes.hpp"
#include "SDK/FN_MiniPartyMember_parameters.hpp"
#include "SDK/FN_AthenaQuickBarSlot_structs.hpp"
#include "SDK/FN_AthenaQuickBarSlot_classes.hpp"
#include "SDK/FN_AthenaQuickBarSlot_parameters.hpp"
#include "SDK/FN_TextStyle-Base-L-B-CreamCan_structs.hpp"
#include "SDK/FN_TextStyle-Base-L-B-CreamCan_classes.hpp"
#include "SDK/FN_TextStyle-Base-L-B-CreamCan_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-Scorpion_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-Scorpion_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-Scorpion_parameters.hpp"
#include "SDK/FN_TextStyle-Header-S-Black_structs.hpp"
#include "SDK/FN_TextStyle-Header-S-Black_classes.hpp"
#include "SDK/FN_TextStyle-Header-S-Black_parameters.hpp"
#include "SDK/FN_RadialPicker_structs.hpp"
#include "SDK/FN_RadialPicker_classes.hpp"
#include "SDK/FN_RadialPicker_parameters.hpp"
#include "SDK/FN_PanelButton_structs.hpp"
#include "SDK/FN_PanelButton_classes.hpp"
#include "SDK/FN_PanelButton_parameters.hpp"
#include "SDK/FN_FrontEndRewards_Queue_structs.hpp"
#include "SDK/FN_FrontEndRewards_Queue_classes.hpp"
#include "SDK/FN_FrontEndRewards_Queue_parameters.hpp"
#include "SDK/FN_Fort_Entry_Music_Controller_BP_structs.hpp"
#include "SDK/FN_Fort_Entry_Music_Controller_BP_classes.hpp"
#include "SDK/FN_Fort_Entry_Music_Controller_BP_parameters.hpp"
#include "SDK/FN_QuestTalkingHeadWidget_structs.hpp"
#include "SDK/FN_QuestTalkingHeadWidget_classes.hpp"
#include "SDK/FN_QuestTalkingHeadWidget_parameters.hpp"
#include "SDK/FN_Subtitles_structs.hpp"
#include "SDK/FN_Subtitles_classes.hpp"
#include "SDK/FN_Subtitles_parameters.hpp"
#include "SDK/FN_InputReflector_structs.hpp"
#include "SDK/FN_InputReflector_classes.hpp"
#include "SDK/FN_InputReflector_parameters.hpp"
#include "SDK/FN_SlateContentCalloutMenu_structs.hpp"
#include "SDK/FN_SlateContentCalloutMenu_classes.hpp"
#include "SDK/FN_SlateContentCalloutMenu_parameters.hpp"
#include "SDK/FN_Border-SolidBG-Purple_structs.hpp"
#include "SDK/FN_Border-SolidBG-Purple_classes.hpp"
#include "SDK/FN_Border-SolidBG-Purple_parameters.hpp"
#include "SDK/FN_ButtonStyle-LeftDark_structs.hpp"
#include "SDK/FN_ButtonStyle-LeftDark_classes.hpp"
#include "SDK/FN_ButtonStyle-LeftDark_parameters.hpp"
#include "SDK/FN_PopupCenterMessageWidget_structs.hpp"
#include "SDK/FN_PopupCenterMessageWidget_classes.hpp"
#include "SDK/FN_PopupCenterMessageWidget_parameters.hpp"
#include "SDK/FN_Reticle_structs.hpp"
#include "SDK/FN_Reticle_classes.hpp"
#include "SDK/FN_Reticle_parameters.hpp"
#include "SDK/FN_ButtonStyle-Right-Dark_structs.hpp"
#include "SDK/FN_ButtonStyle-Right-Dark_classes.hpp"
#include "SDK/FN_ButtonStyle-Right-Dark_parameters.hpp"
#include "SDK/FN_CheckFrontEndRewardsAction_structs.hpp"
#include "SDK/FN_CheckFrontEndRewardsAction_classes.hpp"
#include "SDK/FN_CheckFrontEndRewardsAction_parameters.hpp"
#include "SDK/FN_ReticleStatusWidget_structs.hpp"
#include "SDK/FN_ReticleStatusWidget_classes.hpp"
#include "SDK/FN_ReticleStatusWidget_parameters.hpp"
#include "SDK/FN_FrontEndRewards_Expedition_structs.hpp"
#include "SDK/FN_FrontEndRewards_Expedition_classes.hpp"
#include "SDK/FN_FrontEndRewards_Expedition_parameters.hpp"
#include "SDK/FN_TextStyle-Button-Outline-Disabled_structs.hpp"
#include "SDK/FN_TextStyle-Button-Outline-Disabled_classes.hpp"
#include "SDK/FN_TextStyle-Button-Outline-Disabled_parameters.hpp"
#include "SDK/FN_FrontEndRewards_CurrentReward_structs.hpp"
#include "SDK/FN_FrontEndRewards_CurrentReward_classes.hpp"
#include "SDK/FN_FrontEndRewards_CurrentReward_parameters.hpp"
#include "SDK/FN_RewardsIcon_structs.hpp"
#include "SDK/FN_RewardsIcon_classes.hpp"
#include "SDK/FN_RewardsIcon_parameters.hpp"
#include "SDK/FN_Announce_Gen_Quest_Conversation_FrontEndRewards_structs.hpp"
#include "SDK/FN_Announce_Gen_Quest_Conversation_FrontEndRewards_classes.hpp"
#include "SDK/FN_Announce_Gen_Quest_Conversation_FrontEndRewards_parameters.hpp"
#include "SDK/FN_TextStyle-Button-Outline_structs.hpp"
#include "SDK/FN_TextStyle-Button-Outline_classes.hpp"
#include "SDK/FN_TextStyle-Button-Outline_parameters.hpp"
#include "SDK/FN_FrontEndRewards_ChoiceRewards_structs.hpp"
#include "SDK/FN_FrontEndRewards_ChoiceRewards_classes.hpp"
#include "SDK/FN_FrontEndRewards_ChoiceRewards_parameters.hpp"
#include "SDK/FN_QuestInfo_BulletList_structs.hpp"
#include "SDK/FN_QuestInfo_BulletList_classes.hpp"
#include "SDK/FN_QuestInfo_BulletList_parameters.hpp"
#include "SDK/FN_FrontEndRewards_Conversation_VO_structs.hpp"
#include "SDK/FN_FrontEndRewards_Conversation_VO_classes.hpp"
#include "SDK/FN_FrontEndRewards_Conversation_VO_parameters.hpp"
#include "SDK/FN_FrontEndRewards_EpicQuest_structs.hpp"
#include "SDK/FN_FrontEndRewards_EpicQuest_classes.hpp"
#include "SDK/FN_FrontEndRewards_EpicQuest_parameters.hpp"
#include "SDK/FN_QuestInfo_BulletListEntry_structs.hpp"
#include "SDK/FN_QuestInfo_BulletListEntry_classes.hpp"
#include "SDK/FN_QuestInfo_BulletListEntry_parameters.hpp"
#include "SDK/FN_RotatingStarburstWidget_structs.hpp"
#include "SDK/FN_RotatingStarburstWidget_classes.hpp"
#include "SDK/FN_RotatingStarburstWidget_parameters.hpp"
#include "SDK/FN_ButtonStyle-ItemReward_structs.hpp"
#include "SDK/FN_ButtonStyle-ItemReward_classes.hpp"
#include "SDK/FN_ButtonStyle-ItemReward_parameters.hpp"
#include "SDK/FN_Border_AthenaStore_OfferHeaderBacking_structs.hpp"
#include "SDK/FN_Border_AthenaStore_OfferHeaderBacking_classes.hpp"
#include "SDK/FN_Border_AthenaStore_OfferHeaderBacking_parameters.hpp"
#include "SDK/FN_FrontEndRewards_ListRewards_structs.hpp"
#include "SDK/FN_FrontEndRewards_ListRewards_classes.hpp"
#include "SDK/FN_FrontEndRewards_ListRewards_parameters.hpp"
#include "SDK/FN_Border-TabM-Solid_structs.hpp"
#include "SDK/FN_Border-TabM-Solid_classes.hpp"
#include "SDK/FN_Border-TabM-Solid_parameters.hpp"
#include "SDK/FN_ItemInspectScreen_structs.hpp"
#include "SDK/FN_ItemInspectScreen_classes.hpp"
#include "SDK/FN_ItemInspectScreen_parameters.hpp"
#include "SDK/FN_ItemInspectEvolutionConfirmation_structs.hpp"
#include "SDK/FN_ItemInspectEvolutionConfirmation_classes.hpp"
#include "SDK/FN_ItemInspectEvolutionConfirmation_parameters.hpp"
#include "SDK/FN_CollectionHeader-Level_structs.hpp"
#include "SDK/FN_CollectionHeader-Level_classes.hpp"
#include "SDK/FN_CollectionHeader-Level_parameters.hpp"
#include "SDK/FN_ItemInspectUpgradeCallout_structs.hpp"
#include "SDK/FN_ItemInspectUpgradeCallout_classes.hpp"
#include "SDK/FN_ItemInspectUpgradeCallout_parameters.hpp"
#include "SDK/FN_ItemInspectUpgradeConfirmation_structs.hpp"
#include "SDK/FN_ItemInspectUpgradeConfirmation_classes.hpp"
#include "SDK/FN_ItemInspectUpgradeConfirmation_parameters.hpp"
#include "SDK/FN_ItemInspectEvolutionIngredientsList_structs.hpp"
#include "SDK/FN_ItemInspectEvolutionIngredientsList_classes.hpp"
#include "SDK/FN_ItemInspectEvolutionIngredientsList_parameters.hpp"
#include "SDK/FN_ItemInspectEvolutionIngredientsEntry_structs.hpp"
#include "SDK/FN_ItemInspectEvolutionIngredientsEntry_classes.hpp"
#include "SDK/FN_ItemInspectEvolutionIngredientsEntry_parameters.hpp"
#include "SDK/FN_ItemView_structs.hpp"
#include "SDK/FN_ItemView_classes.hpp"
#include "SDK/FN_ItemView_parameters.hpp"
#include "SDK/FN_QuestInfo_Widget_structs.hpp"
#include "SDK/FN_QuestInfo_Widget_classes.hpp"
#include "SDK/FN_QuestInfo_Widget_parameters.hpp"
#include "SDK/FN_ItemInspectionItemExtraDetailsHostPanel_structs.hpp"
#include "SDK/FN_ItemInspectionItemExtraDetailsHostPanel_classes.hpp"
#include "SDK/FN_ItemInspectionItemExtraDetailsHostPanel_parameters.hpp"
#include "SDK/FN_ItemInspectionMainItemDetailsHostPanel_structs.hpp"
#include "SDK/FN_ItemInspectionMainItemDetailsHostPanel_classes.hpp"
#include "SDK/FN_ItemInspectionMainItemDetailsHostPanel_parameters.hpp"
#include "SDK/FN_Rewards_Header_structs.hpp"
#include "SDK/FN_Rewards_Header_classes.hpp"
#include "SDK/FN_Rewards_Header_parameters.hpp"
#include "SDK/FN_FrontEndRewards_Widget_structs.hpp"
#include "SDK/FN_FrontEndRewards_Widget_classes.hpp"
#include "SDK/FN_FrontEndRewards_Widget_parameters.hpp"
#include "SDK/FN_Rewards_ItemCard_structs.hpp"
#include "SDK/FN_Rewards_ItemCard_classes.hpp"
#include "SDK/FN_Rewards_ItemCard_parameters.hpp"
#include "SDK/FN_DailyRewards_structs.hpp"
#include "SDK/FN_DailyRewards_classes.hpp"
#include "SDK/FN_DailyRewards_parameters.hpp"
#include "SDK/FN_PartyFinder_structs.hpp"
#include "SDK/FN_PartyFinder_classes.hpp"
#include "SDK/FN_PartyFinder_parameters.hpp"
#include "SDK/FN_CollectionHeader-PageProgress_structs.hpp"
#include "SDK/FN_CollectionHeader-PageProgress_classes.hpp"
#include "SDK/FN_CollectionHeader-PageProgress_parameters.hpp"
#include "SDK/FN_MgmtTabsScreen_structs.hpp"
#include "SDK/FN_MgmtTabsScreen_classes.hpp"
#include "SDK/FN_MgmtTabsScreen_parameters.hpp"
#include "SDK/FN_CollectionHeader-Progress-Solid_structs.hpp"
#include "SDK/FN_CollectionHeader-Progress-Solid_classes.hpp"
#include "SDK/FN_CollectionHeader-Progress-Solid_parameters.hpp"
#include "SDK/FN_ItemManagementScreen_structs.hpp"
#include "SDK/FN_ItemManagementScreen_classes.hpp"
#include "SDK/FN_ItemManagementScreen_parameters.hpp"
#include "SDK/FN_CollectionHeader-Progress_structs.hpp"
#include "SDK/FN_CollectionHeader-Progress_classes.hpp"
#include "SDK/FN_CollectionHeader-Progress_parameters.hpp"
#include "SDK/FN_ItemTransform_structs.hpp"
#include "SDK/FN_ItemTransform_classes.hpp"
#include "SDK/FN_ItemTransform_parameters.hpp"
#include "SDK/FN_Border-TabM-Solid-White60pc_structs.hpp"
#include "SDK/FN_Border-TabM-Solid-White60pc_classes.hpp"
#include "SDK/FN_Border-TabM-Solid-White60pc_parameters.hpp"
#include "SDK/FN_ItemTransformConfirmationModal_structs.hpp"
#include "SDK/FN_ItemTransformConfirmationModal_classes.hpp"
#include "SDK/FN_ItemTransformConfirmationModal_parameters.hpp"
#include "SDK/FN_CollectionBookWidget_structs.hpp"
#include "SDK/FN_CollectionBookWidget_classes.hpp"
#include "SDK/FN_CollectionBookWidget_parameters.hpp"
#include "SDK/FN_ItemTransformRequiredItems_structs.hpp"
#include "SDK/FN_ItemTransformRequiredItems_classes.hpp"
#include "SDK/FN_ItemTransformRequiredItems_parameters.hpp"
#include "SDK/FN_Border_Tab_DkBlue_structs.hpp"
#include "SDK/FN_Border_Tab_DkBlue_classes.hpp"
#include "SDK/FN_Border_Tab_DkBlue_parameters.hpp"
#include "SDK/FN_CollectionBookSectionPanel_structs.hpp"
#include "SDK/FN_CollectionBookSectionPanel_classes.hpp"
#include "SDK/FN_CollectionBookSectionPanel_parameters.hpp"
#include "SDK/FN_Border_DropShadow_structs.hpp"
#include "SDK/FN_Border_DropShadow_classes.hpp"
#include "SDK/FN_Border_DropShadow_parameters.hpp"
#include "SDK/FN_Border_LightNavy_VGrad_structs.hpp"
#include "SDK/FN_Border_LightNavy_VGrad_classes.hpp"
#include "SDK/FN_Border_LightNavy_VGrad_parameters.hpp"
#include "SDK/FN_XpBoosts_structs.hpp"
#include "SDK/FN_XpBoosts_classes.hpp"
#include "SDK/FN_XpBoosts_parameters.hpp"
#include "SDK/FN_ButtonStyle-MediumBase_structs.hpp"
#include "SDK/FN_ButtonStyle-MediumBase_classes.hpp"
#include "SDK/FN_ButtonStyle-MediumBase_parameters.hpp"
#include "SDK/FN_XpBar_structs.hpp"
#include "SDK/FN_XpBar_classes.hpp"
#include "SDK/FN_XpBar_parameters.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_structs.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_classes.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_parameters.hpp"
#include "SDK/FN_XpBarXpText_structs.hpp"
#include "SDK/FN_XpBarXpText_classes.hpp"
#include "SDK/FN_XpBarXpText_parameters.hpp"
#include "SDK/FN_ItemTransformResultModal_structs.hpp"
#include "SDK/FN_ItemTransformResultModal_classes.hpp"
#include "SDK/FN_ItemTransformResultModal_parameters.hpp"
#include "SDK/FN_CollectionBookProgressWidget_structs.hpp"
#include "SDK/FN_CollectionBookProgressWidget_classes.hpp"
#include "SDK/FN_CollectionBookProgressWidget_parameters.hpp"
#include "SDK/FN_ButtonStyle-Tab-Secondary_structs.hpp"
#include "SDK/FN_ButtonStyle-Tab-Secondary_classes.hpp"
#include "SDK/FN_ButtonStyle-Tab-Secondary_parameters.hpp"
#include "SDK/FN_ItemTransformSlotScreen_structs.hpp"
#include "SDK/FN_ItemTransformSlotScreen_classes.hpp"
#include "SDK/FN_ItemTransformSlotScreen_parameters.hpp"
#include "SDK/FN_TextStyle-Tab-Secondary-Disabled_structs.hpp"
#include "SDK/FN_TextStyle-Tab-Secondary-Disabled_classes.hpp"
#include "SDK/FN_TextStyle-Tab-Secondary-Disabled_parameters.hpp"
#include "SDK/FN_TextStyle-Tab-Secondary_structs.hpp"
#include "SDK/FN_TextStyle-Tab-Secondary_classes.hpp"
#include "SDK/FN_TextStyle-Tab-Secondary_parameters.hpp"
#include "SDK/FN_BoostsRoot_structs.hpp"
#include "SDK/FN_BoostsRoot_classes.hpp"
#include "SDK/FN_BoostsRoot_parameters.hpp"
#include "SDK/FN_XpBoostDailyBonus_structs.hpp"
#include "SDK/FN_XpBoostDailyBonus_classes.hpp"
#include "SDK/FN_XpBoostDailyBonus_parameters.hpp"
#include "SDK/FN_AccountBonuses_structs.hpp"
#include "SDK/FN_AccountBonuses_classes.hpp"
#include "SDK/FN_AccountBonuses_parameters.hpp"
#include "SDK/FN_ItemTransformResultInfo_structs.hpp"
#include "SDK/FN_ItemTransformResultInfo_classes.hpp"
#include "SDK/FN_ItemTransformResultInfo_parameters.hpp"
#include "SDK/FN_TextStyle-Header-M-Special-Large_structs.hpp"
#include "SDK/FN_TextStyle-Header-M-Special-Large_classes.hpp"
#include "SDK/FN_TextStyle-Header-M-Special-Large_parameters.hpp"
#include "SDK/FN_TextStyle-Power-S_structs.hpp"
#include "SDK/FN_TextStyle-Power-S_classes.hpp"
#include "SDK/FN_TextStyle-Power-S_parameters.hpp"
#include "SDK/FN_ItemTransformKeyPickerTileButton_structs.hpp"
#include "SDK/FN_ItemTransformKeyPickerTileButton_classes.hpp"
#include "SDK/FN_ItemTransformKeyPickerTileButton_parameters.hpp"
#include "SDK/FN_HeroSquadManagementScreen_structs.hpp"
#include "SDK/FN_HeroSquadManagementScreen_classes.hpp"
#include "SDK/FN_HeroSquadManagementScreen_parameters.hpp"
#include "SDK/FN_QuestScreen_structs.hpp"
#include "SDK/FN_QuestScreen_classes.hpp"
#include "SDK/FN_QuestScreen_parameters.hpp"
#include "SDK/FN_JournalQuestRewardDetails_structs.hpp"
#include "SDK/FN_JournalQuestRewardDetails_classes.hpp"
#include "SDK/FN_JournalQuestRewardDetails_parameters.hpp"
#include "SDK/FN_SquadStatsWidget_structs.hpp"
#include "SDK/FN_SquadStatsWidget_classes.hpp"
#include "SDK/FN_SquadStatsWidget_parameters.hpp"
#include "SDK/FN_BP_ItemTransform_TabButton_structs.hpp"
#include "SDK/FN_BP_ItemTransform_TabButton_classes.hpp"
#include "SDK/FN_BP_ItemTransform_TabButton_parameters.hpp"
#include "SDK/FN_ItemTransformKeyInfo_structs.hpp"
#include "SDK/FN_ItemTransformKeyInfo_classes.hpp"
#include "SDK/FN_ItemTransformKeyInfo_parameters.hpp"
#include "SDK/FN_ItemTransformKeyScreen_structs.hpp"
#include "SDK/FN_ItemTransformKeyScreen_classes.hpp"
#include "SDK/FN_ItemTransformKeyScreen_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S-Red_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S-Red_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S-Red_parameters.hpp"
#include "SDK/FN_ItemTransformItemPicker_structs.hpp"
#include "SDK/FN_ItemTransformItemPicker_classes.hpp"
#include "SDK/FN_ItemTransformItemPicker_parameters.hpp"
#include "SDK/FN_ItemTransformResultItems_structs.hpp"
#include "SDK/FN_ItemTransformResultItems_classes.hpp"
#include "SDK/FN_ItemTransformResultItems_parameters.hpp"
#include "SDK/FN_ItemTransformSlotEntry_structs.hpp"
#include "SDK/FN_ItemTransformSlotEntry_classes.hpp"
#include "SDK/FN_ItemTransformSlotEntry_parameters.hpp"
#include "SDK/FN_ItemTransformKeyPicker_structs.hpp"
#include "SDK/FN_ItemTransformKeyPicker_classes.hpp"
#include "SDK/FN_ItemTransformKeyPicker_parameters.hpp"
#include "SDK/FN_ItemTransformSlotItemPicker_structs.hpp"
#include "SDK/FN_ItemTransformSlotItemPicker_classes.hpp"
#include "SDK/FN_ItemTransformSlotItemPicker_parameters.hpp"
#include "SDK/FN_ButtonStyle-BottomBar_structs.hpp"
#include "SDK/FN_ButtonStyle-BottomBar_classes.hpp"
#include "SDK/FN_ButtonStyle-BottomBar_parameters.hpp"
#include "SDK/FN_XpBoostRow_structs.hpp"
#include "SDK/FN_XpBoostRow_classes.hpp"
#include "SDK/FN_XpBoostRow_parameters.hpp"
#include "SDK/FN_ItemTransformSlotItemPickerTileButton_structs.hpp"
#include "SDK/FN_ItemTransformSlotItemPickerTileButton_classes.hpp"
#include "SDK/FN_ItemTransformSlotItemPickerTileButton_parameters.hpp"
#include "SDK/FN_XpBoostCounts_structs.hpp"
#include "SDK/FN_XpBoostCounts_classes.hpp"
#include "SDK/FN_XpBoostCounts_parameters.hpp"
#include "SDK/FN_Border_StatRow_DarkBlue_20pc_structs.hpp"
#include "SDK/FN_Border_StatRow_DarkBlue_20pc_classes.hpp"
#include "SDK/FN_Border_StatRow_DarkBlue_20pc_parameters.hpp"
#include "SDK/FN_XpBoostQuantities_structs.hpp"
#include "SDK/FN_XpBoostQuantities_classes.hpp"
#include "SDK/FN_XpBoostQuantities_parameters.hpp"
#include "SDK/FN_Border_StatRow_DarkBlue_80pc_structs.hpp"
#include "SDK/FN_Border_StatRow_DarkBlue_80pc_classes.hpp"
#include "SDK/FN_Border_StatRow_DarkBlue_80pc_parameters.hpp"
#include "SDK/FN_ItemManagementInventoryPanel_structs.hpp"
#include "SDK/FN_ItemManagementInventoryPanel_classes.hpp"
#include "SDK/FN_ItemManagementInventoryPanel_parameters.hpp"
#include "SDK/FN_ItemManagementDetailsModeBox_structs.hpp"
#include "SDK/FN_ItemManagementDetailsModeBox_classes.hpp"
#include "SDK/FN_ItemManagementDetailsModeBox_parameters.hpp"
#include "SDK/FN_XpBoostInfoText_structs.hpp"
#include "SDK/FN_XpBoostInfoText_classes.hpp"
#include "SDK/FN_XpBoostInfoText_parameters.hpp"
#include "SDK/FN_TextStyle-Button-BottomBar-S-Disabled_structs.hpp"
#include "SDK/FN_TextStyle-Button-BottomBar-S-Disabled_classes.hpp"
#include "SDK/FN_TextStyle-Button-BottomBar-S-Disabled_parameters.hpp"
#include "SDK/FN_ItemManagementModeDetailsPanel_structs.hpp"
#include "SDK/FN_ItemManagementModeDetailsPanel_classes.hpp"
#include "SDK/FN_ItemManagementModeDetailsPanel_parameters.hpp"
#include "SDK/FN_MulchConfirmationItem_structs.hpp"
#include "SDK/FN_MulchConfirmationItem_classes.hpp"
#include "SDK/FN_MulchConfirmationItem_parameters.hpp"
#include "SDK/FN_ItemManagementFocusSwitcher_structs.hpp"
#include "SDK/FN_ItemManagementFocusSwitcher_classes.hpp"
#include "SDK/FN_ItemManagementFocusSwitcher_parameters.hpp"
#include "SDK/FN_QuantitySelector_structs.hpp"
#include "SDK/FN_QuantitySelector_classes.hpp"
#include "SDK/FN_QuantitySelector_parameters.hpp"
#include "SDK/FN_IconTabButton_structs.hpp"
#include "SDK/FN_IconTabButton_classes.hpp"
#include "SDK/FN_IconTabButton_parameters.hpp"
#include "SDK/FN_ItemManangementItemTileButton_structs.hpp"
#include "SDK/FN_ItemManangementItemTileButton_classes.hpp"
#include "SDK/FN_ItemManangementItemTileButton_parameters.hpp"
#include "SDK/FN_HeroSquadBonus_structs.hpp"
#include "SDK/FN_HeroSquadBonus_classes.hpp"
#include "SDK/FN_HeroSquadBonus_parameters.hpp"
#include "SDK/FN_HeroSquadBonuses_structs.hpp"
#include "SDK/FN_HeroSquadBonuses_classes.hpp"
#include "SDK/FN_HeroSquadBonuses_parameters.hpp"
#include "SDK/FN_HeroSquadSlotsView_structs.hpp"
#include "SDK/FN_HeroSquadSlotsView_classes.hpp"
#include "SDK/FN_HeroSquadSlotsView_parameters.hpp"
#include "SDK/FN_ItemManagementMulchConfirmationDialogContent_structs.hpp"
#include "SDK/FN_ItemManagementMulchConfirmationDialogContent_classes.hpp"
#include "SDK/FN_ItemManagementMulchConfirmationDialogContent_parameters.hpp"
#include "SDK/FN_ItemAttributesDetailWidget_structs.hpp"
#include "SDK/FN_ItemAttributesDetailWidget_classes.hpp"
#include "SDK/FN_ItemAttributesDetailWidget_parameters.hpp"
#include "SDK/FN_ItemManagementCompareModeBox_structs.hpp"
#include "SDK/FN_ItemManagementCompareModeBox_classes.hpp"
#include "SDK/FN_ItemManagementCompareModeBox_parameters.hpp"
#include "SDK/FN_ItemManagementMulchModeBox_structs.hpp"
#include "SDK/FN_ItemManagementMulchModeBox_classes.hpp"
#include "SDK/FN_ItemManagementMulchModeBox_parameters.hpp"
#include "SDK/FN_ItemEntry_structs.hpp"
#include "SDK/FN_ItemEntry_classes.hpp"
#include "SDK/FN_ItemEntry_parameters.hpp"
#include "SDK/FN_MonolithicItemDetailsHostPanel_structs.hpp"
#include "SDK/FN_MonolithicItemDetailsHostPanel_classes.hpp"
#include "SDK/FN_MonolithicItemDetailsHostPanel_parameters.hpp"
#include "SDK/FN_StatsModeItemDetailsHostPanel_structs.hpp"
#include "SDK/FN_StatsModeItemDetailsHostPanel_classes.hpp"
#include "SDK/FN_StatsModeItemDetailsHostPanel_parameters.hpp"
#include "SDK/FN_ItemCraftingIngredientList_structs.hpp"
#include "SDK/FN_ItemCraftingIngredientList_classes.hpp"
#include "SDK/FN_ItemCraftingIngredientList_parameters.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_S-B_Blue_structs.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_S-B_Blue_classes.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_S-B_Blue_parameters.hpp"
#include "SDK/FN_ItemCraftingIngredientListEntryHaveNeedVerbose_structs.hpp"
#include "SDK/FN_ItemCraftingIngredientListEntryHaveNeedVerbose_classes.hpp"
#include "SDK/FN_ItemCraftingIngredientListEntryHaveNeedVerbose_parameters.hpp"
#include "SDK/FN_MiscellaneousModeItemDetailsHostPanel_structs.hpp"
#include "SDK/FN_MiscellaneousModeItemDetailsHostPanel_classes.hpp"
#include "SDK/FN_MiscellaneousModeItemDetailsHostPanel_parameters.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-Base_structs.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-Base_classes.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-Base_parameters.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-ComparisonModeItemToCompare_structs.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-ComparisonModeItemToCompare_classes.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-ComparisonModeItemToCompare_parameters.hpp"
#include "SDK/FN_ItemCraftingIngredientsDetailWidget_structs.hpp"
#include "SDK/FN_ItemCraftingIngredientsDetailWidget_classes.hpp"
#include "SDK/FN_ItemCraftingIngredientsDetailWidget_parameters.hpp"
#include "SDK/FN_CollectionBounds_structs.hpp"
#include "SDK/FN_CollectionBounds_classes.hpp"
#include "SDK/FN_CollectionBounds_parameters.hpp"
#include "SDK/FN_ItemIconWidget_structs.hpp"
#include "SDK/FN_ItemIconWidget_classes.hpp"
#include "SDK/FN_ItemIconWidget_parameters.hpp"
#include "SDK/FN_ItemCraftingIngredientListEntryHaveNeed_structs.hpp"
#include "SDK/FN_ItemCraftingIngredientListEntryHaveNeed_classes.hpp"
#include "SDK/FN_ItemCraftingIngredientListEntryHaveNeed_parameters.hpp"
#include "SDK/FN_ItemManagementEquipSlot_structs.hpp"
#include "SDK/FN_ItemManagementEquipSlot_classes.hpp"
#include "SDK/FN_ItemManagementEquipSlot_parameters.hpp"
#include "SDK/FN_ItemManagementInventoryLimitStatusIndicator_structs.hpp"
#include "SDK/FN_ItemManagementInventoryLimitStatusIndicator_classes.hpp"
#include "SDK/FN_ItemManagementInventoryLimitStatusIndicator_parameters.hpp"
#include "SDK/FN_Border_StatRow_WhiteMidOpacity_structs.hpp"
#include "SDK/FN_Border_StatRow_WhiteMidOpacity_classes.hpp"
#include "SDK/FN_Border_StatRow_WhiteMidOpacity_parameters.hpp"
#include "SDK/FN_ItemManagementMulchDetailsPanel_structs.hpp"
#include "SDK/FN_ItemManagementMulchDetailsPanel_classes.hpp"
#include "SDK/FN_ItemManagementMulchDetailsPanel_parameters.hpp"
#include "SDK/FN_RewardListEntryType_structs.hpp"
#include "SDK/FN_RewardListEntryType_classes.hpp"
#include "SDK/FN_RewardListEntryType_parameters.hpp"
#include "SDK/FN_ItemWindow_structs.hpp"
#include "SDK/FN_ItemWindow_classes.hpp"
#include "SDK/FN_ItemWindow_parameters.hpp"
#include "SDK/FN_ObjectivesPage_structs.hpp"
#include "SDK/FN_ObjectivesPage_classes.hpp"
#include "SDK/FN_ObjectivesPage_parameters.hpp"
#include "SDK/FN_StatsListWidget_structs.hpp"
#include "SDK/FN_StatsListWidget_classes.hpp"
#include "SDK/FN_StatsListWidget_parameters.hpp"
#include "SDK/FN_StatItemLarge_structs.hpp"
#include "SDK/FN_StatItemLarge_classes.hpp"
#include "SDK/FN_StatItemLarge_parameters.hpp"
#include "SDK/FN_MissionObjectiveContentWidgetInterface_structs.hpp"
#include "SDK/FN_MissionObjectiveContentWidgetInterface_classes.hpp"
#include "SDK/FN_MissionObjectiveContentWidgetInterface_parameters.hpp"
#include "SDK/FN_Border-TabM-Solid-White20pc_structs.hpp"
#include "SDK/FN_Border-TabM-Solid-White20pc_classes.hpp"
#include "SDK/FN_Border-TabM-Solid-White20pc_parameters.hpp"
#include "SDK/FN_Border-SolidBG_structs.hpp"
#include "SDK/FN_Border-SolidBG_classes.hpp"
#include "SDK/FN_Border-SolidBG_parameters.hpp"
#include "SDK/FN_SimpleSquadSlotButton_structs.hpp"
#include "SDK/FN_SimpleSquadSlotButton_classes.hpp"
#include "SDK/FN_SimpleSquadSlotButton_parameters.hpp"
#include "SDK/FN_MissionObjectiveWidgetProviderInterface_structs.hpp"
#include "SDK/FN_MissionObjectiveWidgetProviderInterface_classes.hpp"
#include "SDK/FN_MissionObjectiveWidgetProviderInterface_parameters.hpp"
#include "SDK/FN_HeroSquadBonusesDetailWidget_structs.hpp"
#include "SDK/FN_HeroSquadBonusesDetailWidget_classes.hpp"
#include "SDK/FN_HeroSquadBonusesDetailWidget_parameters.hpp"
#include "SDK/FN_ItemCraftingIngredientListEntryHaveNeedVerbose_BLACK_structs.hpp"
#include "SDK/FN_ItemCraftingIngredientListEntryHaveNeedVerbose_BLACK_classes.hpp"
#include "SDK/FN_ItemCraftingIngredientListEntryHaveNeedVerbose_BLACK_parameters.hpp"
#include "SDK/FN_MulchRefundItemQuantityList_structs.hpp"
#include "SDK/FN_MulchRefundItemQuantityList_classes.hpp"
#include "SDK/FN_MulchRefundItemQuantityList_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-40pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-40pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-40pc_parameters.hpp"
#include "SDK/FN_AbilitiesPage_structs.hpp"
#include "SDK/FN_AbilitiesPage_classes.hpp"
#include "SDK/FN_AbilitiesPage_parameters.hpp"
#include "SDK/FN_AbilitiesPageTile_structs.hpp"
#include "SDK/FN_AbilitiesPageTile_classes.hpp"
#include "SDK/FN_AbilitiesPageTile_parameters.hpp"
#include "SDK/FN_MovieWidget_structs.hpp"
#include "SDK/FN_MovieWidget_classes.hpp"
#include "SDK/FN_MovieWidget_parameters.hpp"
#include "SDK/FN_DailyRewardsSchedule_structs.hpp"
#include "SDK/FN_DailyRewardsSchedule_classes.hpp"
#include "SDK/FN_DailyRewardsSchedule_parameters.hpp"
#include "SDK/FN_ActiveModifiersPanelContent_structs.hpp"
#include "SDK/FN_ActiveModifiersPanelContent_classes.hpp"
#include "SDK/FN_ActiveModifiersPanelContent_parameters.hpp"
#include "SDK/FN_CollectionBar_structs.hpp"
#include "SDK/FN_CollectionBar_classes.hpp"
#include "SDK/FN_CollectionBar_parameters.hpp"
#include "SDK/FN_CollectionBarTopIcon_structs.hpp"
#include "SDK/FN_CollectionBarTopIcon_classes.hpp"
#include "SDK/FN_CollectionBarTopIcon_parameters.hpp"
#include "SDK/FN_DailyRewardsEpic_structs.hpp"
#include "SDK/FN_DailyRewardsEpic_classes.hpp"
#include "SDK/FN_DailyRewardsEpic_parameters.hpp"
#include "SDK/FN_PlayerBanner_structs.hpp"
#include "SDK/FN_PlayerBanner_classes.hpp"
#include "SDK/FN_PlayerBanner_parameters.hpp"
#include "SDK/FN_TeamScorePanelContent_structs.hpp"
#include "SDK/FN_TeamScorePanelContent_classes.hpp"
#include "SDK/FN_TeamScorePanelContent_parameters.hpp"
#include "SDK/FN_DailyRewardsItem_structs.hpp"
#include "SDK/FN_DailyRewardsItem_classes.hpp"
#include "SDK/FN_DailyRewardsItem_parameters.hpp"
#include "SDK/FN_BannerLibrary_structs.hpp"
#include "SDK/FN_BannerLibrary_classes.hpp"
#include "SDK/FN_BannerLibrary_parameters.hpp"
#include "SDK/FN_BP_LiveStreamerDescription_structs.hpp"
#include "SDK/FN_BP_LiveStreamerDescription_classes.hpp"
#include "SDK/FN_BP_LiveStreamerDescription_parameters.hpp"
#include "SDK/FN_DailyRewardsCurrent_structs.hpp"
#include "SDK/FN_DailyRewardsCurrent_classes.hpp"
#include "SDK/FN_DailyRewardsCurrent_parameters.hpp"
#include "SDK/FN_JournalQuestDetails_structs.hpp"
#include "SDK/FN_JournalQuestDetails_classes.hpp"
#include "SDK/FN_JournalQuestDetails_parameters.hpp"
#include "SDK/FN_BP_QuestExpiresWidget_structs.hpp"
#include "SDK/FN_BP_QuestExpiresWidget_classes.hpp"
#include "SDK/FN_BP_QuestExpiresWidget_parameters.hpp"
#include "SDK/FN_CollectionMissionBadgeDisplayInfo_structs.hpp"
#include "SDK/FN_CollectionMissionBadgeDisplayInfo_classes.hpp"
#include "SDK/FN_CollectionMissionBadgeDisplayInfo_parameters.hpp"
#include "SDK/FN_QuestTreeEntry_structs.hpp"
#include "SDK/FN_QuestTreeEntry_classes.hpp"
#include "SDK/FN_QuestTreeEntry_parameters.hpp"
#include "SDK/FN_MissionOverviewObjective_structs.hpp"
#include "SDK/FN_MissionOverviewObjective_classes.hpp"
#include "SDK/FN_MissionOverviewObjective_parameters.hpp"
#include "SDK/FN_MissionObjectiveProgress_structs.hpp"
#include "SDK/FN_MissionObjectiveProgress_classes.hpp"
#include "SDK/FN_MissionObjectiveProgress_parameters.hpp"
#include "SDK/FN_JournalQuestProgressBar_structs.hpp"
#include "SDK/FN_JournalQuestProgressBar_classes.hpp"
#include "SDK/FN_JournalQuestProgressBar_parameters.hpp"
#include "SDK/FN_MissionRichText_structs.hpp"
#include "SDK/FN_MissionRichText_classes.hpp"
#include "SDK/FN_MissionRichText_parameters.hpp"
#include "SDK/FN_CollectionMultiProgressBar_structs.hpp"
#include "SDK/FN_CollectionMultiProgressBar_classes.hpp"
#include "SDK/FN_CollectionMultiProgressBar_parameters.hpp"
#include "SDK/FN_ObjectivesPanelContent_structs.hpp"
#include "SDK/FN_ObjectivesPanelContent_classes.hpp"
#include "SDK/FN_ObjectivesPanelContent_parameters.hpp"
#include "SDK/FN_PartyInfo_structs.hpp"
#include "SDK/FN_PartyInfo_classes.hpp"
#include "SDK/FN_PartyInfo_parameters.hpp"
#include "SDK/FN_ScoreDetailsRow_structs.hpp"
#include "SDK/FN_ScoreDetailsRow_classes.hpp"
#include "SDK/FN_ScoreDetailsRow_parameters.hpp"
#include "SDK/FN_QuestVerticalRewardInfo_structs.hpp"
#include "SDK/FN_QuestVerticalRewardInfo_classes.hpp"
#include "SDK/FN_QuestVerticalRewardInfo_parameters.hpp"
#include "SDK/FN_EarnedBadgeTile_structs.hpp"
#include "SDK/FN_EarnedBadgeTile_classes.hpp"
#include "SDK/FN_EarnedBadgeTile_parameters.hpp"
#include "SDK/FN_ButtonStyle-BasicVerticalList_structs.hpp"
#include "SDK/FN_ButtonStyle-BasicVerticalList_classes.hpp"
#include "SDK/FN_ButtonStyle-BasicVerticalList_parameters.hpp"
#include "SDK/FN_Border-SolidBG-ShellBlue_structs.hpp"
#include "SDK/FN_Border-SolidBG-ShellBlue_classes.hpp"
#include "SDK/FN_Border-SolidBG-ShellBlue_parameters.hpp"
#include "SDK/FN_Border-LowerTabM_structs.hpp"
#include "SDK/FN_Border-LowerTabM_classes.hpp"
#include "SDK/FN_Border-LowerTabM_parameters.hpp"
#include "SDK/FN_RewardInfoButtonWidget_structs.hpp"
#include "SDK/FN_RewardInfoButtonWidget_classes.hpp"
#include "SDK/FN_RewardInfoButtonWidget_parameters.hpp"
#include "SDK/FN_TeamScoreDetailsContent_structs.hpp"
#include "SDK/FN_TeamScoreDetailsContent_classes.hpp"
#include "SDK/FN_TeamScoreDetailsContent_parameters.hpp"
#include "SDK/FN_Border-LowerTabM-Solid_structs.hpp"
#include "SDK/FN_Border-LowerTabM-Solid_classes.hpp"
#include "SDK/FN_Border-LowerTabM-Solid_parameters.hpp"
#include "SDK/FN_ItemInspectEvolutionChoiceEntry_structs.hpp"
#include "SDK/FN_ItemInspectEvolutionChoiceEntry_classes.hpp"
#include "SDK/FN_ItemInspectEvolutionChoiceEntry_parameters.hpp"
#include "SDK/FN_ButtonStyle-Outline-S_structs.hpp"
#include "SDK/FN_ButtonStyle-Outline-S_classes.hpp"
#include "SDK/FN_ButtonStyle-Outline-S_parameters.hpp"
#include "SDK/FN_TextStyle-Base-M-B-S_structs.hpp"
#include "SDK/FN_TextStyle-Base-M-B-S_classes.hpp"
#include "SDK/FN_TextStyle-Base-M-B-S_parameters.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-ComparisonModeItemToDetail_structs.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-ComparisonModeItemToDetail_classes.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-ComparisonModeItemToDetail_parameters.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-SortLabel_structs.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-SortLabel_classes.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-SortLabel_parameters.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-SortLabelLight_structs.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-SortLabelLight_classes.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-SortLabelLight_parameters.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-B_structs.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-B_classes.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-B_parameters.hpp"
#include "SDK/FN_ScoreBarsWidget_structs.hpp"
#include "SDK/FN_ScoreBarsWidget_classes.hpp"
#include "SDK/FN_ScoreBarsWidget_parameters.hpp"
#include "SDK/FN_HelpWidget_structs.hpp"
#include "SDK/FN_HelpWidget_classes.hpp"
#include "SDK/FN_HelpWidget_parameters.hpp"
#include "SDK/FN_Border-Bottom-Box-Rounded-DkBlue_structs.hpp"
#include "SDK/FN_Border-Bottom-Box-Rounded-DkBlue_classes.hpp"
#include "SDK/FN_Border-Bottom-Box-Rounded-DkBlue_parameters.hpp"
#include "SDK/FN_DayWidget_structs.hpp"
#include "SDK/FN_DayWidget_classes.hpp"
#include "SDK/FN_DayWidget_parameters.hpp"
#include "SDK/FN_ZoneDetails_structs.hpp"
#include "SDK/FN_ZoneDetails_classes.hpp"
#include "SDK/FN_ZoneDetails_parameters.hpp"
#include "SDK/FN_MissionTrackerEntry_structs.hpp"
#include "SDK/FN_MissionTrackerEntry_classes.hpp"
#include "SDK/FN_MissionTrackerEntry_parameters.hpp"
#include "SDK/FN_Border-ItemInfo-Locked_structs.hpp"
#include "SDK/FN_Border-ItemInfo-Locked_classes.hpp"
#include "SDK/FN_Border-ItemInfo-Locked_parameters.hpp"
#include "SDK/FN_DefaultObjectiveContentWidget_structs.hpp"
#include "SDK/FN_DefaultObjectiveContentWidget_classes.hpp"
#include "SDK/FN_DefaultObjectiveContentWidget_parameters.hpp"
#include "SDK/FN_MissionObjectiveProgressBarsManager_structs.hpp"
#include "SDK/FN_MissionObjectiveProgressBarsManager_classes.hpp"
#include "SDK/FN_MissionObjectiveProgressBarsManager_parameters.hpp"
#include "SDK/FN_MissionTrackerSubEntry_structs.hpp"
#include "SDK/FN_MissionTrackerSubEntry_classes.hpp"
#include "SDK/FN_MissionTrackerSubEntry_parameters.hpp"
#include "SDK/FN_MissionTrackerList_structs.hpp"
#include "SDK/FN_MissionTrackerList_classes.hpp"
#include "SDK/FN_MissionTrackerList_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-70pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-70pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-70pc_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-MineShaft_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-MineShaft_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-MineShaft_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-DarkBlueFade_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-DarkBlueFade_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-DarkBlueFade_parameters.hpp"
#include "SDK/FN_QuestTrackerEntry_structs.hpp"
#include "SDK/FN_QuestTrackerEntry_classes.hpp"
#include "SDK/FN_QuestTrackerEntry_parameters.hpp"
#include "SDK/FN_QuestTrackerSubEntry_structs.hpp"
#include "SDK/FN_QuestTrackerSubEntry_classes.hpp"
#include "SDK/FN_QuestTrackerSubEntry_parameters.hpp"
#include "SDK/FN_ScoreBadgeProviderMission_structs.hpp"
#include "SDK/FN_ScoreBadgeProviderMission_classes.hpp"
#include "SDK/FN_ScoreBadgeProviderMission_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-60pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-60pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-60pc_parameters.hpp"
#include "SDK/FN_QuestTrackerMainQuestList_structs.hpp"
#include "SDK/FN_QuestTrackerMainQuestList_classes.hpp"
#include "SDK/FN_QuestTrackerMainQuestList_parameters.hpp"
#include "SDK/FN_QuestTrackerTrackedQuestsList_structs.hpp"
#include "SDK/FN_QuestTrackerTrackedQuestsList_classes.hpp"
#include "SDK/FN_QuestTrackerTrackedQuestsList_parameters.hpp"
#include "SDK/FN_BadgesEarnedPanelContent_structs.hpp"
#include "SDK/FN_BadgesEarnedPanelContent_classes.hpp"
#include "SDK/FN_BadgesEarnedPanelContent_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-Blue-StatNormal_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-Blue-StatNormal_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-Blue-StatNormal_parameters.hpp"
#include "SDK/FN_TextStyle-Header-M-Special_structs.hpp"
#include "SDK/FN_TextStyle-Header-M-Special_classes.hpp"
#include "SDK/FN_TextStyle-Header-M-Special_parameters.hpp"
#include "SDK/FN_TextStyle-Header-M-Blue_structs.hpp"
#include "SDK/FN_TextStyle-Header-M-Blue_classes.hpp"
#include "SDK/FN_TextStyle-Header-M-Blue_parameters.hpp"
#include "SDK/FN_Border-RoundBoxM_structs.hpp"
#include "SDK/FN_Border-RoundBoxM_classes.hpp"
#include "SDK/FN_Border-RoundBoxM_parameters.hpp"
#include "SDK/FN_ButtonStyle-QuestPrimary-L2_structs.hpp"
#include "SDK/FN_ButtonStyle-QuestPrimary-L2_classes.hpp"
#include "SDK/FN_ButtonStyle-QuestPrimary-L2_parameters.hpp"
#include "SDK/FN_ScoreMessageNumber_structs.hpp"
#include "SDK/FN_ScoreMessageNumber_classes.hpp"
#include "SDK/FN_ScoreMessageNumber_parameters.hpp"
#include "SDK/FN_ScoreBarsScoreMessageItem_structs.hpp"
#include "SDK/FN_ScoreBarsScoreMessageItem_classes.hpp"
#include "SDK/FN_ScoreBarsScoreMessageItem_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-NavyBlueFade_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-NavyBlueFade_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-NavyBlueFade_parameters.hpp"
#include "SDK/FN_MulchRefundItemQuantityListEntry_structs.hpp"
#include "SDK/FN_MulchRefundItemQuantityListEntry_classes.hpp"
#include "SDK/FN_MulchRefundItemQuantityListEntry_parameters.hpp"
#include "SDK/FN_ItemCountRecycling_structs.hpp"
#include "SDK/FN_ItemCountRecycling_classes.hpp"
#include "SDK/FN_ItemCountRecycling_parameters.hpp"
#include "SDK/FN_ButtonStyle-QuestSecondary-L_structs.hpp"
#include "SDK/FN_ButtonStyle-QuestSecondary-L_classes.hpp"
#include "SDK/FN_ButtonStyle-QuestSecondary-L_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Blue_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Blue_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Blue_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-S_LightGray_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-S_LightGray_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-S_LightGray_parameters.hpp"
#include "SDK/FN_RewardOrVerticalWidget_structs.hpp"
#include "SDK/FN_RewardOrVerticalWidget_classes.hpp"
#include "SDK/FN_RewardOrVerticalWidget_parameters.hpp"
#include "SDK/FN_PartyFinderListItem_structs.hpp"
#include "SDK/FN_PartyFinderListItem_classes.hpp"
#include "SDK/FN_PartyFinderListItem_parameters.hpp"
#include "SDK/FN_SkillTreepAGEColors_structs.hpp"
#include "SDK/FN_SkillTreepAGEColors_classes.hpp"
#include "SDK/FN_SkillTreepAGEColors_parameters.hpp"
#include "SDK/FN_DailyRewardsMissingFoundersPack_structs.hpp"
#include "SDK/FN_DailyRewardsMissingFoundersPack_classes.hpp"
#include "SDK/FN_DailyRewardsMissingFoundersPack_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-Black_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-Black_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-Black_parameters.hpp"
#include "SDK/FN_ButtonStyle-Solid-Square-CollectionBook_structs.hpp"
#include "SDK/FN_ButtonStyle-Solid-Square-CollectionBook_classes.hpp"
#include "SDK/FN_ButtonStyle-Solid-Square-CollectionBook_parameters.hpp"
#include "SDK/FN_TextScrollStyle-NoFade_structs.hpp"
#include "SDK/FN_TextScrollStyle-NoFade_classes.hpp"
#include "SDK/FN_TextScrollStyle-NoFade_parameters.hpp"
#include "SDK/FN_CollectionBookSectionRewardWidget_structs.hpp"
#include "SDK/FN_CollectionBookSectionRewardWidget_classes.hpp"
#include "SDK/FN_CollectionBookSectionRewardWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-70pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-70pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-70pc_parameters.hpp"
#include "SDK/FN_ArrowCursorWidget_structs.hpp"
#include "SDK/FN_ArrowCursorWidget_classes.hpp"
#include "SDK/FN_ArrowCursorWidget_parameters.hpp"
#include "SDK/FN_CollectionBookPageDetailsWidget_structs.hpp"
#include "SDK/FN_CollectionBookPageDetailsWidget_classes.hpp"
#include "SDK/FN_CollectionBookPageDetailsWidget_parameters.hpp"
#include "SDK/FN_CollectionBookProgressionRewardsPreviewWidget_structs.hpp"
#include "SDK/FN_CollectionBookProgressionRewardsPreviewWidget_classes.hpp"
#include "SDK/FN_CollectionBookProgressionRewardsPreviewWidget_parameters.hpp"
#include "SDK/FN_ItemDescriptionDetailWidget_structs.hpp"
#include "SDK/FN_ItemDescriptionDetailWidget_classes.hpp"
#include "SDK/FN_ItemDescriptionDetailWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Yellow_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Yellow_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Yellow_parameters.hpp"
#include "SDK/FN_MainModeItemDetailsHostPanel_structs.hpp"
#include "SDK/FN_MainModeItemDetailsHostPanel_classes.hpp"
#include "SDK/FN_MainModeItemDetailsHostPanel_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-LightGray35_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-LightGray35_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-LightGray35_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-Gray_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-Gray_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-Gray_parameters.hpp"
#include "SDK/FN_CollectionBookProgressionRewardWidget_structs.hpp"
#include "SDK/FN_CollectionBookProgressionRewardWidget_classes.hpp"
#include "SDK/FN_CollectionBookProgressionRewardWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Header-M_structs.hpp"
#include "SDK/FN_TextStyle-Header-M_classes.hpp"
#include "SDK/FN_TextStyle-Header-M_parameters.hpp"
#include "SDK/FN_CollectionBookPrimaryPanel_structs.hpp"
#include "SDK/FN_CollectionBookPrimaryPanel_classes.hpp"
#include "SDK/FN_CollectionBookPrimaryPanel_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-Blue_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-Blue_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-Blue_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-S_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-S_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-S_parameters.hpp"
#include "SDK/FN_CollectionBookItemPicker_structs.hpp"
#include "SDK/FN_CollectionBookItemPicker_classes.hpp"
#include "SDK/FN_CollectionBookItemPicker_parameters.hpp"
#include "SDK/FN_ItemTooltip_DisplayMode_structs.hpp"
#include "SDK/FN_ItemTooltip_DisplayMode_classes.hpp"
#include "SDK/FN_ItemTooltip_DisplayMode_parameters.hpp"
#include "SDK/FN_CollectionBookRecycleSlotResultsWidget_structs.hpp"
#include "SDK/FN_CollectionBookRecycleSlotResultsWidget_classes.hpp"
#include "SDK/FN_CollectionBookRecycleSlotResultsWidget_parameters.hpp"
#include "SDK/FN_DONOTUSE_structs.hpp"
#include "SDK/FN_DONOTUSE_classes.hpp"
#include "SDK/FN_DONOTUSE_parameters.hpp"
#include "SDK/FN_QuickbarSlotCooldown_structs.hpp"
#include "SDK/FN_QuickbarSlotCooldown_classes.hpp"
#include "SDK/FN_QuickbarSlotCooldown_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Black_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Black_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Black_parameters.hpp"
#include "SDK/FN_MissionPanelContent_structs.hpp"
#include "SDK/FN_MissionPanelContent_classes.hpp"
#include "SDK/FN_MissionPanelContent_parameters.hpp"
#include "SDK/FN_CollectionBookItemPickerButton_structs.hpp"
#include "SDK/FN_CollectionBookItemPickerButton_classes.hpp"
#include "SDK/FN_CollectionBookItemPickerButton_parameters.hpp"
#include "SDK/FN_CollectionBookOverviewWidget_structs.hpp"
#include "SDK/FN_CollectionBookOverviewWidget_classes.hpp"
#include "SDK/FN_CollectionBookOverviewWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Black_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Black_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Black_parameters.hpp"
#include "SDK/FN_CollectionBookPageCompletionRewardWidget_structs.hpp"
#include "SDK/FN_CollectionBookPageCompletionRewardWidget_classes.hpp"
#include "SDK/FN_CollectionBookPageCompletionRewardWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-White50_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-White50_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-White50_parameters.hpp"
#include "SDK/FN_CollectionBookPageListWidget_structs.hpp"
#include "SDK/FN_CollectionBookPageListWidget_classes.hpp"
#include "SDK/FN_CollectionBookPageListWidget_parameters.hpp"
#include "SDK/FN_CollectionBookRewardCardWidget_structs.hpp"
#include "SDK/FN_CollectionBookRewardCardWidget_classes.hpp"
#include "SDK/FN_CollectionBookRewardCardWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Base-ToolTip-B_structs.hpp"
#include "SDK/FN_TextStyle-Base-ToolTip-B_classes.hpp"
#include "SDK/FN_TextStyle-Base-ToolTip-B_parameters.hpp"
#include "SDK/FN_CollectionBookSectionTileWidget_structs.hpp"
#include "SDK/FN_CollectionBookSectionTileWidget_classes.hpp"
#include "SDK/FN_CollectionBookSectionTileWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Base-ToolTip-Body_structs.hpp"
#include "SDK/FN_TextStyle-Base-ToolTip-Body_classes.hpp"
#include "SDK/FN_TextStyle-Base-ToolTip-Body_parameters.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-B_Blue_structs.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-B_Blue_classes.hpp"
#include "SDK/FN_ButtonStyle-TextOnlyBase_XS-B_Blue_parameters.hpp"
#include "SDK/FN_CollectionBookSectionTileRewardWidget_structs.hpp"
#include "SDK/FN_CollectionBookSectionTileRewardWidget_classes.hpp"
#include "SDK/FN_CollectionBookSectionTileRewardWidget_parameters.hpp"
#include "SDK/FN_CollectionBookSlotView_structs.hpp"
#include "SDK/FN_CollectionBookSlotView_classes.hpp"
#include "SDK/FN_CollectionBookSlotView_parameters.hpp"
#include "SDK/FN_ItemPerksListDetailWidget_structs.hpp"
#include "SDK/FN_ItemPerksListDetailWidget_classes.hpp"
#include "SDK/FN_ItemPerksListDetailWidget_parameters.hpp"
#include "SDK/FN_Tooltip-Item_structs.hpp"
#include "SDK/FN_Tooltip-Item_classes.hpp"
#include "SDK/FN_Tooltip-Item_parameters.hpp"
#include "SDK/FN_MiniCraftingIngredientList_structs.hpp"
#include "SDK/FN_MiniCraftingIngredientList_classes.hpp"
#include "SDK/FN_MiniCraftingIngredientList_parameters.hpp"
#include "SDK/FN_MiniCraftingIngredientListEntry_structs.hpp"
#include "SDK/FN_MiniCraftingIngredientListEntry_classes.hpp"
#include "SDK/FN_MiniCraftingIngredientListEntry_parameters.hpp"
#include "SDK/FN_ItemOrWidget_structs.hpp"
#include "SDK/FN_ItemOrWidget_classes.hpp"
#include "SDK/FN_ItemOrWidget_parameters.hpp"
#include "SDK/FN_ItemCount_structs.hpp"
#include "SDK/FN_ItemCount_classes.hpp"
#include "SDK/FN_ItemCount_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Gray_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Gray_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Gray_parameters.hpp"
#include "SDK/FN_MiniItemCraftingIngredientsDetailWidget_structs.hpp"
#include "SDK/FN_MiniItemCraftingIngredientsDetailWidget_classes.hpp"
#include "SDK/FN_MiniItemCraftingIngredientsDetailWidget_parameters.hpp"
#include "SDK/FN_BasicRatingWidget_structs.hpp"
#include "SDK/FN_BasicRatingWidget_classes.hpp"
#include "SDK/FN_BasicRatingWidget_parameters.hpp"
#include "SDK/FN_SurvivorTraitsDetailWidget_structs.hpp"
#include "SDK/FN_SurvivorTraitsDetailWidget_classes.hpp"
#include "SDK/FN_SurvivorTraitsDetailWidget_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionIconTabButton_structs.hpp"
#include "SDK/FN_BP_FortExpeditionIconTabButton_classes.hpp"
#include "SDK/FN_BP_FortExpeditionIconTabButton_parameters.hpp"
#include "SDK/FN_ItemDetailsHeader_structs.hpp"
#include "SDK/FN_ItemDetailsHeader_classes.hpp"
#include "SDK/FN_ItemDetailsHeader_parameters.hpp"
#include "SDK/FN_GE_Homebase_CombinedRatings_structs.hpp"
#include "SDK/FN_GE_Homebase_CombinedRatings_classes.hpp"
#include "SDK/FN_GE_Homebase_CombinedRatings_parameters.hpp"
#include "SDK/FN_SetBonus_AbilityDamage_Low_structs.hpp"
#include "SDK/FN_SetBonus_AbilityDamage_Low_classes.hpp"
#include "SDK/FN_SetBonus_AbilityDamage_Low_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionDetailsWidget_structs.hpp"
#include "SDK/FN_BP_FortExpeditionDetailsWidget_classes.hpp"
#include "SDK/FN_BP_FortExpeditionDetailsWidget_parameters.hpp"
#include "SDK/FN_SetBonus_MaxHealth_High_structs.hpp"
#include "SDK/FN_SetBonus_MaxHealth_High_classes.hpp"
#include "SDK/FN_SetBonus_MaxHealth_High_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionListItem_structs.hpp"
#include "SDK/FN_BP_FortExpeditionListItem_classes.hpp"
#include "SDK/FN_BP_FortExpeditionListItem_parameters.hpp"
#include "SDK/FN_SetBonus_MaxShield_High_structs.hpp"
#include "SDK/FN_SetBonus_MaxShield_High_classes.hpp"
#include "SDK/FN_SetBonus_MaxShield_High_parameters.hpp"
#include "SDK/FN_SetBonus_MeleeDamage_Low_structs.hpp"
#include "SDK/FN_SetBonus_MeleeDamage_Low_classes.hpp"
#include "SDK/FN_SetBonus_MeleeDamage_Low_parameters.hpp"
#include "SDK/FN_SetBonus_RangedDamage_Low_structs.hpp"
#include "SDK/FN_SetBonus_RangedDamage_Low_classes.hpp"
#include "SDK/FN_SetBonus_RangedDamage_Low_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionOverviewWidget_structs.hpp"
#include "SDK/FN_BP_FortExpeditionOverviewWidget_classes.hpp"
#include "SDK/FN_BP_FortExpeditionOverviewWidget_parameters.hpp"
#include "SDK/FN_SetBonus_ShieldRegen_Low_structs.hpp"
#include "SDK/FN_SetBonus_ShieldRegen_Low_classes.hpp"
#include "SDK/FN_SetBonus_ShieldRegen_Low_parameters.hpp"
#include "SDK/FN_SetBonus_TrapDamage_Low_structs.hpp"
#include "SDK/FN_SetBonus_TrapDamage_Low_classes.hpp"
#include "SDK/FN_SetBonus_TrapDamage_Low_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionBuildSquadWidget_structs.hpp"
#include "SDK/FN_BP_FortExpeditionBuildSquadWidget_classes.hpp"
#include "SDK/FN_BP_FortExpeditionBuildSquadWidget_parameters.hpp"
#include "SDK/FN_ItemCalledOutAttributesDetailWidget_structs.hpp"
#include "SDK/FN_ItemCalledOutAttributesDetailWidget_classes.hpp"
#include "SDK/FN_ItemCalledOutAttributesDetailWidget_parameters.hpp"
#include "SDK/FN_SquadSlotDetailsPanel_structs.hpp"
#include "SDK/FN_SquadSlotDetailsPanel_classes.hpp"
#include "SDK/FN_SquadSlotDetailsPanel_parameters.hpp"
#include "SDK/FN_ItemStackCounter_structs.hpp"
#include "SDK/FN_ItemStackCounter_classes.hpp"
#include "SDK/FN_ItemStackCounter_parameters.hpp"
#include "SDK/FN_SetBonus_TrapDurability_High_structs.hpp"
#include "SDK/FN_SetBonus_TrapDurability_High_classes.hpp"
#include "SDK/FN_SetBonus_TrapDurability_High_parameters.hpp"
#include "SDK/FN_GET_AfflictedParent_structs.hpp"
#include "SDK/FN_GET_AfflictedParent_classes.hpp"
#include "SDK/FN_GET_AfflictedParent_parameters.hpp"
#include "SDK/FN_GET_PeriodicDamageParent_structs.hpp"
#include "SDK/FN_GET_PeriodicDamageParent_classes.hpp"
#include "SDK/FN_GET_PeriodicDamageParent_parameters.hpp"
#include "SDK/FN_ItemDetailsStackCounter_structs.hpp"
#include "SDK/FN_ItemDetailsStackCounter_classes.hpp"
#include "SDK/FN_ItemDetailsStackCounter_parameters.hpp"
#include "SDK/FN_GET_PeriodicPhysicalDamage_structs.hpp"
#include "SDK/FN_GET_PeriodicPhysicalDamage_classes.hpp"
#include "SDK/FN_GET_PeriodicPhysicalDamage_parameters.hpp"
#include "SDK/FN_SmasherStrength00_structs.hpp"
#include "SDK/FN_SmasherStrength00_classes.hpp"
#include "SDK/FN_SmasherStrength00_parameters.hpp"
#include "SDK/FN_AnimNotify_PlayForceFeedback_structs.hpp"
#include "SDK/FN_AnimNotify_PlayForceFeedback_classes.hpp"
#include "SDK/FN_AnimNotify_PlayForceFeedback_parameters.hpp"
#include "SDK/FN_AnimNotifyState_DisableSteering_structs.hpp"
#include "SDK/FN_AnimNotifyState_DisableSteering_classes.hpp"
#include "SDK/FN_AnimNotifyState_DisableSteering_parameters.hpp"
#include "SDK/FN_SmasherStrength01_structs.hpp"
#include "SDK/FN_SmasherStrength01_classes.hpp"
#include "SDK/FN_SmasherStrength01_parameters.hpp"
#include "SDK/FN_GET_PeriodicEnergyDamage_structs.hpp"
#include "SDK/FN_GET_PeriodicEnergyDamage_classes.hpp"
#include "SDK/FN_GET_PeriodicEnergyDamage_parameters.hpp"
#include "SDK/FN_GE_OutsideSafeZoneDamage_structs.hpp"
#include "SDK/FN_GE_OutsideSafeZoneDamage_classes.hpp"
#include "SDK/FN_GE_OutsideSafeZoneDamage_parameters.hpp"
#include "SDK/FN_SmasherStrength02_structs.hpp"
#include "SDK/FN_SmasherStrength02_classes.hpp"
#include "SDK/FN_SmasherStrength02_parameters.hpp"
#include "SDK/FN_GE_Map_Athena_Vitality_Health_structs.hpp"
#include "SDK/FN_GE_Map_Athena_Vitality_Health_classes.hpp"
#include "SDK/FN_GE_Map_Athena_Vitality_Health_parameters.hpp"
#include "SDK/FN_GET_Stun_structs.hpp"
#include "SDK/FN_GET_Stun_classes.hpp"
#include "SDK/FN_GET_Stun_parameters.hpp"
#include "SDK/FN_SmasherStrength03_structs.hpp"
#include "SDK/FN_SmasherStrength03_classes.hpp"
#include "SDK/FN_SmasherStrength03_parameters.hpp"
#include "SDK/FN_GE_DBNOResurrectStun_structs.hpp"
#include "SDK/FN_GE_DBNOResurrectStun_classes.hpp"
#include "SDK/FN_GE_DBNOResurrectStun_parameters.hpp"
#include "SDK/FN_SmasherStrength04_structs.hpp"
#include "SDK/FN_SmasherStrength04_classes.hpp"
#include "SDK/FN_SmasherStrength04_parameters.hpp"
#include "SDK/FN_GET_TagContainer_structs.hpp"
#include "SDK/FN_GET_TagContainer_classes.hpp"
#include "SDK/FN_GET_TagContainer_parameters.hpp"
#include "SDK/FN_SmasherStrength05_structs.hpp"
#include "SDK/FN_SmasherStrength05_classes.hpp"
#include "SDK/FN_SmasherStrength05_parameters.hpp"
#include "SDK/FN_SmasherStrength06_structs.hpp"
#include "SDK/FN_SmasherStrength06_classes.hpp"
#include "SDK/FN_SmasherStrength06_parameters.hpp"
#include "SDK/FN_SmasherStrength07_structs.hpp"
#include "SDK/FN_SmasherStrength07_classes.hpp"
#include "SDK/FN_SmasherStrength07_parameters.hpp"
#include "SDK/FN_BP_VictoryDrone_structs.hpp"
#include "SDK/FN_BP_VictoryDrone_classes.hpp"
#include "SDK/FN_BP_VictoryDrone_parameters.hpp"
#include "SDK/FN_AnimNotifyState_TeleportFinished_structs.hpp"
#include "SDK/FN_AnimNotifyState_TeleportFinished_classes.hpp"
#include "SDK/FN_AnimNotifyState_TeleportFinished_parameters.hpp"
#include "SDK/FN_AnimNotifyState_DisablePawnRotation_structs.hpp"
#include "SDK/FN_AnimNotifyState_DisablePawnRotation_classes.hpp"
#include "SDK/FN_AnimNotifyState_DisablePawnRotation_parameters.hpp"
#include "SDK/FN_GE_TransferKnockback_structs.hpp"
#include "SDK/FN_GE_TransferKnockback_classes.hpp"
#include "SDK/FN_GE_TransferKnockback_parameters.hpp"
#include "SDK/FN_GE_DefaultPlayer_BuildTime_structs.hpp"
#include "SDK/FN_GE_DefaultPlayer_BuildTime_classes.hpp"
#include "SDK/FN_GE_DefaultPlayer_BuildTime_parameters.hpp"
#include "SDK/FN_GE_DefaultPlayer_RepairTime_structs.hpp"
#include "SDK/FN_GE_DefaultPlayer_RepairTime_classes.hpp"
#include "SDK/FN_GE_DefaultPlayer_RepairTime_parameters.hpp"
#include "SDK/FN_SmasherStrength08_structs.hpp"
#include "SDK/FN_SmasherStrength08_classes.hpp"
#include "SDK/FN_SmasherStrength08_parameters.hpp"
#include "SDK/FN_SmasherStrength09_structs.hpp"
#include "SDK/FN_SmasherStrength09_classes.hpp"
#include "SDK/FN_SmasherStrength09_parameters.hpp"
#include "SDK/FN_GE_DefaultPlayer_HarvestBuffHit_structs.hpp"
#include "SDK/FN_GE_DefaultPlayer_HarvestBuffHit_classes.hpp"
#include "SDK/FN_GE_DefaultPlayer_HarvestBuffHit_parameters.hpp"
#include "SDK/FN_GE_DefaultPlayer_HarvestBuff_T1_structs.hpp"
#include "SDK/FN_GE_DefaultPlayer_HarvestBuff_T1_classes.hpp"
#include "SDK/FN_GE_DefaultPlayer_HarvestBuff_T1_parameters.hpp"
#include "SDK/FN_GE_DefaultPlayer_HarvestBuff_T2_structs.hpp"
#include "SDK/FN_GE_DefaultPlayer_HarvestBuff_T2_classes.hpp"
#include "SDK/FN_GE_DefaultPlayer_HarvestBuff_T2_parameters.hpp"
#include "SDK/FN_SmasherStrength10_structs.hpp"
#include "SDK/FN_SmasherStrength10_classes.hpp"
#include "SDK/FN_SmasherStrength10_parameters.hpp"
#include "SDK/FN_TheaterMapViewer_structs.hpp"
#include "SDK/FN_TheaterMapViewer_classes.hpp"
#include "SDK/FN_TheaterMapViewer_parameters.hpp"
#include "SDK/FN_SoulSuckCameraShake_structs.hpp"
#include "SDK/FN_SoulSuckCameraShake_classes.hpp"
#include "SDK/FN_SoulSuckCameraShake_parameters.hpp"
#include "SDK/FN_ImpactNumbers_structs.hpp"
#include "SDK/FN_ImpactNumbers_classes.hpp"
#include "SDK/FN_ImpactNumbers_parameters.hpp"
#include "SDK/FN_GE_Generic_StaminaRegenLockout_structs.hpp"
#include "SDK/FN_GE_Generic_StaminaRegenLockout_classes.hpp"
#include "SDK/FN_GE_Generic_StaminaRegenLockout_parameters.hpp"
#include "SDK/FN_GE_Outlander_LaserFocusApplied_structs.hpp"
#include "SDK/FN_GE_Outlander_LaserFocusApplied_classes.hpp"
#include "SDK/FN_GE_Outlander_LaserFocusApplied_parameters.hpp"
#include "SDK/FN_WeakSpot_structs.hpp"
#include "SDK/FN_WeakSpot_classes.hpp"
#include "SDK/FN_WeakSpot_parameters.hpp"
#include "SDK/FN_GE_Outlander_LaserFocusDispel_structs.hpp"
#include "SDK/FN_GE_Outlander_LaserFocusDispel_classes.hpp"
#include "SDK/FN_GE_Outlander_LaserFocusDispel_parameters.hpp"
#include "SDK/FN_GE_ImpactImmunity_structs.hpp"
#include "SDK/FN_GE_ImpactImmunity_classes.hpp"
#include "SDK/FN_GE_ImpactImmunity_parameters.hpp"
#include "SDK/FN_GE_RestoreControlResistance_structs.hpp"
#include "SDK/FN_GE_RestoreControlResistance_classes.hpp"
#include "SDK/FN_GE_RestoreControlResistance_parameters.hpp"
#include "SDK/FN_GE_KnockbackActive_structs.hpp"
#include "SDK/FN_GE_KnockbackActive_classes.hpp"
#include "SDK/FN_GE_KnockbackActive_parameters.hpp"
#include "SDK/FN_GE_StunActive_structs.hpp"
#include "SDK/FN_GE_StunActive_classes.hpp"
#include "SDK/FN_GE_StunActive_parameters.hpp"
#include "SDK/FN_GE_TrapArmGeneric_structs.hpp"
#include "SDK/FN_GE_TrapArmGeneric_classes.hpp"
#include "SDK/FN_GE_TrapArmGeneric_parameters.hpp"
#include "SDK/FN_B_Pickups_structs.hpp"
#include "SDK/FN_B_Pickups_classes.hpp"
#include "SDK/FN_B_Pickups_parameters.hpp"
#include "SDK/FN_GE_Athena_DBNO_Bleed_structs.hpp"
#include "SDK/FN_GE_Athena_DBNO_Bleed_classes.hpp"
#include "SDK/FN_GE_Athena_DBNO_Bleed_parameters.hpp"
#include "SDK/FN_GAB_AthenaDBNO_structs.hpp"
#include "SDK/FN_GAB_AthenaDBNO_classes.hpp"
#include "SDK/FN_GAB_AthenaDBNO_parameters.hpp"
#include "SDK/FN_GAB_AthenaDBNORevive_structs.hpp"
#include "SDK/FN_GAB_AthenaDBNORevive_classes.hpp"
#include "SDK/FN_GAB_AthenaDBNORevive_parameters.hpp"
#include "SDK/FN_Get_DirectDamageParent_structs.hpp"
#include "SDK/FN_Get_DirectDamageParent_classes.hpp"
#include "SDK/FN_Get_DirectDamageParent_parameters.hpp"
#include "SDK/FN_ItemDisplayMode_structs.hpp"
#include "SDK/FN_ItemDisplayMode_classes.hpp"
#include "SDK/FN_ItemDisplayMode_parameters.hpp"
#include "SDK/FN_ItemTextureSet_structs.hpp"
#include "SDK/FN_ItemTextureSet_classes.hpp"
#include "SDK/FN_ItemTextureSet_parameters.hpp"
#include "SDK/FN_GAB_PlayerDBNO_structs.hpp"
#include "SDK/FN_GAB_PlayerDBNO_classes.hpp"
#include "SDK/FN_GAB_PlayerDBNO_parameters.hpp"
#include "SDK/FN_GAB_PlayerDBNOResurrect_structs.hpp"
#include "SDK/FN_GAB_PlayerDBNOResurrect_classes.hpp"
#include "SDK/FN_GAB_PlayerDBNOResurrect_parameters.hpp"
#include "SDK/FN_GET_DirectEnvironmentDamage_structs.hpp"
#include "SDK/FN_GET_DirectEnvironmentDamage_classes.hpp"
#include "SDK/FN_GET_DirectEnvironmentDamage_parameters.hpp"
#include "SDK/FN_SquadSlotItemDetailsHostPanel_structs.hpp"
#include "SDK/FN_SquadSlotItemDetailsHostPanel_classes.hpp"
#include "SDK/FN_SquadSlotItemDetailsHostPanel_parameters.hpp"
#include "SDK/FN_GET_FallingDamage_structs.hpp"
#include "SDK/FN_GET_FallingDamage_classes.hpp"
#include "SDK/FN_GET_FallingDamage_parameters.hpp"
#include "SDK/FN_RewardType_structs.hpp"
#include "SDK/FN_RewardType_classes.hpp"
#include "SDK/FN_RewardType_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionVehicleTileItemWidget_structs.hpp"
#include "SDK/FN_BP_FortExpeditionVehicleTileItemWidget_classes.hpp"
#include "SDK/FN_BP_FortExpeditionVehicleTileItemWidget_parameters.hpp"
#include "SDK/FN_GA_Default_KilledEnemy_structs.hpp"
#include "SDK/FN_GA_Default_KilledEnemy_classes.hpp"
#include "SDK/FN_GA_Default_KilledEnemy_parameters.hpp"
#include "SDK/FN_GAB_GenericApplyKnockback_structs.hpp"
#include "SDK/FN_GAB_GenericApplyKnockback_classes.hpp"
#include "SDK/FN_GAB_GenericApplyKnockback_parameters.hpp"
#include "SDK/FN_GA_DefaultPlayer_ApplyKnockback_structs.hpp"
#include "SDK/FN_GA_DefaultPlayer_ApplyKnockback_classes.hpp"
#include "SDK/FN_GA_DefaultPlayer_ApplyKnockback_parameters.hpp"
#include "SDK/FN_GA_DefaultPlayer_BuildingCreated_structs.hpp"
#include "SDK/FN_GA_DefaultPlayer_BuildingCreated_classes.hpp"
#include "SDK/FN_GA_DefaultPlayer_BuildingCreated_parameters.hpp"
#include "SDK/FN_GA_DefaultPlayer_BuildingRepaired_structs.hpp"
#include "SDK/FN_GA_DefaultPlayer_BuildingRepaired_classes.hpp"
#include "SDK/FN_GA_DefaultPlayer_BuildingRepaired_parameters.hpp"
#include "SDK/FN_GA_DefaultPlayer_Consumable_structs.hpp"
#include "SDK/FN_GA_DefaultPlayer_Consumable_classes.hpp"
#include "SDK/FN_GA_DefaultPlayer_Consumable_parameters.hpp"
#include "SDK/FN_GAB_GenericDeath_structs.hpp"
#include "SDK/FN_GAB_GenericDeath_classes.hpp"
#include "SDK/FN_GAB_GenericDeath_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Gray50_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Gray50_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-B-Gray50_parameters.hpp"
#include "SDK/FN_GA_DefaultPlayer_Death_structs.hpp"
#include "SDK/FN_GA_DefaultPlayer_Death_classes.hpp"
#include "SDK/FN_GA_DefaultPlayer_Death_parameters.hpp"
#include "SDK/FN_GET_FatalDamage_structs.hpp"
#include "SDK/FN_GET_FatalDamage_classes.hpp"
#include "SDK/FN_GET_FatalDamage_parameters.hpp"
#include "SDK/FN_PopupFrame_structs.hpp"
#include "SDK/FN_PopupFrame_classes.hpp"
#include "SDK/FN_PopupFrame_parameters.hpp"
#include "SDK/FN_GA_DefaultPlayer_HarvestBuff_structs.hpp"
#include "SDK/FN_GA_DefaultPlayer_HarvestBuff_classes.hpp"
#include "SDK/FN_GA_DefaultPlayer_HarvestBuff_parameters.hpp"
#include "SDK/FN_GA_DefaultPlayer_HarvestBuffSwitch_structs.hpp"
#include "SDK/FN_GA_DefaultPlayer_HarvestBuffSwitch_classes.hpp"
#include "SDK/FN_GA_DefaultPlayer_HarvestBuffSwitch_parameters.hpp"
#include "SDK/FN_GA_DefaultPlayer_InteractSearch_structs.hpp"
#include "SDK/FN_GA_DefaultPlayer_InteractSearch_classes.hpp"
#include "SDK/FN_GA_DefaultPlayer_InteractSearch_parameters.hpp"
#include "SDK/FN_GA_DefaultPlayer_InteractUse_structs.hpp"
#include "SDK/FN_GA_DefaultPlayer_InteractUse_classes.hpp"
#include "SDK/FN_GA_DefaultPlayer_InteractUse_parameters.hpp"
#include "SDK/FN_GE_HealthRegen_structs.hpp"
#include "SDK/FN_GE_HealthRegen_classes.hpp"
#include "SDK/FN_GE_HealthRegen_parameters.hpp"
#include "SDK/FN_GE_SharedPlayerTrapStatTransfer_structs.hpp"
#include "SDK/FN_GE_SharedPlayerTrapStatTransfer_classes.hpp"
#include "SDK/FN_GE_SharedPlayerTrapStatTransfer_parameters.hpp"
#include "SDK/FN_GAB_GenericStunned_structs.hpp"
#include "SDK/FN_GAB_GenericStunned_classes.hpp"
#include "SDK/FN_GAB_GenericStunned_parameters.hpp"
#include "SDK/FN_NormalBangWrapper_structs.hpp"
#include "SDK/FN_NormalBangWrapper_classes.hpp"
#include "SDK/FN_NormalBangWrapper_parameters.hpp"
#include "SDK/FN_GA_DefaultPlayer_Stunned_structs.hpp"
#include "SDK/FN_GA_DefaultPlayer_Stunned_classes.hpp"
#include "SDK/FN_GA_DefaultPlayer_Stunned_parameters.hpp"
#include "SDK/FN_SupportHeroSquadBonusesDetailWidget_structs.hpp"
#include "SDK/FN_SupportHeroSquadBonusesDetailWidget_classes.hpp"
#include "SDK/FN_SupportHeroSquadBonusesDetailWidget_parameters.hpp"
#include "SDK/FN_GA_TrapBuildGeneric_structs.hpp"
#include "SDK/FN_GA_TrapBuildGeneric_classes.hpp"
#include "SDK/FN_GA_TrapBuildGeneric_parameters.hpp"
#include "SDK/FN_FrontEndRewardWrapperWidget_structs.hpp"
#include "SDK/FN_FrontEndRewardWrapperWidget_classes.hpp"
#include "SDK/FN_FrontEndRewardWrapperWidget_parameters.hpp"
#include "SDK/FN_ItemTextureStylesheet_structs.hpp"
#include "SDK/FN_ItemTextureStylesheet_classes.hpp"
#include "SDK/FN_ItemTextureStylesheet_parameters.hpp"
#include "SDK/FN_RewardsWidget_structs.hpp"
#include "SDK/FN_RewardsWidget_classes.hpp"
#include "SDK/FN_RewardsWidget_parameters.hpp"
#include "SDK/FN_GE_SquadStatBonus_structs.hpp"
#include "SDK/FN_GE_SquadStatBonus_classes.hpp"
#include "SDK/FN_GE_SquadStatBonus_parameters.hpp"
#include "SDK/FN_LegacyItem_DO_NOT_USE_structs.hpp"
#include "SDK/FN_LegacyItem_DO_NOT_USE_classes.hpp"
#include "SDK/FN_LegacyItem_DO_NOT_USE_parameters.hpp"
#include "SDK/FN_Item_TierBadge_structs.hpp"
#include "SDK/FN_Item_TierBadge_classes.hpp"
#include "SDK/FN_Item_TierBadge_parameters.hpp"
#include "SDK/FN_HomeScreenQuestRewardItem_structs.hpp"
#include "SDK/FN_HomeScreenQuestRewardItem_classes.hpp"
#include "SDK/FN_HomeScreenQuestRewardItem_parameters.hpp"
#include "SDK/FN_ItemCooldownMeter_structs.hpp"
#include "SDK/FN_ItemCooldownMeter_classes.hpp"
#include "SDK/FN_ItemCooldownMeter_parameters.hpp"
#include "SDK/FN_LegacyButtonStyle-StarRating_structs.hpp"
#include "SDK/FN_LegacyButtonStyle-StarRating_classes.hpp"
#include "SDK/FN_LegacyButtonStyle-StarRating_parameters.hpp"
#include "SDK/FN_Announce_Gen_Quest_Conversation_structs.hpp"
#include "SDK/FN_Announce_Gen_Quest_Conversation_classes.hpp"
#include "SDK/FN_Announce_Gen_Quest_Conversation_parameters.hpp"
#include "SDK/FN_PlayerTrapArmSpeedModCalculation_structs.hpp"
#include "SDK/FN_PlayerTrapArmSpeedModCalculation_classes.hpp"
#include "SDK/FN_PlayerTrapArmSpeedModCalculation_parameters.hpp"
#include "SDK/FN_PlayerTrapBaseDamageModCalculation_structs.hpp"
#include "SDK/FN_PlayerTrapBaseDamageModCalculation_classes.hpp"
#include "SDK/FN_PlayerTrapBaseDamageModCalculation_parameters.hpp"
#include "SDK/FN_PlayerTrapDiceCritChanceModCalculation_structs.hpp"
#include "SDK/FN_PlayerTrapDiceCritChanceModCalculation_classes.hpp"
#include "SDK/FN_PlayerTrapDiceCritChanceModCalculation_parameters.hpp"
#include "SDK/FN_ItemDragIcon_structs.hpp"
#include "SDK/FN_ItemDragIcon_classes.hpp"
#include "SDK/FN_ItemDragIcon_parameters.hpp"
#include "SDK/FN_RewardsListWidget_structs.hpp"
#include "SDK/FN_RewardsListWidget_classes.hpp"
#include "SDK/FN_RewardsListWidget_parameters.hpp"
#include "SDK/FN_PlayerTrapDiceCritMultiplierModCalculation_structs.hpp"
#include "SDK/FN_PlayerTrapDiceCritMultiplierModCalculation_classes.hpp"
#include "SDK/FN_PlayerTrapDiceCritMultiplierModCalculation_parameters.hpp"
#include "SDK/FN_PlayerTrapHealingSourceModCalculation_structs.hpp"
#include "SDK/FN_PlayerTrapHealingSourceModCalculation_classes.hpp"
#include "SDK/FN_PlayerTrapHealingSourceModCalculation_parameters.hpp"
#include "SDK/FN_PlayerTrapMaxDurabilityModCalculation_structs.hpp"
#include "SDK/FN_PlayerTrapMaxDurabilityModCalculation_classes.hpp"
#include "SDK/FN_PlayerTrapMaxDurabilityModCalculation_parameters.hpp"
#include "SDK/FN_PlayerTrapReloadTimeModCalculation_structs.hpp"
#include "SDK/FN_PlayerTrapReloadTimeModCalculation_classes.hpp"
#include "SDK/FN_PlayerTrapReloadTimeModCalculation_parameters.hpp"
#include "SDK/FN_RewardsChoiceButtonWidget_structs.hpp"
#include "SDK/FN_RewardsChoiceButtonWidget_classes.hpp"
#include "SDK/FN_RewardsChoiceButtonWidget_parameters.hpp"
#include "SDK/FN_Hero_Management_HUDWrapper_structs.hpp"
#include "SDK/FN_Hero_Management_HUDWrapper_classes.hpp"
#include "SDK/FN_Hero_Management_HUDWrapper_parameters.hpp"
#include "SDK/FN_StickyNotification_structs.hpp"
#include "SDK/FN_StickyNotification_classes.hpp"
#include "SDK/FN_StickyNotification_parameters.hpp"
#include "SDK/FN_PlayerChoiceWidget_structs.hpp"
#include "SDK/FN_PlayerChoiceWidget_classes.hpp"
#include "SDK/FN_PlayerChoiceWidget_parameters.hpp"
#include "SDK/FN_BasicStrokeBox_structs.hpp"
#include "SDK/FN_BasicStrokeBox_classes.hpp"
#include "SDK/FN_BasicStrokeBox_parameters.hpp"
#include "SDK/FN_Border-MainL_structs.hpp"
#include "SDK/FN_Border-MainL_classes.hpp"
#include "SDK/FN_Border-MainL_parameters.hpp"
#include "SDK/FN_LegacyButtonStyle-Item_structs.hpp"
#include "SDK/FN_LegacyButtonStyle-Item_classes.hpp"
#include "SDK/FN_LegacyButtonStyle-Item_parameters.hpp"
#include "SDK/FN_MissionDetailsModifierRow_structs.hpp"
#include "SDK/FN_MissionDetailsModifierRow_classes.hpp"
#include "SDK/FN_MissionDetailsModifierRow_parameters.hpp"
#include "SDK/FN_LegacyButtonIconText_structs.hpp"
#include "SDK/FN_LegacyButtonIconText_classes.hpp"
#include "SDK/FN_LegacyButtonIconText_parameters.hpp"
#include "SDK/FN_PlayerChoiceButtonWidget_structs.hpp"
#include "SDK/FN_PlayerChoiceButtonWidget_classes.hpp"
#include "SDK/FN_PlayerChoiceButtonWidget_parameters.hpp"
#include "SDK/FN_LegacyButtonStyle-Close_structs.hpp"
#include "SDK/FN_LegacyButtonStyle-Close_classes.hpp"
#include "SDK/FN_LegacyButtonStyle-Close_parameters.hpp"
#include "SDK/FN_LegacyButtonStyle-Default_structs.hpp"
#include "SDK/FN_LegacyButtonStyle-Default_classes.hpp"
#include "SDK/FN_LegacyButtonStyle-Default_parameters.hpp"
#include "SDK/FN_GAB_Emote_Generic_structs.hpp"
#include "SDK/FN_GAB_Emote_Generic_classes.hpp"
#include "SDK/FN_GAB_Emote_Generic_parameters.hpp"
#include "SDK/FN_BasicNotification_structs.hpp"
#include "SDK/FN_BasicNotification_classes.hpp"
#include "SDK/FN_BasicNotification_parameters.hpp"
#include "SDK/FN_TwitchNotification_structs.hpp"
#include "SDK/FN_TwitchNotification_classes.hpp"
#include "SDK/FN_TwitchNotification_parameters.hpp"
#include "SDK/FN_Announce_ZoneModifiers_structs.hpp"
#include "SDK/FN_Announce_ZoneModifiers_classes.hpp"
#include "SDK/FN_Announce_ZoneModifiers_parameters.hpp"
#include "SDK/FN_PBWA_BG_DoorC_structs.hpp"
#include "SDK/FN_PBWA_BG_DoorC_classes.hpp"
#include "SDK/FN_PBWA_BG_DoorC_parameters.hpp"
#include "SDK/FN_PBWA_BG_DoorS_structs.hpp"
#include "SDK/FN_PBWA_BG_DoorS_classes.hpp"
#include "SDK/FN_PBWA_BG_DoorS_parameters.hpp"
#include "SDK/FN_PBWA_BG_DoorSide_structs.hpp"
#include "SDK/FN_PBWA_BG_DoorSide_classes.hpp"
#include "SDK/FN_PBWA_BG_DoorSide_parameters.hpp"
#include "SDK/FN_PBWA_BG_Solid_structs.hpp"
#include "SDK/FN_PBWA_BG_Solid_classes.hpp"
#include "SDK/FN_PBWA_BG_Solid_parameters.hpp"
#include "SDK/FN_LegacyButtonStyle-Transparent_structs.hpp"
#include "SDK/FN_LegacyButtonStyle-Transparent_classes.hpp"
#include "SDK/FN_LegacyButtonStyle-Transparent_parameters.hpp"
#include "SDK/FN_PBWA_BG_WindowC_structs.hpp"
#include "SDK/FN_PBWA_BG_WindowC_classes.hpp"
#include "SDK/FN_PBWA_BG_WindowC_parameters.hpp"
#include "SDK/FN_Border-MainL-Black_structs.hpp"
#include "SDK/FN_Border-MainL-Black_classes.hpp"
#include "SDK/FN_Border-MainL-Black_parameters.hpp"
#include "SDK/FN_PBWA_BG_Windows_structs.hpp"
#include "SDK/FN_PBWA_BG_Windows_classes.hpp"
#include "SDK/FN_PBWA_BG_Windows_parameters.hpp"
#include "SDK/FN_PBWA_BG_WindowSide_structs.hpp"
#include "SDK/FN_PBWA_BG_WindowSide_classes.hpp"
#include "SDK/FN_PBWA_BG_WindowSide_parameters.hpp"
#include "SDK/FN_PBWA_M1_DoorC_structs.hpp"
#include "SDK/FN_PBWA_M1_DoorC_classes.hpp"
#include "SDK/FN_PBWA_M1_DoorC_parameters.hpp"
#include "SDK/FN_PBWA_M1_DoorS_structs.hpp"
#include "SDK/FN_PBWA_M1_DoorS_classes.hpp"
#include "SDK/FN_PBWA_M1_DoorS_parameters.hpp"
#include "SDK/FN_PBWA_M1_DoorSide_structs.hpp"
#include "SDK/FN_PBWA_M1_DoorSide_classes.hpp"
#include "SDK/FN_PBWA_M1_DoorSide_parameters.hpp"
#include "SDK/FN_PBWA_M1_Solid_structs.hpp"
#include "SDK/FN_PBWA_M1_Solid_classes.hpp"
#include "SDK/FN_PBWA_M1_Solid_parameters.hpp"
#include "SDK/FN_WeaponTooltipStatType_structs.hpp"
#include "SDK/FN_WeaponTooltipStatType_classes.hpp"
#include "SDK/FN_WeaponTooltipStatType_parameters.hpp"
#include "SDK/FN_PBWA_M1_WindowC_structs.hpp"
#include "SDK/FN_PBWA_M1_WindowC_classes.hpp"
#include "SDK/FN_PBWA_M1_WindowC_parameters.hpp"
#include "SDK/FN_PBWA_M1_Windows_structs.hpp"
#include "SDK/FN_PBWA_M1_Windows_classes.hpp"
#include "SDK/FN_PBWA_M1_Windows_parameters.hpp"
#include "SDK/FN_PBWA_M1_WindowSide_structs.hpp"
#include "SDK/FN_PBWA_M1_WindowSide_classes.hpp"
#include "SDK/FN_PBWA_M1_WindowSide_parameters.hpp"
#include "SDK/FN_PBWA_S1_DoorC_structs.hpp"
#include "SDK/FN_PBWA_S1_DoorC_classes.hpp"
#include "SDK/FN_PBWA_S1_DoorC_parameters.hpp"
#include "SDK/FN_PBWA_S1_DoorS_structs.hpp"
#include "SDK/FN_PBWA_S1_DoorS_classes.hpp"
#include "SDK/FN_PBWA_S1_DoorS_parameters.hpp"
#include "SDK/FN_MissionDetailsModifierList_structs.hpp"
#include "SDK/FN_MissionDetailsModifierList_classes.hpp"
#include "SDK/FN_MissionDetailsModifierList_parameters.hpp"
#include "SDK/FN_PBWA_S1_DoorSide_structs.hpp"
#include "SDK/FN_PBWA_S1_DoorSide_classes.hpp"
#include "SDK/FN_PBWA_S1_DoorSide_parameters.hpp"
#include "SDK/FN_PBWA_S1_Solid_structs.hpp"
#include "SDK/FN_PBWA_S1_Solid_classes.hpp"
#include "SDK/FN_PBWA_S1_Solid_parameters.hpp"
#include "SDK/FN_Border-ItemInfo-Header_structs.hpp"
#include "SDK/FN_Border-ItemInfo-Header_classes.hpp"
#include "SDK/FN_Border-ItemInfo-Header_parameters.hpp"
#include "SDK/FN_PBWA_S1_Windows_structs.hpp"
#include "SDK/FN_PBWA_S1_Windows_classes.hpp"
#include "SDK/FN_PBWA_S1_Windows_parameters.hpp"
#include "SDK/FN_FortUITheme_structs.hpp"
#include "SDK/FN_FortUITheme_classes.hpp"
#include "SDK/FN_FortUITheme_parameters.hpp"
#include "SDK/FN_Border-ItemInfoHeader_structs.hpp"
#include "SDK/FN_Border-ItemInfoHeader_classes.hpp"
#include "SDK/FN_Border-ItemInfoHeader_parameters.hpp"
#include "SDK/FN_VehicleObject_structs.hpp"
#include "SDK/FN_VehicleObject_classes.hpp"
#include "SDK/FN_VehicleObject_parameters.hpp"
#include "SDK/FN_PBWA_S1_WindowsC_structs.hpp"
#include "SDK/FN_PBWA_S1_WindowsC_classes.hpp"
#include "SDK/FN_PBWA_S1_WindowsC_parameters.hpp"
#include "SDK/FN_Border-CompareHeader_structs.hpp"
#include "SDK/FN_Border-CompareHeader_classes.hpp"
#include "SDK/FN_Border-CompareHeader_parameters.hpp"
#include "SDK/FN_ColorStylesheet_structs.hpp"
#include "SDK/FN_ColorStylesheet_classes.hpp"
#include "SDK/FN_ColorStylesheet_parameters.hpp"
#include "SDK/FN_Border_DarkBlue_VGrad_structs.hpp"
#include "SDK/FN_Border_DarkBlue_VGrad_classes.hpp"
#include "SDK/FN_Border_DarkBlue_VGrad_parameters.hpp"
#include "SDK/FN_LegacyButtonStyle-Emphasis_structs.hpp"
#include "SDK/FN_LegacyButtonStyle-Emphasis_classes.hpp"
#include "SDK/FN_LegacyButtonStyle-Emphasis_parameters.hpp"
#include "SDK/FN_Border_StatFocus_structs.hpp"
#include "SDK/FN_Border_StatFocus_classes.hpp"
#include "SDK/FN_Border_StatFocus_parameters.hpp"
#include "SDK/FN_Border_StatRow_DarkBlue_structs.hpp"
#include "SDK/FN_Border_StatRow_DarkBlue_classes.hpp"
#include "SDK/FN_Border_StatRow_DarkBlue_parameters.hpp"
#include "SDK/FN_PBWA_S1_WindowsSide_structs.hpp"
#include "SDK/FN_PBWA_S1_WindowsSide_classes.hpp"
#include "SDK/FN_PBWA_S1_WindowsSide_parameters.hpp"
#include "SDK/FN_Border_StatRow_DeltaPositive_structs.hpp"
#include "SDK/FN_Border_StatRow_DeltaPositive_classes.hpp"
#include "SDK/FN_Border_StatRow_DeltaPositive_parameters.hpp"
#include "SDK/FN_Border_StatRow_Locked_structs.hpp"
#include "SDK/FN_Border_StatRow_Locked_classes.hpp"
#include "SDK/FN_Border_StatRow_Locked_parameters.hpp"
#include "SDK/FN_Border_StatRow_Unlocked_structs.hpp"
#include "SDK/FN_Border_StatRow_Unlocked_classes.hpp"
#include "SDK/FN_Border_StatRow_Unlocked_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-S_parameters.hpp"
#include "SDK/FN_Border_StatRow_DeltaNegative_structs.hpp"
#include "SDK/FN_Border_StatRow_DeltaNegative_classes.hpp"
#include "SDK/FN_Border_StatRow_DeltaNegative_parameters.hpp"
#include "SDK/FN_PBWA_W1_DoorC_structs.hpp"
#include "SDK/FN_PBWA_W1_DoorC_classes.hpp"
#include "SDK/FN_PBWA_W1_DoorC_parameters.hpp"
#include "SDK/FN_PBWA_W1_DoorS_structs.hpp"
#include "SDK/FN_PBWA_W1_DoorS_classes.hpp"
#include "SDK/FN_PBWA_W1_DoorS_parameters.hpp"
#include "SDK/FN_PBWA_W1_DoorSide_structs.hpp"
#include "SDK/FN_PBWA_W1_DoorSide_classes.hpp"
#include "SDK/FN_PBWA_W1_DoorSide_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XXS-B-S_structs.hpp"
#include "SDK/FN_TextStyle-Base-XXS-B-S_classes.hpp"
#include "SDK/FN_TextStyle-Base-XXS-B-S_parameters.hpp"
#include "SDK/FN_PBWA_W1_Solid_structs.hpp"
#include "SDK/FN_PBWA_W1_Solid_classes.hpp"
#include "SDK/FN_PBWA_W1_Solid_parameters.hpp"
#include "SDK/FN_PBWA_W1_WindowC_structs.hpp"
#include "SDK/FN_PBWA_W1_WindowC_classes.hpp"
#include "SDK/FN_PBWA_W1_WindowC_parameters.hpp"
#include "SDK/FN_ButtonStyle-MediumTransparentNoCues_structs.hpp"
#include "SDK/FN_ButtonStyle-MediumTransparentNoCues_classes.hpp"
#include "SDK/FN_ButtonStyle-MediumTransparentNoCues_parameters.hpp"
#include "SDK/FN_TextStyle-Header-S_structs.hpp"
#include "SDK/FN_TextStyle-Header-S_classes.hpp"
#include "SDK/FN_TextStyle-Header-S_parameters.hpp"
#include "SDK/FN_ItemCountOverCost_structs.hpp"
#include "SDK/FN_ItemCountOverCost_classes.hpp"
#include "SDK/FN_ItemCountOverCost_parameters.hpp"
#include "SDK/FN_PerkDivider_structs.hpp"
#include "SDK/FN_PerkDivider_classes.hpp"
#include "SDK/FN_PerkDivider_parameters.hpp"
#include "SDK/FN_HeroNotificationHandler_structs.hpp"
#include "SDK/FN_HeroNotificationHandler_classes.hpp"
#include "SDK/FN_HeroNotificationHandler_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-Black_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-Black_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-Black_parameters.hpp"
#include "SDK/FN_Border-Empty_structs.hpp"
#include "SDK/FN_Border-Empty_classes.hpp"
#include "SDK/FN_Border-Empty_parameters.hpp"
#include "SDK/FN_Border-ItemInfo-Blank-DkBlue_structs.hpp"
#include "SDK/FN_Border-ItemInfo-Blank-DkBlue_classes.hpp"
#include "SDK/FN_Border-ItemInfo-Blank-DkBlue_parameters.hpp"
#include "SDK/FN_Border-ItemInfo-Blank_structs.hpp"
#include "SDK/FN_Border-ItemInfo-Blank_classes.hpp"
#include "SDK/FN_Border-ItemInfo-Blank_parameters.hpp"
#include "SDK/FN_Border-ManageListHeader_structs.hpp"
#include "SDK/FN_Border-ManageListHeader_classes.hpp"
#include "SDK/FN_Border-ManageListHeader_parameters.hpp"
#include "SDK/FN_Border-ProgressBar-Collections_structs.hpp"
#include "SDK/FN_Border-ProgressBar-Collections_classes.hpp"
#include "SDK/FN_Border-ProgressBar-Collections_parameters.hpp"
#include "SDK/FN_Border-SolidBG-DkBlue_structs.hpp"
#include "SDK/FN_Border-SolidBG-DkBlue_classes.hpp"
#include "SDK/FN_Border-SolidBG-DkBlue_parameters.hpp"
#include "SDK/FN_Border-TabM-Solid-30pc_structs.hpp"
#include "SDK/FN_Border-TabM-Solid-30pc_classes.hpp"
#include "SDK/FN_Border-TabM-Solid-30pc_parameters.hpp"
#include "SDK/FN_Border-TabM-Solid-White100pc_structs.hpp"
#include "SDK/FN_Border-TabM-Solid-White100pc_classes.hpp"
#include "SDK/FN_Border-TabM-Solid-White100pc_parameters.hpp"
#include "SDK/FN_Border-TabM-Solid-10pc_structs.hpp"
#include "SDK/FN_Border-TabM-Solid-10pc_classes.hpp"
#include "SDK/FN_Border-TabM-Solid-10pc_parameters.hpp"
#include "SDK/FN_Border_DarkNavy_VGrad_structs.hpp"
#include "SDK/FN_Border_DarkNavy_VGrad_classes.hpp"
#include "SDK/FN_Border_DarkNavy_VGrad_parameters.hpp"
#include "SDK/FN_Border_Navy_VGrad_structs.hpp"
#include "SDK/FN_Border_Navy_VGrad_classes.hpp"
#include "SDK/FN_Border_Navy_VGrad_parameters.hpp"
#include "SDK/FN_Border_Solid_DkBlue_structs.hpp"
#include "SDK/FN_Border_Solid_DkBlue_classes.hpp"
#include "SDK/FN_Border_Solid_DkBlue_parameters.hpp"
#include "SDK/FN_PBWA_W1_Windows_structs.hpp"
#include "SDK/FN_PBWA_W1_Windows_classes.hpp"
#include "SDK/FN_PBWA_W1_Windows_parameters.hpp"
#include "SDK/FN_ButtonStyle-Solid-SquareM_structs.hpp"
#include "SDK/FN_ButtonStyle-Solid-SquareM_classes.hpp"
#include "SDK/FN_ButtonStyle-Solid-SquareM_parameters.hpp"
#include "SDK/FN_ButtonStyle-Tab-Manage_structs.hpp"
#include "SDK/FN_ButtonStyle-Tab-Manage_classes.hpp"
#include "SDK/FN_ButtonStyle-Tab-Manage_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-Yellow_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-Yellow_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-Yellow_parameters.hpp"
#include "SDK/FN_TextStyle-Base-S-Picasso_structs.hpp"
#include "SDK/FN_TextStyle-Base-S-Picasso_classes.hpp"
#include "SDK/FN_TextStyle-Base-S-Picasso_parameters.hpp"
#include "SDK/FN_TextStyle-Base-M-B-Yellow_structs.hpp"
#include "SDK/FN_TextStyle-Base-M-B-Yellow_classes.hpp"
#include "SDK/FN_TextStyle-Base-M-B-Yellow_parameters.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Blue_structs.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Blue_classes.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Blue_parameters.hpp"
#include "SDK/FN_PBWA_W1_WindowSide_structs.hpp"
#include "SDK/FN_PBWA_W1_WindowSide_classes.hpp"
#include "SDK/FN_PBWA_W1_WindowSide_parameters.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Black_structs.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Black_classes.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Black_parameters.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Blue50_structs.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Blue50_classes.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Blue50_parameters.hpp"
#include "SDK/FN_Announcement_ZoneModifiers_structs.hpp"
#include "SDK/FN_Announcement_ZoneModifiers_classes.hpp"
#include "SDK/FN_Announcement_ZoneModifiers_parameters.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Red_structs.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Red_classes.hpp"
#include "SDK/FN_TextStyle-Base-M-B_Red_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-I-40pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-I-40pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-I-40pc_parameters.hpp"
#include "SDK/FN_TextStyle-Header-XL_structs.hpp"
#include "SDK/FN_TextStyle-Header-XL_classes.hpp"
#include "SDK/FN_TextStyle-Header-XL_parameters.hpp"
#include "SDK/FN_TextStyle-Header-HomeBasePower_structs.hpp"
#include "SDK/FN_TextStyle-Header-HomeBasePower_classes.hpp"
#include "SDK/FN_TextStyle-Header-HomeBasePower_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-70pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-70pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-70pc_parameters.hpp"
#include "SDK/FN_TextStyle-Base-M-B-DarkGreen_structs.hpp"
#include "SDK/FN_TextStyle-Base-M-B-DarkGreen_classes.hpp"
#include "SDK/FN_TextStyle-Base-M-B-DarkGreen_parameters.hpp"
#include "SDK/FN_AlterationsWidget_structs.hpp"
#include "SDK/FN_AlterationsWidget_classes.hpp"
#include "SDK/FN_AlterationsWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Red_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Red_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Red_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-StaminaBarMax_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-StaminaBarMax_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-HUD-StaminaBarMax_parameters.hpp"
#include "SDK/FN_Border-Slot-Empty_structs.hpp"
#include "SDK/FN_Border-Slot-Empty_classes.hpp"
#include "SDK/FN_Border-Slot-Empty_parameters.hpp"
#include "SDK/FN_IconAndNameWidget_structs.hpp"
#include "SDK/FN_IconAndNameWidget_classes.hpp"
#include "SDK/FN_IconAndNameWidget_parameters.hpp"
#include "SDK/FN_ButtonStyle-Slot-PeopleM_structs.hpp"
#include "SDK/FN_ButtonStyle-Slot-PeopleM_classes.hpp"
#include "SDK/FN_ButtonStyle-Slot-PeopleM_parameters.hpp"
#include "SDK/FN_ButtonStyle-Slot-PeopleL-XL_structs.hpp"
#include "SDK/FN_ButtonStyle-Slot-PeopleL-XL_classes.hpp"
#include "SDK/FN_ButtonStyle-Slot-PeopleL-XL_parameters.hpp"
#include "SDK/FN_ButtonStyle-Slot-PeopleS-XS_structs.hpp"
#include "SDK/FN_ButtonStyle-Slot-PeopleS-XS_classes.hpp"
#include "SDK/FN_ButtonStyle-Slot-PeopleS-XS_parameters.hpp"
#include "SDK/FN_ItemAlterationsListDetailWidget_structs.hpp"
#include "SDK/FN_ItemAlterationsListDetailWidget_classes.hpp"
#include "SDK/FN_ItemAlterationsListDetailWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-Red-70pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-Red-70pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-Red-70pc_parameters.hpp"
#include "SDK/FN_Border-MainModal_structs.hpp"
#include "SDK/FN_Border-MainModal_classes.hpp"
#include "SDK/FN_Border-MainModal_parameters.hpp"
#include "SDK/FN_RewardsListEntry_structs.hpp"
#include "SDK/FN_RewardsListEntry_classes.hpp"
#include "SDK/FN_RewardsListEntry_parameters.hpp"
#include "SDK/FN_Border-ModalHeader-Dark_structs.hpp"
#include "SDK/FN_Border-ModalHeader-Dark_classes.hpp"
#include "SDK/FN_Border-ModalHeader-Dark_parameters.hpp"
#include "SDK/FN_AlterationWidget_structs.hpp"
#include "SDK/FN_AlterationWidget_classes.hpp"
#include "SDK/FN_AlterationWidget_parameters.hpp"
#include "SDK/FN_Lightbox_structs.hpp"
#include "SDK/FN_Lightbox_classes.hpp"
#include "SDK/FN_Lightbox_parameters.hpp"
#include "SDK/FN_InfoEntry_structs.hpp"
#include "SDK/FN_InfoEntry_classes.hpp"
#include "SDK/FN_InfoEntry_parameters.hpp"
#include "SDK/FN_SquadSlotGroup_structs.hpp"
#include "SDK/FN_SquadSlotGroup_classes.hpp"
#include "SDK/FN_SquadSlotGroup_parameters.hpp"
#include "SDK/FN_TextStyle-Header-M-S_structs.hpp"
#include "SDK/FN_TextStyle-Header-M-S_classes.hpp"
#include "SDK/FN_TextStyle-Header-M-S_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Yellow_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Yellow_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Yellow_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-70pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-70pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-70pc_parameters.hpp"
#include "SDK/FN_Border_StatRow_WhiteLowOpacity_structs.hpp"
#include "SDK/FN_Border_StatRow_WhiteLowOpacity_classes.hpp"
#include "SDK/FN_Border_StatRow_WhiteLowOpacity_parameters.hpp"
#include "SDK/FN_TextScrollStyle-Base_structs.hpp"
#include "SDK/FN_TextScrollStyle-Base_classes.hpp"
#include "SDK/FN_TextScrollStyle-Base_parameters.hpp"
#include "SDK/FN_ButtonStyle-TransparentWithCues-XS-B-White_structs.hpp"
#include "SDK/FN_ButtonStyle-TransparentWithCues-XS-B-White_classes.hpp"
#include "SDK/FN_ButtonStyle-TransparentWithCues-XS-B-White_parameters.hpp"
#include "SDK/FN_BP_SimpleItemWidget_structs.hpp"
#include "SDK/FN_BP_SimpleItemWidget_classes.hpp"
#include "SDK/FN_BP_SimpleItemWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Blue_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Blue_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Blue_parameters.hpp"
#include "SDK/FN_Border-1pxOutline_structs.hpp"
#include "SDK/FN_Border-1pxOutline_classes.hpp"
#include "SDK/FN_Border-1pxOutline_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionPickVehicleWidget_structs.hpp"
#include "SDK/FN_BP_FortExpeditionPickVehicleWidget_classes.hpp"
#include "SDK/FN_BP_FortExpeditionPickVehicleWidget_parameters.hpp"
#include "SDK/FN_Border-Bang_structs.hpp"
#include "SDK/FN_Border-Bang_classes.hpp"
#include "SDK/FN_Border-Bang_parameters.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-MulchModeItemToDetail_structs.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-MulchModeItemToDetail_classes.hpp"
#include "SDK/FN_ItemManagementTileButtonStyle-MulchModeItemToDetail_parameters.hpp"
#include "SDK/FN_ButtonStyle-MediumTransparentWithCues_structs.hpp"
#include "SDK/FN_ButtonStyle-MediumTransparentWithCues_classes.hpp"
#include "SDK/FN_ButtonStyle-MediumTransparentWithCues_parameters.hpp"
#include "SDK/FN_StatsListItemWIdget_structs.hpp"
#include "SDK/FN_StatsListItemWIdget_classes.hpp"
#include "SDK/FN_StatsListItemWIdget_parameters.hpp"
#include "SDK/FN_TooltipStat_structs.hpp"
#include "SDK/FN_TooltipStat_classes.hpp"
#include "SDK/FN_TooltipStat_parameters.hpp"
#include "SDK/FN_ButtonStyle-HighlightEmphasized_structs.hpp"
#include "SDK/FN_ButtonStyle-HighlightEmphasized_classes.hpp"
#include "SDK/FN_ButtonStyle-HighlightEmphasized_parameters.hpp"
#include "SDK/FN_TextStyle-MediumButton_structs.hpp"
#include "SDK/FN_TextStyle-MediumButton_classes.hpp"
#include "SDK/FN_TextStyle-MediumButton_parameters.hpp"
#include "SDK/FN_SquadSlotItemPicker_structs.hpp"
#include "SDK/FN_SquadSlotItemPicker_classes.hpp"
#include "SDK/FN_SquadSlotItemPicker_parameters.hpp"
#include "SDK/FN_PerksList_structs.hpp"
#include "SDK/FN_PerksList_classes.hpp"
#include "SDK/FN_PerksList_parameters.hpp"
#include "SDK/FN_PerkWidgetNew_structs.hpp"
#include "SDK/FN_PerkWidgetNew_classes.hpp"
#include "SDK/FN_PerkWidgetNew_parameters.hpp"
#include "SDK/FN_TextStyle-MediumButton-Disabled_structs.hpp"
#include "SDK/FN_TextStyle-MediumButton-Disabled_classes.hpp"
#include "SDK/FN_TextStyle-MediumButton-Disabled_parameters.hpp"
#include "SDK/FN_TextStyle-Button-BottomBar-S-Selected_structs.hpp"
#include "SDK/FN_TextStyle-Button-BottomBar-S-Selected_classes.hpp"
#include "SDK/FN_TextStyle-Button-BottomBar-S-Selected_parameters.hpp"
#include "SDK/FN_PerkTierWidgetNew_structs.hpp"
#include "SDK/FN_PerkTierWidgetNew_classes.hpp"
#include "SDK/FN_PerkTierWidgetNew_parameters.hpp"
#include "SDK/FN_FortHeroSupportPerkWidget_structs.hpp"
#include "SDK/FN_FortHeroSupportPerkWidget_classes.hpp"
#include "SDK/FN_FortHeroSupportPerkWidget_parameters.hpp"
#include "SDK/FN_SurvivorSquadBonusTraitsDetailWidget_structs.hpp"
#include "SDK/FN_SurvivorSquadBonusTraitsDetailWidget_classes.hpp"
#include "SDK/FN_SurvivorSquadBonusTraitsDetailWidget_parameters.hpp"
#include "SDK/FN_PrimaryHeroActiveAbilitiesListDetailWidget_structs.hpp"
#include "SDK/FN_PrimaryHeroActiveAbilitiesListDetailWidget_classes.hpp"
#include "SDK/FN_PrimaryHeroActiveAbilitiesListDetailWidget_parameters.hpp"
#include "SDK/FN_TextStyle-Button-BottomBar-S_structs.hpp"
#include "SDK/FN_TextStyle-Button-BottomBar-S_classes.hpp"
#include "SDK/FN_TextStyle-Button-BottomBar-S_parameters.hpp"
#include "SDK/FN_ItemDetailsHeaderItemDisplayText_structs.hpp"
#include "SDK/FN_ItemDetailsHeaderItemDisplayText_classes.hpp"
#include "SDK/FN_ItemDetailsHeaderItemDisplayText_parameters.hpp"
#include "SDK/FN_ItemDetailsHeaderRarityTypeText_structs.hpp"
#include "SDK/FN_ItemDetailsHeaderRarityTypeText_classes.hpp"
#include "SDK/FN_ItemDetailsHeaderRarityTypeText_parameters.hpp"
#include "SDK/FN_ItemDetailsHeaderTagListText_structs.hpp"
#include "SDK/FN_ItemDetailsHeaderTagListText_classes.hpp"
#include "SDK/FN_ItemDetailsHeaderTagListText_parameters.hpp"
#include "SDK/FN_ItemRatingIndicator_structs.hpp"
#include "SDK/FN_ItemRatingIndicator_classes.hpp"
#include "SDK/FN_ItemRatingIndicator_parameters.hpp"
#include "SDK/FN_StatNumericTextBlock_structs.hpp"
#include "SDK/FN_StatNumericTextBlock_classes.hpp"
#include "SDK/FN_StatNumericTextBlock_parameters.hpp"
#include "SDK/FN_SquadSlotItemPickerTileButton_structs.hpp"
#include "SDK/FN_SquadSlotItemPickerTileButton_classes.hpp"
#include "SDK/FN_SquadSlotItemPickerTileButton_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionReturnsWidget_structs.hpp"
#include "SDK/FN_BP_FortExpeditionReturnsWidget_classes.hpp"
#include "SDK/FN_BP_FortExpeditionReturnsWidget_parameters.hpp"
#include "SDK/FN_BP_FortMaterialProgressBar_structs.hpp"
#include "SDK/FN_BP_FortMaterialProgressBar_classes.hpp"
#include "SDK/FN_BP_FortMaterialProgressBar_parameters.hpp"
#include "SDK/FN_ViewInfoButton_NoText_structs.hpp"
#include "SDK/FN_ViewInfoButton_NoText_classes.hpp"
#include "SDK/FN_ViewInfoButton_NoText_parameters.hpp"
#include "SDK/FN_Tooltip-Custom-S_structs.hpp"
#include "SDK/FN_Tooltip-Custom-S_classes.hpp"
#include "SDK/FN_Tooltip-Custom-S_parameters.hpp"
#include "SDK/FN_CraftingIngredient_structs.hpp"
#include "SDK/FN_CraftingIngredient_classes.hpp"
#include "SDK/FN_CraftingIngredient_parameters.hpp"
#include "SDK/FN_EFortUIThemeColor_structs.hpp"
#include "SDK/FN_EFortUIThemeColor_classes.hpp"
#include "SDK/FN_EFortUIThemeColor_parameters.hpp"
#include "SDK/FN_ColorLibrary_structs.hpp"
#include "SDK/FN_ColorLibrary_classes.hpp"
#include "SDK/FN_ColorLibrary_parameters.hpp"
#include "SDK/FN_ItemTooltipContent_structs.hpp"
#include "SDK/FN_ItemTooltipContent_classes.hpp"
#include "SDK/FN_ItemTooltipContent_parameters.hpp"
#include "SDK/FN_BP_ExpeditionSquadSlotsView_structs.hpp"
#include "SDK/FN_BP_ExpeditionSquadSlotsView_classes.hpp"
#include "SDK/FN_BP_ExpeditionSquadSlotsView_parameters.hpp"
#include "SDK/FN_SquadIcon_structs.hpp"
#include "SDK/FN_SquadIcon_classes.hpp"
#include "SDK/FN_SquadIcon_parameters.hpp"
#include "SDK/FN_PerkWidget_structs.hpp"
#include "SDK/FN_PerkWidget_classes.hpp"
#include "SDK/FN_PerkWidget_parameters.hpp"
#include "SDK/FN_Tooltip-BasicMultiLine-S_structs.hpp"
#include "SDK/FN_Tooltip-BasicMultiLine-S_classes.hpp"
#include "SDK/FN_Tooltip-BasicMultiLine-S_parameters.hpp"
#include "SDK/FN_InfoWindow_structs.hpp"
#include "SDK/FN_InfoWindow_classes.hpp"
#include "SDK/FN_InfoWindow_parameters.hpp"
#include "SDK/FN_ExpeditionSquadSlotButton_structs.hpp"
#include "SDK/FN_ExpeditionSquadSlotButton_classes.hpp"
#include "SDK/FN_ExpeditionSquadSlotButton_parameters.hpp"
#include "SDK/FN_Item_TierStar_structs.hpp"
#include "SDK/FN_Item_TierStar_classes.hpp"
#include "SDK/FN_Item_TierStar_parameters.hpp"
#include "SDK/FN_ItemDurabilityMeter_structs.hpp"
#include "SDK/FN_ItemDurabilityMeter_classes.hpp"
#include "SDK/FN_ItemDurabilityMeter_parameters.hpp"
#include "SDK/FN_E_OutlanderFragmentTypes_structs.hpp"
#include "SDK/FN_E_OutlanderFragmentTypes_classes.hpp"
#include "SDK/FN_E_OutlanderFragmentTypes_parameters.hpp"
#include "SDK/FN_XpBarToolTip_structs.hpp"
#include "SDK/FN_XpBarToolTip_classes.hpp"
#include "SDK/FN_XpBarToolTip_parameters.hpp"
#include "SDK/FN_TooltipStatWrapper_structs.hpp"
#include "SDK/FN_TooltipStatWrapper_classes.hpp"
#include "SDK/FN_TooltipStatWrapper_parameters.hpp"
#include "SDK/FN_TooltipStatWidget_structs.hpp"
#include "SDK/FN_TooltipStatWidget_classes.hpp"
#include "SDK/FN_TooltipStatWidget_parameters.hpp"
#include "SDK/FN_IconTextButton_structs.hpp"
#include "SDK/FN_IconTextButton_classes.hpp"
#include "SDK/FN_IconTextButton_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionExpiresWidget_structs.hpp"
#include "SDK/FN_BP_FortExpeditionExpiresWidget_classes.hpp"
#include "SDK/FN_BP_FortExpeditionExpiresWidget_parameters.hpp"
#include "SDK/FN_Tooltip-Basic-S_structs.hpp"
#include "SDK/FN_Tooltip-Basic-S_classes.hpp"
#include "SDK/FN_Tooltip-Basic-S_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionMasterWidget_structs.hpp"
#include "SDK/FN_BP_FortExpeditionMasterWidget_classes.hpp"
#include "SDK/FN_BP_FortExpeditionMasterWidget_parameters.hpp"
#include "SDK/FN_BP_FortExpeditionListView_structs.hpp"
#include "SDK/FN_BP_FortExpeditionListView_classes.hpp"
#include "SDK/FN_BP_FortExpeditionListView_parameters.hpp"
#include "SDK/FN_HeroCoreStat_structs.hpp"
#include "SDK/FN_HeroCoreStat_classes.hpp"
#include "SDK/FN_HeroCoreStat_parameters.hpp"
#include "SDK/FN_TooltipLibrary_structs.hpp"
#include "SDK/FN_TooltipLibrary_classes.hpp"
#include "SDK/FN_TooltipLibrary_parameters.hpp"
#include "SDK/FN_Tooltip-CoreStat_structs.hpp"
#include "SDK/FN_Tooltip-CoreStat_classes.hpp"
#include "SDK/FN_Tooltip-CoreStat_parameters.hpp"
#include "SDK/FN_HorizontalTabList_structs.hpp"
#include "SDK/FN_HorizontalTabList_classes.hpp"
#include "SDK/FN_HorizontalTabList_parameters.hpp"
#include "SDK/FN_Tooltip-DescriptionText_structs.hpp"
#include "SDK/FN_Tooltip-DescriptionText_classes.hpp"
#include "SDK/FN_Tooltip-DescriptionText_parameters.hpp"
#include "SDK/FN_LegacyAlterationGroup_Widget_structs.hpp"
#include "SDK/FN_LegacyAlterationGroup_Widget_classes.hpp"
#include "SDK/FN_LegacyAlterationGroup_Widget_parameters.hpp"
#include "SDK/FN_LegacyAlteration_Widget_structs.hpp"
#include "SDK/FN_LegacyAlteration_Widget_classes.hpp"
#include "SDK/FN_LegacyAlteration_Widget_parameters.hpp"
#include "SDK/FN_LegacyPerksWidget_structs.hpp"
#include "SDK/FN_LegacyPerksWidget_classes.hpp"
#include "SDK/FN_LegacyPerksWidget_parameters.hpp"
#include "SDK/FN_WorkerTooltipStatsWidget_structs.hpp"
#include "SDK/FN_WorkerTooltipStatsWidget_classes.hpp"
#include "SDK/FN_WorkerTooltipStatsWidget_parameters.hpp"
#include "SDK/FN_SlotLibrary_structs.hpp"
#include "SDK/FN_SlotLibrary_classes.hpp"
#include "SDK/FN_SlotLibrary_parameters.hpp"
#include "SDK/FN_LegacyPerkTierWidget_structs.hpp"
#include "SDK/FN_LegacyPerkTierWidget_classes.hpp"
#include "SDK/FN_LegacyPerkTierWidget_parameters.hpp"
#include "SDK/FN_ItemUIFunctionLibrary_structs.hpp"
#include "SDK/FN_ItemUIFunctionLibrary_classes.hpp"
#include "SDK/FN_ItemUIFunctionLibrary_parameters.hpp"
#include "SDK/FN_TooltipDurabilityMeter_structs.hpp"
#include "SDK/FN_TooltipDurabilityMeter_classes.hpp"
#include "SDK/FN_TooltipDurabilityMeter_parameters.hpp"
#include "SDK/FN_GE_Map_Resistance_To_Shield_structs.hpp"
#include "SDK/FN_GE_Map_Resistance_To_Shield_classes.hpp"
#include "SDK/FN_GE_Map_Resistance_To_Shield_parameters.hpp"
#include "SDK/FN_Border-StatBG_structs.hpp"
#include "SDK/FN_Border-StatBG_classes.hpp"
#include "SDK/FN_Border-StatBG_parameters.hpp"
#include "SDK/FN_GE_Map_Athena_Inventory_StartingSize_structs.hpp"
#include "SDK/FN_GE_Map_Athena_Inventory_StartingSize_classes.hpp"
#include "SDK/FN_GE_Map_Athena_Inventory_StartingSize_parameters.hpp"
#include "SDK/FN_GE_Map_Tech_to_AbilityDamage_TrapDamage_structs.hpp"
#include "SDK/FN_GE_Map_Tech_to_AbilityDamage_TrapDamage_classes.hpp"
#include "SDK/FN_GE_Map_Tech_to_AbilityDamage_TrapDamage_parameters.hpp"
#include "SDK/FN_GE_Athena_DBNO_Start_structs.hpp"
#include "SDK/FN_GE_Athena_DBNO_Start_classes.hpp"
#include "SDK/FN_GE_Athena_DBNO_Start_parameters.hpp"
#include "SDK/FN_Harvest_CameraShake_structs.hpp"
#include "SDK/FN_Harvest_CameraShake_classes.hpp"
#include "SDK/FN_Harvest_CameraShake_parameters.hpp"
#include "SDK/FN_GE_Map_Commando_Vitality_Health_structs.hpp"
#include "SDK/FN_GE_Map_Commando_Vitality_Health_classes.hpp"
#include "SDK/FN_GE_Map_Commando_Vitality_Health_parameters.hpp"
#include "SDK/FN_FortUIStylesheet_structs.hpp"
#include "SDK/FN_FortUIStylesheet_classes.hpp"
#include "SDK/FN_FortUIStylesheet_parameters.hpp"
#include "SDK/FN_AnimNotify_MeleeAnimTrails_Off_structs.hpp"
#include "SDK/FN_AnimNotify_MeleeAnimTrails_Off_classes.hpp"
#include "SDK/FN_AnimNotify_MeleeAnimTrails_Off_parameters.hpp"
#include "SDK/FN_AnimNotify_MeleeAnimTrails_On_structs.hpp"
#include "SDK/FN_AnimNotify_MeleeAnimTrails_On_classes.hpp"
#include "SDK/FN_AnimNotify_MeleeAnimTrails_On_parameters.hpp"
#include "SDK/FN_AnimNotify_PlayFireFX_structs.hpp"
#include "SDK/FN_AnimNotify_PlayFireFX_classes.hpp"
#include "SDK/FN_AnimNotify_PlayFireFX_parameters.hpp"
#include "SDK/FN_Melee_CameraShake_structs.hpp"
#include "SDK/FN_Melee_CameraShake_classes.hpp"
#include "SDK/FN_Melee_CameraShake_parameters.hpp"
#include "SDK/FN_GE_Map_Commando_Prowess_TrapDamage_structs.hpp"
#include "SDK/FN_GE_Map_Commando_Prowess_TrapDamage_classes.hpp"
#include "SDK/FN_GE_Map_Commando_Prowess_TrapDamage_parameters.hpp"
#include "SDK/FN_LegacyBasicGradientFill_structs.hpp"
#include "SDK/FN_LegacyBasicGradientFill_classes.hpp"
#include "SDK/FN_LegacyBasicGradientFill_parameters.hpp"
#include "SDK/FN_LegacyRatingWidget_structs.hpp"
#include "SDK/FN_LegacyRatingWidget_classes.hpp"
#include "SDK/FN_LegacyRatingWidget_parameters.hpp"
#include "SDK/FN_B_AthenaAlwaysLoadedContentHack_structs.hpp"
#include "SDK/FN_B_AthenaAlwaysLoadedContentHack_classes.hpp"
#include "SDK/FN_B_AthenaAlwaysLoadedContentHack_parameters.hpp"
#include "SDK/FN_Tooltip-DisplayAttribute_structs.hpp"
#include "SDK/FN_Tooltip-DisplayAttribute_classes.hpp"
#include "SDK/FN_Tooltip-DisplayAttribute_parameters.hpp"
#include "SDK/FN_ButtonStyle-RatingWidget_structs.hpp"
#include "SDK/FN_ButtonStyle-RatingWidget_classes.hpp"
#include "SDK/FN_ButtonStyle-RatingWidget_parameters.hpp"
#include "SDK/FN_StyleLibrary_structs.hpp"
#include "SDK/FN_StyleLibrary_classes.hpp"
#include "SDK/FN_StyleLibrary_parameters.hpp"
#include "SDK/FN_TooltipIngredientList_structs.hpp"
#include "SDK/FN_TooltipIngredientList_classes.hpp"
#include "SDK/FN_TooltipIngredientList_parameters.hpp"
#include "SDK/FN_F_MED_BLK_Red_Head_01_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_F_MED_BLK_Red_Head_01_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_F_MED_BLK_Red_Head_01_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_BulletWhipTrackerComponent_structs.hpp"
#include "SDK/FN_BulletWhipTrackerComponent_classes.hpp"
#include "SDK/FN_BulletWhipTrackerComponent_parameters.hpp"
#include "SDK/FN_F_MED_CAU_Armstrong_01_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_F_MED_CAU_Armstrong_01_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_F_MED_CAU_Armstrong_01_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_M_MED_ASN_Jae_Head_01_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_M_MED_ASN_Jae_Head_01_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_M_MED_ASN_Jae_Head_01_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_M_MED_BLK_Sydney_01_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_M_MED_BLK_Sydney_01_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_M_MED_BLK_Sydney_01_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_M_MED_HIS_Sparks_Head_01_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_M_MED_HIS_Sparks_Head_01_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_M_MED_HIS_Sparks_Head_01_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_GE_Map_Commando_Defense_ShieldStrength_ShieldRegen_structs.hpp"
#include "SDK/FN_GE_Map_Commando_Defense_ShieldStrength_ShieldRegen_classes.hpp"
#include "SDK/FN_GE_Map_Commando_Defense_ShieldStrength_ShieldRegen_parameters.hpp"
#include "SDK/FN_B_Melee_Generic_structs.hpp"
#include "SDK/FN_B_Melee_Generic_classes.hpp"
#include "SDK/FN_B_Melee_Generic_parameters.hpp"
#include "SDK/FN_GE_Map_Commando_Offense_AllWeaponDamage_structs.hpp"
#include "SDK/FN_GE_Map_Commando_Offense_AllWeaponDamage_classes.hpp"
#include "SDK/FN_GE_Map_Commando_Offense_AllWeaponDamage_parameters.hpp"
#include "SDK/FN_M_MED_Commando_Head_HW01_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_M_MED_Commando_Head_HW01_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_M_MED_Commando_Head_HW01_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_B_Melee_Edged_Generic_structs.hpp"
#include "SDK/FN_B_Melee_Edged_Generic_classes.hpp"
#include "SDK/FN_B_Melee_Edged_Generic_parameters.hpp"
#include "SDK/FN_B_Melee_Impact_Generic_structs.hpp"
#include "SDK/FN_B_Melee_Impact_Generic_classes.hpp"
#include "SDK/FN_B_Melee_Impact_Generic_parameters.hpp"
#include "SDK/FN_B_Prj_Bullet_Sniper_structs.hpp"
#include "SDK/FN_B_Prj_Bullet_Sniper_classes.hpp"
#include "SDK/FN_B_Prj_Bullet_Sniper_parameters.hpp"
#include "SDK/FN_B_StockBattleBus_structs.hpp"
#include "SDK/FN_B_StockBattleBus_classes.hpp"
#include "SDK/FN_B_StockBattleBus_parameters.hpp"
#include "SDK/FN_MenuScreen_Commando_structs.hpp"
#include "SDK/FN_MenuScreen_Commando_classes.hpp"
#include "SDK/FN_MenuScreen_Commando_parameters.hpp"
#include "SDK/FN_B_Melee_Impact_Pickaxe_Athena_structs.hpp"
#include "SDK/FN_B_Melee_Impact_Pickaxe_Athena_classes.hpp"
#include "SDK/FN_B_Melee_Impact_Pickaxe_Athena_parameters.hpp"
#include "SDK/FN_B_Melee_Impact_Pickaxe_Flamingo_Athena_structs.hpp"
#include "SDK/FN_B_Melee_Impact_Pickaxe_Flamingo_Athena_classes.hpp"
#include "SDK/FN_B_Melee_Impact_Pickaxe_Flamingo_Athena_parameters.hpp"
#include "SDK/FN_B_Melee_Edged_Scythe_structs.hpp"
#include "SDK/FN_B_Melee_Edged_Scythe_classes.hpp"
#include "SDK/FN_B_Melee_Edged_Scythe_parameters.hpp"
#include "SDK/FN_AnimNotify_PlayReloadFX_structs.hpp"
#include "SDK/FN_AnimNotify_PlayReloadFX_classes.hpp"
#include "SDK/FN_AnimNotify_PlayReloadFX_parameters.hpp"
#include "SDK/FN_Female_Commando_01_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_Female_Commando_01_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_Female_Commando_01_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_PBWA_BG_ArchwayLarge_structs.hpp"
#include "SDK/FN_PBWA_BG_ArchwayLarge_classes.hpp"
#include "SDK/FN_PBWA_BG_ArchwayLarge_parameters.hpp"
#include "SDK/FN_PBWA_BG_BalconyD_structs.hpp"
#include "SDK/FN_PBWA_BG_BalconyD_classes.hpp"
#include "SDK/FN_PBWA_BG_BalconyD_parameters.hpp"
#include "SDK/FN_PBWA_BG_BalconyO_structs.hpp"
#include "SDK/FN_PBWA_BG_BalconyO_classes.hpp"
#include "SDK/FN_PBWA_BG_BalconyO_parameters.hpp"
#include "SDK/FN_PBWA_BG_Brace_structs.hpp"
#include "SDK/FN_PBWA_BG_Brace_classes.hpp"
#include "SDK/FN_PBWA_BG_Brace_parameters.hpp"
#include "SDK/FN_PBWA_BG_HalfWall_structs.hpp"
#include "SDK/FN_PBWA_BG_HalfWall_classes.hpp"
#include "SDK/FN_PBWA_BG_HalfWall_parameters.hpp"
#include "SDK/FN_PBWA_BG_HalfWallDoor_structs.hpp"
#include "SDK/FN_PBWA_BG_HalfWallDoor_classes.hpp"
#include "SDK/FN_PBWA_BG_HalfWallDoor_parameters.hpp"
#include "SDK/FN_PBWA_BG_HalfWallDoorS_structs.hpp"
#include "SDK/FN_PBWA_BG_HalfWallDoorS_classes.hpp"
#include "SDK/FN_PBWA_BG_HalfWallDoorS_parameters.hpp"
#include "SDK/FN_PBWA_BG_QuarterWallHalf_structs.hpp"
#include "SDK/FN_PBWA_BG_QuarterWallHalf_classes.hpp"
#include "SDK/FN_PBWA_BG_QuarterWallHalf_parameters.hpp"
#include "SDK/FN_PBWA_BG_QuarterWallS_structs.hpp"
#include "SDK/FN_PBWA_BG_QuarterWallS_classes.hpp"
#include "SDK/FN_PBWA_BG_QuarterWallS_parameters.hpp"
#include "SDK/FN_PBWA_BG_RoofC_structs.hpp"
#include "SDK/FN_PBWA_BG_RoofC_classes.hpp"
#include "SDK/FN_PBWA_BG_RoofC_parameters.hpp"
#include "SDK/FN_PBWA_BG_RoofI_structs.hpp"
#include "SDK/FN_PBWA_BG_RoofI_classes.hpp"
#include "SDK/FN_PBWA_BG_RoofI_parameters.hpp"
#include "SDK/FN_PBWA_BG_RoofS_structs.hpp"
#include "SDK/FN_PBWA_BG_RoofS_classes.hpp"
#include "SDK/FN_PBWA_BG_RoofS_parameters.hpp"
#include "SDK/FN_PBWA_BG_RoofWall_structs.hpp"
#include "SDK/FN_PBWA_BG_RoofWall_classes.hpp"
#include "SDK/FN_PBWA_BG_RoofWall_parameters.hpp"
#include "SDK/FN_PBWA_BG_StairR_structs.hpp"
#include "SDK/FN_PBWA_BG_StairR_classes.hpp"
#include "SDK/FN_PBWA_BG_StairR_parameters.hpp"
#include "SDK/FN_PBWA_BG_StairW_structs.hpp"
#include "SDK/FN_PBWA_BG_StairW_classes.hpp"
#include "SDK/FN_PBWA_BG_StairW_parameters.hpp"
#include "SDK/FN_F_Med_Soldier_01_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_F_Med_Soldier_01_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_F_Med_Soldier_01_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_F_MED_ASN_Sarah_Head_01_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_F_MED_ASN_Sarah_Head_01_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_F_MED_ASN_Sarah_Head_01_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_F_Med_Head_01_Skeleton_AnimBlueprint_structs.hpp"
#include "SDK/FN_F_Med_Head_01_Skeleton_AnimBlueprint_classes.hpp"
#include "SDK/FN_F_Med_Head_01_Skeleton_AnimBlueprint_parameters.hpp"
#include "SDK/FN_PBW_BP_Parent_structs.hpp"
#include "SDK/FN_PBW_BP_Parent_classes.hpp"
#include "SDK/FN_PBW_BP_Parent_parameters.hpp"
#include "SDK/FN_PBWA_M1_HalfWall_structs.hpp"
#include "SDK/FN_PBWA_M1_HalfWall_classes.hpp"
#include "SDK/FN_PBWA_M1_HalfWall_parameters.hpp"
#include "SDK/FN_PBWA_M1_HalfWallDoor_structs.hpp"
#include "SDK/FN_PBWA_M1_HalfWallDoor_classes.hpp"
#include "SDK/FN_PBWA_M1_HalfWallDoor_parameters.hpp"
#include "SDK/FN_PBWA_M1_HalfWallDoorS_structs.hpp"
#include "SDK/FN_PBWA_M1_HalfWallDoorS_classes.hpp"
#include "SDK/FN_PBWA_M1_HalfWallDoorS_parameters.hpp"
#include "SDK/FN_PBWA_M1_QuarterWallS_structs.hpp"
#include "SDK/FN_PBWA_M1_QuarterWallS_classes.hpp"
#include "SDK/FN_PBWA_M1_QuarterWallS_parameters.hpp"
#include "SDK/FN_SK_M_Med_Soldier_04_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_SK_M_Med_Soldier_04_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_SK_M_Med_Soldier_04_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_PBWA_M1_RoofWall_structs.hpp"
#include "SDK/FN_PBWA_M1_RoofWall_classes.hpp"
#include "SDK/FN_PBWA_M1_RoofWall_parameters.hpp"
#include "SDK/FN_PBWA_S1_HalfWallDoor_structs.hpp"
#include "SDK/FN_PBWA_S1_HalfWallDoor_classes.hpp"
#include "SDK/FN_PBWA_S1_HalfWallDoor_parameters.hpp"
#include "SDK/FN_PBWA_S1_HalfWallDoorSide_structs.hpp"
#include "SDK/FN_PBWA_S1_HalfWallDoorSide_classes.hpp"
#include "SDK/FN_PBWA_S1_HalfWallDoorSide_parameters.hpp"
#include "SDK/FN_PBWA_S1_HalfWallS_structs.hpp"
#include "SDK/FN_PBWA_S1_HalfWallS_classes.hpp"
#include "SDK/FN_PBWA_S1_HalfWallS_parameters.hpp"
#include "SDK/FN_PBWA_S1_QuarterWallS_structs.hpp"
#include "SDK/FN_PBWA_S1_QuarterWallS_classes.hpp"
#include "SDK/FN_PBWA_S1_QuarterWallS_parameters.hpp"
#include "SDK/FN_PBWA_S1_RoofWall_structs.hpp"
#include "SDK/FN_PBWA_S1_RoofWall_classes.hpp"
#include "SDK/FN_PBWA_S1_RoofWall_parameters.hpp"
#include "SDK/FN_PBWA_W1_Archway_structs.hpp"
#include "SDK/FN_PBWA_W1_Archway_classes.hpp"
#include "SDK/FN_PBWA_W1_Archway_parameters.hpp"
#include "SDK/FN_PBWA_W1_ArchwayLarge_structs.hpp"
#include "SDK/FN_PBWA_W1_ArchwayLarge_classes.hpp"
#include "SDK/FN_PBWA_W1_ArchwayLarge_parameters.hpp"
#include "SDK/FN_PBWA_W1_ArchwayLargeSupport_structs.hpp"
#include "SDK/FN_PBWA_W1_ArchwayLargeSupport_classes.hpp"
#include "SDK/FN_PBWA_W1_ArchwayLargeSupport_parameters.hpp"
#include "SDK/FN_PBWA_W1_BalconyD_structs.hpp"
#include "SDK/FN_PBWA_W1_BalconyD_classes.hpp"
#include "SDK/FN_PBWA_W1_BalconyD_parameters.hpp"
#include "SDK/FN_PBWA_W1_BalconyI_structs.hpp"
#include "SDK/FN_PBWA_W1_BalconyI_classes.hpp"
#include "SDK/FN_PBWA_W1_BalconyI_parameters.hpp"
#include "SDK/FN_PBWA_W1_BalconyO_structs.hpp"
#include "SDK/FN_PBWA_W1_BalconyO_classes.hpp"
#include "SDK/FN_PBWA_W1_BalconyO_parameters.hpp"
#include "SDK/FN_PBWA_W1_BalconyS_structs.hpp"
#include "SDK/FN_PBWA_W1_BalconyS_classes.hpp"
#include "SDK/FN_PBWA_W1_BalconyS_parameters.hpp"
#include "SDK/FN_PBWA_W1_Brace_structs.hpp"
#include "SDK/FN_PBWA_W1_Brace_classes.hpp"
#include "SDK/FN_PBWA_W1_Brace_parameters.hpp"
#include "SDK/FN_PBWA_W1_Floor_structs.hpp"
#include "SDK/FN_PBWA_W1_Floor_classes.hpp"
#include "SDK/FN_PBWA_W1_Floor_parameters.hpp"
#include "SDK/FN_PBWA_W1_HalfWallDoor_structs.hpp"
#include "SDK/FN_PBWA_W1_HalfWallDoor_classes.hpp"
#include "SDK/FN_PBWA_W1_HalfWallDoor_parameters.hpp"
#include "SDK/FN_PBWA_W1_HalfWallDoorS_structs.hpp"
#include "SDK/FN_PBWA_W1_HalfWallDoorS_classes.hpp"
#include "SDK/FN_PBWA_W1_HalfWallDoorS_parameters.hpp"
#include "SDK/FN_PBWA_W1_HalfWallHalf_structs.hpp"
#include "SDK/FN_PBWA_W1_HalfWallHalf_classes.hpp"
#include "SDK/FN_PBWA_W1_HalfWallHalf_parameters.hpp"
#include "SDK/FN_PBWA_W1_HalfWallS_structs.hpp"
#include "SDK/FN_PBWA_W1_HalfWallS_classes.hpp"
#include "SDK/FN_PBWA_W1_HalfWallS_parameters.hpp"
#include "SDK/FN_PBWA_W1_Pillar_structs.hpp"
#include "SDK/FN_PBWA_W1_Pillar_classes.hpp"
#include "SDK/FN_PBWA_W1_Pillar_parameters.hpp"
#include "SDK/FN_PBWA_W1_QuarterWallHalf_structs.hpp"
#include "SDK/FN_PBWA_W1_QuarterWallHalf_classes.hpp"
#include "SDK/FN_PBWA_W1_QuarterWallHalf_parameters.hpp"
#include "SDK/FN_PBWA_W1_QuarterWallS_structs.hpp"
#include "SDK/FN_PBWA_W1_QuarterWallS_classes.hpp"
#include "SDK/FN_PBWA_W1_QuarterWallS_parameters.hpp"
#include "SDK/FN_PBWA_W1_RoofC_structs.hpp"
#include "SDK/FN_PBWA_W1_RoofC_classes.hpp"
#include "SDK/FN_PBWA_W1_RoofC_parameters.hpp"
#include "SDK/FN_PBWA_W1_RoofD_structs.hpp"
#include "SDK/FN_PBWA_W1_RoofD_classes.hpp"
#include "SDK/FN_PBWA_W1_RoofD_parameters.hpp"
#include "SDK/FN_PBWA_W1_RoofI_structs.hpp"
#include "SDK/FN_PBWA_W1_RoofI_classes.hpp"
#include "SDK/FN_PBWA_W1_RoofI_parameters.hpp"
#include "SDK/FN_PBWA_W1_RoofO_structs.hpp"
#include "SDK/FN_PBWA_W1_RoofO_classes.hpp"
#include "SDK/FN_PBWA_W1_RoofO_parameters.hpp"
#include "SDK/FN_PBWA_W1_RoofS_structs.hpp"
#include "SDK/FN_PBWA_W1_RoofS_classes.hpp"
#include "SDK/FN_PBWA_W1_RoofS_parameters.hpp"
#include "SDK/FN_SkillTreePages_structs.hpp"
#include "SDK/FN_SkillTreePages_classes.hpp"
#include "SDK/FN_SkillTreePages_parameters.hpp"
#include "SDK/FN_PBWA_W1_RoofWall_structs.hpp"
#include "SDK/FN_PBWA_W1_RoofWall_classes.hpp"
#include "SDK/FN_PBWA_W1_RoofWall_parameters.hpp"
#include "SDK/FN_PBWA_W1_StairF_structs.hpp"
#include "SDK/FN_PBWA_W1_StairF_classes.hpp"
#include "SDK/FN_PBWA_W1_StairF_parameters.hpp"
#include "SDK/FN_PBWA_W1_StairR_structs.hpp"
#include "SDK/FN_PBWA_W1_StairR_classes.hpp"
#include "SDK/FN_PBWA_W1_StairR_parameters.hpp"
#include "SDK/FN_PBWA_W1_StairT_structs.hpp"
#include "SDK/FN_PBWA_W1_StairT_classes.hpp"
#include "SDK/FN_PBWA_W1_StairT_parameters.hpp"
#include "SDK/FN_SK_M_MED_Head_01_Skeleton_AnimBP_structs.hpp"
#include "SDK/FN_SK_M_MED_Head_01_Skeleton_AnimBP_classes.hpp"
#include "SDK/FN_SK_M_MED_Head_01_Skeleton_AnimBP_parameters.hpp"
#include "SDK/FN_PBWA_W1_StairW_structs.hpp"
#include "SDK/FN_PBWA_W1_StairW_classes.hpp"
#include "SDK/FN_PBWA_W1_StairW_parameters.hpp"
#include "SDK/FN_BP_ZT_CriticalMission_structs.hpp"
#include "SDK/FN_BP_ZT_CriticalMission_classes.hpp"
#include "SDK/FN_BP_ZT_CriticalMission_parameters.hpp"
#include "SDK/FN_BP_ZT_Survival_structs.hpp"
#include "SDK/FN_BP_ZT_Survival_classes.hpp"
#include "SDK/FN_BP_ZT_Survival_parameters.hpp"
#include "SDK/FN_Combined_Horde_structs.hpp"
#include "SDK/FN_Combined_Horde_classes.hpp"
#include "SDK/FN_Combined_Horde_parameters.hpp"
#include "SDK/FN_BP_ZT_BuildTheBase_structs.hpp"
#include "SDK/FN_BP_ZT_BuildTheBase_classes.hpp"
#include "SDK/FN_BP_ZT_BuildTheBase_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_H_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_H_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_H_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_L_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_L_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_L_parameters.hpp"
#include "SDK/FN_Border-SolidBG-ToolTip_structs.hpp"
#include "SDK/FN_Border-SolidBG-ToolTip_classes.hpp"
#include "SDK/FN_Border-SolidBG-ToolTip_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_M_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_M_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_M_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_VH_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_VH_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_VH_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_VL_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_VL_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Canny_VL_parameters.hpp"
#include "SDK/FN_WeaponTooltipDPSWidget_structs.hpp"
#include "SDK/FN_WeaponTooltipDPSWidget_classes.hpp"
#include "SDK/FN_WeaponTooltipDPSWidget_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_H_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_H_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_H_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_L_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_L_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_L_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_M_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_M_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_M_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_VH_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_VH_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_VH_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_VL_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_VL_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Plank_VL_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_SW_H_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_SW_H_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_SW_H_parameters.hpp"
#include "SDK/FN_Border-SolidBG-ToolTipShadow_structs.hpp"
#include "SDK/FN_Border-SolidBG-ToolTipShadow_classes.hpp"
#include "SDK/FN_Border-SolidBG-ToolTipShadow_parameters.hpp"
#include "SDK/FN_Button-SkillTree-M-People_structs.hpp"
#include "SDK/FN_Button-SkillTree-M-People_classes.hpp"
#include "SDK/FN_Button-SkillTree-M-People_parameters.hpp"
#include "SDK/FN_En_SplineForwardAxes_01_structs.hpp"
#include "SDK/FN_En_SplineForwardAxes_01_classes.hpp"
#include "SDK/FN_En_SplineForwardAxes_01_parameters.hpp"
#include "SDK/FN_Button-SkillTree-M_structs.hpp"
#include "SDK/FN_Button-SkillTree-M_classes.hpp"
#include "SDK/FN_Button-SkillTree-M_parameters.hpp"
#include "SDK/FN_LegacyPerkWidget_structs.hpp"
#include "SDK/FN_LegacyPerkWidget_classes.hpp"
#include "SDK/FN_LegacyPerkWidget_parameters.hpp"
#include "SDK/FN_E_Outlander_FragmentTypes_structs.hpp"
#include "SDK/FN_E_Outlander_FragmentTypes_classes.hpp"
#include "SDK/FN_E_Outlander_FragmentTypes_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_SW_M_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_SW_M_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_SW_M_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_SW_VH_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_SW_VH_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_SW_VH_parameters.hpp"
#include "SDK/FN_DefaultSkillNodeStyle_structs.hpp"
#include "SDK/FN_DefaultSkillNodeStyle_classes.hpp"
#include "SDK/FN_DefaultSkillNodeStyle_parameters.hpp"
#include "SDK/FN_BluGlow_MorphAnimation_structs.hpp"
#include "SDK/FN_BluGlow_MorphAnimation_classes.hpp"
#include "SDK/FN_BluGlow_MorphAnimation_parameters.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Twine_structs.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Twine_classes.hpp"
#include "SDK/FN_BP_ZT_ChallengeTheHorde_Twine_parameters.hpp"
#include "SDK/FN_BP_SplineVolumeTrail_v1b_structs.hpp"
#include "SDK/FN_BP_SplineVolumeTrail_v1b_classes.hpp"
#include "SDK/FN_BP_SplineVolumeTrail_v1b_parameters.hpp"
#include "SDK/FN_BP_ZT_Athena_structs.hpp"
#include "SDK/FN_BP_ZT_Athena_classes.hpp"
#include "SDK/FN_BP_ZT_Athena_parameters.hpp"
#include "SDK/FN_B_AntiMatCharge_CameraShake_structs.hpp"
#include "SDK/FN_B_AntiMatCharge_CameraShake_classes.hpp"
#include "SDK/FN_B_AntiMatCharge_CameraShake_parameters.hpp"
#include "SDK/FN_CameraShake_AntiMaterialCharge_FullyCharged_structs.hpp"
#include "SDK/FN_CameraShake_AntiMaterialCharge_FullyCharged_classes.hpp"
#include "SDK/FN_CameraShake_AntiMaterialCharge_FullyCharged_parameters.hpp"
#include "SDK/FN_NavFilter_MissionBots_structs.hpp"
#include "SDK/FN_NavFilter_MissionBots_classes.hpp"
#include "SDK/FN_NavFilter_MissionBots_parameters.hpp"
#include "SDK/FN_BP_ZT_Athena_Faceoff_structs.hpp"
#include "SDK/FN_BP_ZT_Athena_Faceoff_classes.hpp"
#include "SDK/FN_BP_ZT_Athena_Faceoff_parameters.hpp"
#include "SDK/FN_BP_ZT_Athena_Streaming_structs.hpp"
#include "SDK/FN_BP_ZT_Athena_Streaming_classes.hpp"
#include "SDK/FN_BP_ZT_Athena_Streaming_parameters.hpp"
#include "SDK/FN_BP_ZT_PVE_structs.hpp"
#include "SDK/FN_BP_ZT_PVE_classes.hpp"
#include "SDK/FN_BP_ZT_PVE_parameters.hpp"
#include "SDK/FN_SchematicTooltipCraftingIngredient_structs.hpp"
#include "SDK/FN_SchematicTooltipCraftingIngredient_classes.hpp"
#include "SDK/FN_SchematicTooltipCraftingIngredient_parameters.hpp"
#include "SDK/FN_NavLink_Floor_structs.hpp"
#include "SDK/FN_NavLink_Floor_classes.hpp"
#include "SDK/FN_NavLink_Floor_parameters.hpp"
#include "SDK/FN_Button-SkillTree-L_structs.hpp"
#include "SDK/FN_Button-SkillTree-L_classes.hpp"
#include "SDK/FN_Button-SkillTree-L_parameters.hpp"
#include "SDK/FN_MissionBotPawn_structs.hpp"
#include "SDK/FN_MissionBotPawn_classes.hpp"
#include "SDK/FN_MissionBotPawn_parameters.hpp"
#include "SDK/FN_TooltipHeroBonuses_structs.hpp"
#include "SDK/FN_TooltipHeroBonuses_classes.hpp"
#include "SDK/FN_TooltipHeroBonuses_parameters.hpp"
#include "SDK/FN_Button-SkillTree-None_structs.hpp"
#include "SDK/FN_Button-SkillTree-None_classes.hpp"
#include "SDK/FN_Button-SkillTree-None_parameters.hpp"
#include "SDK/FN_Button-SkillTree-S_structs.hpp"
#include "SDK/FN_Button-SkillTree-S_classes.hpp"
#include "SDK/FN_Button-SkillTree-S_parameters.hpp"
#include "SDK/FN_T1_Research_structs.hpp"
#include "SDK/FN_T1_Research_classes.hpp"
#include "SDK/FN_T1_Research_parameters.hpp"
#include "SDK/FN_T2_Horde_structs.hpp"
#include "SDK/FN_T2_Horde_classes.hpp"
#include "SDK/FN_T2_Horde_parameters.hpp"
#include "SDK/FN_T2_Main_structs.hpp"
#include "SDK/FN_T2_Main_classes.hpp"
#include "SDK/FN_T2_Main_parameters.hpp"
#include "SDK/FN_T2_Research_structs.hpp"
#include "SDK/FN_T2_Research_classes.hpp"
#include "SDK/FN_T2_Research_parameters.hpp"
#include "SDK/FN_T3_Horde_structs.hpp"
#include "SDK/FN_T3_Horde_classes.hpp"
#include "SDK/FN_T3_Horde_parameters.hpp"
#include "SDK/FN_VinderTech_Umbrella_AnimBP_structs.hpp"
#include "SDK/FN_VinderTech_Umbrella_AnimBP_classes.hpp"
#include "SDK/FN_VinderTech_Umbrella_AnimBP_parameters.hpp"
#include "SDK/FN_T3_Main_structs.hpp"
#include "SDK/FN_T3_Main_classes.hpp"
#include "SDK/FN_T3_Main_parameters.hpp"
#include "SDK/FN_T3_Research_structs.hpp"
#include "SDK/FN_T3_Research_classes.hpp"
#include "SDK/FN_T3_Research_parameters.hpp"
#include "SDK/FN_BluGlo_Node_structs.hpp"
#include "SDK/FN_BluGlo_Node_classes.hpp"
#include "SDK/FN_BluGlo_Node_parameters.hpp"
#include "SDK/FN_PlayerPawn_Generic_Parent_structs.hpp"
#include "SDK/FN_PlayerPawn_Generic_Parent_classes.hpp"
#include "SDK/FN_PlayerPawn_Generic_Parent_parameters.hpp"
#include "SDK/FN_Fortnite_M_Avg_Player_MenusScreen_AnimBP_structs.hpp"
#include "SDK/FN_Fortnite_M_Avg_Player_MenusScreen_AnimBP_classes.hpp"
#include "SDK/FN_Fortnite_M_Avg_Player_MenusScreen_AnimBP_parameters.hpp"
#include "SDK/FN_DefenderAnimBlueprint_structs.hpp"
#include "SDK/FN_DefenderAnimBlueprint_classes.hpp"
#include "SDK/FN_DefenderAnimBlueprint_parameters.hpp"
#include "SDK/FN_HuskStrength00_structs.hpp"
#include "SDK/FN_HuskStrength00_classes.hpp"
#include "SDK/FN_HuskStrength00_parameters.hpp"
#include "SDK/FN_HuskStrength01_structs.hpp"
#include "SDK/FN_HuskStrength01_classes.hpp"
#include "SDK/FN_HuskStrength01_parameters.hpp"
#include "SDK/FN_HuskStrength02_structs.hpp"
#include "SDK/FN_HuskStrength02_classes.hpp"
#include "SDK/FN_HuskStrength02_parameters.hpp"
#include "SDK/FN_HuskStrength03_structs.hpp"
#include "SDK/FN_HuskStrength03_classes.hpp"
#include "SDK/FN_HuskStrength03_parameters.hpp"
#include "SDK/FN_HuskStrength04_structs.hpp"
#include "SDK/FN_HuskStrength04_classes.hpp"
#include "SDK/FN_HuskStrength04_parameters.hpp"
#include "SDK/FN_HuskStrength05_structs.hpp"
#include "SDK/FN_HuskStrength05_classes.hpp"
#include "SDK/FN_HuskStrength05_parameters.hpp"
#include "SDK/FN_HuskStrength06_structs.hpp"
#include "SDK/FN_HuskStrength06_classes.hpp"
#include "SDK/FN_HuskStrength06_parameters.hpp"
#include "SDK/FN_HuskStrength07_structs.hpp"
#include "SDK/FN_HuskStrength07_classes.hpp"
#include "SDK/FN_HuskStrength07_parameters.hpp"
#include "SDK/FN_HuskStrength08_structs.hpp"
#include "SDK/FN_HuskStrength08_classes.hpp"
#include "SDK/FN_HuskStrength08_parameters.hpp"
#include "SDK/FN_HuskStrength09_structs.hpp"
#include "SDK/FN_HuskStrength09_classes.hpp"
#include "SDK/FN_HuskStrength09_parameters.hpp"
#include "SDK/FN_HuskStrength10_structs.hpp"
#include "SDK/FN_HuskStrength10_classes.hpp"
#include "SDK/FN_HuskStrength10_parameters.hpp"
#include "SDK/FN_PBWA_BG_Archway_structs.hpp"
#include "SDK/FN_PBWA_BG_Archway_classes.hpp"
#include "SDK/FN_PBWA_BG_Archway_parameters.hpp"
#include "SDK/FN_PBWA_BG_ArchwayLargeSupport_structs.hpp"
#include "SDK/FN_PBWA_BG_ArchwayLargeSupport_classes.hpp"
#include "SDK/FN_PBWA_BG_ArchwayLargeSupport_parameters.hpp"
#include "SDK/FN_PBWA_BG_BalconyI_structs.hpp"
#include "SDK/FN_PBWA_BG_BalconyI_classes.hpp"
#include "SDK/FN_PBWA_BG_BalconyI_parameters.hpp"
#include "SDK/FN_PBWA_BG_BalconyS_structs.hpp"
#include "SDK/FN_PBWA_BG_BalconyS_classes.hpp"
#include "SDK/FN_PBWA_BG_BalconyS_parameters.hpp"
#include "SDK/FN_PBWA_BG_Floor_structs.hpp"
#include "SDK/FN_PBWA_BG_Floor_classes.hpp"
#include "SDK/FN_PBWA_BG_Floor_parameters.hpp"
#include "SDK/FN_PBWA_BG_HalfWallHalf_structs.hpp"
#include "SDK/FN_PBWA_BG_HalfWallHalf_classes.hpp"
#include "SDK/FN_PBWA_BG_HalfWallHalf_parameters.hpp"
#include "SDK/FN_PBWA_BG_RoofD_structs.hpp"
#include "SDK/FN_PBWA_BG_RoofD_classes.hpp"
#include "SDK/FN_PBWA_BG_RoofD_parameters.hpp"
#include "SDK/FN_PBWA_BG_RoofO_structs.hpp"
#include "SDK/FN_PBWA_BG_RoofO_classes.hpp"
#include "SDK/FN_PBWA_BG_RoofO_parameters.hpp"
#include "SDK/FN_PBWA_BG_StairF_structs.hpp"
#include "SDK/FN_PBWA_BG_StairF_classes.hpp"
#include "SDK/FN_PBWA_BG_StairF_parameters.hpp"
#include "SDK/FN_PBWA_BG_StairT_structs.hpp"
#include "SDK/FN_PBWA_BG_StairT_classes.hpp"
#include "SDK/FN_PBWA_BG_StairT_parameters.hpp"
#include "SDK/FN_PBWA_M1_Archway_structs.hpp"
#include "SDK/FN_PBWA_M1_Archway_classes.hpp"
#include "SDK/FN_PBWA_M1_Archway_parameters.hpp"
#include "SDK/FN_PBWA_M1_ArchwayLarge_structs.hpp"
#include "SDK/FN_PBWA_M1_ArchwayLarge_classes.hpp"
#include "SDK/FN_PBWA_M1_ArchwayLarge_parameters.hpp"
#include "SDK/FN_PBWA_M1_ArchwayLargeSupport_structs.hpp"
#include "SDK/FN_PBWA_M1_ArchwayLargeSupport_classes.hpp"
#include "SDK/FN_PBWA_M1_ArchwayLargeSupport_parameters.hpp"
#include "SDK/FN_PBWA_M1_BalconyD_structs.hpp"
#include "SDK/FN_PBWA_M1_BalconyD_classes.hpp"
#include "SDK/FN_PBWA_M1_BalconyD_parameters.hpp"
#include "SDK/FN_PBWA_M1_BalconyI_structs.hpp"
#include "SDK/FN_PBWA_M1_BalconyI_classes.hpp"
#include "SDK/FN_PBWA_M1_BalconyI_parameters.hpp"
#include "SDK/FN_PBWA_M1_BalconyO_structs.hpp"
#include "SDK/FN_PBWA_M1_BalconyO_classes.hpp"
#include "SDK/FN_PBWA_M1_BalconyO_parameters.hpp"
#include "SDK/FN_PBWA_M1_BalconyS_structs.hpp"
#include "SDK/FN_PBWA_M1_BalconyS_classes.hpp"
#include "SDK/FN_PBWA_M1_BalconyS_parameters.hpp"
#include "SDK/FN_PBWA_M1_Brace_structs.hpp"
#include "SDK/FN_PBWA_M1_Brace_classes.hpp"
#include "SDK/FN_PBWA_M1_Brace_parameters.hpp"
#include "SDK/FN_PBWA_M1_Floor_structs.hpp"
#include "SDK/FN_PBWA_M1_Floor_classes.hpp"
#include "SDK/FN_PBWA_M1_Floor_parameters.hpp"
#include "SDK/FN_PBWA_M1_HalfWallHalf_structs.hpp"
#include "SDK/FN_PBWA_M1_HalfWallHalf_classes.hpp"
#include "SDK/FN_PBWA_M1_HalfWallHalf_parameters.hpp"
#include "SDK/FN_PBWA_M1_Pillar_structs.hpp"
#include "SDK/FN_PBWA_M1_Pillar_classes.hpp"
#include "SDK/FN_PBWA_M1_Pillar_parameters.hpp"
#include "SDK/FN_PBWA_M1_QuarterWallHalf_structs.hpp"
#include "SDK/FN_PBWA_M1_QuarterWallHalf_classes.hpp"
#include "SDK/FN_PBWA_M1_QuarterWallHalf_parameters.hpp"
#include "SDK/FN_PBWA_M1_RoofC_structs.hpp"
#include "SDK/FN_PBWA_M1_RoofC_classes.hpp"
#include "SDK/FN_PBWA_M1_RoofC_parameters.hpp"
#include "SDK/FN_PBWA_M1_RoofD_structs.hpp"
#include "SDK/FN_PBWA_M1_RoofD_classes.hpp"
#include "SDK/FN_PBWA_M1_RoofD_parameters.hpp"
#include "SDK/FN_PBWA_M1_RoofI_structs.hpp"
#include "SDK/FN_PBWA_M1_RoofI_classes.hpp"
#include "SDK/FN_PBWA_M1_RoofI_parameters.hpp"
#include "SDK/FN_PBWA_M1_RoofO_structs.hpp"
#include "SDK/FN_PBWA_M1_RoofO_classes.hpp"
#include "SDK/FN_PBWA_M1_RoofO_parameters.hpp"
#include "SDK/FN_PBWA_M1_RoofS_structs.hpp"
#include "SDK/FN_PBWA_M1_RoofS_classes.hpp"
#include "SDK/FN_PBWA_M1_RoofS_parameters.hpp"
#include "SDK/FN_PBWA_M1_StairF_structs.hpp"
#include "SDK/FN_PBWA_M1_StairF_classes.hpp"
#include "SDK/FN_PBWA_M1_StairF_parameters.hpp"
#include "SDK/FN_PBWA_M1_StairR_structs.hpp"
#include "SDK/FN_PBWA_M1_StairR_classes.hpp"
#include "SDK/FN_PBWA_M1_StairR_parameters.hpp"
#include "SDK/FN_PBWA_M1_StairT_structs.hpp"
#include "SDK/FN_PBWA_M1_StairT_classes.hpp"
#include "SDK/FN_PBWA_M1_StairT_parameters.hpp"
#include "SDK/FN_B_Fragment_Container_structs.hpp"
#include "SDK/FN_B_Fragment_Container_classes.hpp"
#include "SDK/FN_B_Fragment_Container_parameters.hpp"
#include "SDK/FN_PBWA_M1_StairW_structs.hpp"
#include "SDK/FN_PBWA_M1_StairW_classes.hpp"
#include "SDK/FN_PBWA_M1_StairW_parameters.hpp"
#include "SDK/FN_PBWA_S1_Archway_structs.hpp"
#include "SDK/FN_PBWA_S1_Archway_classes.hpp"
#include "SDK/FN_PBWA_S1_Archway_parameters.hpp"
#include "SDK/FN_DoorMetaObstacle_structs.hpp"
#include "SDK/FN_DoorMetaObstacle_classes.hpp"
#include "SDK/FN_DoorMetaObstacle_parameters.hpp"
#include "SDK/FN_PBWA_S1_ArchwayLarge_structs.hpp"
#include "SDK/FN_PBWA_S1_ArchwayLarge_classes.hpp"
#include "SDK/FN_PBWA_S1_ArchwayLarge_parameters.hpp"
#include "SDK/FN_PBWA_S1_ArchwayLargeSupport_structs.hpp"
#include "SDK/FN_PBWA_S1_ArchwayLargeSupport_classes.hpp"
#include "SDK/FN_PBWA_S1_ArchwayLargeSupport_parameters.hpp"
#include "SDK/FN_PBWA_S1_BalconyD_structs.hpp"
#include "SDK/FN_PBWA_S1_BalconyD_classes.hpp"
#include "SDK/FN_PBWA_S1_BalconyD_parameters.hpp"
#include "SDK/FN_PBWA_S1_BalconyI_structs.hpp"
#include "SDK/FN_PBWA_S1_BalconyI_classes.hpp"
#include "SDK/FN_PBWA_S1_BalconyI_parameters.hpp"
#include "SDK/FN_PBWA_S1_BalconyO_structs.hpp"
#include "SDK/FN_PBWA_S1_BalconyO_classes.hpp"
#include "SDK/FN_PBWA_S1_BalconyO_parameters.hpp"
#include "SDK/FN_PBWA_S1_BalconyS_structs.hpp"
#include "SDK/FN_PBWA_S1_BalconyS_classes.hpp"
#include "SDK/FN_PBWA_S1_BalconyS_parameters.hpp"
#include "SDK/FN_GET_MoveSpeed_structs.hpp"
#include "SDK/FN_GET_MoveSpeed_classes.hpp"
#include "SDK/FN_GET_MoveSpeed_parameters.hpp"
#include "SDK/FN_GE_Outlander_FragmentTeamSpeedBoost_structs.hpp"
#include "SDK/FN_GE_Outlander_FragmentTeamSpeedBoost_classes.hpp"
#include "SDK/FN_GE_Outlander_FragmentTeamSpeedBoost_parameters.hpp"
#include "SDK/FN_PBWA_S1_Brace_structs.hpp"
#include "SDK/FN_PBWA_S1_Brace_classes.hpp"
#include "SDK/FN_PBWA_S1_Brace_parameters.hpp"
#include "SDK/FN_PBWA_S1_Floor_structs.hpp"
#include "SDK/FN_PBWA_S1_Floor_classes.hpp"
#include "SDK/FN_PBWA_S1_Floor_parameters.hpp"
#include "SDK/FN_GE_Outlander_LlamaFragment_structs.hpp"
#include "SDK/FN_GE_Outlander_LlamaFragment_classes.hpp"
#include "SDK/FN_GE_Outlander_LlamaFragment_parameters.hpp"
#include "SDK/FN_PBWA_S1_HalfWallHalf_structs.hpp"
#include "SDK/FN_PBWA_S1_HalfWallHalf_classes.hpp"
#include "SDK/FN_PBWA_S1_HalfWallHalf_parameters.hpp"
#include "SDK/FN_GE_Outlander_PhaseShiftClearCD_OLD_structs.hpp"
#include "SDK/FN_GE_Outlander_PhaseShiftClearCD_OLD_classes.hpp"
#include "SDK/FN_GE_Outlander_PhaseShiftClearCD_OLD_parameters.hpp"
#include "SDK/FN_PBWA_S1_Pillar_structs.hpp"
#include "SDK/FN_PBWA_S1_Pillar_classes.hpp"
#include "SDK/FN_PBWA_S1_Pillar_parameters.hpp"
#include "SDK/FN_PBWA_S1_QuarterWallHalf_structs.hpp"
#include "SDK/FN_PBWA_S1_QuarterWallHalf_classes.hpp"
#include "SDK/FN_PBWA_S1_QuarterWallHalf_parameters.hpp"
#include "SDK/FN_SkillTreeGroups_structs.hpp"
#include "SDK/FN_SkillTreeGroups_classes.hpp"
#include "SDK/FN_SkillTreeGroups_parameters.hpp"
#include "SDK/FN_SkillTreeGroupColors_structs.hpp"
#include "SDK/FN_SkillTreeGroupColors_classes.hpp"
#include "SDK/FN_SkillTreeGroupColors_parameters.hpp"
#include "SDK/FN_T1_Main_structs.hpp"
#include "SDK/FN_T1_Main_classes.hpp"
#include "SDK/FN_T1_Main_parameters.hpp"
#include "SDK/FN_Button-SkillTree-Next-L_structs.hpp"
#include "SDK/FN_Button-SkillTree-Next-L_classes.hpp"
#include "SDK/FN_Button-SkillTree-Next-L_parameters.hpp"
#include "SDK/FN_T4_Horde_structs.hpp"
#include "SDK/FN_T4_Horde_classes.hpp"
#include "SDK/FN_T4_Horde_parameters.hpp"
#include "SDK/FN_PBWA_S1_RoofC_structs.hpp"
#include "SDK/FN_PBWA_S1_RoofC_classes.hpp"
#include "SDK/FN_PBWA_S1_RoofC_parameters.hpp"
#include "SDK/FN_PBWA_S1_RoofD_structs.hpp"
#include "SDK/FN_PBWA_S1_RoofD_classes.hpp"
#include "SDK/FN_PBWA_S1_RoofD_parameters.hpp"
#include "SDK/FN_Border-ItemInfoHeader_BrightBar_structs.hpp"
#include "SDK/FN_Border-ItemInfoHeader_BrightBar_classes.hpp"
#include "SDK/FN_Border-ItemInfoHeader_BrightBar_parameters.hpp"
#include "SDK/FN_PBWA_S1_RoofI_structs.hpp"
#include "SDK/FN_PBWA_S1_RoofI_classes.hpp"
#include "SDK/FN_PBWA_S1_RoofI_parameters.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Purple_structs.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Purple_classes.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Purple_parameters.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Blue_structs.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Blue_classes.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Blue_parameters.hpp"
#include "SDK/FN_SkillTreeColorSetup_structs.hpp"
#include "SDK/FN_SkillTreeColorSetup_classes.hpp"
#include "SDK/FN_SkillTreeColorSetup_parameters.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Green_structs.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Green_classes.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Green_parameters.hpp"
#include "SDK/FN_DuplicateResOutMesh_structs.hpp"
#include "SDK/FN_DuplicateResOutMesh_classes.hpp"
#include "SDK/FN_DuplicateResOutMesh_parameters.hpp"
#include "SDK/FN_PBWA_S1_RoofO_structs.hpp"
#include "SDK/FN_PBWA_S1_RoofO_classes.hpp"
#include "SDK/FN_PBWA_S1_RoofO_parameters.hpp"
#include "SDK/FN_VinderTech_GliderChute_AnimBP_structs.hpp"
#include "SDK/FN_VinderTech_GliderChute_AnimBP_classes.hpp"
#include "SDK/FN_VinderTech_GliderChute_AnimBP_parameters.hpp"
#include "SDK/FN_AnimNotify_PlayPlayerLandsWindEffect_structs.hpp"
#include "SDK/FN_AnimNotify_PlayPlayerLandsWindEffect_classes.hpp"
#include "SDK/FN_AnimNotify_PlayPlayerLandsWindEffect_parameters.hpp"
#include "SDK/FN_AnimNotify_FootStep_structs.hpp"
#include "SDK/FN_AnimNotify_FootStep_classes.hpp"
#include "SDK/FN_AnimNotify_FootStep_parameters.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Orange_structs.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Orange_classes.hpp"
#include "SDK/FN_Border_HordeZoneLabel_Orange_parameters.hpp"
#include "SDK/FN_AnimNotify_FootStep_Left_structs.hpp"
#include "SDK/FN_AnimNotify_FootStep_Left_classes.hpp"
#include "SDK/FN_AnimNotify_FootStep_Left_parameters.hpp"
#include "SDK/FN_PBWA_S1_RoofS_structs.hpp"
#include "SDK/FN_PBWA_S1_RoofS_classes.hpp"
#include "SDK/FN_PBWA_S1_RoofS_parameters.hpp"
#include "SDK/FN_AnimNotify_FootStep_Right_structs.hpp"
#include "SDK/FN_AnimNotify_FootStep_Right_classes.hpp"
#include "SDK/FN_AnimNotify_FootStep_Right_parameters.hpp"
#include "SDK/FN_BP_AnimNotify_CameraShake_structs.hpp"
#include "SDK/FN_BP_AnimNotify_CameraShake_classes.hpp"
#include "SDK/FN_BP_AnimNotify_CameraShake_parameters.hpp"
#include "SDK/FN_BP_GCSteps_structs.hpp"
#include "SDK/FN_BP_GCSteps_classes.hpp"
#include "SDK/FN_BP_GCSteps_parameters.hpp"
#include "SDK/FN_PBWA_S1_StairF_structs.hpp"
#include "SDK/FN_PBWA_S1_StairF_classes.hpp"
#include "SDK/FN_PBWA_S1_StairF_parameters.hpp"
#include "SDK/FN_BluGloRequestHandler_structs.hpp"
#include "SDK/FN_BluGloRequestHandler_classes.hpp"
#include "SDK/FN_BluGloRequestHandler_parameters.hpp"
#include "SDK/FN_PBWA_S1_StairR_structs.hpp"
#include "SDK/FN_PBWA_S1_StairR_classes.hpp"
#include "SDK/FN_PBWA_S1_StairR_parameters.hpp"
#include "SDK/FN_PBWA_S1_StairT_structs.hpp"
#include "SDK/FN_PBWA_S1_StairT_classes.hpp"
#include "SDK/FN_PBWA_S1_StairT_parameters.hpp"
#include "SDK/FN_B_PlayerHealthDamage_CameraLensEffect_structs.hpp"
#include "SDK/FN_B_PlayerHealthDamage_CameraLensEffect_classes.hpp"
#include "SDK/FN_B_PlayerHealthDamage_CameraLensEffect_parameters.hpp"
#include "SDK/FN_B_PlayerShieldDamage_CameraLensEffect_structs.hpp"
#include "SDK/FN_B_PlayerShieldDamage_CameraLensEffect_classes.hpp"
#include "SDK/FN_B_PlayerShieldDamage_CameraLensEffect_parameters.hpp"
#include "SDK/FN__WaterMeshBlueprintMaster_structs.hpp"
#include "SDK/FN__WaterMeshBlueprintMaster_classes.hpp"
#include "SDK/FN__WaterMeshBlueprintMaster_parameters.hpp"
#include "SDK/FN_BP_Waterfall_structs.hpp"
#include "SDK/FN_BP_Waterfall_classes.hpp"
#include "SDK/FN_BP_Waterfall_parameters.hpp"
#include "SDK/FN_s_pool_floor_structs.hpp"
#include "SDK/FN_s_pool_floor_classes.hpp"
#include "SDK/FN_s_pool_floor_parameters.hpp"
#include "SDK/FN_S_Pool_Inner_structs.hpp"
#include "SDK/FN_S_Pool_Inner_classes.hpp"
#include "SDK/FN_S_Pool_Inner_parameters.hpp"
#include "SDK/FN_S_Pool_Outer_structs.hpp"
#include "SDK/FN_S_Pool_Outer_classes.hpp"
#include "SDK/FN_S_Pool_Outer_parameters.hpp"
#include "SDK/FN_S_Stream_2W_GeoSlope_2L_a_structs.hpp"
#include "SDK/FN_S_Stream_2W_GeoSlope_2L_a_classes.hpp"
#include "SDK/FN_S_Stream_2W_GeoSlope_2L_a_parameters.hpp"
#include "SDK/FN_S_Stream_2W_SBend_3L_a_structs.hpp"
#include "SDK/FN_S_Stream_2W_SBend_3L_a_classes.hpp"
#include "SDK/FN_S_Stream_2W_SBend_3L_a_parameters.hpp"
#include "SDK/FN_S_Stream_2W_Straight_05L_a_structs.hpp"
#include "SDK/FN_S_Stream_2W_Straight_05L_a_classes.hpp"
#include "SDK/FN_S_Stream_2W_Straight_05L_a_parameters.hpp"
#include "SDK/FN_S_Stream_2W_Straight_1L_a_structs.hpp"
#include "SDK/FN_S_Stream_2W_Straight_1L_a_classes.hpp"
#include "SDK/FN_S_Stream_2W_Straight_1L_a_parameters.hpp"
#include "SDK/FN_S_Stream_2W_Straight_2L_a_structs.hpp"
#include "SDK/FN_S_Stream_2W_Straight_2L_a_classes.hpp"
#include "SDK/FN_S_Stream_2W_Straight_2L_a_parameters.hpp"
#include "SDK/FN_S_Stream_2W_ToShoreline_1L_a_structs.hpp"
#include "SDK/FN_S_Stream_2W_ToShoreline_1L_a_classes.hpp"
#include "SDK/FN_S_Stream_2W_ToShoreline_1L_a_parameters.hpp"
#include "SDK/FN_S_Stream_2W_TSection_2L_a_structs.hpp"
#include "SDK/FN_S_Stream_2W_TSection_2L_a_classes.hpp"
#include "SDK/FN_S_Stream_2W_TSection_2L_a_parameters.hpp"
#include "SDK/FN_S_Stream_2W_Turn_2L_a_structs.hpp"
#include "SDK/FN_S_Stream_2W_Turn_2L_a_classes.hpp"
#include "SDK/FN_S_Stream_2W_Turn_2L_a_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_1x1_structs.hpp"
#include "SDK/FN_S_Water_Shallow_1x1_classes.hpp"
#include "SDK/FN_S_Water_Shallow_1x1_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_1x2_structs.hpp"
#include "SDK/FN_S_Water_Shallow_1x2_classes.hpp"
#include "SDK/FN_S_Water_Shallow_1x2_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_1x3_structs.hpp"
#include "SDK/FN_S_Water_Shallow_1x3_classes.hpp"
#include "SDK/FN_S_Water_Shallow_1x3_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_1x4_structs.hpp"
#include "SDK/FN_S_Water_Shallow_1x4_classes.hpp"
#include "SDK/FN_S_Water_Shallow_1x4_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_1x5_structs.hpp"
#include "SDK/FN_S_Water_Shallow_1x5_classes.hpp"
#include "SDK/FN_S_Water_Shallow_1x5_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_2x2_structs.hpp"
#include "SDK/FN_S_Water_Shallow_2x2_classes.hpp"
#include "SDK/FN_S_Water_Shallow_2x2_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_2x3_structs.hpp"
#include "SDK/FN_S_Water_Shallow_2x3_classes.hpp"
#include "SDK/FN_S_Water_Shallow_2x3_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_2x4_structs.hpp"
#include "SDK/FN_S_Water_Shallow_2x4_classes.hpp"
#include "SDK/FN_S_Water_Shallow_2x4_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_2x5_structs.hpp"
#include "SDK/FN_S_Water_Shallow_2x5_classes.hpp"
#include "SDK/FN_S_Water_Shallow_2x5_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_3x3_structs.hpp"
#include "SDK/FN_S_Water_Shallow_3x3_classes.hpp"
#include "SDK/FN_S_Water_Shallow_3x3_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_3x4_structs.hpp"
#include "SDK/FN_S_Water_Shallow_3x4_classes.hpp"
#include "SDK/FN_S_Water_Shallow_3x4_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_3x5_structs.hpp"
#include "SDK/FN_S_Water_Shallow_3x5_classes.hpp"
#include "SDK/FN_S_Water_Shallow_3x5_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_4x4_structs.hpp"
#include "SDK/FN_S_Water_Shallow_4x4_classes.hpp"
#include "SDK/FN_S_Water_Shallow_4x4_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_4x5_structs.hpp"
#include "SDK/FN_S_Water_Shallow_4x5_classes.hpp"
#include "SDK/FN_S_Water_Shallow_4x5_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_5x5_structs.hpp"
#include "SDK/FN_S_Water_Shallow_5x5_classes.hpp"
#include "SDK/FN_S_Water_Shallow_5x5_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x1_structs.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x1_classes.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x1_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x2_structs.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x2_classes.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x2_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x3_structs.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x3_classes.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x3_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x4_structs.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x4_classes.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x4_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x5_structs.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x5_classes.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x5_parameters.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x_Half_structs.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x_Half_classes.hpp"
#include "SDK/FN_S_Water_Shallow_Half_x_Half_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x1_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x1_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x1_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x2_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x2_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x2_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x3_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x3_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x3_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x4_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x4_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x4_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x5_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x5_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_1x5_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x2_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x2_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x2_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x3_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x3_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x3_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x4_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x4_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x4_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x5_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x5_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_2x5_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_3x3_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_3x3_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_3x3_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_3x4_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_3x4_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_3x4_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_3x5_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_3x5_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_3x5_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_4x4_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_4x4_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_4x4_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_4x5_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_4x5_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_4x5_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_5x5_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_5x5_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_5x5_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x1_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x1_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x1_parameters.hpp"
#include "SDK/FN_FlowMapMaterials_structs.hpp"
#include "SDK/FN_FlowMapMaterials_classes.hpp"
#include "SDK/FN_FlowMapMaterials_parameters.hpp"
#include "SDK/FN_PBWA_S1_StairW_structs.hpp"
#include "SDK/FN_PBWA_S1_StairW_classes.hpp"
#include "SDK/FN_PBWA_S1_StairW_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x2_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x2_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x2_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x3_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x3_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x3_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x4_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x4_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x4_parameters.hpp"
#include "SDK/FN_WaterMeshAssetsToReplace_structs.hpp"
#include "SDK/FN_WaterMeshAssetsToReplace_classes.hpp"
#include "SDK/FN_WaterMeshAssetsToReplace_parameters.hpp"
#include "SDK/FN_PlayerAnimAssets_Struct_structs.hpp"
#include "SDK/FN_PlayerAnimAssets_Struct_classes.hpp"
#include "SDK/FN_PlayerAnimAssets_Struct_parameters.hpp"
#include "SDK/FN_Direction_structs.hpp"
#include "SDK/FN_Direction_classes.hpp"
#include "SDK/FN_Direction_parameters.hpp"
#include "SDK/FN_ECardinalDirection_structs.hpp"
#include "SDK/FN_ECardinalDirection_classes.hpp"
#include "SDK/FN_ECardinalDirection_parameters.hpp"
#include "SDK/FN_ArrayOfWaterMeshWaterBlueprintPairs_structs.hpp"
#include "SDK/FN_ArrayOfWaterMeshWaterBlueprintPairs_classes.hpp"
#include "SDK/FN_ArrayOfWaterMeshWaterBlueprintPairs_parameters.hpp"
#include "SDK/FN_FortMetaNavAreaDef_structs.hpp"
#include "SDK/FN_FortMetaNavAreaDef_classes.hpp"
#include "SDK/FN_FortMetaNavAreaDef_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x5_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x5_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x5_parameters.hpp"
#include "SDK/FN_NavLink_RoofC_structs.hpp"
#include "SDK/FN_NavLink_RoofC_classes.hpp"
#include "SDK/FN_NavLink_RoofC_parameters.hpp"
#include "SDK/FN_BluGloManager_structs.hpp"
#include "SDK/FN_BluGloManager_classes.hpp"
#include "SDK/FN_BluGloManager_parameters.hpp"
#include "SDK/FN_StaticMeshMaterialArrayCombo_structs.hpp"
#include "SDK/FN_StaticMeshMaterialArrayCombo_classes.hpp"
#include "SDK/FN_StaticMeshMaterialArrayCombo_parameters.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x_Half_structs.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x_Half_classes.hpp"
#include "SDK/FN_S_TRV_Water_Shallow_Half_x_Half_parameters.hpp"
#include "SDK/FN_Tiered_BluGlo_Parent_structs.hpp"
#include "SDK/FN_Tiered_BluGlo_Parent_classes.hpp"
#include "SDK/FN_Tiered_BluGlo_Parent_parameters.hpp"
#include "SDK/FN_FortNavArea_JumpDown_structs.hpp"
#include "SDK/FN_FortNavArea_JumpDown_classes.hpp"
#include "SDK/FN_FortNavArea_JumpDown_parameters.hpp"
#include "SDK/FN_FortNavArea_JumpDownSmashable_structs.hpp"
#include "SDK/FN_FortNavArea_JumpDownSmashable_classes.hpp"
#include "SDK/FN_FortNavArea_JumpDownSmashable_parameters.hpp"
#include "SDK/FN_B_RiftImpact_CameraShake_structs.hpp"
#include "SDK/FN_B_RiftImpact_CameraShake_classes.hpp"
#include "SDK/FN_B_RiftImpact_CameraShake_parameters.hpp"
#include "SDK/FN_FortNavArea_JumpDownSmashable2_structs.hpp"
#include "SDK/FN_FortNavArea_JumpDownSmashable2_classes.hpp"
#include "SDK/FN_FortNavArea_JumpDownSmashable2_parameters.hpp"
#include "SDK/FN_AnimNotifyState_HolsterWeapon_structs.hpp"
#include "SDK/FN_AnimNotifyState_HolsterWeapon_classes.hpp"
#include "SDK/FN_AnimNotifyState_HolsterWeapon_parameters.hpp"
#include "SDK/FN_FortNavArea_JumpDownSmashable3_structs.hpp"
#include "SDK/FN_FortNavArea_JumpDownSmashable3_classes.hpp"
#include "SDK/FN_FortNavArea_JumpDownSmashable3_parameters.hpp"
#include "SDK/FN_GE_Resist_Damage_AoE_structs.hpp"
#include "SDK/FN_GE_Resist_Damage_AoE_classes.hpp"
#include "SDK/FN_GE_Resist_Damage_AoE_parameters.hpp"
#include "SDK/FN_GE_SpecialEvent_Halloween_PumpkinHead_structs.hpp"
#include "SDK/FN_GE_SpecialEvent_Halloween_PumpkinHead_classes.hpp"
#include "SDK/FN_GE_SpecialEvent_Halloween_PumpkinHead_parameters.hpp"
#include "SDK/FN_En_ShellTypes_01_structs.hpp"
#include "SDK/FN_En_ShellTypes_01_classes.hpp"
#include "SDK/FN_En_ShellTypes_01_parameters.hpp"
#include "SDK/FN_InterfacePlayerPawn_structs.hpp"
#include "SDK/FN_InterfacePlayerPawn_classes.hpp"
#include "SDK/FN_InterfacePlayerPawn_parameters.hpp"
#include "SDK/FN_GE_Athena_Revive_structs.hpp"
#include "SDK/FN_GE_Athena_Revive_classes.hpp"
#include "SDK/FN_GE_Athena_Revive_parameters.hpp"
#include "SDK/FN_SkillTreeGroupBG-H_structs.hpp"
#include "SDK/FN_SkillTreeGroupBG-H_classes.hpp"
#include "SDK/FN_SkillTreeGroupBG-H_parameters.hpp"
#include "SDK/FN_PlayerWindParticleEmitters_structs.hpp"
#include "SDK/FN_PlayerWindParticleEmitters_classes.hpp"
#include "SDK/FN_PlayerWindParticleEmitters_parameters.hpp"
#include "SDK/FN_GE_Generic_Revive_structs.hpp"
#include "SDK/FN_GE_Generic_Revive_classes.hpp"
#include "SDK/FN_GE_Generic_Revive_parameters.hpp"
#include "SDK/FN_NodeBackground_structs.hpp"
#include "SDK/FN_NodeBackground_classes.hpp"
#include "SDK/FN_NodeBackground_parameters.hpp"
#include "SDK/FN_GE_Generic_HealthRegen_structs.hpp"
#include "SDK/FN_GE_Generic_HealthRegen_classes.hpp"
#include "SDK/FN_GE_Generic_HealthRegen_parameters.hpp"
#include "SDK/FN_EnumEventWorldItemDrop_structs.hpp"
#include "SDK/FN_EnumEventWorldItemDrop_classes.hpp"
#include "SDK/FN_EnumEventWorldItemDrop_parameters.hpp"
#include "SDK/FN_SurvivorBadgeTypes_structs.hpp"
#include "SDK/FN_SurvivorBadgeTypes_classes.hpp"
#include "SDK/FN_SurvivorBadgeTypes_parameters.hpp"
#include "SDK/FN_GE_ExtraLifeRevive_structs.hpp"
#include "SDK/FN_GE_ExtraLifeRevive_classes.hpp"
#include "SDK/FN_GE_ExtraLifeRevive_parameters.hpp"
#include "SDK/FN_PlayerTakeDamage_CameraShake_structs.hpp"
#include "SDK/FN_PlayerTakeDamage_CameraShake_classes.hpp"
#include "SDK/FN_PlayerTakeDamage_CameraShake_parameters.hpp"
#include "SDK/FN_SkillTreeGroupBG-H-Wide_structs.hpp"
#include "SDK/FN_SkillTreeGroupBG-H-Wide_classes.hpp"
#include "SDK/FN_SkillTreeGroupBG-H-Wide_parameters.hpp"
#include "SDK/FN_DecoTool_structs.hpp"
#include "SDK/FN_DecoTool_classes.hpp"
#include "SDK/FN_DecoTool_parameters.hpp"
#include "SDK/FN_B_Rift_Portals_structs.hpp"
#include "SDK/FN_B_Rift_Portals_classes.hpp"
#include "SDK/FN_B_Rift_Portals_parameters.hpp"
#include "SDK/FN_TracerGeneric_structs.hpp"
#include "SDK/FN_TracerGeneric_classes.hpp"
#include "SDK/FN_TracerGeneric_parameters.hpp"
#include "SDK/FN_GE_Generic_ShieldRegen_structs.hpp"
#include "SDK/FN_GE_Generic_ShieldRegen_classes.hpp"
#include "SDK/FN_GE_Generic_ShieldRegen_parameters.hpp"
#include "SDK/FN_GE_HealthRegen_Delay_Damaged_structs.hpp"
#include "SDK/FN_GE_HealthRegen_Delay_Damaged_classes.hpp"
#include "SDK/FN_GE_HealthRegen_Delay_Damaged_parameters.hpp"
#include "SDK/FN_Car_RV_structs.hpp"
#include "SDK/FN_Car_RV_classes.hpp"
#include "SDK/FN_Car_RV_parameters.hpp"
#include "SDK/FN_Fortnite_M_Avg_Player_AnimBlueprint_structs.hpp"
#include "SDK/FN_Fortnite_M_Avg_Player_AnimBlueprint_classes.hpp"
#include "SDK/FN_Fortnite_M_Avg_Player_AnimBlueprint_parameters.hpp"
#include "SDK/FN_PlayerPawn_Generic_structs.hpp"
#include "SDK/FN_PlayerPawn_Generic_classes.hpp"
#include "SDK/FN_PlayerPawn_Generic_parameters.hpp"
#include "SDK/FN_Struct_SurvivorScriptedAbilities_structs.hpp"
#include "SDK/FN_Struct_SurvivorScriptedAbilities_classes.hpp"
#include "SDK/FN_Struct_SurvivorScriptedAbilities_parameters.hpp"
#include "SDK/FN_GE_ShieldRegen_Delay_Damaged_structs.hpp"
#include "SDK/FN_GE_ShieldRegen_Delay_Damaged_classes.hpp"
#include "SDK/FN_GE_ShieldRegen_Delay_Damaged_parameters.hpp"
#include "SDK/FN_PlayerPawn_Athena_Generic_structs.hpp"
#include "SDK/FN_PlayerPawn_Athena_Generic_classes.hpp"
#include "SDK/FN_PlayerPawn_Athena_Generic_parameters.hpp"
#include "SDK/FN_SpeechBubbleWidget_structs.hpp"
#include "SDK/FN_SpeechBubbleWidget_classes.hpp"
#include "SDK/FN_SpeechBubbleWidget_parameters.hpp"
#include "SDK/FN_SkillTreeGroupBG-Vert_structs.hpp"
#include "SDK/FN_SkillTreeGroupBG-Vert_classes.hpp"
#include "SDK/FN_SkillTreeGroupBG-Vert_parameters.hpp"
#include "SDK/FN_PlayerPawn_Outlander_structs.hpp"
#include "SDK/FN_PlayerPawn_Outlander_classes.hpp"
#include "SDK/FN_PlayerPawn_Outlander_parameters.hpp"
#include "SDK/FN_ProjectileHuskRanged_structs.hpp"
#include "SDK/FN_ProjectileHuskRanged_classes.hpp"
#include "SDK/FN_ProjectileHuskRanged_parameters.hpp"
#include "SDK/FN_B_Ranged_Generic_structs.hpp"
#include "SDK/FN_B_Ranged_Generic_classes.hpp"
#include "SDK/FN_B_Ranged_Generic_parameters.hpp"
#include "SDK/FN_PlayerPawn_Athena_structs.hpp"
#include "SDK/FN_PlayerPawn_Athena_classes.hpp"
#include "SDK/FN_PlayerPawn_Athena_parameters.hpp"
#include "SDK/FN_WindManager_structs.hpp"
#include "SDK/FN_WindManager_classes.hpp"
#include "SDK/FN_WindManager_parameters.hpp"
#include "SDK/FN_EnemyPawn_Parent_structs.hpp"
#include "SDK/FN_EnemyPawn_Parent_classes.hpp"
#include "SDK/FN_EnemyPawn_Parent_parameters.hpp"
#include "SDK/FN_AnimNotify_PlayFeedbackLine_structs.hpp"
#include "SDK/FN_AnimNotify_PlayFeedbackLine_classes.hpp"
#include "SDK/FN_AnimNotify_PlayFeedbackLine_parameters.hpp"
#include "SDK/FN_B_PlayerHealthDamage_LensEffect_Direction_structs.hpp"
#include "SDK/FN_B_PlayerHealthDamage_LensEffect_Direction_classes.hpp"
#include "SDK/FN_B_PlayerHealthDamage_LensEffect_Direction_parameters.hpp"
#include "SDK/FN_MissionBlueprintFunctionLibrary_structs.hpp"
#include "SDK/FN_MissionBlueprintFunctionLibrary_classes.hpp"
#include "SDK/FN_MissionBlueprintFunctionLibrary_parameters.hpp"
#include "SDK/FN_B_PlayerShieldDamage_LensEffect_Direction_structs.hpp"
#include "SDK/FN_B_PlayerShieldDamage_LensEffect_Direction_classes.hpp"
#include "SDK/FN_B_PlayerShieldDamage_LensEffect_Direction_parameters.hpp"
#include "SDK/FN_PlayerCameraMode1P_structs.hpp"
#include "SDK/FN_PlayerCameraMode1P_classes.hpp"
#include "SDK/FN_PlayerCameraMode1P_parameters.hpp"
#include "SDK/FN_PlayerCameraMode1P_Targeting_structs.hpp"
#include "SDK/FN_PlayerCameraMode1P_Targeting_classes.hpp"
#include "SDK/FN_PlayerCameraMode1P_Targeting_parameters.hpp"
#include "SDK/FN_B_DtB_FloatingRift_structs.hpp"
#include "SDK/FN_B_DtB_FloatingRift_classes.hpp"
#include "SDK/FN_B_DtB_FloatingRift_parameters.hpp"
#include "SDK/FN_SharedMissionLists_structs.hpp"
#include "SDK/FN_SharedMissionLists_classes.hpp"
#include "SDK/FN_SharedMissionLists_parameters.hpp"
#include "SDK/FN_AIGoalManager_structs.hpp"
#include "SDK/FN_AIGoalManager_classes.hpp"
#include "SDK/FN_AIGoalManager_parameters.hpp"
#include "SDK/FN_MainPlayerCamera_structs.hpp"
#include "SDK/FN_MainPlayerCamera_classes.hpp"
#include "SDK/FN_MainPlayerCamera_parameters.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Blaster_structs.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Blaster_classes.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Blaster_parameters.hpp"
#include "SDK/FN_StructuralBuildingsAroundPrimaryAssignments_structs.hpp"
#include "SDK/FN_StructuralBuildingsAroundPrimaryAssignments_classes.hpp"
#include "SDK/FN_StructuralBuildingsAroundPrimaryAssignments_parameters.hpp"
#include "SDK/FN_EnemyPawn_Interface_structs.hpp"
#include "SDK/FN_EnemyPawn_Interface_classes.hpp"
#include "SDK/FN_EnemyPawn_Interface_parameters.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Flinger_structs.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Flinger_classes.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Flinger_parameters.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Bombshell_Poison_structs.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Bombshell_Poison_classes.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Bombshell_Poison_parameters.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Bombshell_structs.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Bombshell_classes.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Bombshell_parameters.hpp"
#include "SDK/FN_StructuralBuildingsInCachedPathOfPrimaryAssignments_structs.hpp"
#include "SDK/FN_StructuralBuildingsInCachedPathOfPrimaryAssignments_classes.hpp"
#include "SDK/FN_StructuralBuildingsInCachedPathOfPrimaryAssignments_parameters.hpp"
#include "SDK/FN_v1_PlayerCameraModeBase_structs.hpp"
#include "SDK/FN_v1_PlayerCameraModeBase_classes.hpp"
#include "SDK/FN_v1_PlayerCameraModeBase_parameters.hpp"
#include "SDK/FN_v1_PlayerCameraModeMelee_structs.hpp"
#include "SDK/FN_v1_PlayerCameraModeMelee_classes.hpp"
#include "SDK/FN_v1_PlayerCameraModeMelee_parameters.hpp"
#include "SDK/FN_v1_PlayerCameraModeRanged_structs.hpp"
#include "SDK/FN_v1_PlayerCameraModeRanged_classes.hpp"
#include "SDK/FN_v1_PlayerCameraModeRanged_parameters.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingPistol_structs.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingPistol_classes.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingPistol_parameters.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingRifle_structs.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingRifle_classes.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingRifle_parameters.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingScope_structs.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingScope_classes.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingScope_parameters.hpp"
#include "SDK/FN_CinematicCamera_MatineeTransition_structs.hpp"
#include "SDK/FN_CinematicCamera_MatineeTransition_classes.hpp"
#include "SDK/FN_CinematicCamera_MatineeTransition_parameters.hpp"
#include "SDK/FN_Default3PCamera_structs.hpp"
#include "SDK/FN_Default3PCamera_classes.hpp"
#include "SDK/FN_Default3PCamera_parameters.hpp"
#include "SDK/FN_Ranged3PCamera_structs.hpp"
#include "SDK/FN_Ranged3PCamera_classes.hpp"
#include "SDK/FN_Ranged3PCamera_parameters.hpp"
#include "SDK/FN_Sniper3PCamera_structs.hpp"
#include "SDK/FN_Sniper3PCamera_classes.hpp"
#include "SDK/FN_Sniper3PCamera_parameters.hpp"
#include "SDK/FN_Targeting3PCamera_structs.hpp"
#include "SDK/FN_Targeting3PCamera_classes.hpp"
#include "SDK/FN_Targeting3PCamera_parameters.hpp"
#include "SDK/FN_Targeting3PCamera_LongRange_structs.hpp"
#include "SDK/FN_Targeting3PCamera_LongRange_classes.hpp"
#include "SDK/FN_Targeting3PCamera_LongRange_parameters.hpp"
#include "SDK/FN_Targeting3PCamera_MidRange_structs.hpp"
#include "SDK/FN_Targeting3PCamera_MidRange_classes.hpp"
#include "SDK/FN_Targeting3PCamera_MidRange_parameters.hpp"
#include "SDK/FN_Targeting3PCamera_Scope_structs.hpp"
#include "SDK/FN_Targeting3PCamera_Scope_classes.hpp"
#include "SDK/FN_Targeting3PCamera_Scope_parameters.hpp"
#include "SDK/FN_Targeting3PCamera_VeryShortRange_structs.hpp"
#include "SDK/FN_Targeting3PCamera_VeryShortRange_classes.hpp"
#include "SDK/FN_Targeting3PCamera_VeryShortRange_parameters.hpp"
#include "SDK/FN_v2_PlayerCameraModeBase_structs.hpp"
#include "SDK/FN_v2_PlayerCameraModeBase_classes.hpp"
#include "SDK/FN_v2_PlayerCameraModeBase_parameters.hpp"
#include "SDK/FN_v2_PlayerCameraModeMelee_structs.hpp"
#include "SDK/FN_v2_PlayerCameraModeMelee_classes.hpp"
#include "SDK/FN_v2_PlayerCameraModeMelee_parameters.hpp"
#include "SDK/FN_v2_PlayerCameraModeRanged_structs.hpp"
#include "SDK/FN_v2_PlayerCameraModeRanged_classes.hpp"
#include "SDK/FN_v2_PlayerCameraModeRanged_parameters.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingPistol_structs.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingPistol_classes.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingPistol_parameters.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingRifle_structs.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingRifle_classes.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingRifle_parameters.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingScope_structs.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingScope_classes.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingScope_parameters.hpp"
#include "SDK/FN_B_DtB_LightningZap_structs.hpp"
#include "SDK/FN_B_DtB_LightningZap_classes.hpp"
#include "SDK/FN_B_DtB_LightningZap_parameters.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingVeryShortRange_structs.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingVeryShortRange_classes.hpp"
#include "SDK/FN_v2_PlayerCameraModeTargetingVeryShortRange_parameters.hpp"
#include "SDK/FN_v3_PlayerCameraModeBase_structs.hpp"
#include "SDK/FN_v3_PlayerCameraModeBase_classes.hpp"
#include "SDK/FN_v3_PlayerCameraModeBase_parameters.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingVeryShortRange_structs.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingVeryShortRange_classes.hpp"
#include "SDK/FN_v1_PlayerCameraModeTargetingVeryShortRange_parameters.hpp"
#include "SDK/FN_GCN_Athena_Bush_structs.hpp"
#include "SDK/FN_GCN_Athena_Bush_classes.hpp"
#include "SDK/FN_GCN_Athena_Bush_parameters.hpp"
#include "SDK/FN_GCNL_PurpleStuff_structs.hpp"
#include "SDK/FN_GCNL_PurpleStuff_classes.hpp"
#include "SDK/FN_GCNL_PurpleStuff_parameters.hpp"
#include "SDK/FN_GCNS_GM_OnDeathExplosion_structs.hpp"
#include "SDK/FN_GCNS_GM_OnDeathExplosion_classes.hpp"
#include "SDK/FN_GCNS_GM_OnDeathExplosion_parameters.hpp"
#include "SDK/FN_GCN_RiftZapPlayer_structs.hpp"
#include "SDK/FN_GCN_RiftZapPlayer_classes.hpp"
#include "SDK/FN_GCN_RiftZapPlayer_parameters.hpp"
#include "SDK/FN_GCNStatic_MantisStrike_structs.hpp"
#include "SDK/FN_GCNStatic_MantisStrike_classes.hpp"
#include "SDK/FN_GCNStatic_MantisStrike_parameters.hpp"
#include "SDK/FN_GCNS_Constructor_ContainmentUnitFX_structs.hpp"
#include "SDK/FN_GCNS_Constructor_ContainmentUnitFX_classes.hpp"
#include "SDK/FN_GCNS_Constructor_ContainmentUnitFX_parameters.hpp"
#include "SDK/FN_GCNS_Constructor_ElectrifiedFloors_structs.hpp"
#include "SDK/FN_GCNS_Constructor_ElectrifiedFloors_classes.hpp"
#include "SDK/FN_GCNS_Constructor_ElectrifiedFloors_parameters.hpp"
#include "SDK/FN_GCNS_Constructor_KineticOverload_structs.hpp"
#include "SDK/FN_GCNS_Constructor_KineticOverload_classes.hpp"
#include "SDK/FN_GCNS_Constructor_KineticOverload_parameters.hpp"
#include "SDK/FN_GCNS_Outlander_HeavyStuffDamage_structs.hpp"
#include "SDK/FN_GCNS_Outlander_HeavyStuffDamage_classes.hpp"
#include "SDK/FN_GCNS_Outlander_HeavyStuffDamage_parameters.hpp"
#include "SDK/FN_NearestPlayerContext_structs.hpp"
#include "SDK/FN_NearestPlayerContext_classes.hpp"
#include "SDK/FN_NearestPlayerContext_parameters.hpp"
#include "SDK/FN_GCNS_Cart_InstantHealFX_structs.hpp"
#include "SDK/FN_GCNS_Cart_InstantHealFX_classes.hpp"
#include "SDK/FN_GCNS_Cart_InstantHealFX_parameters.hpp"
#include "SDK/FN_GCNL_Enemy_Frozen_structs.hpp"
#include "SDK/FN_GCNL_Enemy_Frozen_classes.hpp"
#include "SDK/FN_GCNL_Enemy_Frozen_parameters.hpp"
#include "SDK/FN_GCNL_GM_OnLowHealth_Enrage_structs.hpp"
#include "SDK/FN_GCNL_GM_OnLowHealth_Enrage_classes.hpp"
#include "SDK/FN_GCNL_GM_OnLowHealth_Enrage_parameters.hpp"
#include "SDK/FN_GCNL_GM_ReflectDamage_structs.hpp"
#include "SDK/FN_GCNL_GM_ReflectDamage_classes.hpp"
#include "SDK/FN_GCNL_GM_ReflectDamage_parameters.hpp"
#include "SDK/FN_GCNL_GM_MaxHealthIncrease_Major_structs.hpp"
#include "SDK/FN_GCNL_GM_MaxHealthIncrease_Major_classes.hpp"
#include "SDK/FN_GCNL_GM_MaxHealthIncrease_Major_parameters.hpp"
#include "SDK/FN_GCNS_Commando_IfItBleedsTick_structs.hpp"
#include "SDK/FN_GCNS_Commando_IfItBleedsTick_classes.hpp"
#include "SDK/FN_GCNS_Commando_IfItBleedsTick_parameters.hpp"
#include "SDK/FN_GCN_RiftStaminaDrain_structs.hpp"
#include "SDK/FN_GCN_RiftStaminaDrain_classes.hpp"
#include "SDK/FN_GCN_RiftStaminaDrain_parameters.hpp"
#include "SDK/FN_GCN_NPC_Ice_structs.hpp"
#include "SDK/FN_GCN_NPC_Ice_classes.hpp"
#include "SDK/FN_GCN_NPC_Ice_parameters.hpp"
#include "SDK/FN_GCN_TakerMarkedForDeath_structs.hpp"
#include "SDK/FN_GCN_TakerMarkedForDeath_classes.hpp"
#include "SDK/FN_GCN_TakerMarkedForDeath_parameters.hpp"
#include "SDK/FN_GCN_BluGloPylon_BuildSpeed_Activate_structs.hpp"
#include "SDK/FN_GCN_BluGloPylon_BuildSpeed_Activate_classes.hpp"
#include "SDK/FN_GCN_BluGloPylon_BuildSpeed_Activate_parameters.hpp"
#include "SDK/FN_GCN_BluGloPylon_Energy_Activate_structs.hpp"
#include "SDK/FN_GCN_BluGloPylon_Energy_Activate_classes.hpp"
#include "SDK/FN_GCN_BluGloPylon_Energy_Activate_parameters.hpp"
#include "SDK/FN_GCN_BluGloPylon_Health_Activate_structs.hpp"
#include "SDK/FN_GCN_BluGloPylon_Health_Activate_classes.hpp"
#include "SDK/FN_GCN_BluGloPylon_Health_Activate_parameters.hpp"
#include "SDK/FN_GCN_BluGloPylon_RunSpeed_Activate_structs.hpp"
#include "SDK/FN_GCN_BluGloPylon_RunSpeed_Activate_classes.hpp"
#include "SDK/FN_GCN_BluGloPylon_RunSpeed_Activate_parameters.hpp"
#include "SDK/FN_GCN_BluGloPylon_Shield_Activate_structs.hpp"
#include "SDK/FN_GCN_BluGloPylon_Shield_Activate_classes.hpp"
#include "SDK/FN_GCN_BluGloPylon_Shield_Activate_parameters.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Taker_structs.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Taker_classes.hpp"
#include "SDK/FN_CircleAroundPrimaryAssignmentGoals_Taker_parameters.hpp"
#include "SDK/FN_MusicManager_structs.hpp"
#include "SDK/FN_MusicManager_classes.hpp"
#include "SDK/FN_MusicManager_parameters.hpp"
#include "SDK/FN_B_IngameMap_SceneCaptureBlurryNew_structs.hpp"
#include "SDK/FN_B_IngameMap_SceneCaptureBlurryNew_classes.hpp"
#include "SDK/FN_B_IngameMap_SceneCaptureBlurryNew_parameters.hpp"
#include "SDK/FN_B_IngameMap_SceneCaptureNew_structs.hpp"
#include "SDK/FN_B_IngameMap_SceneCaptureNew_classes.hpp"
#include "SDK/FN_B_IngameMap_SceneCaptureNew_parameters.hpp"
#include "SDK/FN_CCTeamStatsGameplayEffect_structs.hpp"
#include "SDK/FN_CCTeamStatsGameplayEffect_classes.hpp"
#include "SDK/FN_CCTeamStatsGameplayEffect_parameters.hpp"
#include "SDK/FN_B_FortGlobalAbilityTargetingActor_structs.hpp"
#include "SDK/FN_B_FortGlobalAbilityTargetingActor_classes.hpp"
#include "SDK/FN_B_FortGlobalAbilityTargetingActor_parameters.hpp"
#include "SDK/FN_Athena_PlayerCameraModeBase_structs.hpp"
#include "SDK/FN_Athena_PlayerCameraModeBase_classes.hpp"
#include "SDK/FN_Athena_PlayerCameraModeBase_parameters.hpp"
#include "SDK/FN_Athena_PlayerCamera_DBNO_structs.hpp"
#include "SDK/FN_Athena_PlayerCamera_DBNO_classes.hpp"
#include "SDK/FN_Athena_PlayerCamera_DBNO_parameters.hpp"
#include "SDK/FN_Athena_PlayerCameraModeMelee_structs.hpp"
#include "SDK/FN_Athena_PlayerCameraModeMelee_classes.hpp"
#include "SDK/FN_Athena_PlayerCameraModeMelee_parameters.hpp"
#include "SDK/FN_Athena_PlayerCameraModeSkydiveDive_structs.hpp"
#include "SDK/FN_Athena_PlayerCameraModeSkydiveDive_classes.hpp"
#include "SDK/FN_Athena_PlayerCameraModeSkydiveDive_parameters.hpp"
#include "SDK/FN_Athena_PlayerCameraModeSkydiveGlide_structs.hpp"
#include "SDK/FN_Athena_PlayerCameraModeSkydiveGlide_classes.hpp"
#include "SDK/FN_Athena_PlayerCameraModeSkydiveGlide_parameters.hpp"
#include "SDK/FN_Athena_PlayerCameraModeSkydiveParachute_structs.hpp"
#include "SDK/FN_Athena_PlayerCameraModeSkydiveParachute_classes.hpp"
#include "SDK/FN_Athena_PlayerCameraModeSkydiveParachute_parameters.hpp"
#include "SDK/FN_PlayerCameraMode_DBNO_structs.hpp"
#include "SDK/FN_PlayerCameraMode_DBNO_classes.hpp"
#include "SDK/FN_PlayerCameraMode_DBNO_parameters.hpp"
#include "SDK/FN_v3_PlayerCameraModeMelee_structs.hpp"
#include "SDK/FN_v3_PlayerCameraModeMelee_classes.hpp"
#include "SDK/FN_v3_PlayerCameraModeMelee_parameters.hpp"
#include "SDK/FN_v3_PlayerCameraModeRanged_structs.hpp"
#include "SDK/FN_v3_PlayerCameraModeRanged_classes.hpp"
#include "SDK/FN_v3_PlayerCameraModeRanged_parameters.hpp"
#include "SDK/FN_UsedPlacementActorsContext_structs.hpp"
#include "SDK/FN_UsedPlacementActorsContext_classes.hpp"
#include "SDK/FN_UsedPlacementActorsContext_parameters.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingAssault_structs.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingAssault_classes.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingAssault_parameters.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingPistol_structs.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingPistol_classes.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingPistol_parameters.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingRifle_structs.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingRifle_classes.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingRifle_parameters.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingScope_structs.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingScope_classes.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingScope_parameters.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingVeryShortRange_structs.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingVeryShortRange_classes.hpp"
#include "SDK/FN_v3_PlayerCameraModeTargetingVeryShortRange_parameters.hpp"
#include "SDK/FN_CombatManager_structs.hpp"
#include "SDK/FN_CombatManager_classes.hpp"
#include "SDK/FN_CombatManager_parameters.hpp"
#include "SDK/FN_UsedPrimaryMissionPlacementActorsContext_structs.hpp"
#include "SDK/FN_UsedPrimaryMissionPlacementActorsContext_classes.hpp"
#include "SDK/FN_UsedPrimaryMissionPlacementActorsContext_parameters.hpp"
#include "SDK/FN_TeamID_Default_structs.hpp"
#include "SDK/FN_TeamID_Default_classes.hpp"
#include "SDK/FN_TeamID_Default_parameters.hpp"
#include "SDK/FN_TeamID_Lime_structs.hpp"
#include "SDK/FN_TeamID_Lime_classes.hpp"
#include "SDK/FN_TeamID_Lime_parameters.hpp"
#include "SDK/FN_TeamID_Orange_structs.hpp"
#include "SDK/FN_TeamID_Orange_classes.hpp"
#include "SDK/FN_TeamID_Orange_parameters.hpp"
#include "SDK/FN_UIMapManager_structs.hpp"
#include "SDK/FN_UIMapManager_classes.hpp"
#include "SDK/FN_UIMapManager_parameters.hpp"
#include "SDK/FN_LF_SurvivorShelterIndoor_structs.hpp"
#include "SDK/FN_LF_SurvivorShelterIndoor_classes.hpp"
#include "SDK/FN_LF_SurvivorShelterIndoor_parameters.hpp"
#include "SDK/FN_UsedProximityDangerZoneContext_structs.hpp"
#include "SDK/FN_UsedProximityDangerZoneContext_classes.hpp"
#include "SDK/FN_UsedProximityDangerZoneContext_parameters.hpp"
#include "SDK/FN_LF_SurvivorShelterOutdoor1_structs.hpp"
#include "SDK/FN_LF_SurvivorShelterOutdoor1_classes.hpp"
#include "SDK/FN_LF_SurvivorShelterOutdoor1_parameters.hpp"
#include "SDK/FN_LF_SurvivorShelterOutdoor3_structs.hpp"
#include "SDK/FN_LF_SurvivorShelterOutdoor3_classes.hpp"
#include "SDK/FN_LF_SurvivorShelterOutdoor3_parameters.hpp"
#include "SDK/FN_PlayerSpawnPlacementActor_structs.hpp"
#include "SDK/FN_PlayerSpawnPlacementActor_classes.hpp"
#include "SDK/FN_PlayerSpawnPlacementActor_parameters.hpp"
#include "SDK/FN_GCNS_Taker_TRV_Lifesuck_structs.hpp"
#include "SDK/FN_GCNS_Taker_TRV_Lifesuck_classes.hpp"
#include "SDK/FN_GCNS_Taker_TRV_Lifesuck_parameters.hpp"
#include "SDK/FN_GCNS_GM_OnDeath_SpeedUpFriends_structs.hpp"
#include "SDK/FN_GCNS_GM_OnDeath_SpeedUpFriends_classes.hpp"
#include "SDK/FN_GCNS_GM_OnDeath_SpeedUpFriends_parameters.hpp"
#include "SDK/FN_GCNS_GM_OnDeathAlliedHeal_structs.hpp"
#include "SDK/FN_GCNS_GM_OnDeathAlliedHeal_classes.hpp"
#include "SDK/FN_GCNS_GM_OnDeathAlliedHeal_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B_parameters.hpp"
#include "SDK/FN_GCNS_GM_OnDeathAreaHeal_structs.hpp"
#include "SDK/FN_GCNS_GM_OnDeathAreaHeal_classes.hpp"
#include "SDK/FN_GCNS_GM_OnDeathAreaHeal_parameters.hpp"
#include "SDK/FN_ThrowingStarTest_structs.hpp"
#include "SDK/FN_ThrowingStarTest_classes.hpp"
#include "SDK/FN_ThrowingStarTest_parameters.hpp"
#include "SDK/FN_GCNS_GM_OnDmgLifeLeech_structs.hpp"
#include "SDK/FN_GCNS_GM_OnDmgLifeLeech_classes.hpp"
#include "SDK/FN_GCNS_GM_OnDmgLifeLeech_parameters.hpp"
#include "SDK/FN_GCNS_GM_OnDmgReceived_DmgShare_structs.hpp"
#include "SDK/FN_GCNS_GM_OnDmgReceived_DmgShare_classes.hpp"
#include "SDK/FN_GCNS_GM_OnDmgReceived_DmgShare_parameters.hpp"
#include "SDK/FN_GCNS_GM_Player_OnShieldDestroyed_AOE_structs.hpp"
#include "SDK/FN_GCNS_GM_Player_OnShieldDestroyed_AOE_classes.hpp"
#include "SDK/FN_GCNS_GM_Player_OnShieldDestroyed_AOE_parameters.hpp"
#include "SDK/FN_Vivox_structs.hpp"
#include "SDK/FN_Vivox_classes.hpp"
#include "SDK/FN_Vivox_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Blue-60pc_structs.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Blue-60pc_classes.hpp"
#include "SDK/FN_TextStyle-Base-XS-B-Blue-60pc_parameters.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_L_structs.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_L_classes.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_L_parameters.hpp"
#include "SDK/FN_WebBrowser_structs.hpp"
#include "SDK/FN_WebBrowser_classes.hpp"
#include "SDK/FN_WebBrowser_parameters.hpp"
#include "SDK/FN_GCNS_GM_PlayerOnDmgLifeLeech_structs.hpp"
#include "SDK/FN_GCNS_GM_PlayerOnDmgLifeLeech_classes.hpp"
#include "SDK/FN_GCNS_GM_PlayerOnDmgLifeLeech_parameters.hpp"
#include "SDK/FN_T4_Main_structs.hpp"
#include "SDK/FN_T4_Main_classes.hpp"
#include "SDK/FN_T4_Main_parameters.hpp"
#include "SDK/FN_SessionMessages_structs.hpp"
#include "SDK/FN_SessionMessages_classes.hpp"
#include "SDK/FN_SessionMessages_parameters.hpp"
#include "SDK/FN_Serialization_structs.hpp"
#include "SDK/FN_Serialization_classes.hpp"
#include "SDK/FN_Serialization_parameters.hpp"
#include "SDK/FN_EngineMessages_structs.hpp"
#include "SDK/FN_EngineMessages_classes.hpp"
#include "SDK/FN_EngineMessages_parameters.hpp"
#include "SDK/FN_AssetRegistry_structs.hpp"
#include "SDK/FN_AssetRegistry_classes.hpp"
#include "SDK/FN_AssetRegistry_parameters.hpp"
#include "SDK/FN_VectorVM_structs.hpp"
#include "SDK/FN_VectorVM_classes.hpp"
#include "SDK/FN_VectorVM_parameters.hpp"
#include "SDK/FN_ScheduledEvents_structs.hpp"
#include "SDK/FN_ScheduledEvents_classes.hpp"
#include "SDK/FN_ScheduledEvents_parameters.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_M_structs.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_M_classes.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_M_parameters.hpp"
#include "SDK/FN_TcpMessaging_structs.hpp"
#include "SDK/FN_TcpMessaging_classes.hpp"
#include "SDK/FN_TcpMessaging_parameters.hpp"
#include "SDK/FN_WmfMediaFactory_structs.hpp"
#include "SDK/FN_WmfMediaFactory_classes.hpp"
#include "SDK/FN_WmfMediaFactory_parameters.hpp"
#include "SDK/FN_AvfMediaFactory_structs.hpp"
#include "SDK/FN_AvfMediaFactory_classes.hpp"
#include "SDK/FN_AvfMediaFactory_parameters.hpp"
#include "SDK/FN_LightPropagationVolumeRuntime_structs.hpp"
#include "SDK/FN_LightPropagationVolumeRuntime_classes.hpp"
#include "SDK/FN_LightPropagationVolumeRuntime_parameters.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_S_structs.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_S_classes.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_S_parameters.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_XL_structs.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_XL_classes.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_XL_parameters.hpp"
#include "SDK/FN_PurchaseFlow_structs.hpp"
#include "SDK/FN_PurchaseFlow_classes.hpp"
#include "SDK/FN_PurchaseFlow_parameters.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_XS_structs.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_XS_classes.hpp"
#include "SDK/FN_ItemCardPowerRatingTextStyle_XS_parameters.hpp"
#include "SDK/FN_DmgTypeBP_Environmental_structs.hpp"
#include "SDK/FN_DmgTypeBP_Environmental_classes.hpp"
#include "SDK/FN_DmgTypeBP_Environmental_parameters.hpp"
#include "SDK/FN_GCNS_Generic_BotTurret_Impact_structs.hpp"
#include "SDK/FN_GCNS_Generic_BotTurret_Impact_classes.hpp"
#include "SDK/FN_GCNS_Generic_BotTurret_Impact_parameters.hpp"
#include "SDK/FN_ButtonStyle-Primary-M_structs.hpp"
#include "SDK/FN_ButtonStyle-Primary-M_classes.hpp"
#include "SDK/FN_ButtonStyle-Primary-M_parameters.hpp"
#include "SDK/FN_OodleHandlerComponent_structs.hpp"
#include "SDK/FN_OodleHandlerComponent_classes.hpp"
#include "SDK/FN_OodleHandlerComponent_parameters.hpp"
#include "SDK/FN_TextStyle-Button-Primary-M-Disabled_structs.hpp"
#include "SDK/FN_TextStyle-Button-Primary-M-Disabled_classes.hpp"
#include "SDK/FN_TextStyle-Button-Primary-M-Disabled_parameters.hpp"
#include "SDK/FN_GCNS_TestAntiMaterial_structs.hpp"
#include "SDK/FN_GCNS_TestAntiMaterial_classes.hpp"
#include "SDK/FN_GCNS_TestAntiMaterial_parameters.hpp"
#include "SDK/FN_BuildPatchServices_structs.hpp"
#include "SDK/FN_BuildPatchServices_classes.hpp"
#include "SDK/FN_BuildPatchServices_parameters.hpp"
#include "SDK/FN_UACCommon_structs.hpp"
#include "SDK/FN_UACCommon_classes.hpp"
#include "SDK/FN_UACCommon_parameters.hpp"
#include "SDK/FN_Overlay_structs.hpp"
#include "SDK/FN_Overlay_classes.hpp"
#include "SDK/FN_Overlay_parameters.hpp"
#include "SDK/FN_GCNL_GM_OnDmg_SpeedBuff_structs.hpp"
#include "SDK/FN_GCNL_GM_OnDmg_SpeedBuff_classes.hpp"
#include "SDK/FN_GCNL_GM_OnDmg_SpeedBuff_parameters.hpp"
#include "SDK/FN_TextStyle-Button-Primary-M_structs.hpp"
#include "SDK/FN_TextStyle-Button-Primary-M_classes.hpp"
#include "SDK/FN_TextStyle-Button-Primary-M_parameters.hpp"
#include "SDK/FN_GCNL_GM_OnDmgCorrosion_structs.hpp"
#include "SDK/FN_GCNL_GM_OnDmgCorrosion_classes.hpp"
#include "SDK/FN_GCNL_GM_OnDmgCorrosion_parameters.hpp"
#include "SDK/FN_MoviePlayer_structs.hpp"
#include "SDK/FN_MoviePlayer_classes.hpp"
#include "SDK/FN_MoviePlayer_parameters.hpp"
#include "SDK/FN_SocialDefaults_structs.hpp"
#include "SDK/FN_SocialDefaults_classes.hpp"
#include "SDK/FN_SocialDefaults_parameters.hpp"
#include "SDK/FN_AudioMixer_structs.hpp"
#include "SDK/FN_AudioMixer_classes.hpp"
#include "SDK/FN_AudioMixer_parameters.hpp"
#include "SDK/FN_GCNL_GM_OnDmgReceived_Slow_structs.hpp"
#include "SDK/FN_GCNL_GM_OnDmgReceived_Slow_classes.hpp"
#include "SDK/FN_GCNL_GM_OnDmgReceived_Slow_parameters.hpp"
#include "SDK/FN_InvisibleCursorWidget_structs.hpp"
#include "SDK/FN_InvisibleCursorWidget_classes.hpp"
#include "SDK/FN_InvisibleCursorWidget_parameters.hpp"
#include "SDK/FN_MediaAssets_structs.hpp"
#include "SDK/FN_MediaAssets_classes.hpp"
#include "SDK/FN_MediaAssets_parameters.hpp"
#include "SDK/FN_GameLiveStreaming_structs.hpp"
#include "SDK/FN_GameLiveStreaming_classes.hpp"
#include "SDK/FN_GameLiveStreaming_parameters.hpp"
#include "SDK/FN_Niagara_structs.hpp"
#include "SDK/FN_Niagara_classes.hpp"
#include "SDK/FN_Niagara_parameters.hpp"
#include "SDK/FN_EngineSettings_structs.hpp"
#include "SDK/FN_EngineSettings_classes.hpp"
#include "SDK/FN_EngineSettings_parameters.hpp"
#include "SDK/FN_MovieSceneCapture_structs.hpp"
#include "SDK/FN_MovieSceneCapture_classes.hpp"
#include "SDK/FN_MovieSceneCapture_parameters.hpp"
#include "SDK/FN_SubtitlesWidgets_structs.hpp"
#include "SDK/FN_SubtitlesWidgets_classes.hpp"
#include "SDK/FN_SubtitlesWidgets_parameters.hpp"
#include "SDK/FN_GeometryCache_structs.hpp"
#include "SDK/FN_GeometryCache_classes.hpp"
#include "SDK/FN_GeometryCache_parameters.hpp"
#include "SDK/FN_ActorSequence_structs.hpp"
#include "SDK/FN_ActorSequence_classes.hpp"
#include "SDK/FN_ActorSequence_parameters.hpp"
#include "SDK/FN_EpicGameplayStatsRuntime_structs.hpp"
#include "SDK/FN_EpicGameplayStatsRuntime_classes.hpp"
#include "SDK/FN_EpicGameplayStatsRuntime_parameters.hpp"
#include "SDK/FN_CinematicCamera_structs.hpp"
#include "SDK/FN_CinematicCamera_classes.hpp"
#include "SDK/FN_CinematicCamera_parameters.hpp"
#include "SDK/FN_MaterialShaderQualitySettings_structs.hpp"
#include "SDK/FN_MaterialShaderQualitySettings_classes.hpp"
#include "SDK/FN_MaterialShaderQualitySettings_parameters.hpp"
#include "SDK/FN_TextStyle-Base-XXS-B_structs.hpp"
#include "SDK/FN_TextStyle-Base-XXS-B_classes.hpp"
#include "SDK/FN_TextStyle-Base-XXS-B_parameters.hpp"
#include "SDK/FN_GCNL_GM_OnDmgVulnerability_structs.hpp"
#include "SDK/FN_GCNL_GM_OnDmgVulnerability_classes.hpp"
#include "SDK/FN_GCNL_GM_OnDmgVulnerability_parameters.hpp"
#include "SDK/FN_Landscape_structs.hpp"
#include "SDK/FN_Landscape_classes.hpp"
#include "SDK/FN_Landscape_parameters.hpp"
#include "SDK/FN_GCN_NPC_Fire_structs.hpp"
#include "SDK/FN_GCN_NPC_Fire_classes.hpp"
#include "SDK/FN_GCN_NPC_Fire_parameters.hpp"
#include "SDK/FN_GCNL_Taker_TRV_DaytimeVulnerability_structs.hpp"
#include "SDK/FN_GCNL_Taker_TRV_DaytimeVulnerability_classes.hpp"
#include "SDK/FN_GCNL_Taker_TRV_DaytimeVulnerability_parameters.hpp"
#include "SDK/FN_HeadMountedDisplay_structs.hpp"
#include "SDK/FN_HeadMountedDisplay_classes.hpp"
#include "SDK/FN_HeadMountedDisplay_parameters.hpp"
#include "SDK/FN_GCN_NPC_Lightning_structs.hpp"
#include "SDK/FN_GCN_NPC_Lightning_classes.hpp"
#include "SDK/FN_GCN_NPC_Lightning_parameters.hpp"
#include "SDK/FN_Qos_structs.hpp"
#include "SDK/FN_Qos_classes.hpp"
#include "SDK/FN_Qos_parameters.hpp"
#include "SDK/FN_GCN_Generic_Stunned_structs.hpp"
#include "SDK/FN_GCN_Generic_Stunned_classes.hpp"
#include "SDK/FN_GCN_Generic_Stunned_parameters.hpp"
#include "SDK/FN_GCN_Commando_IncendiaryRoundsActive_structs.hpp"
#include "SDK/FN_GCN_Commando_IncendiaryRoundsActive_classes.hpp"
#include "SDK/FN_GCN_Commando_IncendiaryRoundsActive_parameters.hpp"
#include "SDK/FN_GCN_Commando_IncendiaryRoundsDOT_structs.hpp"
#include "SDK/FN_GCN_Commando_IncendiaryRoundsDOT_classes.hpp"
#include "SDK/FN_GCN_Commando_IncendiaryRoundsDOT_parameters.hpp"
#include "SDK/FN_LevelSequence_structs.hpp"
#include "SDK/FN_LevelSequence_classes.hpp"
#include "SDK/FN_LevelSequence_parameters.hpp"
#include "SDK/FN_GCN_Commando_Warcry_structs.hpp"
#include "SDK/FN_GCN_Commando_Warcry_classes.hpp"
#include "SDK/FN_GCN_Commando_Warcry_parameters.hpp"
#include "SDK/FN_GCN_Commando_DebilitatingShots_structs.hpp"
#include "SDK/FN_GCN_Commando_DebilitatingShots_classes.hpp"
#include "SDK/FN_GCN_Commando_DebilitatingShots_parameters.hpp"
#include "SDK/FN_GCN_Outlander_DefensiveBuff_structs.hpp"
#include "SDK/FN_GCN_Outlander_DefensiveBuff_classes.hpp"
#include "SDK/FN_GCN_Outlander_DefensiveBuff_parameters.hpp"
#include "SDK/FN_T4_Research_structs.hpp"
#include "SDK/FN_T4_Research_classes.hpp"
#include "SDK/FN_T4_Research_parameters.hpp"
#include "SDK/FN_GCN_Outlander_BearFragmentDebuff_structs.hpp"
#include "SDK/FN_GCN_Outlander_BearFragmentDebuff_classes.hpp"
#include "SDK/FN_GCN_Outlander_BearFragmentDebuff_parameters.hpp"
#include "SDK/FN_SkillTreeBPLibrary_structs.hpp"
#include "SDK/FN_SkillTreeBPLibrary_classes.hpp"
#include "SDK/FN_SkillTreeBPLibrary_parameters.hpp"
#include "SDK/FN_DefaultSkillNode_structs.hpp"
#include "SDK/FN_DefaultSkillNode_classes.hpp"
#include "SDK/FN_DefaultSkillNode_parameters.hpp"
#include "SDK/FN_KeybindWidget_structs.hpp"
#include "SDK/FN_KeybindWidget_classes.hpp"
#include "SDK/FN_KeybindWidget_parameters.hpp"
#include "SDK/FN_DefaultUIDataConfiguration_structs.hpp"
#include "SDK/FN_DefaultUIDataConfiguration_classes.hpp"
#include "SDK/FN_DefaultUIDataConfiguration_parameters.hpp"

using namespace SDK;

typedef __int8 int8;
typedef __int16 int16;
typedef __int32 int32;
typedef __int64 int64;

typedef unsigned __int8 uint8;
typedef unsigned __int16 uint16;
typedef unsigned __int32 uint32;
typedef unsigned __int64 uint64;

static auto Malloc = reinterpret_cast<void* (*)(int32_t Size, int32_t Alignment)>(uintptr_t(GetModuleHandle(0)) + 0x128A100);
static auto Free = reinterpret_cast<void (*)(void* Memory)>(uintptr_t(GetModuleHandle(0)) + 0x1281200);

#define NumBitsPerDWORD ((int32)32)
#define NumBitsPerDWORDLogTwo ((int32)5)

static FORCEINLINE uint32 CountLeadingZeros(uint32 Value)
{
    unsigned long Log2;
    if (_BitScanReverse(&Log2, Value) != 0)
    {
        return 31 - Log2;
    }

    return 32;
}

template <class ArrayType>
class TArray_
{
    friend struct FString_;

    template <typename ArrayType>
    friend class TSparseArray;

    template <typename SetType>
    friend class TSet_;

public:

    TArray_() = default;

    TArray_(int InitMax)
        : Count(0)
        , MaxElements(InitMax)
    {
        Data = Malloc(InitMax * sizeof(ArrayType), alignof(ArrayType));
    }

    FORCEINLINE int Num() const
    {
        return Count;
    }
    FORCEINLINE int Max() const
    {
        return MaxElements;
    }
    FORCEINLINE ArrayType*& GetData()
    {
        return Data;
    }
    FORCEINLINE int Slack() const
    {
        return MaxElements - Count;
    }
    FORCEINLINE void Reserve(const int NumElements)
    {
        Data = Slack() >= NumElements ? Data : (ArrayType*)Realloc(Data, (MaxElements = Count + NumElements) * sizeof(ArrayType), 0);
    }
    FORCEINLINE void Reset(int MinSizeAfterReset = 5)
    {
        MaxElements = MinSizeAfterReset;
        Count = 0;

        if (Data)
            Free(Data);

        Data = (ArrayType*)Malloc(MinSizeAfterReset * sizeof(ArrayType), alignof(ArrayType));
    }
    FORCEINLINE bool RemoveSingle(const int Index)
    {
        if (Index < Count)
        {
            if (Index != Count - 1)
                Data[Index] = Data[Count - 1];

            --Count;

            return true;
        }
        return false;
    }
    FORCEINLINE void RemoveAt(int Index, int Lenght = 1)
    {
        for (; Lenght != 0; --Lenght)
        {
            if (!RemoveSingle(Index++))
                break;
        }
    }
    FORCEINLINE void Add(const ArrayType& InData)
    {
        Reserve(1);
        Data[Count] = InData;
        ++Count;
    }
    FORCEINLINE void MultiAdd(int Num, ArrayType InData...)
    {
        va_list MyList;
        va_start(MyList, Num);

        Reserve(Num);

        for (int i = 0; i < Num; i++)
        {
            Add(va_arg(MyList, ArrayType));
        }
        va_end(MyList);
    };
    FORCEINLINE void FreeArray()
    {
        Free(Data);
        Data = nullptr;
        Count = 0;
        MaxElements = 0;
    }

    FORCEINLINE explicit operator bool() const
    {
        return Count > 0 && Data;
    }
    FORCEINLINE ArrayType& operator[](int i)
    {
        return Data[i];
    };

    FORCEINLINE const ArrayType& operator[](int i) const
    {
        return Data[i];
    };

    FORCEINLINE int MaxIndex()
    {
        return Count - 1;
    }

    class FBaseArrayIterator
    {
        int32 Index;
        TArray_<ArrayType>& ITArray_;

    public:
        FBaseArrayIterator(TArray_<ArrayType>& Array)
            : ITArray_(Array)
            , Index(Array.Count)
        {
        }
        FBaseArrayIterator(TArray_<ArrayType>& Array, int32 CurrentIndex)
            : ITArray_(Array)
            , Index(CurrentIndex)
        {
        }

        FORCEINLINE ArrayType& operator*()
        {
            return ITArray_[Index];
        }
        FORCEINLINE ArrayType& operator->()
        {
            return ITArray_[Index];
        }
        FORCEINLINE FBaseArrayIterator& operator++()
        {
            ++Index;
            return *this;
        }
        FORCEINLINE FBaseArrayIterator& operator--()
        {
            --Index;
            return *this;
        }
        FORCEINLINE bool operator==(const FBaseArrayIterator& other) const
        {
            return Index == other.Index;
        }
        FORCEINLINE bool operator!=(const FBaseArrayIterator& other) const
        {
            return Index != other.Index;
        }
    };

    FORCEINLINE FBaseArrayIterator begin()
    {
        return FBaseArrayIterator(*this, 0);
    }
    FORCEINLINE FBaseArrayIterator begin() const
    {
        return FBaseArrayIterator(*this, 0);
    }
    FORCEINLINE FBaseArrayIterator end()
    {
        return FBaseArrayIterator(*this);
    }
    FORCEINLINE FBaseArrayIterator end() const
    {
        return FBaseArrayIterator(*this);
    }

private:
    ArrayType* Data;
    int32_t Count;
    int32_t MaxElements;
};

struct FString_ : private TArray_<wchar_t>
{
    inline FString_() {};

    FString_(const wchar_t* other)
    {
        MaxElements = Count = *other ? std::wcslen(other) + 1 : 0;

        if (Count)
        {
            Data = const_cast<wchar_t*>(other);
        }
    };

    FORCEINLINE bool IsValid() const
    {
        return Data != nullptr;
    }

    FORCEINLINE const wchar_t* ToWString() const
    {
        return Data;
    }
    FORCEINLINE void Free()
    {
        this->FreeArray();
    }

    FORCEINLINE std::string ToString() const
    {
        auto length = wcslen(Data);

        std::string str(length, '\0');

        std::use_facet<std::ctype<wchar_t>>(std::locale()).narrow(Data, Data + length, '?', &str[0]);

        return str;
    }

    FORCEINLINE explicit operator bool() const
    {
        return this->TArray_<wchar_t>::operator bool();
    }
};

// Allocates Elements directly where the instance is created rather than on the heap
template <int32 NumElements>
class TInlineAllocator
{
private:
    template <int32 Size, int32 Alignment>
    struct alignas(Alignment) TAlligendBytes
    {
        uint8 Pad[Size];
    };

    template <typename ElementType>
    struct TTypeCompatibleBytes : public TAlligendBytes<sizeof(ElementType), alignof(ElementType)>
    {
    };

public:
    template <typename ElementType>
    class ForElementType
    {
        friend class TBiTArray_;

    private:
        TTypeCompatibleBytes<ElementType> InlineData[NumElements];

        ElementType* SecondaryData;

    public:

        FORCEINLINE int32 NumInlineBytes() const
        {
            return sizeof(ElementType) * NumElements;
        }
        FORCEINLINE int32 NumInlineBits() const
        {
            return NumInlineBytes() * 8;
        }

        FORCEINLINE ElementType& operator[](int32 Index)
        {
            return *(ElementType*)(&InlineData[Index]);
        }
        FORCEINLINE const ElementType& operator[](int32 Index) const
        {
            return *(ElementType*)(&InlineData[Index]);
        }

        FORCEINLINE void operator=(void* InElements)
        {
            SecondaryData = InElements;
        }

        FORCEINLINE ElementType& GetInlineElement(int32 Index)
        {
            return *(ElementType*)(&InlineData[Index]);
        }
        FORCEINLINE const ElementType& GetInlineElement(int32 Index) const
        {
            return *(ElementType*)(&InlineData[Index]);
        }
        FORCEINLINE ElementType& GetSecondaryElement(int32 Index)
        {
            return SecondaryData[Index];
        }
        FORCEINLINE const ElementType& GetSecondaryElement(int32 Index) const
        {
            return SecondaryData[Index];
        }
    };
};

class TBiTArray_
{
private:
    template <typename ArrayType>
    friend class TSparseArray;
    template <typename SetType>
    friend class TSet_;

private:
    TInlineAllocator<4>::ForElementType<uint32> Data;
    int32 NumBits;
    int32 MaxBits;

    struct FRelativeBitReference
    {
    public:
        FORCEINLINE explicit FRelativeBitReference(int32 BitIndex)
            : DWORDIndex(BitIndex >> NumBitsPerDWORDLogTwo)
            , Mask(1 << (BitIndex & ((NumBitsPerDWORD)-1)))
        {
        }

        int32 DWORDIndex;
        uint32 Mask;
    };

public:
    struct FBitReference
    {
        FORCEINLINE FBitReference(uint32& InData, uint32 InMask)
            : Data(InData)
            , Mask(InMask)
        {
        }
        FORCEINLINE const FBitReference(const uint32& InData, const uint32 InMask)
            : Data(const_cast<uint32&>(InData))
            , Mask(InMask)
        {
        }

        FORCEINLINE void SetBit(const bool Value)
        {
            Value ? Data |= Mask : Data &= ~Mask;

            // 10011101 - Data			 // 10011101 - Data
            // 00000010 - Mask - true |	 // 00000010 - Mask - false
            // 10011111	-  |=			 // 11111101 -  ~
            //							 // 10011111 -  &=
        }

        FORCEINLINE operator bool() const
        {
            return (Data & Mask) != 0;
        }
        FORCEINLINE void operator=(const bool Value)
        {
            this->SetBit(Value);
        }

    private:
        uint32& Data;
        uint32 Mask;
    };

public:
    class FBitIterator : public FRelativeBitReference
    {
    private:
        int32 Index;
        const TBiTArray_& IteratedArray;

    public:
        FORCEINLINE const FBitIterator(const TBiTArray_& ToIterate, const int32 StartIndex) // Begin
            : IteratedArray(ToIterate)
            , Index(StartIndex)
            , FRelativeBitReference(StartIndex)
        {
        }
        FORCEINLINE const FBitIterator(const TBiTArray_& ToIterate) // End
            : IteratedArray(ToIterate)
            , Index(ToIterate.NumBits)
            , FRelativeBitReference(ToIterate.NumBits)
        {
        }

        FORCEINLINE explicit operator bool() const
        {
            return Index < IteratedArray.Num();
        }
        FORCEINLINE FBitIterator& operator++()
        {
            ++Index;
            this->Mask <<= 1;
            if (!this->Mask)
            {
                this->Mask = 1;
                ++this->DWORDIndex;
            }
            return *this;
        }
        FORCEINLINE bool operator*() const
        {
            // Thesis: Once there are more elements in the BiTArray_ than InlineData can hold it'll just allocate all of 
            // them through SecondaryElements, leaving InlineData all true

            if (IteratedArray.NumBits < IteratedArray.Data.NumInlineBits())
            {
                return (bool)FBitReference(IteratedArray.Data.GetInlineElement(this->DWORDIndex), this->Mask);
            }
            else
            {
                return (bool)FBitReference(IteratedArray.Data.GetSecondaryElement(this->DWORDIndex), this->Mask);
            }
        }
        FORCEINLINE bool operator==(const FBitIterator& OtherIt) const
        {
            return Index == OtherIt.Index;
        }
        FORCEINLINE bool operator!=(const FBitIterator& OtherIt) const
        {
            return Index </*=*/ OtherIt.Index;
        }
        FORCEINLINE bool operator < (const int32 Other) const
        {
            return Index < Other;
        }
        FORCEINLINE bool operator > (const int32 Other) const
        {
            return Index < Other;
        }

        FORCEINLINE int32 GetIndex() const
        {
            return Index;
        }
    };

    class FSetBitIterator : public FRelativeBitReference
    {
    private:
        const TBiTArray_& IteratedArray;

        uint32 UnvisitedBitMask;
        int32  CurrentBitIndex;
        int32  BaseBitIndex;

    public:
        FORCEINLINE FSetBitIterator(const TBiTArray_& ToIterate, int32 StartIndex)
            : FRelativeBitReference(StartIndex)
            , IteratedArray(const_cast<TBiTArray_&>(ToIterate))
            , UnvisitedBitMask((~0U) << (StartIndex & (NumBitsPerDWORD - 1)))
            , CurrentBitIndex(StartIndex)
            , BaseBitIndex(StartIndex & ~(NumBitsPerDWORD - 1))
        {
            if (StartIndex != IteratedArray.NumBits)
            {
                FindNexTSet_Bit();
            }
        }
        FORCEINLINE FSetBitIterator(const TBiTArray_& ToIterate)
            : FRelativeBitReference(ToIterate.NumBits)
            , IteratedArray(const_cast<TBiTArray_&>(ToIterate))
            , UnvisitedBitMask(0)
            , CurrentBitIndex(ToIterate.NumBits)
            , BaseBitIndex(ToIterate.NumBits)
        {
        }

        FORCEINLINE FSetBitIterator& operator++()
        {
            UnvisitedBitMask &= ~this->Mask;

            FindNexTSet_Bit();

            return *this;
        }
        FORCEINLINE bool operator*() const
        {
            return true;
        }

        FORCEINLINE bool operator==(const FSetBitIterator& Other) const
        {
            return CurrentBitIndex == Other.CurrentBitIndex;
        }
        FORCEINLINE bool operator!=(const FSetBitIterator& Other) const
        {
            return CurrentBitIndex </*=*/ Other.CurrentBitIndex;
        }

        FORCEINLINE explicit operator bool() const
        {
            return CurrentBitIndex < IteratedArray.NumBits;
        }

        FORCEINLINE int32 GetIndex() const
        {
            return CurrentBitIndex;
        }

    private:

        void FindNexTSet_Bit()
        {
            //InlineData is the first 16-bytes of TBiTArray_
            const uint32* ArrayData = (IteratedArray.NumBits < IteratedArray.Data.NumInlineBits() ? (uint32*)(&IteratedArray) : IteratedArray.Data.SecondaryData);

            const int32 ArrayNum = IteratedArray.NumBits;
            const int32 LastDWORDIndex = (ArrayNum - 1) / NumBitsPerDWORD;

            uint32 RemainingBitMask = ArrayData[this->DWORDIndex] & UnvisitedBitMask;
            while (!RemainingBitMask)
            {
                ++this->DWORDIndex;
                BaseBitIndex += NumBitsPerDWORD;

                if (this->DWORDIndex > LastDWORDIndex)
                {
                    CurrentBitIndex += ArrayNum;
                    return;
                }

                RemainingBitMask = ArrayData[this->DWORDIndex];
                UnvisitedBitMask = ~0;
            }

            const uint32 NewRemainingBitMask = RemainingBitMask & (RemainingBitMask - 1);

            this->Mask = NewRemainingBitMask ^ RemainingBitMask;

            CurrentBitIndex = BaseBitIndex + NumBitsPerDWORD - 1 - CountLeadingZeros(this->Mask);

            if (CurrentBitIndex > ArrayNum)
            {
                CurrentBitIndex = ArrayNum;
            }
        }
    };

public:
    FORCEINLINE FBitIterator Iterator(int32 StartIndex)
    {
        return FBitIterator(*this, StartIndex);
    }
    FORCEINLINE FSetBitIterator SetBitIterator(int32 StartIndex)
    {
        return FSetBitIterator(*this, StartIndex);
    }

    FORCEINLINE FBitIterator begin()
    {
        return FBitIterator(*this, 0);
    }
    FORCEINLINE const FBitIterator begin() const
    {
        return FBitIterator(*this, 0);
    }
    FORCEINLINE FBitIterator end()
    {
        return FBitIterator(*this);
    }
    FORCEINLINE const FBitIterator end() const
    {
        return  FBitIterator(*this);
    }

    FORCEINLINE FSetBitIterator SetBitsItBegin()
    {
        return FSetBitIterator(*this, 0);
    }
    FORCEINLINE const FSetBitIterator SetBitsItBegin() const
    {
        return FSetBitIterator(*this, 0);
    }
    FORCEINLINE const FSetBitIterator SetBitsItEnd()
    {
        return FSetBitIterator(*this);
    }
    FORCEINLINE const FSetBitIterator SetBitsItEnd() const
    {
        return FSetBitIterator(*this);
    }

    FORCEINLINE int32 Num() const
    {
        return NumBits;
    }
    FORCEINLINE int32 Max() const
    {
        return MaxBits;
    }
    FORCEINLINE bool IsSet(int32 Index) const
    {
        return *FBitIterator(*this, Index);
    }
    FORCEINLINE void Set(const int32 Index, const bool Value, bool bIsSettingAllZero = false)
    {
        const int32 DWORDIndex = (Index >> ((int32)5));
        const int32 Mask = (1 << (Index & (((int32)32) - 1)));

        if (!bIsSettingAllZero)
            NumBits = Index >= NumBits ? Index < MaxBits ? Index + 1 : NumBits : NumBits;

        FBitReference(Data[DWORDIndex], Mask).SetBit(Value);
    }
    FORCEINLINE void ZeroAll()
    {
        for (int i = 0; i < MaxBits; i++)
        {
            Set(i, false, true);
        }
    }
};

template <typename ElementType>
union TSparseArrayElementOrListLink
{
    TSparseArrayElementOrListLink(ElementType& InElement)
        : ElementData(InElement)
    {
    }
    TSparseArrayElementOrListLink(ElementType&& InElement)
        : ElementData(InElement)
    {
    }

    TSparseArrayElementOrListLink(int32 InPrevFree, int32 InNextFree)
        : PrevFreeIndex(InPrevFree)
        , NextFreeIndex(InNextFree)
    {
    }

    TSparseArrayElementOrListLink<ElementType> operator=(const TSparseArrayElementOrListLink<ElementType>& Other)
    {
        return TSparseArrayElementOrListLink(Other.NextFreeIndex, Other.PrevFreeIndex);
    }

    /** If the element is allocated, its value is stored here. */
    ElementType ElementData;

    struct
    {
        /** If the element isn't allocated, this is a link to the previous element in the array's free list. */
        int32 PrevFreeIndex;

        /** If the element isn't allocated, this is a link to the next element in the array's free list. */
        int32 NextFreeIndex;
    };
};

template <typename ArrayType>
class TSparseArray
{
private:
    template <typename SetType>
    friend class TSet_;

public:
    typedef TSparseArrayElementOrListLink<ArrayType> FSparseArrayElement;

private:
    TArray_<FSparseArrayElement> Data;
    TBiTArray_ AllocationFlags;
    int32 FirstFreeIndex;
    int32 NumFreeIndices;

public:
    class FBaseIterator
    {
    private:
        TSparseArray<ArrayType>& IteratedArray;
        TBiTArray_::FSetBitIterator BiTArray_It;

    public:
        FORCEINLINE FBaseIterator(const TSparseArray<ArrayType>& Array, const TBiTArray_::FSetBitIterator BitIterator)
            : IteratedArray(const_cast<TSparseArray<ArrayType>&>(Array))
            , BiTArray_It(const_cast<TBiTArray_::FSetBitIterator&>(BitIterator))
        {
        }

        FORCEINLINE explicit operator bool() const
        {
            return (bool)BiTArray_It;
        }
        FORCEINLINE TSparseArray<ArrayType>::FBaseIterator& operator++()
        {
            ++BiTArray_It;
            return *this;
        }
        FORCEINLINE ArrayType& operator*()
        {
            return IteratedArray[BiTArray_It.GetIndex()].ElementData;
        }
        FORCEINLINE const ArrayType& operator*() const
        {
            return IteratedArray[BiTArray_It.GetIndex()].ElementData;
        }
        FORCEINLINE ArrayType& operator->()
        {
            return IteratedArray[BiTArray_It.GetIndex()].ElementData;
        }
        FORCEINLINE const ArrayType& operator->() const
        {
            return IteratedArray[BiTArray_It.GetIndex()].ElementData;
        }
        FORCEINLINE bool operator==(const TSparseArray<ArrayType>::FBaseIterator& Other) const
        {
            return BiTArray_It == Other.BiTArray_It;
        }
        FORCEINLINE bool operator!=(const TSparseArray<ArrayType>::FBaseIterator& Other) const
        {
            return BiTArray_It != Other.BiTArray_It;
        }

        FORCEINLINE int32 GetIndex() const
        {
            return BiTArray_It.GetIndex();
        }
        FORCEINLINE bool IsElementValid() const
        {
            return *BiTArray_It;
        }
    };

public:
    FORCEINLINE TSparseArray<ArrayType>::FBaseIterator begin()
    {
        return TSparseArray<ArrayType>::FBaseIterator(*this, TBiTArray_::FSetBitIterator(AllocationFlags, 0));
    }
    FORCEINLINE const TSparseArray<ArrayType>::FBaseIterator begin() const
    {
        return TSparseArray<ArrayType>::FBaseIterator(*this, TBiTArray_::FSetBitIterator(AllocationFlags, 0));
    }
    FORCEINLINE TSparseArray<ArrayType>::FBaseIterator end()
    {
        return TSparseArray<ArrayType>::FBaseIterator(*this, TBiTArray_::FSetBitIterator(AllocationFlags));
    }
    FORCEINLINE const TSparseArray<ArrayType>::FBaseIterator end() const
    {
        return TSparseArray<ArrayType>::FBaseIterator(*this, TBiTArray_::FSetBitIterator(AllocationFlags));
    }

    FORCEINLINE FSparseArrayElement& operator[](uint32 Index)
    {
        return *(FSparseArrayElement*)&Data[Index].ElementData;
    }
    FORCEINLINE const FSparseArrayElement& operator[](uint32 Index) const
    {
        return *(const FSparseArrayElement*)&Data[Index].ElementData;
    }

    FORCEINLINE int32 Num() const
    {
        return Data.Num() - NumFreeIndices;
    }
    FORCEINLINE int32 GetNumFreeIndices() const
    {
        return NumFreeIndices;
    }
    FORCEINLINE int32 GetFirstFreeIndex() const
    {
        return FirstFreeIndex;
    }
    FORCEINLINE const TArray_<FSparseArrayElement>& GetData() const
    {
        return Data;
    }
    FORCEINLINE const TBiTArray_& GetAllocationFlags() const
    {
        return AllocationFlags;
    }
    FORCEINLINE bool IsIndexValid(int32 IndexToCheck) const
    {
        return AllocationFlags.IsSet(IndexToCheck);
    }

    FORCEINLINE bool RemoveAt(const int32 IndexToRemove)
    {
        if (IndexToRemove >= 0 && IndexToRemove < Data.Num() && AllocationFlags.IsSet(IndexToRemove))
        {
            int32 PreviousFreeIndex = -1;
            int32 NextFreeIndex = -1;

            if (NumFreeIndices == 0)
            {
                FirstFreeIndex = IndexToRemove;
                Data[IndexToRemove] = { -1, -1 };
            }
            else
            {
                for (auto It = AllocationFlags.begin(); It != AllocationFlags.end(); ++It)
                {
                    if (!It)
                    {
                        if (It.GetIndex() < IndexToRemove)
                        {
                            Data[IndexToRemove].PrevFreeIndex = It.GetIndex();
                        }
                        else if (It.GetIndex() > IndexToRemove)
                        {
                            Data[IndexToRemove].NextFreeIndex = It.GetIndex();
                            break;
                        }
                    }
                }
            }
            AllocationFlags.Set(IndexToRemove, false);
            NumFreeIndices++;

            return true;
        }
        return false;
    }
    FORCEINLINE int32 Add(ArrayType InElement)
    {
        FSparseArrayElement Element(InElement);

        int32 NextFree;
        int32 OutIndex;
        if (FirstFreeIndex >= 1)
        {
            NextFree = Data[FirstFreeIndex].NextFreeIndex;
            Data[FirstFreeIndex] = Element;
            --NumFreeIndices;

            AllocationFlags.Set(FirstFreeIndex, true);

            if (NumFreeIndices >= 1)
            {
                OutIndex = NextFree;
                FirstFreeIndex = NextFree;
                Data[NextFree].PrevFreeIndex = -1;

                return OutIndex;
            }
        }
        else
        {
            Data.Add(Element);
            AllocationFlags.Set(Data.Num() - 1, true);

            return Data.Num() - 1;
        }
    }
};

template <typename ElementType>
class TSet_Element
{
public:
    ElementType Value;
    mutable int32 HashNextId;
    mutable int32 HashIndex;

    TSet_Element(ElementType InValue, int32 InHashNextId, int32 InHashIndex)
        : Value(InValue)
        , HashNextId(InHashNextId)
        , HashIndex(InHashIndex)
    {
    }

    FORCEINLINE TSet_Element<ElementType>& operator=(const TSet_Element<ElementType>& Other)
    {
        Value = Other.Value;
    }

    FORCEINLINE bool operator==(const TSet_Element& Other) const
    {
        return Value == Other.Value;
    }
    FORCEINLINE bool operator!=(const TSet_Element& Other) const
    {
        return Value != Other.Value;
    }
};

template <typename SetType>
class TSet_
{
private:
    friend TSparseArray;

public:
    typedef TSet_Element<SetType> ElementType;
    typedef TSparseArrayElementOrListLink<ElementType> ArrayElementType;

private:
    TSparseArray<ElementType> Elements;

    mutable TInlineAllocator<1>::ForElementType<int> Hash;
    mutable int32 HashSize;

public:
    class FBaseIterator
    {
    private:
        TSet_<SetType>& IteratedSet;
        TSparseArray<ElementType>::FBaseIterator ElementIt;

    public:
        FORCEINLINE FBaseIterator(const TSet_<SetType>& InSet, TSparseArray<TSet_Element<SetType>>::FBaseIterator InElementIt)
            : IteratedSet(const_cast<TSet_<SetType>&>(InSet))
            , ElementIt(InElementIt)
        {
        }

        FORCEINLINE explicit operator bool() const
        {
            return (bool)ElementIt;
        }
        FORCEINLINE TSet_<SetType>::FBaseIterator& operator++()
        {
            ++ElementIt;
            return *this;
        }
        FORCEINLINE bool operator==(const TSet_<SetType>::FBaseIterator& OtherIt) const
        {
            return ElementIt == OtherIt.ElementIt;
        }
        FORCEINLINE bool operator!=(const TSet_<SetType>::FBaseIterator& OtherIt) const
        {
            return ElementIt != OtherIt.ElementIt;
        }
        FORCEINLINE TSet_<SetType>::FBaseIterator& operator=(TSet_<SetType>::FBaseIterator& OtherIt)
        {
            return ElementIt = OtherIt.ElementIt;
        }
        FORCEINLINE SetType& operator*()
        {
            return (*ElementIt).Value;
        }
        FORCEINLINE const SetType& operator*() const
        {
            return (*ElementIt).Value;
        }
        FORCEINLINE ElementType& operator->()
        {
            return *ElementIt;
        }
        FORCEINLINE const ElementType& operator->() const
        {
            return *ElementIt;
        }
        FORCEINLINE const int32 GetIndex() const
        {
            return ElementIt.GetIndex();
        }
        FORCEINLINE ElementType& GeTSet_Element()
        {
            return *ElementIt;
        }
        FORCEINLINE const ElementType& GeTSet_Element() const
        {
            return *ElementIt;
        }
        FORCEINLINE bool IsElementValid() const
        {
            return ElementIt.IsElementValid();
        }
    };

public:
    FORCEINLINE TSet_<SetType>::FBaseIterator begin()
    {
        return TSet_<SetType>::FBaseIterator(*this, Elements.begin());
    }
    FORCEINLINE const TSet_<SetType>::FBaseIterator begin() const
    {
        return TSet_<SetType>::FBaseIterator(*this, Elements.begin());
    }
    FORCEINLINE TSet_<SetType>::FBaseIterator end()
    {
        return TSet_<SetType>::FBaseIterator(*this, Elements.end());
    }
    FORCEINLINE const TSet_<SetType>::FBaseIterator end() const
    {
        return TSet_<SetType>::FBaseIterator(*this, Elements.end());
    }

    FORCEINLINE SetType& operator[](int Index)
    {
        return Elements[Index].ElementData.Value;
    }

    FORCEINLINE int32 Num() const
    {
        return Elements.Num();
    }
    FORCEINLINE bool IsValid() const
    {
        return Elements.Data.Data != nullptr && Elements.AllocationFlags.MaxBits > 0;
    }
    FORCEINLINE TSparseArray<ElementType>& GetElements()
    {
        return Elements;
    }
    FORCEINLINE const TSparseArray<ElementType>& GetElements() const
    {
        return Elements;
    }
    FORCEINLINE const TBiTArray_& GetAllocationFlags() const
    {
        return Elements.GetAllocationFlags();
    }
    FORCEINLINE bool IsIndexValid(int32 IndexToCheck) const
    {
        return Elements.IsIndexValid(IndexToCheck);
    }
    FORCEINLINE const bool Contains(const SetType& ElementToLookFor) const
    {
        if (Num() <= 0)
            return false;

        for (SetType Element : *this)
        {
            if (Element == ElementToLookFor)
                return true;
        }
        return false;
    }
    FORCEINLINE const int32 Find(const SetType& ElementToLookFor) const
    {
        for (auto It = this->begin(); It != this->end(); ++It)
        {
            if (*It == ElementToLookFor)
            {
                return It.GetIndex();
            }
        }
        return -1;
    }
    FORCEINLINE bool Remove(const SetType& ElementToRemove)
    {
        return Elements.RemoveAt(Find(ElementToRemove));
    }
    FORCEINLINE int32 AddSingle(SetType InElement, int32 InHashIndex = 0, int32 InHashNextId = 0)
    {
        // ToDo: Check for duplication

        if (!this->IsValid())
            return this->Initialize();

        return Elements.AddSingle({ InElement, InHashIndex, InHashNextId });
    }
    FORCEINLINE void Initialize(const int32 NumElementsToInitWith = 5)
    {
        if (this->IsValid())
            return;

        Elements.Data.MaxElements = NumElementsToInitWith;
        Elements.Data.Count = NumElementsToInitWith;
        Elements.Data.Data = (ArrayElementType*)(Malloc(NumElementsToInitWith * sizeof(ElementType), alignof(ElementType)));
        for (int i = 0; i < NumElementsToInitWith; i++)
        {
            Elements.Data.Data[i].PrevFreeIndex = i - 1;
            Elements.Data.Data[i].NextFreeIndex = i + 1;
        }

        Elements.FirstFreeIndex = 0;
        Elements.NumFreeIndices = NumElementsToInitWith;

        Elements.AllocationFlags.MaxBits = 128;
        Elements.AllocationFlags.NumBits = NumElementsToInitWith;
        Elements.AllocationFlags.ZeroAll();

        Hash = Malloc(NumElementsToInitWith * sizeof(ElementType), alignof(ElementType));
        HashSize = NumElementsToInitWith * sizeof(ElementType);
    }
};

template <typename KeyType, typename ValueType>
class TPair
{
private:
    KeyType First;
    ValueType Second;

public:
    TPair(KeyType Key, ValueType Value)
        : First(Key)
        , Second(Value)
    {
    }

public:
    FORCEINLINE KeyType& Key()
    {
        return First;
    }
    FORCEINLINE const KeyType& Key() const
    {
        return First;
    }
    FORCEINLINE ValueType& Value()
    {
        return Second;
    }
    FORCEINLINE const ValueType& Value() const
    {
        return Second;
    }
};

template <typename KeyType, typename ValueType>
class TMap_
{
public:
    typedef TPair<KeyType, ValueType> ElementType;

private:
    TSet_<ElementType> Pairs;

public:
    class FBaseIterator
    {
    private:
        TMap_<KeyType, ValueType>& IteratedMap;
        TSet_<ElementType>::FBaseIterator SetIt;

    public:
        FBaseIterator(TMap_<KeyType, ValueType>& InMap, TSet_<ElementType>::FBaseIterator InSet)
            : IteratedMap(InMap)
            , SetIt(InSet)
        {
        }
        FORCEINLINE TMap_<KeyType, ValueType>::FBaseIterator operator++()
        {
            ++SetIt;
            return *this;
        }
        FORCEINLINE TMap_<KeyType, ValueType>::ElementType& operator*()
        {
            return *SetIt;
        }
        FORCEINLINE const TMap_<KeyType, ValueType>::ElementType& operator*() const
        {
            return *SetIt;
        }
        FORCEINLINE bool operator==(const TMap_<KeyType, ValueType>::FBaseIterator& Other) const
        {
            return SetIt == Other.SetIt;
        }
        FORCEINLINE bool operator!=(const TMap_<KeyType, ValueType>::FBaseIterator& Other) const
        {
            return SetIt != Other.SetIt;
        }
        FORCEINLINE bool IsElementValid() const
        {
            return SetIt.IsElementValid();
        }
    };

    FORCEINLINE TMap_<KeyType, ValueType>::FBaseIterator begin()
    {
        return TMap_<KeyType, ValueType>::FBaseIterator(*this, Pairs.begin());
    }
    FORCEINLINE const TMap_<KeyType, ValueType>::FBaseIterator begin() const
    {
        return TMap_<KeyType, ValueType>::FBaseIterator(*this, Pairs.begin());
    }
    FORCEINLINE TMap_<KeyType, ValueType>::FBaseIterator end()
    {
        return TMap_<KeyType, ValueType>::FBaseIterator(*this, Pairs.end());
    }
    FORCEINLINE const TMap_<KeyType, ValueType>::FBaseIterator end() const
    {
        return TMap_<KeyType, ValueType>::FBaseIterator(*this, Pairs.end());
    }
    FORCEINLINE ValueType& operator[](const KeyType& Key)
    {
        return this->GetByKey(Key);
    }
    FORCEINLINE const ValueType& operator[](const KeyType& Key) const
    {
        return this->GetByKey(Key);
    }
    FORCEINLINE int32 Num() const
    {
        return Pairs.Num();
    }
    FORCEINLINE bool IsValid() const
    {
        return Pairs.IsValid();
    }
    FORCEINLINE bool IsIndexValid(int32 IndexToCheck) const
    {
        return Pairs.IsIndexValid(IndexToCheck);
    }
    FORCEINLINE void Initialize(const int32 NumElementsToInitWith = 5)
    {
        return Pairs.Initialize(NumElementsToInitWith);
    }
    FORCEINLINE bool Contains(const KeyType& ElementToLookFor) const
    {
        for (auto Element : *this)
        {
            if (Element.Key() == ElementToLookFor)
                return true;
        }
        return false;
    }
    FORCEINLINE int32 AddSingle(KeyType InKey, ValueType InValue)
    {
        return Pairs.AddSingle({ InKey, InValue });
    }
    FORCEINLINE int32 AddSingle(ElementType InElement)
    {
        return Pairs.AddSingle(InElement);
    }
    FORCEINLINE ValueType& GetByKey(const KeyType& Key)
    {
        for (auto Pair : *this)
        {
            if (Pair.Key() == Key)
            {
                return Pair.Value();
            }
        }
    }
    FORCEINLINE ValueType GetByKeyNoRef(const KeyType& Key)
    {
        for (auto Pair : *this)
        {
            if (Pair.Key() == Key)
            {
                return Pair.Value();
            }
        }
    }
};


template <typename PtrType>
class TUniquePtr
{
private:

    PtrType* Ptr;

public:
    bool IsValid() const
    {
        return Ptr != nullptr;
    }

    FORCEINLINE explicit operator bool() const
    {
        return IsValid();
    }

    FORCEINLINE bool operator!() const
    {
        return !IsValid();
    }

    FORCEINLINE PtrType* operator->() const
    {
        return Ptr;
    }

    FORCEINLINE PtrType& operator*() const
    {
        return *Ptr;
    }

    FORCEINLINE const PtrType*& Get() const
    {
        return Ptr;
    }

    FORCEINLINE PtrType*& Get()
    {
        return Ptr;
    }
};

template <class ObjectType>
class TSharedPtr
{
public:
    ObjectType* Object;

    int32 SharedReferenceCount;
    int32 WeakReferenceCount;

    FORCEINLINE ObjectType* Get()
    {
        return Object;
    }
    FORCEINLINE ObjectType* Get() const
    {
        return Object;
    }
    FORCEINLINE ObjectType& operator*()
    {
        return *Object;
    }
    FORCEINLINE const ObjectType& operator*() const
    {
        return *Object;
    }
    FORCEINLINE ObjectType* operator->()
    {
        return Object;
    }
};

struct FActorDestructionInfo
{
    TWeakObjectPtr<ULevel>		Level;
    TWeakObjectPtr<UObject>		ObjOuter;
    FVector			            DestroyedPosition;
    int32       	            NetGUID;
    FString			            PathName;

    FName			            StreamingLevelName;
};

struct FNetworkObjectInfo
{
    AActor* Actor;
    TWeakObjectPtr<AActor> WeakActor;
    double NextUpdateTime;
    double LastNetReplicateTime;
    float OptimalNetUpdateDelta;
    float LastNetUpdateTime;
    uint32 bPendingNetUpdate : 1;
    uint32 bForceRelevantNextUpdate : 1;
    TSet_<TWeakObjectPtr<UNetConnection>> DormantConnections;
    TSet_<TWeakObjectPtr<UNetConnection>> RecentlyDormantConnections;
};

class FNetworkObjectList
{
public:
    typedef TSet_<TSharedPtr<FNetworkObjectInfo>> FNetworkObjecTSet_;

    FNetworkObjecTSet_ AllNetworkObjects;
    FNetworkObjecTSet_ ActiveNetworkObjects;
    FNetworkObjecTSet_ ObjectsDormantOnAllConnections;

    TMap_<TWeakObjectPtr<UNetConnection>, int32> NumDormantObjectsPerConnection;
};

// ScriptStruct Engine.NetViewer
// 0x0030
struct FNetViewer
{
    class UNetConnection* Connection;                                               // 0x0000(0x0008) (ZeroConstructor, IsPlainOldData)
    class AActor* InViewer;                                                 // 0x0008(0x0008) (ZeroConstructor, IsPlainOldData)
    class AActor* ViewTarget;                                               // 0x0010(0x0008) (ZeroConstructor, IsPlainOldData)
    struct FVector                                     ViewLocation;                                             // 0x0018(0x000C) (IsPlainOldData)
    struct FVector                                     ViewDir;                                                  // 0x0024(0x000C) (IsPlainOldData)

    FNetViewer()
        : Connection(0)
        , InViewer(0)
        , ViewTarget(0)
        , ViewLocation(FVector({ 0, 0, 0 }))
        , ViewDir(FVector({ 0, 0, 0 }))
    {
    }

    FNetViewer(UNetConnection* InConnection, float DeltaSeconds)
        : Connection(InConnection)
        , InViewer((InConnection->PlayerController ? InConnection->PlayerController : InConnection->OwningActor))
        , ViewTarget(InConnection->ViewTarget)
        , ViewLocation(FVector({ 0, 0, 0 }))
        , ViewDir(FVector({ 0, 0, 0 }))
    {
        APlayerController* ViewingController = InConnection->PlayerController;

        FVector ViewTargetLoc = ViewTarget->K2_GetActorLocation();

        if (ViewTargetLoc)
        {
            ViewLocation = ViewTargetLoc;

            if (ViewingController)
            {
                FRotator ViewRotation = ViewingController->ControlRotation;
                ViewingController->GetActorEyesViewPoint(&ViewLocation, &ViewRotation);
                ViewDir = ViewRotation.Vector();
            }
        }
    }

    inline const bool operator==(const ::FNetViewer& Other) const
    {
        return Connection == Other.Connection && InViewer == Other.InViewer && ViewTarget == Other.ViewTarget;
    }
    inline const bool operator!=(const ::FNetViewer& Other) const
    {
        return Connection != Other.Connection || InViewer != Other.InViewer || ViewTarget != Other.ViewTarget;
    }
};

struct FActorPriority
{
    int32 Priority;

    FNetworkObjectInfo* ActorInfo;
    UActorChannel* Channel;

    struct FActorDestructionInfo* DestructionInfo;

    FActorPriority()
        : Priority(0)
        , ActorInfo(NULL)
        , Channel(NULL)
        , DestructionInfo(NULL)
    {
    }

    FActorPriority(UNetConnection* InConnection, UActorChannel* InChannel, FNetworkObjectInfo* InActorInfo, const TArray<::FNetViewer>& Viewers, bool bLowBandwidth)
        : ActorInfo(InActorInfo)
        , Channel(InChannel)
        , DestructionInfo(NULL)
    {
        static auto RoundToInt = [](float F)
        {
            return _mm_cvt_ss2si(_mm_set_ss(F + F + 0.5f)) >> 1;
        };

        float Time = Channel ? (InConnection->Driver->Time - Channel->LastUpdateTime) : InConnection->Driver->SpawnPrioritySeconds;

        Priority = 0;
        for (int32 i = 0; i < Viewers.Num(); i++)
        {
            // Priority = Util::Max(Priority, RoundToInt(65536.0f * GetNetPriority(ActorInfo->Actor, Viewer.ViewLocation, Viewer.ViewDir, Viewer.InViewer, Viewer.ViewTarget, InChannel, Time, bLowBandwidth)));
        }
    }
    FActorPriority(UNetConnection* InConnection, FActorDestructionInfo* Info, const TArray<::FNetViewer>& Viewers)
        : ActorInfo(NULL)
        , Channel(NULL)
        , DestructionInfo(Info)
    {
        Priority = 0;

        for (int32 i = 0; i < Viewers.Num(); i++)
        {
            float Time = InConnection->Driver->SpawnPrioritySeconds;

            FVector Dir = DestructionInfo->DestroyedPosition - Viewers[i].ViewLocation;
            float DistSq = Dir.SizeSquared();

            // adjust priority based on distance and whether actor is in front of viewer
            if ((Viewers[i].ViewDir | Dir) < 0.f)
            {
                if (DistSq > (2000.f * 2000.f))
                {
                    Time *= 0.2f;
                }
                else if (DistSq > (500.f * 500.f))
                {
                    Time *= 0.4f;
                }
            }
            else if (DistSq > (3162.f * 3162.f))
            {
                Time *= 0.4f;
            }

            Priority = max(Priority, int(65536.0f * Time));
        }
    }
};